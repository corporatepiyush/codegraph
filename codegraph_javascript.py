...
def walk_cursor_nodes(node: Any) -> Iterator[Any]:
    """walk_cursor without the depth: no per-node tuple allocated.

    Same pre-order, same nodes; for walks that never read the depth this
    removes one tuple per node of every file."""
    cursor = node.walk()
    while True:
        yield cursor.node
        if cursor.goto_first_child():
...
#: The fixed (non-metric) columns every symbols INSERT writes, in order.
#: Constant across every symbol; hoisted out of insert_symbol, which used
#: to rebuild this 14-string list per symbol.
_SYMBOL_FIXED_COLS = ["file_id", "module_id", "parent_id", "name", "qual_name",
                      "kind", "line_start", "line_end", "n_lines",
                      "byte_start", "byte_end", "signature", "return_type",
                      "visibility"]
...
def discover(analyzer: Analyzer, root: str, db: sqlite3.Connection,
    ...
    name_re = TEST_NAME_RE_BY_LANG.get(analyzer.LANG, TEST_NAME_RE)
    mod_id: dict[str, int] = {}
    out: list[FileRec] = []
    ...
    for dirpath, dirnames, filenames in os.walk(root, onerror=_walk_error):
        ...
        for fn in sorted(filenames):
            ...
            # One pass for blank / sloc / comment / longest-line instead of
            # four generator sweeps (each re-running strip() per line).
            # Values are identical to the four separate sums they replace.
            blank = cmt = sloc_n = max_len = 0
            for l in lines:
                ls = l.lstrip()
                if not ls:
                    blank += 1
                    ...
                    sloc_n += 1
                    if ls[:3] in _LINE_COMMENT_PREFIXES:
                        cmt += 1
                n = len(l)
                if n > max_len:
                    max_len = n
            ...
            if not rel.isascii() and _has_surrogates(rel):
...
#: Line prefixes that make a line comment-ish for the `comment_lines` file
#: column in `discover`. Same list the per-symbol sloc prefix test uses.
_LINE_COMMENT_PREFIXES = ("//", "#", "/*", "*", "*/", '"""', "'''", "--",
                          ";;", "%")

def scan_markers(rec: FileRec, bufs: Buffers) -> None:
    """TODO/FIXME/HACK and friends, with their line and text."""
    search = MARKER_RE.search
    for i, line in enumerate(rec.text.splitlines(), 1):
        # The comment-ish test is a substring scan in C; run it BEFORE the
        # regex so most lines never reach the pattern. Same conjunction,
        # fewer regex calls -- the row produced is identical.
...
class TreeSitterAnalyzer(Analyzer):
    ...
    def __init__(self) -> None:
        ...
        #: lazily-built frozen copies of the class-level node-type tables
        self._fs: Optional[tuple] = None
        #: type string -> precomputed measure() decision tuple. Built on the
        #: first node of each type the walk meets; one dict probe per node
        #: replaces ~10 set memberships and an is_named call.
        self._measure_plan: Optional[dict[str, tuple]] = None

    def _type_frozensets(self) -> tuple:
        """Frozen copies of the node-type tables, built once per instance.

        measure() used to call set() on eight class-level tuples on EVERY
        invocation -- once per function body, type body and module scope.
        """
        fs = self._fs
        if fs is None:
            fs = self._fs = (
                frozenset(self.LOOP_NODES),
                frozenset(self.BRANCH_NODES),
                frozenset(self.NEST_NODES),
                frozenset(self.CALL_NODES),
                frozenset(self.OPERATOR_NODES),
                frozenset(self.STRING_NODES),
                frozenset(self.NUMBER_NODES),
                frozenset(self.COMMENT_NODES),
                frozenset(self.IF_NODES),
                frozenset(self.FUNC_KINDS) | frozenset(self.TYPE_KINDS),
            )
        return fs

    def _prune_types(self) -> frozenset:
        """Node types measure() skips in `prune` mode: everything that gets
        its own symbol."""
        return self._type_frozensets()[9]
    ...
    def emit_module_scope(self, root: Any, rec: FileRec,
        ...
        if not self.MODULE_SCOPE_SYMBOL:
            return
        stats = self.measure(root, rec, prune=self._prune_types())
    ...
    def walk_scope(self, node: Any, rec: FileRec, db: sqlite3.Connection,
        ...
        fk_get = self.FUNC_KINDS.get
        tk_get = self.TYPE_KINDS.get
        ...
        while stack:
            ...
            if kind:
                ...
                stack.extend([(c, inner) for c in
                              reversed((body or cur).named_children)])
                continue
            kind = tk_get(cur.type)
            if kind:
                ...
                stack.extend([(c, inner) for c in
                              reversed((body or cur).named_children)])
                continue
            stack.extend([(c, sc) for c in reversed(cur.named_children)])
    ...
    def emit_function(self, node: Any, rec: FileRec, db: sqlite3.Connection,
        ...
        name0 = self.node_name(node, rec)
        name = name0 or "(anonymous)"
        ...
        m.update(self.function_flags(node, rec, scope, name0, sig))
        ...
        sid = self.insert_symbol(
            ...
            self.visibility_of(node, rec, name0), m)
    ...
    def emit_type(self, node: Any, rec: FileRec, db: sqlite3.Connection,
        ...
        body = node.child_by_field_name(self.BODY_FIELD) or node
        bstats = self.measure(body, rec, prune=self._prune_types())
        ...
        m.update(self.type_flags(node, rec, scope, name0))
        doc = self.docstring_lines(node, rec)
        m["n_doc_lines"] = doc
        m["has_doc"] = 1 if doc else 0
        sid = self.insert_symbol(
            ...
            self.visibility_of(node, rec, name), m)
        self.emit_attributes(node, rec, sid, bufs)
    ...
    def insert_symbol(self, db: sqlite3.Connection, rec: FileRec, name: str,
        ...
        for k, dflt in spec:
            v = mget(k, dflt)
            row.append(int(v) if type(v) is bool else v)
    ...
    def measure(self, body: Any, rec: FileRec,
        ...
        Every type-dependent decision below is precomputed once per node TYPE
        in `self._measure_plan` and unpacked from one tuple, so the per-node
        cost is a single dict probe instead of ten set memberships plus an
        `is_named` call. The plan is a pure function of the class-level node
        tables, which never change at run time.
        """
        st = BodyStats()
        src = rec.data
        else_field = self.ELSE_FIELD
        (loops, branches, nests, calls, operators, strings, numbers,
         comments, ifs, _prune) = self._type_frozensets()
        counters = self.COUNTERS
        flags = self.FLAG_NODES
        want_elif = bool(ifs) and bool(else_field)
        extra_loops = self.extra_loop_ids(body, rec)
        check_extra = bool(extra_loops)
        plan = self._measure_plan
        if plan is None:
            plan = self._measure_plan = {}
        plan_get = plan.get
        counts = st.counts
        ...
        while True:
            ...
            if e is None:
                ...
                # so requiring a named node is free everywhere else. Named-ness
                # is a property of the TYPE, so it is folded into the plan.
                named = node.is_named
                kind = 0
                if t in calls:
                    kind = 1
                    ...
                    kind = 5
                e = (named,
                     want_elif and t in ifs,
                     named and t in nests,
                     t in loops,
                     named and t in branches,
                     counters.get(t),
                     named and t == "assignment_expression",
                     flags.get(t),
                     kind)
                plan[t] = e
            (named, elif_cand, nest_m, loop_m, branch_m, counter_key,
             is_assign, flag_key, kind) = e
            ...
            if nest_m and not is_elif and node is not body:
                nest_stack.append(depth)
                if len(nest_stack) > st.max_nesting:
                    st.max_nesting = len(nest_stack)
            nest = len(nest_stack)
            if named and (loop_m or (check_extra and node.id in extra_loops)):
                loop_stack.append(depth)
                loop_depth += 1
                if loop_depth > st.max_loop_depth:
                    st.max_loop_depth = loop_depth
                st.cyclomatic += 1
                st.cognitive += nest if nest > 1 else 1
                counts["n_loops"] = cget("n_loops", 0) + 1
            elif branch_m:
                st.cyclomatic += 1
                st.cognitive += (1 if is_elif else (nest if nest > 1 else 1))
                counts["n_branches"] = cget("n_branches", 0) + 1
                if is_elif:
                    counts["n_elif"] = cget("n_elif", 0) + 1
                if loop_depth:
                    counts["branch_in_loop"] = cget("branch_in_loop", 0) + 1

            if counter_key is not None:
                counts[counter_key] = cget(counter_key, 0) + 1
            if is_assign:
            ...
            if kind == 0:
                if node.child_count == 0:
                    st.n_tokens += 1
                    st.n_operands += 1
                    # text_of inlined: this is the single hottest text read in
                    # the build (every leaf token of every body).
                    st.operands.add(src[node.start_byte:node.end_byte]
                                    .decode("utf-8", "replace")[:40])
            elif kind == 1:
                on_call(node, src, st, loop_depth, nest, t)
            elif kind == 2:
                st.n_operators += 1
                st.operators.add(t)
            elif kind == 3:
                txt = text_of_(node, src)
                counts["n_string_lit"] = cget("n_string_lit", 0) + 1
                ...
                st.n_operands += 1
                st.operands.add(txt)
                if txt not in MAGIC_STR and NUM_RE.match(txt) is not None:
                    counts["n_magic"] = cget("n_magic", 0) + 1
                    st.literals.append(("number", txt,
                                        node.start_point[0] + 1, True))
                if "." in txt or "e" in txt.lower():
                    counts["n_float_lit"] = cget("n_float_lit", 0) + 1
            else:                       # kind == 5: comments
                counts["n_comment_lines"] = cget("n_comment_lines", 0) + \
                    (node.end_point[0] - node.start_point[0] + 1)

            on_node(node, src, st, loop_depth, nest, t)

            if prune is not None and node is not body and t in prune:
                descend = False
            else:
                descend = True
    ...
    def on_call(self, node: Any, src: bytes, st: BodyStats,
                loop_depth: int, nest: int, t: str = "") -> None:
        st.bump("n_calls")
        if loop_depth:
    ...
    def on_node(self, node: Any, src: bytes, st: BodyStats,
                loop_depth: int, nest: int, t: str = "") -> None:
        """Hook: anything the COUNTERS table cannot express. `t` is the
        node's type, precomputed by the walk that calls this."""

    # -- per-symbol detail a language may override ------------------------
    def function_flags(self, node: Any, rec: FileRec, scope: Scope,
    ...
    def type_flags(self, node: Any, rec: FileRec, scope: Scope,
    ...
    def visibility_of(self, node: Any, rec: FileRec,
...
#: Frozensets for the hot per-call-node membership tests in on_call: hashing
#: once beats a linear tuple scan, and the sets are built once, not per call.
_CHILD_PROCESS_BASES = frozenset(("exec", "execSync", "spawnSync"))
_FS_RW_BASES = frozenset(("readFile", "readFileSync", "writeFile",
                          "writeFileSync"))
_FS_WRITE_BASES = frozenset(("writeFile", "writeFileSync"))
_FS_SYNC_BASES = frozenset(("readFileSync", "writeFileSync", "existsSync",
                            "statSync"))
_ARRAY_GROW_BASES = frozenset(("push", "concat", "unshift"))
_SEARCH_BASES = frozenset(("indexOf", "includes", "find"))
_WEAK_HASH_BASES = frozenset(("createHash", "createHmac", "createCipher"))
_SEND_FILE_BASES = frozenset(("sendFile", "sendfile", "download"))
_ZIP_EXTRACT_BASES = frozenset(("extract", "extractAll", "extractAllTo",
                                "extractFiles"))
...
class JavaScriptAnalyzer(TreeSitterAnalyzer):
    ...
    # -- cross-function pack: config, executors, callbacks, sinks ----------
        ("n_env_read", "INT NOT NULL DEFAULT 0"),          # no-process-env
        ("n_promise_exec_async", "INT NOT NULL DEFAULT 0"),
        ("n_promise_exec_return", "INT NOT NULL DEFAULT 0"),
        ("n_async_arg", "INT NOT NULL DEFAULT 0"),
        ("n_regex_dynamic", "INT NOT NULL DEFAULT 0"),     # non-literal RegExp
        ("n_send_file", "INT NOT NULL DEFAULT 0"),         # res.sendFile family
        ("n_lazy_require", "INT NOT NULL DEFAULT 0"),      # require() in fn body
        ("n_server_start", "INT NOT NULL DEFAULT 0"),      # listen/createServer
    )
...
CREATE TABLE timers(
    ...
    at_module_scope INT NOT NULL DEFAULT 0,
    in_loop INT NOT NULL DEFAULT 0,
    is_async_cb INT NOT NULL DEFAULT 0
) STRICT;
...
-- One row per route/middleware registration (`app.get(path, ...)`). Express 4
-- does not catch a rejected promise from a handler (the request hangs and the
-- rejection is unhandled), and a named handler that resolves to nothing in
-- this tree is a 500 at request time -- both are questions about the
-- registration site, so they are recorded where they happen.
CREATE TABLE routes(
    id INTEGER PRIMARY KEY,
    file_id INT NOT NULL REFERENCES files(id),
    symbol_id INT REFERENCES symbols(id),
    ...
    handler TEXT NOT NULL DEFAULT '',
    handler_inline INT NOT NULL DEFAULT 0,
    handler_async INT NOT NULL DEFAULT 0,
    n_middlewares INT NOT NULL DEFAULT 0
) STRICT;
...
CREATE INDEX idx_route_file ON routes(file_id, method);
CREATE INDEX idx_route_handler ON routes(handler);
-- uncaught-exception-swallowed asks, per listener row, whether the installer
-- also exec()s. Without this partial index that is a hazards scan per row.
CREATE INDEX idx_haz_sym_exec ON hazards(symbol_id) WHERE category='exec';
"""
...
"""
    ...
    def parse_file(self, rec: FileRec, db: sqlite3.Connection,
        ...
        self._span_starts = []
        self._exported = set()
        self._mc_cands: Optional[dict[str, tuple]] = None
    ...
    def on_call(self, node: Any, src: bytes, st: BodyStats,
        ...
        # One lowercase copy per call node: the checks below used to call
        # .lower() three times (zip, logger, auth), each allocating.
        lname = name.lower()
        if base in _CHILD_PROCESS_BASES and "child_process" in name:
            bump("n_child_process")           # eslint-plugin-security
        if base == "fetch" or name.startswith(
                ("axios.", "got.", "superagent.", "node-fetch", "http.request",
                 "https.request")):
            # G27: SSRF sink -- fetch a URL that may come from the client
            bump("n_fetch")
        if base in _FS_RW_BASES and name.startswith("fs."):
            ...
            if first is not None and first.type not in ("string",
                ...
                bump("n_dynamic_open")
        if base in _FS_WRITE_BASES and name.startswith("fs."):
            args = node.child_by_field_name("arguments")
            if args is not None and any(
                ...
                bump("n_upload_save")
        if "unzip" in lname or ("zip" in lname and base in _ZIP_EXTRACT_BASES):
            # G29: archive extraction -- entry containment is a check, not a
            # name; this ranks where archives are opened.
            bump("n_zip_read")
        if base == "assign" and name.startswith("Object."):
            # G30: mass assignment -- Object.assign(obj, input) merges every
            # key; the spread {...req.body} shape is invisible here.
            bump("n_mass_assign")
        if base in LOG_LEVELS and "logger" in lname:
            # G14: a logging call -- injection into the log line is the risk
            # when the message contains request input.
            bump("n_log_call")
        if name.startswith("console.") and base in LOG_LEVELS:
            # G21: a console call -- sensitive data in logs is the risk when
            # the message contains request input.
            bump("n_console_log")
        if any(k in lname for k in AUTH_MARKERS):
            # G01: an auth-family call -- the marker that makes the
            # unauthenticated-input surface query exclude this function.
            bump("n_auth_call")
        if base in _FS_SYNC_BASES:
            bump("n_fs_sync")                 # detect-non-literal-fs-filename
        if name == "Object.assign" and loop_depth:
            bump("n_assign_in_loop")
        if base == "parse" and name.startswith("JSON") and loop_depth:
            bump("n_json_parse_in_loop")
        if base in _ARRAY_GROW_BASES and loop_depth:
            bump("n_array_grow_in_loop")      # unicorn/no-array-push-push
        if base in _SEARCH_BASES and loop_depth:
            bump("n_search_in_loop")          # accidental O(n^2)
        if name == "Math.random":
            bump("n_math_random")             # security/detect-pseudoRandomBytes
        if base in _WEAK_HASH_BASES:
            ...
            if "md5" in _at or "sha1" in _at or "rc4" in _at or "des" in _at:
                bump("n_weak_hash")              # the algorithm is an ARGUMENT
        if base == "then" and loop_depth:
            bump("n_then_in_loop")            # promise/no-promise-in-callback
        if base == "catch" and loop_depth:
            bump("n_catch_in_loop")
        if name == "process.exit":
            bump("n_process_exit")            # unicorn/no-process-exit
        if name == "Buffer" or name.startswith("Buffer."):
            bump("n_buffer_call")             # node/no-deprecated-api
        if base == "setPrototypeOf" or base == "__defineGetter__":
            bump("n_proto_mutate")
        if fn.type == "import":
            bump("n_import_dynamic")
        ...
        if name in ("eval", "Function") or base == "eval":
            ...
            if args is not None:
                ...
                if kid is not None and kid.type not in ("string",):
                    bump("n_require_dynamic")
                elif kid is not None and not _at_module_scope(node):
                    # eslint-plugin-n/global-require: a literal require() at
                    # call depth -- a dependency the import graph cannot show
                    # and a load cost on every call.
                    bump("n_lazy_require")
            ...
            bump("n_then")
            bump("n_promise_chain")
        elif base in ("catch", "finally") and "." in name:
            bump("n_catch_handler")
            bump("n_promise_chain")
        ...
        if SETSTATE_RE.match(base) or name.endswith(".setState"):
            bump("n_setstate")
        if is_new and base == "RegExp":
            _ra = node.child_by_field_name("arguments")
            _r0 = _ra.named_children[0] \
                if _ra is not None and _ra.named_children else None
            if _r0 is not None and _r0.type not in ("string",
                                                    "template_string"):
                # security/detect-non-literal-regexp: a pattern built at run
                # time from data -- the ReDoS/injection half that a scan of
                # regex LITERALS (n_regex_redos) never sees.
                bump("n_regex_dynamic")
        if is_new and base == "Promise":
            _pa = node.child_by_field_name("arguments")
            _ex = _pa.named_children[0] \
                if _pa is not None and _pa.named_children else None
            if _ex is not None and _ex.type in ANON_FN_NODES:
                if any(c.type == "async" for c in _ex.children):
                    # no-async-promise-executor: a throw inside this executor
                    # never rejects the promise; the caller waits forever.
                    bump("n_promise_exec_async")
                if _executor_returns_value(_ex):
                    # no-promise-executor-return: the returned value is
                    # discarded, but it reads as if it resolved the promise.
                    bump("n_promise_exec_return")
        _call_args = node.child_by_field_name("arguments")
        if _call_args is not None:
            for _a in _call_args.named_children:
                if _a.type in ANON_FN_NODES and any(
                        c.type == "async" for c in _a.children):
                    # An async function handed to a caller that does not await
                    # its return value (forEach/map/emitter/timer): every
                    # rejection inside it is unhandled.
                    bump("n_async_arg")
                    break
        if base in _SEND_FILE_BASES and "." in name:
            # Semgrep express tainted-file-path family: res.sendFile(path)
            # streams whatever the path resolves to.
            bump("n_send_file")
        if base in ("listen", "createServer") and not is_new:
            # nodebestpractices 2.6/8.6: something now LISTENS. Pairing that
            # with a SIGTERM handler is the graceful-shutdown contract.
            bump("n_server_start")
        if is_new:
            if base == "Map" or base == "WeakMap":
                ...
                bump("n_new_set")
            elif base == "WeakRef" or base == "FinalizationRegistry":
                bump("n_weak_ref")
    ...
    def on_node(self, node: Any, src: bytes, st: BodyStats,
        ...
        if t == "binary_expression":
            ...
            if base == "process.env":
                # eslint-plugin-n/no-process-env: configuration read at call
                # depth instead of injected at the edge.
                st.bump("n_env_read")
    ...
    def parse_imports(self, root: Any, rec: FileRec, bufs: Buffers) -> None:
        ...
        # Module-cache candidates are collected in THIS walk rather than a
        # second full-tree traversal in parse_file_extra: same cursor, same
        # pre-order, same rows -- one whole-tree pass per file fewer.
        mc_cands: dict[str, tuple] = {}
        self._mc_cands = mc_cands

        for n in walk_cursor_nodes(root):
            t = n.type
            if t == "import_statement":
                ...
                self._cache_candidate(n, src, mc_cands)
    ...
    def parse_file_extra(self, root: Any, rec: FileRec,
        ...
        # Collected during the parse_imports walk (one traversal, not two).
        # The fallback covers a future call order that skips parse_imports.
        caches = self._mc_cands
        if caches is None:
            caches = {}
            for n in walk_cursor_nodes(root):
                if n.type == "variable_declarator":
                    self._cache_candidate(n, src, caches)
        ...
        for n, _depth, _in_loop, _in_cond in walk_cursor_ctx(root):
            ...
            if t == "call_expression" or t == "new_expression":
                ...
                left = n.child_by_field_name("left")
                if left is not None and cache_use:
        ...
        if base in LISTENER_ADD and not new:
            ...
            bufs.rows("timers").append(
                (rec.fid, sid, line, "set", base, kind, handle[:120],
                 ...
                 int(k0 is not None and k0.type in ANON_FN_NODES
                     and any(c.type == "async" for c in k0.children))))
            ...
            if first.startswith("/") or base == "use":
                # The terminal callback is what handles the request; anything
                # fn-shaped before it is middleware in the chain.
                fns = [k for k in kids if k.type in ANON_FN_NODES
                       or k.type == "identifier"]
                term = fns[-1] if fns else None
                bufs.rows("routes").append(
                    (rec.fid, sid, line, base[:20], first[:120],
                     _txt(term, src).strip()[:120] if term is not None else "",
                     int(term is not None and term.type in ANON_FN_NODES),
                     int(term is not None and term.type in ANON_FN_NODES
                         and any(c.type == "async" for c in term.children)),
                     max(0, len(fns) - 1)))
                for k in kids[1:]:
    ...
    def _cache_candidate(self, n: Any, src: bytes,
                         out: dict[str, tuple]) -> None:
        """One `variable_declarator` considered as a module-scope container.

        `const cache = new Map()` and friends. Module scope specifically,
        because that is the scope that lives as long as the process. The same
        `new Map()` inside a function is a local and dies with the call.
        Called from the parse_imports walk for every variable_declarator; a
        later declarator of the same name overwrites an earlier one, exactly
        as the standalone second walk this replaced did.
        """
    ...
    def flush_extra(self, db: sqlite3.Connection, bufs: Buffers) -> None:
        for tbl, sql in (
            ...
            ("timers",
             ...
             "at_module_scope,in_loop,is_async_cb) "
             "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)"),
            ("module_caches",
            ...
            ("secret_candidates",
             ...
             "VALUES(?,?,?,?)"),
            ("routes",
             "INSERT INTO routes(file_id,symbol_id,line,method,path,handler,"
             "handler_inline,handler_async,n_middlewares) "
             "VALUES(?,?,?,?,?,?,?,?,?)"),
        ):
...
def _executor_returns_value(executor: Any) -> bool:
    """True when a Promise executor body ends with `return <value>`.

    no-promise-executor-return: the value is silently discarded, but a return
    in an executor reads as if it resolved the promise. The walk is bounded to
    the executor node, which only exists on `new Promise(...)` sites.
    """
    body = executor.child_by_field_name("body")
    for c in walk(body if body is not None else executor):
...
(
    ...
    "Empty or silent catch blocks: errors caught and swallowed (ESLint no-empty)",
    "ANSWERS where an error is caught and discarded: an EMPTY catch block, or a\n"
    ...
    """SELECT s.name, s.n_catch_empty AS empty_catches,
        s.n_catch_broad AS silent_catches, s.n_try AS try_blocks,
        s.n_throw AS throws_, s.fan_in, s.is_handler AS handler,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE (s.n_catch_empty > 0 OR s.n_catch_broad > 0)
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_catch_empty DESC, s.n_catch_broad DESC, s.fan_in DESC
    LIMIT :lim"""),
(
    "test-only-callers",
    "Private functions whose only callers live in test files",
    "ANSWERS production code that production code does not use. A private\n"
    ...
    """WITH callers AS (
        SELECT e.callee_id AS id,
            COUNT(DISTINCT CASE WHEN cf.is_test = 0 AND cs.is_test = 0
                                THEN e.caller_id END) AS prod_callers,
            COUNT(DISTINCT e.caller_id) AS all_callers
        FROM edges e
        JOIN symbols cs ON cs.id = e.caller_id
        JOIN files cf ON cf.id = cs.file_id
        WHERE e.is_self = 0
        GROUP BY e.callee_id)
    SELECT s.name, c.all_callers - c.prod_callers AS test_callers,
        c.all_callers AS callers, s.sloc, s.cyclomatic AS cyclo,
    ...
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE c.prod_callers = 0
      AND (s.is_public = 0 OR substr(s.name, 1, 1) = '_')
      AND s.kind IN ('function','method','closure')
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY test_callers DESC, s.sloc DESC LIMIT :lim"""),
(
    "unawaited-async-majority",
    "Async functions most callers treat as fire-and-forget (CodeQL js/missing-await)",
    "ANSWERS the aggregate shape behind unhandled rejections: async functions\n"
    ...
    """WITH per AS (
        SELECT e.callee_id AS id,
            COUNT(DISTINCT CASE WHEN cf.is_test = 0 THEN e.caller_id END)
                AS prod_callers,
            COUNT(DISTINCT CASE WHEN cf.is_test = 0 AND cal.n_await = 0
                                 AND cal.n_then = 0
                                 AND cal.n_catch_handler = 0
                                THEN e.caller_id END) AS discarding
        FROM edges e
        JOIN symbols cal ON cal.id = e.caller_id
        JOIN files cf ON cf.id = cal.file_id
        WHERE e.is_self = 0
        GROUP BY e.callee_id)
    SELECT s.name AS async_fn, p.prod_callers AS callers, p.discarding,
        CAST(100.0 * p.discarding / NULLIF(p.prod_callers, 0) AS INT)
            AS pct_discarding,
        s.n_net + s.n_io AS side_effects, s.n_throw AS throws_,
        f.path || ':' || s.line_start AS at
    FROM per p
    JOIN symbols s ON s.id = p.id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.is_async = 1 AND p.prod_callers > 1 AND p.discarding > 0
      AND 2 * p.discarding >= p.prod_callers
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY pct_discarding DESC, side_effects DESC LIMIT :lim"""),
(
    "handler-writes-shared-cache",
    "Request handlers writing module-scope containers: state that outlives the request",
    "ANSWERS handlers that write to a container declared OUTSIDE themselves\n"
    ...
    """SELECT s.name, s.n_cache_write AS container_writes,
        s.n_new_map + s.n_new_set AS containers_made,
        s.is_async AS handler_async, s.fan_in,
        COALESCE(m.name,'') AS module_,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.is_handler = 1 AND s.n_cache_write > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_cache_write DESC, s.fan_in DESC LIMIT :lim"""),
(
    "listener-add-in-request-path",
    "Event listeners registered inside request handlers (retention multiplied by traffic)",
    "ANSWERS the multiplicative retention leak: a listener registered per\n"
    ...
    """SELECT s.name, s.n_listener_add AS listener_adds,
        s.n_listener_remove AS listener_removes,
        s.n_listener_inline AS inline_handlers,
        s.n_timer_set AS timers_set, s.is_async AS handler_async,
        s.fan_in, COALESCE(m.name,'') AS module_,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.is_handler = 1 AND s.n_listener_add > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_listener_add DESC, s.fan_in DESC LIMIT :lim"""),
(
    "json-parse-unguarded",
    "JSON.parse of untrusted input with no try in the function",
    "ANSWERS where malformed input kills the request -- or since Node 15, an\n"
    ...
    """SELECT s.name, s.n_json_parse AS parses, s.n_try AS try_blocks,
        s.is_handler AS handler,
        (SELECT COUNT(*) FROM user_input_sites u
         WHERE u.symbol_id = s.id) AS input_reads,
        s.fan_in, f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_json_parse > 0 AND s.n_try = 0
      AND (s.is_handler = 1
           OR EXISTS (SELECT 1 FROM user_input_sites u
                      WHERE u.symbol_id = s.id))
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.is_handler DESC, input_reads DESC, s.n_json_parse DESC
    LIMIT :lim"""),
(
    "effect-dep-array-misuse",
    "React effect/callback hooks missing the dependency array entirely, or passing []",
    "ANSWERS the two exhaustive-deps shapes that are bugs rather than style:\n"
    ...
    """SELECT h.name, h.has_dep_array AS no_dep_array, h.n_deps AS deps,
        h.in_loop, h.in_condition,
        h.registers_listener AS registers_listener,
        h.registers_timer AS registers_timer, h.has_cleanup AS cleanup,
        COALESCE(s.name,'(module scope)') AS in_fn,
        f.path || ':' || h.line AS at
    FROM hooks h
    ...
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE h.name IN ('useEffect','useLayoutEffect','useCallback','useMemo',
                     'useImperativeHandle','useInsertionEffect')
      AND (h.has_dep_array = 0 OR h.n_deps = 0)
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY h.has_dep_array ASC, h.registers_listener DESC, h.line
    LIMIT :lim"""),
(
    "effect-registers-no-cleanup",
    "Effects that subscribe or start timers and return no cleanup",
    "ANSWERS the React shape of the retention leak: an effect whose body\n"
    ...
    """SELECT h.name, h.registers_listener AS registers_listener,
        h.registers_timer AS registers_timer, h.has_dep_array AS no_dep_array,
        h.n_deps AS deps, h.in_condition,
        COALESCE(s.name,'(module scope)') AS in_fn,
        f.path || ':' || h.line AS at
    FROM hooks h
    JOIN files f ON f.id = h.file_id
    LEFT JOIN symbols s ON s.id = h.symbol_id
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE (h.registers_listener = 1 OR h.registers_timer = 1)
      AND h.has_cleanup = 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY h.registers_listener DESC, h.registers_timer DESC, h.line
    LIMIT :lim"""),
(
    "process-exit-in-exported",
    "process.exit() inside exported functions: a library killing its host",
    "ANSWERS the cross-file version of process-exit-in-handler: an EXPORTED\n"
    ...
    "    is not captured.",
    """SELECT s.name, s.n_process_exit AS exit_calls, s.is_entrypoint,
        s.fan_in, COALESCE(m.name,'') AS module_,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_process_exit > 0 AND s.is_exported = 1
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC, s.n_process_exit DESC LIMIT :lim"""),
(
    "weak-crypto-hash",
    "md5/sha1/DES chosen as the algorithm argument (OWASP crypto",
    "ANSWERS functions that select a broken algorithm by passing its name as\n"
    ...
    "    count alone.",
    """SELECT s.name, s.n_weak_hash AS weak_hashes,
        (SELECT COUNT(*) FROM user_input_sites u
         WHERE u.symbol_id = s.id) AS input_reads,
        s.is_handler AS handler,
        s.n_crypto AS crypto_hazards, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_weak_hash > 0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY input_reads DESC, s.n_weak_hash DESC, s.fan_in DESC
    LIMIT :lim"""),
(
    "predictable-random-token",
    "Math.random in functions that also touch auth: guessable session material",
    "ANSWERS the co-occurrence that turns Math.random from a smell into a\n"
    ...
    "    name matches.",
    """SELECT s.name, s.n_math_random AS random_calls,
        s.n_auth_call AS auth_calls, s.is_handler AS handler,
        s.is_exported AS exported, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_math_random > 0 AND s.n_auth_call > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_auth_call DESC, s.n_math_random DESC, s.fan_in DESC
    LIMIT :lim"""),
(
    "timer-callback-as-string",
    "setTimeout/setInterval whose callback is a string: implied eval",
    "ANSWERS timers whose callback is a STRING (`setTimeout(\"doIt()\", 100)`).\n"
    ...
    """SELECT COALESCE(s.name,'(module scope)') AS in_fn, t.api, t.kind,
        t.at_module_scope AS module_scope, t.in_loop,
        t.is_repeating AS repeating,
        f.path || ':' || t.line AS at
    FROM timers t
    JOIN files f ON f.id = t.file_id
    LEFT JOIN symbols s ON s.id = t.symbol_id
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE t.op = 'set' AND t.callback_is_string = 1
      AND f.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY t.is_repeating DESC, t.at_module_scope DESC, t.in_loop DESC
    LIMIT :lim"""),
(
    "uncaught-exception-swallowed",
    "process.on('uncaughtException'/'unhandledRejection') handlers",
    "ANSWERS where the process-level safety nets are installed. Both events\n"
    ...
    "    helper shows 0.",
    """SELECT l.event, l.target,
        COALESCE(s.name,'(module scope)') AS in_fn,
        (SELECT COALESCE(SUM(hz.n), 0) FROM hazards hz
         WHERE hz.symbol_id = l.symbol_id AND hz.category = 'exec')
            AS exit_calls,
        f.path || ':' || l.line AS at
    FROM listeners l
    JOIN files f ON f.id = l.file_id
    ...
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE l.op = 'add'
      AND l.event IN ('uncaughtException', 'unhandledRejection')
      AND f.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY (l.target = 'process') DESC, l.event LIMIT :lim"""),
(
    "cache-scoped-to-call",
    "A Map/Set built inside a function and written to: a cache that caches nothing",
    "ANSWERS functions that construct a fresh Map or Set AND write entries to\n"
    ...
    """SELECT s.name, s.n_new_map + s.n_new_set AS containers,
        s.n_cache_write AS writes, s.fan_in, s.is_handler AS handler,
        s.is_recursive AS recursive_, f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE (s.n_new_map > 0 OR s.n_new_set > 0) AND s.n_cache_write > 0
      AND s.kind IN ('function','method','closure')
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_cache_write DESC, s.fan_in DESC LIMIT :lim"""),
(
    "module-scope-sync-io",
    "Synchronous fs/crypto calls at module load: the cold-start tax every require pays",
    "ANSWERS top-level *Sync calls (the `<module>` symbol of each file).\n"
    ...
    """SELECT s.name AS module_symbol, s.n_sync_calls AS sync_calls,
        s.n_io AS io_hazards, s.n_exec AS exec_hazards,
        s.is_entrypoint AS entry, COALESCE(m.name,'') AS module_,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.kind='module' AND s.n_sync_calls > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_sync_calls DESC, s.n_exec DESC LIMIT :lim"""),
(
    "private-symbol-imported",
    "Underscore-prefixed symbols imported across files",
    "ANSWERS files that import another file's `_`-named export -- the\n"
    ...
    """SELECT i.name, i.alias, i.source AS from_file,
        COALESCE(m.name,'') AS module_,
        f.path || ':' || i.line AS at
    FROM import_names i
    JOIN files f ON f.id = i.file_id
    JOIN files tf ON tf.id = i.source_id
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE substr(i.name, 1, 1) = '_'
      AND f.is_test = 0 AND tf.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY f.path, i.line LIMIT :lim"""),
(
    "then-in-loop",
    ".then() inside a loop body: detached chains, interleaved order",
    "ANSWERS promise chains created PER ITERATION (`for (...) p.then(...)`).\n"
    ...
    """SELECT s.name, s.n_then_in_loop AS then_sites_in_loop,
        s.await_in_loop AS awaits_in_loop, s.n_then AS then_sites,
        s.n_catch_handler AS catch_handlers, s.n_loops AS loops,
        s.fan_in, f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_then_in_loop > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_then_in_loop DESC, s.fan_in DESC LIMIT :lim"""),
(
    "module-scope-floating-promise",
    "Fire-and-forget async calls at module load: startup races nothing can await",
    "ANSWERS top-level statement-position async calls (`init()` at import\n"
    ...
    "    line to fix.",
    """SELECT s.name AS module_symbol, s.n_floating_promise AS detached_calls,
        s.n_await AS awaits, s.n_net AS net_hazards, s.n_io AS io_hazards,
        COALESCE(m.name,'') AS module_,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.kind='module' AND s.n_floating_promise > 1 AND s.n_await = 0
      AND (s.n_net + s.n_io + s.n_exec + s.n_timer_set
           + s.n_child_process + s.n_server_start) > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_floating_promise DESC LIMIT :lim"""),
(
    "module-top-level-await",
    "Top-level await in a module: every importer waits for it",
    "ANSWERS files whose top level awaits (the `<module>` symbol). TLA\n"
    ...
    """SELECT s.name AS module_symbol, s.n_await AS top_level_awaits,
        s.n_floating_promise AS detached_calls, s.n_net AS net_hazards,
        s.is_entrypoint AS entry, COALESCE(m.name,'') AS module_,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.kind='module' AND s.n_await > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_await DESC LIMIT :lim"""),
(
    "duplicate-listener-registration",
    "The same event registered twice on the same target in one module",
    "ANSWERS files that call add-style APIs twice for one (target, event)\n"
    ...
    """WITH reg AS (
        SELECT l.file_id, l.target, l.event, COUNT(*) AS adds,
            SUM(l.handler_inline) AS inline_adds, MIN(l.line) AS first_line
        FROM listeners l
        JOIN files f ON f.id = l.file_id
        WHERE l.op = 'add' AND f.is_test = 0
        GROUP BY l.file_id, l.target, l.event
        HAVING COUNT(*) > 1)
    SELECT r.target, r.event, r.adds, r.inline_adds,
        COALESCE(m.name,'') AS module_,
        f.path || ':' || r.first_line AS at
    FROM reg r
    JOIN files f ON f.id = r.file_id
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE COALESCE(m.name,'') LIKE :mod
    ORDER BY r.adds DESC, r.inline_adds DESC LIMIT :lim"""),
(
    "graceful-shutdown-missing",
    "A server starts here, and nothing in the module handles SIGTERM/SIGINT",
    "ANSWERS files that call listen/createServer (n_server_start) with no\n"
    ...
    """WITH starts AS (
        SELECT s.file_id, MIN(s.line_start) AS line,
            SUM(s.n_server_start) AS starts
        FROM symbols s JOIN files f ON f.id = s.file_id
        WHERE s.n_server_start > 0 AND f.is_test = 0
        GROUP BY s.file_id)
    SELECT f.path, st.starts, st.line,
        COALESCE(m.name,'') AS module_,
        f.path || ':' || st.line AS at
    FROM starts st
    JOIN files f ON f.id = st.file_id
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE NOT EXISTS (
              SELECT 1 FROM listeners l
              WHERE l.file_id = st.file_id AND l.op = 'add'
                AND l.event IN ('SIGTERM','SIGINT','SIGUSR2','exit',
                                'beforeExit'))
      AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY st.starts DESC, f.path LIMIT :lim"""),
(
    "env-read-at-depth",
    "process.env read deep in the call tree: configuration far from the edge",
    "ANSWERS functions that read process.env and how far UP the call graph\n"
    ...
    """WITH RECURSIVE up(sym, depth) AS (        -- depth bound: 4 hops
        SELECT s.id, 0 FROM symbols s
        WHERE s.n_env_read > 0
        UNION
        SELECT e.caller_id, u.depth + 1
        FROM up u JOIN edges e ON e.callee_id = u.sym
        WHERE u.depth < 4 AND e.is_self = 0),
    best AS (SELECT sym, MAX(depth) AS depth FROM up GROUP BY sym)
    SELECT s.name, b.depth AS hops_to_edge, s.n_env_read AS env_reads,
        s.fan_in, s.is_handler AS handler, s.is_entrypoint AS entry,
        COALESCE(m.name,'') AS module_,
        f.path || ':' || s.line_start AS at
    FROM best b
    JOIN symbols s ON s.id = b.sym
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_env_read > 0 AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY b.depth DESC, s.fan_in DESC LIMIT :lim"""),
(
    "promise-executor-async",
    "new Promise(async ...): executor throws never reject the caller's promise",
    "ANSWERS executors declared async. A throw inside them rejects the\n"
    ...
    """SELECT s.name, s.n_promise_exec_async AS async_executors,
        s.n_promise_all AS batches, s.n_await AS awaits, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_promise_exec_async > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_promise_exec_async DESC, s.fan_in DESC LIMIT :lim"""),
(
    "promise-executor-return",
    "A Promise executor's return value is silently discarded (no-promise-executor-return)",
    "ANSWERS executors that `return` a value. In any other function return\n"
    ...
    """SELECT s.name, s.n_promise_exec_return AS executor_returns,
        s.n_promise_exec_async AS async_executors, s.n_await AS awaits,
        s.fan_in, f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_promise_exec_return > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_promise_exec_return DESC, s.fan_in DESC LIMIT :lim"""),
(
    "async-callback-in-array-method",
    "An async function passed where the return value is discarded (forEach, emitters, timers)",
    "ANSWERS call sites that hand an ASYNC function to an API that will not\n"
    ...
    """SELECT s.name, s.n_async_arg AS async_args,
        s.n_listener_add AS listener_adds, s.n_timer_set AS timers_set,
        s.n_callbacks AS callback_args, s.fan_in, s.is_handler AS handler,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_async_arg > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_async_arg DESC, s.n_listener_add DESC, s.fan_in DESC
    LIMIT :lim"""),
(
    "regex-from-input",
    "new RegExp built from non-literal data: ReDoS and injection by construction",
    "ANSWERS patterns compiled at run time from a variable\n"
    ...
    """SELECT s.name, s.n_regex_dynamic AS dynamic_regexes,
        (SELECT COUNT(*) FROM user_input_sites u
         WHERE u.symbol_id = s.id) AS input_reads,
        s.n_regex_lit AS regex_literals,
        s.regex_in_loop, s.fan_in, s.is_handler AS handler,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_regex_dynamic > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_regex_dynamic DESC, input_reads DESC LIMIT :lim"""),
(
    "sendfile-traversal-surface",
    "res.sendFile/res.download in functions that read request input (express taint)",
    "ANSWERS the express-shaped path traversal: a handler chain that reads\n"
    ...
    """SELECT s.name, s.n_send_file AS file_sends,
        (SELECT COUNT(*) FROM user_input_sites u
         WHERE u.symbol_id = s.id) AS input_reads,
        s.n_dynamic_open AS fs_dynamic_opens, s.is_handler AS handler,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_send_file > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY input_reads DESC, s.n_send_file DESC LIMIT :lim"""),
(
    "lazy-require-in-function",
    "require() at call depth: a dependency the import graph cannot show (n/global-require)",
    "ANSWERS literal require() calls INSIDE function bodies. Each one is a\n"
    ...
    """SELECT s.name, s.n_lazy_require AS lazy_requires,
        s.n_require_dynamic AS dynamic_requires, s.n_calls AS calls_,
        s.fan_in, s.is_handler AS handler,
        COALESCE(m.name,'') AS module_,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_lazy_require > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_lazy_require DESC, s.fan_in DESC LIMIT :lim"""),
(
    "async-route-handler",
    "Async handlers/middleware on routes: Express 4 does not catch their rejections",
    "ANSWERS route registrations whose terminal callback is async. Express 4\n"
    ...
    """SELECT r.method, r.path, r.handler, r.n_middlewares AS middlewares,
        r.handler_inline AS inline,
        (SELECT COUNT(*) FROM symbols s WHERE s.name = r.handler
           AND s.kind IN ('function','method','closure')) AS defs_in_tree,
        COALESCE(m.name,'') AS module_,
        f.path || ':' || r.line AS at
    FROM routes r
    JOIN files f ON f.id = r.file_id
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE r.handler_async = 1 AND f.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY r.n_middlewares DESC, r.line LIMIT :lim"""),
(
    "route-handler-unresolved",
    "A route names its handler, and nothing in this tree defines it",
    "ANSWERS registrations like `app.get('/x', showUser)` where no function\n"
    ...
    JOIN files f ON f.id = r.file_id
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE r.handler_inline = 0 AND r.handler <> ''
      AND NOT EXISTS (SELECT 1 FROM symbols s WHERE s.name = r.handler
                        AND s.kind IN ('function','method','closure'))
      AND f.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY r.line LIMIT :lim"""),
(
    "repeating-async-timer",
    "setInterval with an async callback: an uncollected rejection every tick",
    "ANSWERS timers whose callback is an async function (timers.is_async_cb).\n"
    ...
    """SELECT COALESCE(s.name,'(module scope)') AS in_fn, t.api, t.kind,
        t.at_module_scope AS module_scope, t.in_loop,
        t.is_repeating AS repeating, t.is_assigned AS handle_kept,
        COALESCE(s.is_async, 0) AS fn_is_async,
        f.path || ':' || t.line AS at
    FROM timers t
    JOIN files f ON f.id = t.file_id
    LEFT JOIN symbols s ON s.id = t.symbol_id
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE t.op = 'set' AND t.is_async_cb = 1
      AND f.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY t.is_repeating DESC, t.in_loop DESC, t.at_module_scope DESC
    LIMIT :lim""")
]
...
(
    "async-surface",
    "Where the async surface concentrates, module by module",
    "ANSWERS how much of each module speaks in promises: the share of async\n"
    ...
    """SELECT m.name AS module_,
        COUNT(*) AS fns,
        SUM(s.is_async) AS async_fns,
        CAST(100.0 * SUM(s.is_async) / NULLIF(COUNT(*), 0) AS INT)
            AS pct_async,
        COALESCE(SUM(s.n_await), 0) AS awaits,
        COALESCE(SUM(s.await_in_loop), 0) AS awaits_in_loop,
        COALESCE(SUM(s.n_promise_chain), 0) AS then_links,
        COALESCE(SUM(s.n_catch_handler), 0) AS catch_handlers,
        COALESCE(SUM(s.n_promise_all), 0) AS batches
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    JOIN modules m ON m.id = s.module_id
    WHERE s.kind IN ('function','method','closure')
      AND f.is_test = 0 AND f.is_generated = 0 AND m.name LIKE :mod
    GROUP BY m.id
    ORDER BY async_fns DESC, then_links DESC LIMIT :lim"""),
(
    "throw-catch-balance",
    "Throws vs catch blocks, module by module: where errors can escape",
    "ANSWERS the arithmetic of error propagation per module: how many throw\n"
    ...
    """SELECT m.name AS module_,
        COALESCE(SUM(s.n_throw), 0) AS throws_,
        COALESCE(SUM(s.n_try), 0) AS try_blocks,
        COALESCE(SUM(s.n_catch), 0) AS catch_blocks,
        COALESCE(SUM(s.n_catch_empty), 0) AS empty_catches,
        CAST(100.0 * MIN(COALESCE(SUM(s.n_catch), 0),
                         COALESCE(SUM(s.n_throw), 0))
             / NULLIF(COALESCE(SUM(s.n_throw), 0), 0) AS INT)
            AS pct_paired
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    JOIN modules m ON m.id = s.module_id
    ...
    GROUP BY m.id
    HAVING throws_ > 0
    ORDER BY throws_ DESC, empty_catches DESC LIMIT :lim"""),
(
    "blocking-surface",
    "Event-loop blocking operations per module: the latency triage map",
    "ANSWERS where the *Sync calls, exec calls and JSON megaparses live,\n"
    ...
    """SELECT m.name AS module_,
        COALESCE(SUM(s.n_sync_calls), 0) AS sync_calls,
        COALESCE(SUM(s.n_fs_sync), 0) AS sync_fs,
        COALESCE(SUM(s.n_exec), 0) AS exec_calls,
        COALESCE(SUM(s.n_json_parse), 0) AS json_ops,
        COALESCE(SUM(s.io_in_loop), 0) AS io_in_loop,
        COALESCE(SUM(s.n_child_process), 0) AS child_process,
        (COALESCE(SUM(s.n_sync_calls), 0) * 3
         + COALESCE(SUM(s.n_fs_sync), 0) * 2
         + COALESCE(SUM(s.n_exec), 0) * 2
         + COALESCE(SUM(s.n_json_parse), 0)) AS blocking_weight
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    JOIN modules m ON m.id = s.module_id
    WHERE f.is_test = 0 AND f.is_generated = 0 AND m.name LIKE :mod
    GROUP BY m.id
    HAVING blocking_weight > 0
    ORDER BY blocking_weight DESC LIMIT :lim"""),
(
    "env-coupling",
    "process.env reads per module: the configuration-at-distance map",
    "ANSWERS which modules reach for the environment directly and how many\n"
    ...
    """SELECT m.name AS module_,
        COALESCE(SUM(s.n_env_read), 0) AS env_reads,
        COUNT(DISTINCT CASE WHEN s.n_env_read > 0 THEN s.id END)
            AS reader_fns,
        COUNT(DISTINCT CASE WHEN s.n_env_read > 0 AND s.is_handler = 1
                            THEN s.id END) AS handlers_reading_env,
        COUNT(DISTINCT CASE WHEN s.n_env_read > 0 AND s.is_entrypoint = 1
                            THEN s.id END) AS entry_readers
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    JOIN modules m ON m.id = s.module_id
    WHERE f.is_test = 0 AND f.is_generated = 0 AND m.name LIKE :mod
    GROUP BY m.id
    HAVING env_reads > 0
    ORDER BY env_reads DESC, handlers_reading_env DESC LIMIT :lim"""),
(
    "listener-balance",
    "Event-listener add/remove balance per module: the retention ledger",
    "ANSWERS the module-level ledger behind retention-leak-frontier: how\n"
    ...
    """WITH agg AS (
        SELECT f.module_id,
            SUM(CASE WHEN l.op = 'add' THEN 1 ELSE 0 END) AS adds,
            SUM(CASE WHEN l.op = 'remove' THEN 1 ELSE 0 END) AS removes,
            SUM(CASE WHEN l.op = 'add' AND l.handler_inline = 1
                      AND l.has_signal = 0 THEN 1 ELSE 0 END) AS inline_anon,
            SUM(CASE WHEN l.op = 'add' AND l.at_module_scope = 1
                     THEN 1 ELSE 0 END) AS at_scope,
            SUM(CASE WHEN l.op = 'add' AND l.in_loop = 1
                     THEN 1 ELSE 0 END) AS in_loop
        FROM listeners l
        JOIN files f ON f.id = l.file_id
        WHERE f.is_test = 0
        GROUP BY f.module_id)
    SELECT m.name AS module_, a.adds, a.removes, a.inline_anon,
        a.at_scope, a.in_loop,
        MAX(0, a.adds - a.removes) AS unbalanced
    FROM agg a
    JOIN modules m ON m.id = a.module_id
    WHERE m.name LIKE :mod
    ORDER BY unbalanced DESC, a.inline_anon DESC LIMIT :lim"""),
(
    "input-surface-map",
    "Where request input enters the code, by kind and module",
    "ANSWERS the attack surface in one table: how many Express-style input\n"
    ...
    """SELECT m.name AS module_,
        COUNT(*) FILTER (WHERE u.kind = 'query') AS query_reads,
        COUNT(*) FILTER (WHERE u.kind = 'body') AS body_reads,
        COUNT(*) FILTER (WHERE u.kind = 'header') AS header_reads,
        COUNT(*) FILTER (WHERE u.kind = 'cookie') AS cookie_reads,
        COUNT(*) FILTER (WHERE u.kind = 'form') AS form_reads,
        COUNT(*) FILTER (WHERE u.kind = 'path') AS path_reads,
        COUNT(DISTINCT u.symbol_id) AS fns_reading_input,
        COUNT(DISTINCT u.file_id) AS files_reading_input
    FROM user_input_sites u
    JOIN files f ON f.id = u.file_id
    JOIN modules m ON m.id = f.module_id
    WHERE f.is_test = 0 AND m.name LIKE :mod
    GROUP BY m.id
    ORDER BY fns_reading_input DESC, body_reads DESC LIMIT :lim"""),
(
    "promise-chain-depth",
    "The longest .then chains: debugging cost ranked per function",
    "ANSWERS which functions carry the deepest promise chains. Every\n"
    ...
    """SELECT s.name, s.n_promise_chain AS chain_links, s.n_then AS then_sites,
        s.n_catch_handler AS handlers, s.n_await AS awaits,
        s.n_then_in_loop AS then_in_loop, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_promise_chain >= 2
      AND s.kind IN ('function','method','closure')
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_promise_chain DESC, s.n_then DESC, s.fan_in DESC
    LIMIT :lim"""),
(
    "callback-fanout",
    "Callback and closure density: the indirection meter",
    "ANSWERS how much of a function's work is delegated to nameless callbacks\n"
    ...
    """SELECT s.name, s.n_callbacks AS callback_args, s.n_closures AS closures,
        s.n_lambda AS lambdas, s.max_nesting AS nest,
        s.n_callbacks * (1 + s.max_nesting) AS indirection,
        s.fan_in, s.cyclomatic AS cyclo,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_callbacks >= 3 AND s.kind IN ('function','method')
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY indirection DESC, s.n_callbacks DESC LIMIT :lim"""),
(
    "production-reach",
    "Share of each module's functions that production code actually reaches",
    "ANSWERS the reachability ledger: per module, how many functions have at\n"
    ...
    """WITH perfn AS (
        SELECT s.id, s.module_id, s.fan_in,
            (SELECT COUNT(DISTINCT CASE WHEN cf.is_test = 0
                                         AND cs.is_test = 0
                                        THEN e.caller_id END)
             FROM edges e
             JOIN symbols cs ON cs.id = e.caller_id
             JOIN files cf ON cf.id = cs.file_id
             WHERE e.callee_id = s.id AND e.is_self = 0) AS prod_callers
        FROM symbols s
        JOIN files f ON f.id = s.file_id
        WHERE s.kind IN ('function','method','closure')
          AND f.is_test = 0 AND f.is_generated = 0)
    SELECT m.name AS module_,
        COUNT(*) AS fns,
        SUM(CASE WHEN p.prod_callers = 0 AND p.fan_in = 0 THEN 1 ELSE 0 END)
            AS unreferenced,
        SUM(CASE WHEN p.prod_callers = 0 AND p.fan_in > 0 THEN 1 ELSE 0 END)
            AS test_only,
        CAST(100.0 * SUM(CASE WHEN p.prod_callers > 0 THEN 1 ELSE 0 END)
             / NULLIF(COUNT(*), 0) AS INT) AS pct_prod_reachable
    FROM perfn p
    JOIN modules m ON m.id = p.module_id
    WHERE m.name LIKE :mod
    GROUP BY m.id
    ORDER BY pct_prod_reachable ASC, test_only DESC LIMIT :lim"""),
(
    "module-coupling",
    "Import-graph coupling per module: internal edges and package weight",
    "ANSWERS how wired-in each module is: how many sibling files it imports,\n"
    ...
    """SELECT m.name AS module_,
        (SELECT COUNT(DISTINCT i.target_id)
         ...
         WHERE sf.module_id = m.id AND i.is_external = 0
           AND i.target_id IS NOT NULL AND sf.is_test = 0)
            AS imports_internal,
        (SELECT COUNT(DISTINCT i.file_id)
         FROM imports i
         JOIN files tf ON tf.id = i.target_id
         JOIN files sf ON sf.id = i.file_id
         WHERE tf.module_id = m.id AND i.is_external = 0
           AND sf.is_test = 0 AND tf.is_test = 0)
            AS imported_by_files,
        (SELECT COUNT(DISTINCT i.target)
         FROM imports i
         JOIN files sf ON sf.id = i.file_id
         WHERE sf.module_id = m.id AND i.is_external = 1
           AND sf.is_test = 0)
            AS external_packages,
        (SELECT COUNT(DISTINCT i.target_id)
         FROM imports i
         JOIN files sf ON sf.id = i.file_id
         WHERE sf.module_id = m.id AND i.is_external = 0
        ...
        + (SELECT COUNT(DISTINCT i.file_id)
           ...
           WHERE tf.module_id = m.id AND i.is_external = 0
            ...
            AS coupling
    FROM modules m
    WHERE m.name LIKE :mod AND m.kind = 'source'
    ORDER BY coupling DESC, external_packages DESC LIMIT :lim"""),
(
    "middleware-chain-depth",
    "Route registrations ranked by middleware chain length",
    "ANSWERS the depth of the middleware stack each route assembles before\n"
    ...
    """SELECT r.method, r.path, r.n_middlewares AS middlewares, r.handler,
        r.handler_inline AS inline,
        COALESCE(m.name,'') AS module_,
        f.path || ':' || r.line AS at
    FROM routes r
    JOIN files f ON f.id = r.file_id
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE r.n_middlewares > 0 AND f.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY r.n_middlewares DESC, r.line LIMIT :lim"""),
(
    "route-surface",
    "API shape per module: methods, middleware mounts, async handlers",
    "ANSWERS the HTTP surface each module exposes: how many routes by\n"
    ...
    """SELECT m.name AS module_,
        COUNT(*) AS routes_,
        SUM(CASE WHEN r.method = 'get' THEN 1 ELSE 0 END) AS get_,
        SUM(CASE WHEN r.method = 'post' THEN 1 ELSE 0 END) AS post_,
        SUM(CASE WHEN r.method IN ('put','patch') THEN 1 ELSE 0 END)
            AS put_patch,
        SUM(CASE WHEN r.method = 'delete' THEN 1 ELSE 0 END) AS delete_,
        SUM(CASE WHEN r.method = 'use' THEN 1 ELSE 0 END)
            AS middleware_mounts,
        SUM(r.handler_async) AS async_handlers,
        COUNT(DISTINCT r.file_id) AS files_
    FROM routes r
    JOIN files f ON f.id = r.file_id
    JOIN modules m ON m.id = f.module_id
    WHERE f.is_test = 0 AND m.name LIKE :mod
    GROUP BY m.id
    ORDER BY routes_ DESC, middleware_mounts DESC LIMIT :lim""")
]
