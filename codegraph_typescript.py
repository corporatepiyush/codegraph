...
#: Line prefixes that mark a comment line, for the per-file comment_lines
#: count. Hoisted to module level: the tuple was rebuilt once per LINE of
#: every file read.
...
def discover(analyzer: Analyzer, root: str, db: sqlite3.Connection,
    ...
    for dirpath, dirnames, filenames in os.walk(root, onerror=_walk_error):
        ...
        for fn in sorted(filenames):
            ...
            # One pass for blank / code / comment lines and max line length;
            # this used to be four full passes (two of them re-running
            # lstrip/strip per line).
            blank = 0
            cmt = 0
            n_code = 0
            maxlen = 0
            for l in lines:
                ...
                if not l.strip():
                    ...
                    n_code += 1
                    if l.lstrip()[:3] in _LINE_COMMENT_PREFIXES:
                        cmt += 1
            ...
            file_rows.append(
                (fid, rel, os.path.dirname(rel) or ".", fn, ext, analyzer.LANG,
                 mid, st.st_size, len(lines), n_code,
                 blank, cmt, maxlen,
...
def scan_markers(rec: FileRec, bufs: Buffers) -> None:
    ...
    The splitlines() list is parked on the record for parse_file_extra, which
    used to split the same text a second time a few milliseconds later; the
    build loop drops it together with rec.text/rec.data.
    """
    lines = rec.text.splitlines()
    rec._scan_lines = lines
...
class TreeSitterAnalyzer(Analyzer):
    ...
    def __init__(self) -> None:
        ...
        # -- dispatch sets for `measure`, built once, not once per body ------
        # `measure()` used to rebuild eleven set()s on every call -- once per
        # function body, type body and module scope. The tables are class
        # constants, so the sets are loop-invariant: build them here, as
        # frozensets (hash-set membership in the hot walker).
        self._loops_set = frozenset(self.LOOP_NODES)
        self._branches_set = frozenset(self.BRANCH_NODES)
        self._nests_set = frozenset(self.NEST_NODES)
        self._calls_set = frozenset(self.CALL_NODES)
        self._operators_set = frozenset(self.OPERATOR_NODES)
        self._strings_set = frozenset(self.STRING_NODES)
        self._numbers_set = frozenset(self.NUMBER_NODES)
        self._comments_set = frozenset(self.COMMENT_NODES)
        self._ifs_set = frozenset(self.IF_NODES)
        self._prune_set = (frozenset(self.FUNC_KINDS)
                           | frozenset(self.TYPE_KINDS))
        #: node types whose subtrees `measure` should never skip
        self._special_set = (self._calls_set | self._operators_set
                             | self._strings_set | self._numbers_set
                             | self._comments_set)
        #: `on_node` hook gate: node types the language's on_node inspects.
        #: None (the base default) means "always call on_node", so a subclass
        #: that overrides on_node with new branches keeps working unchanged.
        self._on_node_gate: Optional[frozenset] = getattr(
            self, "ON_NODE_TYPES", None)
        # COUNTERS and FLAG_NODES are both keyed by node type and both feed
        # `counts`; `measure` used to probe the two dicts separately for
        # every node. One merged dict answers with one lookup -- valid only
        # when no node type appears in both, so overlapping tables keep the
        # two-lookup path (where the flag would overwrite the counter's
        # bump, in that order).
        self._counter_flag: Optional[dict[str, tuple[str, bool]]]
        if set(self.COUNTERS) & set(self.FLAG_NODES):
            self._counter_flag = None
        else:
            merged: dict[str, tuple[str, bool]] = {}
            for _t, _col in self.COUNTERS.items():
                merged[_t] = (_col, False)
            for _t, _col in self.FLAG_NODES.items():
                merged[_t] = (_col, True)
            self._counter_flag = merged
    ...
    def emit_module_scope(self, root: Any, rec: FileRec,
        ...
        if not self.MODULE_SCOPE_SYMBOL:
            return
        stats = self.measure(root, rec, prune=self._prune_set)
    ...
    def walk_scope(self, node: Any, rec: FileRec, db: sqlite3.Connection,
        ...
        a crash.
        """
        # Local-bound dispatch: three attribute walks per node in the old
        # form, and `named_children` (a fresh Python list per node) built
        # twice -- once to reverse, once to extend. `named_child(i)` walking
        # i downward pushes children in the SAME first-child-first order as
        # `reversed(named_children)` did, so symbol ids come out identical.
        ...
        stack: list[tuple[Any, Scope]] = []
        kids = node.named_children
        for i in range(len(kids) - 1, -1, -1):
            stack.append((kids[i], scope))
        while stack:
            cur, sc = stack.pop()
            ...
            if kind:
                ...
                body = body or cur
                n = body.named_child_count
                while n:
                    n -= 1
                    stack.append((body.named_child(n), inner))
                continue
            ...
            if kind:
                ...
                while n:
                    ...
                    stack.append((body.named_child(n), inner))
                continue
            n = cur.named_child_count
            while n:
                n -= 1
                stack.append((cur.named_child(n), sc))
    ...
    def emit_function(self, node: Any, rec: FileRec, db: sqlite3.Connection,
        ...
        # One signature decode serves both the flags pass and the symbol row:
        # both used to call signature_of() separately.
        signature = self.signature_of(node, rec)
        m.update(self.function_flags(node, rec, scope, signature))
    ...
    def emit_type(self, node: Any, rec: FileRec, db: sqlite3.Connection,
        ...
        name = self.node_name(node, rec) or "(anonymous)"
        qual = scope.qual_prefix + name
        # One decode of the declaration text serves the flags pass, the
        # signature header and type_extra: all three used to re-decode the
        # same span (a large type can be hundreds of lines).
        txt = _txt(node, rec.data)
        ...
        body = node.child_by_field_name(self.BODY_FIELD) or node
        bstats = self.measure(body, rec, prune=self._prune_set)
        ...
        m.update(self.type_flags(node, rec, scope, txt))
        doc = self.docstring_lines(node, rec)
        m["n_doc_lines"] = doc
        m["has_doc"] = 1 if doc else 0
        sid = self.insert_symbol(
            db, rec, name, kind, node, qual, scope.symbol_id,
            txt.split("{")[0].strip()[:300], "",
            self.visibility_of(node, rec), m)
        self.emit_attributes(node, rec, sid, bufs)
        ...
        self.type_extra(node, rec, db, bufs, sid, scope, txt)
        self.by_name.setdefault(name, []).append(
            (sid, rec.fid, rec.mid, scope.type_name))
        self.by_qual[qual] = sid
        return sid
    ...
    def insert_symbol(self, db: sqlite3.Connection, rec: FileRec, name: str,
        ...
        for k, dflt in spec:
            v = m_get(k, dflt)
            row_append(int(v) if type(v) is bool else v)
    ...
    def measure(self, body: Any, rec: FileRec,
        ...
        st = BodyStats()
        src = rec.data
        # Loop-invariant dispatch tables, built once per analyzer (see
        # __init__), local-bound here so the walk touches no attribute paths.
        loops = self._loops_set
        branches = self._branches_set
        nests = self._nests_set
        calls = self._calls_set
        counters_get = self.COUNTERS.get
        operators = self._operators_set
        strings = self._strings_set
        numbers = self._numbers_set
        comments = self._comments_set
        # One union lookup replaces five for the two most common node shapes
        # (plain interior nodes and leaves): a node outside this set is
        # neither a call, operator, string, number nor comment, so only the
        # leaf test can apply. Same branches, same order, same results.
        special = self._special_set
        counter_flag = self._counter_flag
        if counter_flag is not None:
            counters_get = None
            cf_get = counter_flag.get
        else:
            counters_get = self.COUNTERS.get
            cf_get = None
        flags_get = self.FLAG_NODES.get
        ifs = self._ifs_set

        else_field = self.ELSE_FIELD
        want_elif = bool(ifs) and bool(else_field)
        on_node_gate = self._on_node_gate
        ...
        while True:
            ...
            if named and (t in loops
                          or (extra_loops and node.id in extra_loops)):
                loop_stack.append(depth)
                loop_depth += 1
                st.max_loop_depth = max(st.max_loop_depth, loop_depth)
                st.cyclomatic += 1
                st.cognitive += max(1, len(nest_stack))
                counts["n_loops"] = counts.get("n_loops", 0) + 1
            elif named and t in branches:
                st.cyclomatic += 1
                st.cognitive += 1 if is_elif else max(1, len(nest_stack))
                counts["n_branches"] = counts.get("n_branches", 0) + 1
                if is_elif:
                    counts["n_elif"] = counts.get("n_elif", 0) + 1
                if loop_depth:
                    counts["branch_in_loop"] = \
                        counts.get("branch_in_loop", 0) + 1
            ...
            if cf_get is not None:
                ...
                if cf is not None:
                    ...
                    if _isflag:
                        ...
                        counts[_col] = counts.get(_col, 0) + 1
            else:
                key = counters_get(t)
                if key is not None:
            ...
            # Per-node language hook, gated for languages that declare the
            # exact set of node types their on_node inspects: the hook used
            # to be invoked for every node in every body, where the vast
            # majority are identifiers and punctuation it never looks at.
            if on_node_gate is None or t in on_node_gate:
    ...
    def on_call(self, node: Any, src: bytes, st: BodyStats,
        ...
        # Async callback arguments: an async function passed where a
        # sync (void-returning) one is expected -- forEach, on, setTimeout,
        # Promise executor. Checked before the dynamic early-return below so
        # `new Promise(async ...)` (fn field is `constructor`, unseen by
        # CALL_FUNC_FIELD) is covered too. Indexed named_child(i) iteration:
        # `named_children` built a fresh list for every call node.
        _args = node.child_by_field_name("arguments")
        _async_args = 0
        if _args is not None:
            for _i in range(_args.named_child_count, 0, -1):
                _a = _args.named_child(_i - 1)
                if _a.type in ("arrow_function", "function_expression",
                               "function") \
                        and src[_a.start_byte:_a.start_byte + 5] == b"async":
                    _async_args += 1
        if _async_args:
            st.bump("n_async_callback", _async_args)
            _fn0 = node.child_by_field_name(self.CALL_FUNC_FIELD)
            if _fn0 is None:
                # new_expression names its callee `constructor`, not `function`
                _fn0 = node.child_by_field_name("constructor")
            if _fn0 is not None and _txt(_fn0, src).strip() == "Promise":
                st.bump("n_async_executor")
        ...
        # One lower() copy serves the zip check, the logger check and the
        # auth-marker scan, which used to re-lower the name -- the auth scan
        # once per marker, six times per call.
        _nl = name.lower()
        if name == "super" or name.startswith("super."):
            # super() / super.x() -- the strict-override pairing signal
        ...
        if "unzip" in _nl or ("zip" in _nl and _b in (
        ...
        if _b in LOG_LEVELS and "logger" in _nl:
    ...
    def type_flags(self, node: Any, rec: FileRec,
                   scope: Scope, txt: str = "") -> dict[str, Any]:
        return {}
...
#: Handler-shape detection for function_flags. Precompiled: two re.search()
#: calls per function went through the re module's pattern cache each time.
_HANDLER_REQ_RE = re.compile(r'\b(req|request|ctx|event)\b')
_HANDLER_RES_RE = re.compile(r'\b(res|response|reply)\b')
...
#: The capture is shape-based (object text), so an untyped `req: any`
#: handler fires the same rows as a typed one; a base/member not in these
#: maps records nothing.
...
#: Listener/timer method families for the on_node call branch. The same six
#: membership tests used to run as two separate if/elif ladders per call
#: expression -- one to bump a counter, one to decide the listeners-table op.
_LISTENER_ADD_BASES = frozenset(("addEventListener", "addListener", "on",
                                 "subscribe", "observe", "once"))
_LISTENER_REMOVE_BASES = frozenset(("removeEventListener", "removeListener",
                                    "off", "unsubscribe", "disconnect"))
_TIMER_SET_BASES = frozenset(("setTimeout", "setInterval", "setImmediate",
                              "requestAnimationFrame"))
_TIMER_CLEAR_BASES = frozenset(("clearTimeout", "clearInterval",
                                "cancelAnimationFrame"))
_PROMISE_COMBINERS = frozenset(("all", "allSettled", "race"))
...
class TypeScriptAnalyzer(TreeSitterAnalyzer):
    ...
    ("n_elif", "INT NOT NULL DEFAULT 0"),
        ...
        # -- async-boundary pack: promises crossing call edges ---------------
        ("n_async_callback", "INT NOT NULL DEFAULT 0"),    # async fn as arg
        ("n_async_executor", "INT NOT NULL DEFAULT 0"),    # new Promise(async
        ("n_super_calls", "INT NOT NULL DEFAULT 0"),       # super() / super.x
        ("n_promise_all_in_loop", "INT NOT NULL DEFAULT 0"),
        # -- accessor-shape pack: get/set pairing and purity -----------------
        ("is_getter", "INT NOT NULL DEFAULT 0"),
        ("is_setter", "INT NOT NULL DEFAULT 0"),
    )
...
-- Await expressions whose operand is a CALL: one row per `await f()` site.
-- `base` is the callee's last dot-segment, so a join on symbols.name is exact
-- membership rather than a substring scan. This is the awaited half of the
-- promise-boundary picture: callsites says who calls f, this says who awaits
-- it -- the transitive-floating-promise queries read the gap between them.
CREATE TABLE awaited_calls(
    symbol_id INT NOT NULL REFERENCES symbols(id),
    file_id INT NOT NULL REFERENCES files(id),
    name TEXT NOT NULL,
    base TEXT NOT NULL,
...
CREATE INDEX idx_awaited_base ON awaited_calls(base);
CREATE INDEX idx_awaited_sym ON awaited_calls(symbol_id);
CREATE INDEX idx_lis_async ON listeners(symbol_id) WHERE is_async=1;
...
"""
    ...
    def __init__(self) -> None:
        ...
        #: per-file memos for node_name / _is_exported. Both climb the same
        # few parents on every call -- emit_function, walk_scope,
        # visibility_of, function_flags, type_flags and type_extra each ask
        # the same question about the same node. Cleared per file in
        # parse_file: tree-sitter node ids are tree-local, so a cache
        # carried across files could answer with another file's symbol.
        self._name_cache: dict[int, str] = {}
        self._exp_cache: dict[int, bool] = {}
    ...
    def parse_file(self, rec: FileRec, db: sqlite3.Connection,
                   bufs: Buffers) -> None:
        # tree-sitter node ids are only unique within one parsed tree.
    ...
    def node_name(self, node: Any, rec: FileRec) -> str:
        ...
        Memoised per file: emit_function, walk_scope, visibility_of and
        function_flags all ask for the same node's name, and the arrow-function
        path climbs up to four `parent` hops per answer. Same tree, same
        answer, one climb.
        """
            ...
            while cur is not None and hops < 4:
                if cur.type in ("variable_declarator", "public_field_definition",
                    ...
                    for field in ("name", "left", "key"):
                        ...
                        if got is not None:
                            v = _txt(got, rec.data).strip().strip('"\'`')
                            cache[nid] = v
                            return v
    ...
    def _is_exported(self, node: Any) -> bool:
        ...
        while p is not None and p.type in ("variable_declarator",
            ...
            p = p.parent
        v = p is not None and p.type == "export_statement"
        cache[nid] = v
        return v

    def function_flags(self, node: Any, rec: FileRec,
                       scope: Scope, sig: str = "") -> dict[str, Any]:
        src = rec.data
        ...
        # get/set are ANONYMOUS keyword children of method_definition;
        # `override` arrives as a named override_modifier node. All three
        # change what calls to this member mean (accessor pairing,
        # super-contract), so they are captured on the declaration itself.
        _kinds = {c.type for c in node.children}
        is_get = 1 if "get" in _kinds else 0
        is_set = 1 if "set" in _kinds else 0
        is_ovr = 1 if "override_modifier" in _kinds else 0
        ...
        # btxt feeds ONLY is_component, whose first conjunct is
        # name[:1].isupper() -- so the body decode (600 chars of it) is
        # computed only when that conjunct can pass.
        if name[:1].isupper() and body is not None:
            btxt = _txt(body, src)[:600]
        else:
            btxt = ""
        return dict(
            ...
            is_handler=1 if _HANDLER_REQ_RE.search(ptxt) \
                            and _HANDLER_RES_RE.search(ptxt) else 0,
    ...
    def type_flags(self, node: Any, rec: FileRec,
        ...
        depth = 0
        max_depth = 0
        counter_get = _TYPE_COUNTERS.get
        cursor = node.walk()
        while True:
            n = cursor.node
            t = n.type
    ...
    def on_node(self, node: Any, src: bytes, st: BodyStats,
                loop_depth: int, nest: int) -> None:
        t = node.type
        # Branch order is free: every arm tests equality with a DISTINCT node
        # type, so the arms can sit in frequency order instead. Calls and
        # member accesses dominate real bodies and used to sit at the END of
        # a twelve-arm chain.
        if t == "call_expression":
            ...
            elif base in _PROMISE_COMBINERS and "Promise" in name:
                st.bump("n_promise_all")
                if loop_depth:
                    # Promise.all inside a loop fans out n x args
                    # concurrent -- the unbounded-concurrency shape.
                    st.bump("n_promise_all_in_loop")
            elif base == "then":
                st.bump("n_then_chain")
            elif base in _LISTENER_ADD_BASES:
                st.bump("n_listener_add")
                op = "add"
            elif base in _LISTENER_REMOVE_BASES:
                st.bump("n_listener_remove")
                op = "remove"
            elif base in _TIMER_SET_BASES:
                st.bump("n_timer_set")
            elif base in _TIMER_CLEAR_BASES:
            ...
            if op:
                ...
                if args is not None and args.named_children:
                    ...
                    lis_async = int(any(
                        a.type in ("arrow_function", "function_expression",
                                   "function")
                        and src[a.start_byte:a.start_byte + 5] == b"async"
                        for a in args.named_children))
                st.extra_rows.append(
                    ("listener", name.rsplit(".", 1)[0][:80], ev, op,
                     node.start_point[0] + 1, int(loop_depth > 0), lis_async))
            ...
            if prop is not None:
                ...
                # user_input_sites rows: captured on THIS walk instead of a
                # second pass per symbol (function_extra used to re-walk the
                # whole body).
                obj = node.child_by_field_name("object")
                if obj is not None:
                    base_obj = _txt(obj, src)
                    kind = USER_INPUT_MEMBERS.get(p)
            ...
            # Awaited-call rows: the awaited half of the promise-boundary
            # picture, captured on the same walk (see awaited_calls).
            kids = node.named_children
            if kids and kids[0].type == "call_expression":
                callee = kids[0].child_by_field_name("function")
                if callee is not None:
                    nm = _txt(callee, src).strip()
                    if nm:
                        st.extra_rows.append(
                            ("await", nm[:160], nm.rsplit(".", 1)[-1][:120],
                             node.start_point[0] + 1))
        elif t == "this":
            st.bump("n_this_refs")
    ...
    #: The exact node types on_node inspects. Declaring them lets `measure`
    #: skip the hook for the identifiers and punctuation that dominate every
    #: body -- the hook used to be called for all ~1.5M nodes per repo and
    #: matched on twelve `t ==` tests before returning None from most.
    ON_NODE_TYPES = frozenset((
        "call_expression", "member_expression", "binary_expression",
        "as_expression", "assignment_expression", "template_substitution",
        "await_expression", "this", "subscript_expression", "union_type",
        "comment", "regex",
    ))
    ...
    def function_extra(self, node: Any, rec: FileRec, db: sqlite3.Connection,
        ...
        for row in stats.extra_rows:
            if row[0] == "listener":
                _, recv, ev, op, line, in_loop, is_async = row
                bufs.rows("listeners").append(
                    (sid, rec.fid, op, recv, ev,
                     line, in_loop, is_async))
            elif row[0] == "input":
                ...
                _, nm, base, line = row
                bufs.rows("awaited_calls").append(
                    (sid, rec.fid, nm, base, line))
        ...
        for i, mem in enumerate(members):
            ...
            bufs.fields.append(
                (sid, i, mname[:120], mtype[:200], "",
                 mem.start_point[0] + 1, 0, int("readonly" in mtxt[:20]),
                 int("readonly" not in mtxt[:20] and mem.type != "method_definition"
                     and mem.type != "method_signature"),
    ...
    def emit_attributes(self, node: Any, rec: FileRec, sid: int,
                        bufs: Buffers) -> None:
        # A decorator on `export class X` hangs off the export_statement,
        # not the class_declaration -- without climbing, every exported
        # decorated class read as undecorated. A decorator on a MEMBER hangs
        # off the class_body, immediately before the member it decorates;
        # "immediately" = no other member starts between them.
        decos: list[Any] = []
        par = node.parent
        if par is not None and par.type == "export_statement":
            decos.extend(c for c in par.children if c.type == "decorator")
        if par is not None and par.type == "class_body":
            members = par.named_children
            for c in par.children:
                if c.type != "decorator" or c.end_byte > node.start_byte:
                    continue
                between = [m0 for m0 in members
                           if m0.id != node.id
                           and m0.start_byte >= c.end_byte
                           and m0.end_byte <= node.start_byte]
                if not between:
                    decos.append(c)
        decos.extend(c for c in node.children if c.type == "decorator")
        seen: set[int] = set()
        for c in decos:
            if c.id in seen:
                continue
            ...
            bufs.attributes.append(
                (sid, rec.fid, name[:120], txt[:200],
                 c.start_point[0] + 1))

    def parse_imports(self, root: Any, rec: FileRec, bufs: Buffers) -> None:
        src = rec.data
        # Inlined cursor walk, same visit order as walk_cursor(root): the
        # generator handed back a (node, depth) tuple per node -- hundreds of
        # thousands per repo -- and this loop read neither element.
        cursor = root.walk()
        while True:
            n = cursor.node
            t = n.type
            if t == "import_statement":
                ...
                txt = _txt(n, src)
                # Pre-order cursor scan, same visit order as walk(n): the
                # walk() form materialised a children list per node.
                names = []
                sub = n.walk()
                _done = False
                while not _done:
                    ...
                    if sub.goto_first_child():
                        continue
                    while not sub.goto_next_sibling():
                        if not sub.goto_parent():
                            _done = True
                            break
                for spec in names:
                ...
                is_star = "*" in txt.partition("from")[0]
                ...
                # Pre-order scan of the statement subtree for export
                # specifiers -- same visit order as walk(), without
                # materialising a children list per node. Declarations with
                # no specifier list are common here and can be large, so the
                # scan is cursor-only: no per-node allocations at all.
                names: list[str] = []
                sub = n.walk()
                done = False
                while not done:
    ...
    def parse_file_extra(self, root: Any, rec: FileRec,
        ...
        # Reuse the split scan_markers already did for this same file; if it
        # is gone (direct call), split here as before.
        lines = getattr(rec, "_scan_lines", None)
        if lines is None:
            lines = rec.text.splitlines()
        for i, line in enumerate(lines, 1):
            # Lexical gate: every SUPPRESS_RE match contains "ts-" or
            # "eslint", so two plain substring scans replace a
            # case-insensitive alternation regex on the lines that match
            # neither -- which is nearly all of them.
            if "ts-" not in line and "eslint" not in line:
                continue
    ...
    def flush_extra(self, db: sqlite3.Connection, bufs: Buffers) -> None:
        for tbl, sql in (
            ...
            ("secret_candidates",
             ...
             "VALUES(?,?,?,?)"),
            ("awaited_calls",
             "INSERT INTO awaited_calls(symbol_id,file_id,name,base,line) "
             "VALUES(?,?,?,?,?)"),
        ):
...
(
    ...
    """SELECT s.name, s.n_any_total AS any_count,
        s.n_any_params AS any_params,
        s.returns_any AS returns_any,
        s.n_as_any AS as_any,
        s.n_non_null AS non_null_assertions,
        s.fan_in, s.is_exported,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_any_total > 0 AND f.is_test=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_any_total DESC, s.fan_in DESC LIMIT :lim"""),
(
    "ts-ignore",
    ...
    """SELECT s.name, s.n_ts_ignore AS ts_ignores,
        s.n_ts_expect_error AS ts_expect_errors,
        s.n_eslint_disable AS eslint_disables,
        s.n_suppressions AS total_suppressions,
        s.fan_in, s.cyclomatic AS cyclo,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE (s.n_ts_ignore + s.n_ts_expect_error) > 0 AND f.is_test=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC,
        s.n_ts_ignore + s.n_ts_expect_error DESC LIMIT :lim"""),
(
    "non-null-assertion",
    ...
    """SELECT s.name, s.n_non_null AS non_null_assertions,
        s.n_optional_chain AS optional_chains,
        s.n_non_null AS non_null_assertions,
        s.fan_in, s.cyclomatic AS cyclo,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_non_null > 2 AND f.is_test=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_non_null DESC, s.fan_in DESC LIMIT :lim"""),
(
    ...
    """SELECT s.name, s.n_as_assertion AS as_assertions,
        s.n_as_any AS as_any,
        s.n_angle_assertion AS angle_assertions,
        s.n_satisfies AS satisfies,
        s.fan_in, s.cyclomatic AS cyclo,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE (s.n_as_assertion + s.n_angle_assertion) > 2 AND f.is_test=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC,
        s.n_as_assertion + s.n_angle_assertion DESC LIMIT :lim"""),
(
    "floating-promise",
    ...
    """SELECT s.name, s.n_floating_promise AS floating_promises,
        s.n_await AS awaits, s.n_then_chain AS then_chains,
        s.n_promise_all AS promise_alls,
        s.fan_in, s.cyclomatic AS cyclo,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_floating_promise > 0 AND f.is_test=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC, s.n_floating_promise DESC LIMIT :lim"""),
(
    "async-in-loop",
    ...
    """SELECT s.name, s.await_in_loop AS awaits_in_loop,
        s.n_await AS total_awaits,
        s.n_loops AS loops, s.n_promise_all AS promise_alls,
    ...
    WHERE s.await_in_loop > 0 AND f.is_test=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.await_in_loop DESC, s.n_loops DESC LIMIT :lim"""),
(
    "dom-xss-sink",
    ...
    """SELECT s.name,         s.n_innerhtml AS innerhtml_uses,
        s.n_json_parse AS json_parses,
        s.fan_in, s.is_handler,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_innerhtml > 0 AND f.is_test=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC, s.n_innerhtml DESC LIMIT :lim"""),
(
    "redos-surface",
    ...
    """SELECT s.name, s.n_regex_redos AS redos_patterns,
        s.n_json_parse AS json_parses,
        s.fan_in, s.is_handler,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
...
(
    ...
    """SELECT f.path,
        COUNT(*) FILTER (WHERE s.kind IN ('interface','type','enum','module'))
            AS type_space,
        COUNT(*) FILTER (WHERE s.kind IN ('class','function','method','closure'))
            AS value_space,
        COUNT(*) AS total,
        CAST(100.0 * COUNT(*) FILTER (WHERE s.kind IN
...
(
    ...
    """SELECT s.name AS enum_name, f.path,
        COUNT(*) FILTER (WHERE substr(em.value,1,1) IN ('''','"'))
            AS string_members,
        COUNT(*) FILTER (WHERE substr(em.value,1,1) NOT IN ('''','"'))
            AS numeric_members,
...
(
    ...
    """SELECT s.name, s.n_child_process AS child_process_calls,
        s.n_exec AS exec_calls, s.fan_in, s.sloc,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
...
# -- async-boundary pack: promises across call edges -------------------------
(
    "sync-caller-of-async",
    ...
    """SELECT callee.name AS async_callee, caller.name AS caller_fn,
        caller.is_async AS caller_is_async, caller.n_await AS caller_awaits,
        caller.n_returns AS caller_returns, callee.fan_in AS callee_fan_in,
        f.path || ':' || cs.line AS at
    FROM callsites cs
    JOIN symbols callee ON callee.id = cs.callee_id AND callee.is_async = 1
    JOIN symbols caller ON caller.id = cs.caller_id
        AND (caller.is_async = 0 OR caller.n_await = 0)
    JOIN files f ON f.id = caller.file_id
    LEFT JOIN modules m ON m.id = caller.module_id
    WHERE f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY callee.fan_in DESC, caller.fan_in DESC, cs.line
    LIMIT :lim"""),
(
    "never-awaited-api",
    ...
    """WITH async_fns AS (
        SELECT s.id, s.fan_in FROM symbols s
        WHERE s.is_async = 1 AND s.fan_in > 0),
    caller_stats AS (
        SELECT cs.callee_id,
            COUNT(DISTINCT cs.caller_id) AS callers,
            SUM(CASE WHEN c.is_async = 0 OR c.n_await = 0
                     THEN 1 ELSE 0 END) AS non_awaiting
        FROM callsites cs
        JOIN symbols c ON c.id = cs.caller_id
        GROUP BY cs.callee_id)
SELECT s.name AS async_api, cs.callers AS callers,
    cs.non_awaiting AS non_awaiting_callers, s.is_exported AS exported,
    s.n_await AS awaits, s.fan_in,
    f.path || ':' || s.line_start AS at
FROM async_fns a
JOIN caller_stats cs ON cs.callee_id = a.id
JOIN symbols s ON s.id = a.id
JOIN files f ON f.id = s.file_id
LEFT JOIN modules m ON m.id = s.module_id
WHERE cs.non_awaiting = cs.callers
  AND f.is_test = 0 AND f.is_generated = 0
  AND COALESCE(m.name,'') LIKE :mod
ORDER BY cs.callers DESC, s.fan_in DESC LIMIT :lim"""),
(
    ...
    """SELECT s.name, s.n_async_callback AS async_arguments,
        s.n_listener_add AS listener_adds, s.n_timer_set AS timer_starts,
        s.n_calls AS calls, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_async_callback > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_async_callback DESC, s.fan_in DESC LIMIT :lim"""),
(
    ...
    """SELECT s.name AS registrar, l.op, l.event, l.target, l.in_loop,
        CASE WHEN l.op = 'add' THEN 1 ELSE 0 END AS is_add,
        s.fan_in, f.path || ':' || l.line AS at
    FROM listeners l
    JOIN symbols s ON s.id = l.symbol_id
    JOIN files f ON f.id = l.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE l.is_async = 1
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY is_add DESC, s.fan_in DESC, l.line LIMIT :lim"""),
(
    ...
    """SELECT s.name, s.n_async_executor AS async_executors,
        s.n_promise_all AS promise_alls, s.n_await AS awaits,
        s.fan_in, f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_async_executor > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_async_executor DESC, s.fan_in DESC LIMIT :lim"""),
(
    "await-of-non-thenable",
    ...
    """SELECT s.name AS awaiting_fn, a.base AS awaited_callee,
        c.return_type AS declared_return,
        CASE WHEN instr(c.return_type, 'void') > 0 THEN 2 ELSE 1 END
            AS severity,
        COUNT(DISTINCT c.id) AS matching_defs,
        s.fan_in, f.path || ':' || a.line AS at
    FROM awaited_calls a
    JOIN symbols s ON s.id = a.symbol_id
    JOIN symbols c ON c.name = a.base
        AND c.kind IN ('function', 'method', 'closure')
        AND c.is_async = 0
        AND instr(c.return_type, 'Promise') = 0
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id, a.base, a.line
    ORDER BY severity DESC, s.fan_in DESC, a.line LIMIT :lim"""),
(
    ...
    """SELECT p.name AS class_, COUNT(DISTINCT cs.callee_id) AS async_callees,
        GROUP_CONCAT(DISTINCT callee.name) AS callees_called,
        ctor.n_io AS io, ctor.n_net AS net, ctor.fan_in,
        f.path || ':' || ctor.line_start AS at
    FROM callsites cs
    JOIN symbols ctor ON ctor.id = cs.caller_id AND ctor.name = 'constructor'
    JOIN symbols callee ON callee.id = cs.callee_id AND callee.is_async = 1
    LEFT JOIN symbols p ON p.id = ctor.parent_id
    JOIN files f ON f.id = ctor.file_id
    LEFT JOIN modules m ON m.id = ctor.module_id
    WHERE f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY ctor.id
    ORDER BY async_callees DESC, ctor.fan_in DESC LIMIT :lim"""),
(
    "import-cycle-chain",
    ...
    """WITH RECURSIVE walk(a, b, depth, path, first_line) AS (
        SELECT i.file_id, i.target_id, 1, fa.path || '|' || ft.path, i.line
        FROM imports i
        JOIN files fa ON fa.id = i.file_id
        JOIN files ft ON ft.id = i.target_id
        WHERE i.is_type_only = 0 AND i.target_id IS NOT NULL
        UNION ALL
        SELECT w.a, i.target_id, w.depth + 1, w.path || '|' || ft2.path,
            w.first_line
        FROM walk w
        JOIN imports i ON i.file_id = w.b
            AND i.is_type_only = 0 AND i.target_id IS NOT NULL
        JOIN files ft2 ON ft2.id = i.target_id
        WHERE w.depth < 5
          AND (ft2.id = w.a
               OR instr(w.path || '|', ft2.path || '|') = 0)),
    closed AS (
        SELECT a, b, depth, path, first_line FROM walk
        WHERE b = a AND depth >= 3 AND depth <= 5)
SELECT fa.path AS start_file, c.path AS cycle, c.depth AS hops,
    fa.path || ':' || c.first_line AS at
FROM closed c
JOIN files fa ON fa.id = c.a
LEFT JOIN modules m ON m.id = fa.module_id
WHERE fa.is_test = 0 AND fa.is_generated = 0
  AND COALESCE(m.name,'') LIKE :mod
ORDER BY c.depth ASC, c.path ASC LIMIT :lim"""),
(
    "unprotected-async-entry",
    ...
    """SELECT s.name, s.fan_in, s.n_await AS awaits, s.n_calls AS calls,
        s.is_exported AS exported, s.is_handler AS handler,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.is_async = 1 AND s.n_await > 0
      AND s.n_try = 0 AND s.n_catch = 0 AND s.n_then_chain = 0
      AND s.fan_in > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC, s.n_await DESC LIMIT :lim"""),
(
    "unconsumed-disposables",
    ...
    """SELECT fa.name AS factory, fa.return_type,
        (SELECT COUNT(DISTINCT cs.caller_id) FROM callsites cs
          WHERE cs.callee_id = fa.id) AS callers,
        (SELECT COUNT(DISTINCT cs.caller_id) FROM callsites cs
          JOIN symbols c2 ON c2.id = cs.caller_id
          WHERE cs.callee_id = fa.id AND c2.n_dispose_call = 0)
            AS callers_never_dispose,
        fa.fan_in, f.path || ':' || fa.line_start AS at
    FROM symbols fa
    JOIN files f ON f.id = fa.file_id
    LEFT JOIN modules m ON m.id = fa.module_id
    WHERE fa.return_type <> ''
      AND (instr(fa.return_type, 'Disposable') > 0
           OR instr(fa.return_type, 'IDisposable') > 0)
      AND fa.fan_in > 0
      AND (SELECT COUNT(DISTINCT cs.caller_id) FROM callsites cs
           JOIN symbols c2 ON c2.id = cs.caller_id
           WHERE cs.callee_id = fa.id AND c2.n_dispose_call = 0) > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY callers_never_dispose DESC, fa.fan_in DESC LIMIT :lim"""),
(
    "unvalidated-dto-into-io",
    ...
    """SELECT s.name AS consumer, t.name AS unvalidated_dto, p.name AS arg,
        (SELECT COUNT(*) FROM fields fl WHERE fl.symbol_id = t.id)
            AS dto_fields,
        (s.n_io + s.n_net + s.n_fetch) AS io_calls,
        s.is_handler AS handler, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM params p
    JOIN symbols s ON s.id = p.symbol_id AND s.kind IN ('function','method')
    JOIN symbols t ON t.name = LTRIM(p.type, ': ')
        AND t.kind IN ('class', 'interface')
        AND NOT EXISTS (SELECT 1 FROM attributes att
                        WHERE att.symbol_id = t.id)
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE p.type <> ''
      AND (s.n_io > 0 OR s.n_net > 0 OR s.n_fetch > 0 OR s.is_handler = 1)
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY (s.fan_in + s.n_io + s.n_net + s.n_fetch) DESC, dto_fields DESC
    LIMIT :lim"""),
(
    ...
    """WITH mut AS (
        SELECT symbol_id, COUNT(*) AS n_mutable FROM fields
        WHERE is_mutable = 1 GROUP BY symbol_id),
    class_input AS (
        SELECT m2.parent_id AS class_id, SUM(i.n) AS input_reads
        FROM symbols m2
        JOIN (SELECT symbol_id, COUNT(*) AS n FROM user_input_sites
              GROUP BY symbol_id) i ON i.symbol_id = m2.id
        WHERE m2.parent_id IS NOT NULL
        GROUP BY m2.parent_id)
SELECT c.name AS class_, mut.n_mutable AS mutable_fields,
    ci.input_reads AS input_reads_in_methods,
    c.is_exported AS exported, c.fan_in,
    f.path || ':' || c.line_start AS at
FROM symbols c
JOIN mut ON mut.symbol_id = c.id
JOIN class_input ci ON ci.class_id = c.id
JOIN files f ON f.id = c.file_id
LEFT JOIN modules m ON m.id = c.module_id
WHERE c.kind = 'class'
  AND f.is_test = 0 AND f.is_generated = 0
  AND COALESCE(m.name,'') LIKE :mod
ORDER BY (ci.input_reads * mut.n_mutable) DESC, c.fan_in DESC LIMIT :lim"""),
("unsound-reachable", "as any, ! and suppressions reachable from exported or handler entry points",
    ...
    """WITH RECURSIVE walk(root, sym, depth) AS (
        ...
        FROM walk w JOIN edges e ON e.caller_id = w.sym
        WHERE w.depth < 4 AND e.is_self = 0),      -- depth bound: 4 hops
    reach AS (SELECT sym, MIN(depth) AS depth FROM walk GROUP BY sym)
SELECT s.name, r.depth AS hops, s.n_as_any AS as_any,
    s.n_non_null AS bang, s.n_suppressions AS suppressions,
    s.n_any_total AS anys, s.fan_in,
    f.path || ':' || s.line_start AS at
FROM reach r
JOIN symbols s ON s.id = r.sym
JOIN files f ON f.id = s.file_id
LEFT JOIN modules m ON m.id = s.module_id
WHERE r.depth > 0
  AND (s.n_as_any > 0 OR s.n_non_null > 0 OR s.n_suppressions > 0)
  AND f.is_test = 0 AND f.is_generated = 0
  AND COALESCE(m.name,'') LIKE :mod
ORDER BY (s.n_as_any*3 + s.n_non_null + s.n_suppressions) DESC, r.depth ASC
LIMIT :lim"""),
(
    "decorated-method-zero-caller",
    ...
    """SELECT s.name, p.name AS class_,
        COUNT(DISTINCT a.name) AS n_decorators,
        GROUP_CONCAT(DISTINCT a.name) AS decorators,
        s.is_static AS static, s.sloc, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    ...
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.kind = 'method' AND s.fan_in = 0
      AND s.is_declaration_only = 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id
    ORDER BY s.sloc DESC, n_decorators DESC LIMIT :lim"""),
(
    ...
    """SELECT p.name AS class_, s.name AS property,
        MAX(s.is_getter) AS has_getter, MAX(s.is_setter) AS has_setter,
        COUNT(DISTINCT s.id) AS declarations,
        MAX(s.fan_in) AS fan_in,
        f.path || ':' || MIN(s.line_start) AS at
    FROM symbols s
    JOIN symbols p ON p.id = s.parent_id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.kind = 'method' AND (s.is_getter = 1 OR s.is_setter = 1)
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY p.id, s.name
    HAVING MAX(s.is_getter) <> MAX(s.is_setter)
    ORDER BY declarations DESC, MAX(s.fan_in) DESC LIMIT :lim"""),
(
    "async-side-effect-getter",
    ...
    """SELECT s.name AS getter, p.name AS class_, s.n_await AS awaits,
        s.n_io AS io, s.n_net AS net, s.n_fetch AS fetches, s.n_dom AS dom,
        s.fan_in, f.path || ':' || s.line_start AS at
    FROM symbols s
    LEFT JOIN symbols p ON p.id = s.parent_id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.is_getter = 1
      AND (s.n_await + s.n_io + s.n_net + s.n_fetch + s.n_dom) > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY (s.n_await*3 + s.n_io + s.n_net + s.n_fetch + s.n_dom) DESC,
...
(
    ...
    """SELECT s.name, s.fan_in, s.n_returns AS returns_,
        s.n_then_chain AS then_chains, s.is_exported AS exported,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.is_async = 1 AND s.n_await = 0 AND s.n_then_chain = 0
      AND s.fan_in > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC, s.n_returns DESC LIMIT :lim"""),
(
    "then-without-catch",
    ...
    """SELECT s.name, s.n_then_chain AS then_calls, s.n_await AS awaits,
        s.n_catch AS catch_blocks, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_then_chain > 0 AND s.n_catch = 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_then_chain DESC, s.fan_in DESC LIMIT :lim"""),
(
    "discarded-promise-chain",
    ...
    """SELECT s.name, s.n_then_chain AS then_calls, s.return_type,
        s.fan_in, s.is_exported AS exported,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_then_chain > 0 AND s.fan_in > 0
      AND (s.return_type = '' OR instr(s.return_type, 'void') > 0)
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_then_chain DESC, s.fan_in DESC LIMIT :lim"""),
(
    "handler-unguarded-await",
    ...
    """SELECT s.name, s.n_await AS awaits, s.n_catch AS catch_blocks,
        (SELECT COUNT(*) FROM user_input_sites u WHERE u.symbol_id = s.id)
            AS input_reads,
        s.fan_in, f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.is_handler = 1 AND s.n_await > 0 AND s.n_try = 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_await DESC, input_reads DESC LIMIT :lim"""),
(
    ...
    """SELECT s.name, s.n_timer_set AS timers_started,
        s.n_async_callback AS async_arguments,
        s.n_timer_clear AS timers_cleared, s.timer_in_loop AS timers_in_loop,
        s.fan_in, f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_timer_set > 0 AND s.n_async_callback > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_timer_set DESC, s.n_async_callback DESC LIMIT :lim"""),
(
    ...
    """SELECT s.name AS method, p.name AS class_, p.fan_in AS class_fan_in,
    ...
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.is_override = 1 AND s.n_super_calls = 0 AND s.kind = 'method'
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY p.fan_in DESC, s.sloc DESC LIMIT :lim"""),
(
    ...
    """WITH pair AS (
        SELECT DISTINCT ca.parent_id AS a, cb.parent_id AS b
        FROM edges e
        JOIN symbols ca ON ca.id = e.caller_id AND ca.parent_id IS NOT NULL
        JOIN symbols cb ON cb.id = e.callee_id AND cb.parent_id IS NOT NULL
        WHERE e.is_self = 0 AND ca.parent_id <> cb.parent_id),
    mutual AS (
        SELECT p1.a, p1.b FROM pair p1
        JOIN pair p2 ON p2.a = p1.b AND p2.b = p1.a
        WHERE p1.a < p1.b)
SELECT fa.name AS class_a, fb.name AS class_b,
    (SELECT COUNT(*) FROM pair
      WHERE (pair.a = fa.id AND pair.b = fb.id)
         OR (pair.a = fb.id AND pair.b = fa.id)) AS coupled_pairs,
    fa.fan_in AS fan_in_a, fb.fan_in AS fan_in_b,
    f.path || ':' || fa.line_start AS at
FROM mutual mx
JOIN symbols fa ON fa.id = mx.a
JOIN symbols fb ON fb.id = mx.b
JOIN files f ON f.id = fa.file_id
LEFT JOIN modules m ON m.id = fa.module_id
WHERE f.is_test = 0 AND f.is_generated = 0
  AND COALESCE(m.name,'') LIKE :mod
ORDER BY coupled_pairs DESC, fan_in_a DESC LIMIT :lim"""),
(
    "mixed-await-styles",
    ...
    """SELECT s.name, s.n_await AS awaits, s.n_then_chain AS then_calls,
        s.n_catch AS catch_blocks, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_await > 0 AND s.n_then_chain > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY (s.n_await + s.n_then_chain) DESC, s.fan_in DESC LIMIT :lim"""),
(
    "async-void-api",
    ...
    """SELECT s.name, s.return_type, s.n_await AS awaits, s.fan_in,
        s.is_exported AS exported, s.is_handler AS handler,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.is_async = 1 AND instr(s.return_type, 'void') > 0
      AND s.fan_in > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC, s.n_await DESC LIMIT :lim"""),
(
    ...
    """SELECT s.name, s.n_promise_all_in_loop AS all_in_loop,
        s.n_promise_all AS total_promise_all, s.await_in_loop AS awaits_in_loop,
        s.n_async_callback AS async_arguments, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_promise_all_in_loop > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_promise_all_in_loop DESC, s.await_in_loop DESC LIMIT :lim"""),
(
    "module-scope-io",
    ...
    """SELECT s.name AS module_init, s.n_io AS io, s.n_net AS net,
        s.n_fetch AS fetches, s.n_fs_sync AS sync_fs,
        s.n_child_process AS child_proc, s.n_timer_set AS timers_started,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.kind = 'module' AND s.name = '<module>'
      AND (s.n_io + s.n_net + s.n_fetch + s.n_fs_sync
           + s.n_child_process + s.n_timer_set) > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY (s.n_io + s.n_net + s.n_fetch + s.n_fs_sync
              + s.n_child_process + s.n_timer_set) DESC LIMIT :lim"""),
(
    "untyped-handler-input",
    ...
    """SELECT s.name, s.n_any_params AS any_params,
        (SELECT COUNT(*) FROM user_input_sites u WHERE u.symbol_id = s.id)
            AS input_reads,
        s.is_handler AS handler, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.is_handler = 1 AND s.n_any_params > 0
      AND s.kind IN ('function', 'method')
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_any_params DESC, input_reads DESC LIMIT :lim"""),
(
    "deep-then-chain",
    ...
    """SELECT s.name, s.n_then_chain AS then_calls, s.n_await AS awaits,
        s.n_catch AS catch_blocks, s.cognitive AS cog, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_then_chain >= 3
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_then_chain DESC, s.cognitive DESC LIMIT :lim"""),
]
...
(
    ...
    """SELECT COALESCE(m.name,'(root)') AS module,
        ...
        CAST(100.0 * SUM(s.is_async) / NULLIF(COUNT(DISTINCT s.id), 0) AS INT)
            AS pct_async,
        COALESCE(SUM(s.n_await), 0) AS awaits,
        COALESCE(SUM(s.n_then_chain), 0) AS then_chains,
        COALESCE(SUM(s.n_promise_all), 0) AS promise_alls,
        COALESCE(SUM(s.n_floating_promise), 0) AS floating,
        COALESCE(SUM(s.n_async_callback), 0) AS async_callback_args,
    ...
    GROUP BY m.id
    HAVING async_fns > 0
    ORDER BY async_fns DESC, then_chains DESC LIMIT :lim"""),
(
    "unsound-index",
    ...
    """SELECT COALESCE(m.name,'(root)') AS module,
        COALESCE(SUM(s.n_as_any), 0) AS as_any,
        COALESCE(SUM(s.n_non_null), 0) AS non_null_bang,
        COALESCE(SUM(s.n_suppressions), 0) AS suppressions,
        COALESCE(SUM(s.n_any_total), 0) AS any_refs,
        COALESCE(SUM(s.sloc), 0) AS sloc,
        CAST(1000.0 * (COALESCE(SUM(s.n_as_any), 0) * 3
            + COALESCE(SUM(s.n_non_null), 0)
            + COALESCE(SUM(s.n_suppressions), 0) * 2
            + COALESCE(SUM(s.n_any_total), 0))
            / NULLIF(COALESCE(SUM(s.sloc), 0), 0) AS INT)
            AS unsound_per_kloc
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.kind IN ('function','method','closure')
      ...
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY m.id
    HAVING unsound_per_kloc > 0
    ORDER BY unsound_per_kloc DESC LIMIT :lim"""),
(
    "attack-surface",
    ...
    """WITH inp AS (
        ...
        JOIN symbols s2 ON s2.id = u.symbol_id
        GROUP BY s2.module_id)
SELECT COALESCE(m.name,'(root)') AS module,
    SUM(s.is_handler) AS handlers,
    COALESCE(MAX(inp.n), 0) AS input_reads,
    COALESCE(SUM(s.n_fetch), 0) AS fetches,
    COALESCE(SUM(s.n_fs_sync), 0) AS sync_fs_calls,
    COALESCE(SUM(s.n_child_process), 0) AS child_proc_calls,
    COALESCE(SUM(s.n_console_log), 0) AS console_calls,
    COALESCE(SUM(s.n_auth_call), 0) AS auth_calls
FROM symbols s
JOIN files f ON f.id = s.file_id
LEFT JOIN modules m ON m.id = s.module_id
LEFT JOIN inp ON inp.mid = m.id
WHERE s.kind IN ('function','method','closure')
  AND f.is_test = 0 AND f.is_generated = 0
  AND COALESCE(m.name,'') LIKE :mod
GROUP BY m.id
HAVING handlers > 0 OR input_reads > 0
ORDER BY (handlers * 2 + input_reads) DESC LIMIT :lim"""),
(
    "exposure-map",
    ...
    """WITH RECURSIVE reach(root, sym, depth) AS (
        ...
        WHERE r.depth < 6 AND e.is_self = 0),      -- depth bound: 6 hops
    agg AS (SELECT root, COUNT(DISTINCT sym) AS reach_n
            FROM reach GROUP BY root)
SELECT COALESCE(m.name,'(root)') AS module,
    COUNT(DISTINCT agg.root) AS entry_points,
    SUM(agg.reach_n) AS symbols_reachable,
    MAX(agg.reach_n) AS max_from_one_entry,
    ROUND(AVG(agg.reach_n), 1) AS avg_from_entry
FROM agg
JOIN symbols s ON s.id = agg.root
LEFT JOIN modules m ON m.id = s.module_id
WHERE COALESCE(m.name,'') LIKE :mod
GROUP BY m.id
ORDER BY symbols_reachable DESC LIMIT :lim"""),
(
    "untested-risk",
    ...
    """SELECT s.name, s.risk_score AS risk, s.fan_in, s.cyclomatic AS cyclo,
        s.n_any_total AS anys, s.n_exec AS exec_calls,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.kind IN ('function','method','closure') AND s.risk_score > 0
      AND f.is_test = 0 AND s.is_test = 0 AND f.is_generated = 0
      AND NOT EXISTS (
          SELECT 1 FROM edges e
          JOIN symbols c ON c.id = e.caller_id
          JOIN files cf ON cf.id = c.file_id
          WHERE e.callee_id = s.id AND (cf.is_test = 1 OR c.is_test = 1))
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.risk_score DESC LIMIT :lim"""),
(
    "generic-hotspots",
    ...
    """SELECT s.name, s.kind, s.n_generic_params AS tparams,
        s.n_type_args AS type_args, s.n_callsites AS call_sites,
        s.n_type_args * MAX(s.n_callsites, 1) + s.n_generic_params
            AS instantiation_load,
        s.n_conditional_type AS conds, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE (s.n_type_args > 0 OR s.n_generic_params > 0)
      AND f.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY instantiation_load DESC, s.n_type_args DESC LIMIT :lim"""),
(
    "handler-latency-budget",
    ...
    "    the totals.",
    """SELECT s.name, s.n_await AS awaits, s.await_in_loop AS awaits_in_loop,
        s.n_net AS net_calls, s.n_fetch AS fetches, s.n_fs_sync AS sync_fs,
        s.n_io AS io,
        (s.n_await + s.await_in_loop * 5 + s.n_net + s.n_fetch + s.n_io)
            AS serialized_ops,
        s.fan_in, f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.is_handler = 1
      AND (s.n_await + s.n_net + s.n_fetch + s.n_io) > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY serialized_ops DESC LIMIT :lim"""),
(
    "heavy-constructors",
    ...
    """SELECT s.name AS constructor, p.name AS class_, s.cyclomatic AS cyclo,
        s.n_calls AS calls, s.n_io AS io, s.n_net AS net,
        s.n_params, s.sloc, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    LEFT JOIN symbols p ON p.id = s.parent_id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.name = 'constructor' AND s.kind = 'method'
      AND (s.cyclomatic > 3 OR s.n_io > 0 OR s.n_net > 0 OR s.n_calls > 5)
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY (s.cyclomatic + s.n_io * 3 + s.n_net * 3 + s.n_calls) DESC
    LIMIT :lim"""),
(
    "framework-magic",
    ...
    "    defect count.",
    """WITH dec AS (
        SELECT symbol_id, COUNT(DISTINCT name) AS n
        FROM attributes GROUP BY symbol_id)
SELECT COALESCE(m.name,'(root)') AS module,
    COUNT(DISTINCT s.id) AS methods,
    SUM(CASE WHEN d.n > 0 THEN 1 ELSE 0 END) AS decorated_methods,
    SUM(CASE WHEN d.n > 0 AND s.fan_in = 0 THEN 1 ELSE 0 END)
        AS decorated_zero_caller,
    CAST(100.0 * SUM(CASE WHEN d.n > 0 THEN 1 ELSE 0 END)
        / NULLIF(COUNT(DISTINCT s.id), 0) AS INT) AS pct_decorated
FROM symbols s
JOIN files f ON f.id = s.file_id
LEFT JOIN dec d ON d.symbol_id = s.id
LEFT JOIN modules m ON m.id = s.module_id
WHERE s.kind = 'method'
  AND f.is_test = 0 AND f.is_generated = 0
  AND COALESCE(m.name,'') LIKE :mod
GROUP BY m.id
HAVING decorated_methods > 0
ORDER BY decorated_methods DESC LIMIT :lim"""),
(
    "error-handling-map",
    ...
    """SELECT COALESCE(m.name,'(root)') AS module,
        ...
        SUM(CASE WHEN s.is_async = 1 AND s.n_await > 0 AND s.n_try = 0
                 THEN 1 ELSE 0 END) AS async_fns_no_try
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.kind IN ('function','method','closure')
      ...
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY m.id
    HAVING throws + catches > 0
    ORDER BY (throws + catches) DESC LIMIT :lim"""),
]
