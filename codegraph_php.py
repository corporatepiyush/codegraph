...
# csv, json and subprocess are imported lazily, at their only use sites:
# --csv, composer manifests (and --json) and --install-deps. Every run paid
# the import cost; only those paths read the modules.
...
def _walk_nodes(node: Any) -> Iterator[Any]:
    """`walk_cursor` without the depth: same traversal, no per-node tuple.

    The two whole-file passes (`parse_imports`, `parse_file_extra`) never
    read the depth, so yielding one tuple per node -- 275k of them on a
    125-file tree -- was allocation with no reader.
    """
    cursor = node.walk()
    while True:
        yield cursor.node
...
def discover(analyzer: Analyzer, root: str, db: sqlite3.Connection,
    ...
    for dirpath, dirnames, filenames in os.walk(root, onerror=_walk_error):
        ...
        for fn in sorted(filenames):
            ...
            # what is read, not what parsing it costs. Scanned with find()
            # rather than data.split(b"\n"), which materialised a list
            # holding every line of the file just to throw it away.
            ...
            # blank / comment / sloc / max-line-len used to be four passes
            # over every line (str.strip() twice per line); one pass, same
            # numbers. st == l.strip() and st[:3] == l.lstrip()[:3] for every
            # line, so the comment test is unchanged.
            blank = sloc = cmt = 0
            max_line = 0
            _cmt_prefixes = ("//", "#", "/*", "*", "*/", '"""',
                             "'''", "--", ";;", "%")
            ...
            # Surrogates are never ASCII, so the overwhelming majority of
            # paths skip the per-character scan entirely.
...
class TreeSitterAnalyzer(Analyzer):
    ...
    #: node types on_node inspects; None means "may inspect anything", so
    #: measure() must call it for every node. A language that declares the
    #: set gets one frozenset probe per node instead of a Python call for
    #: every node in every body.
    ON_NODE_TYPES: Optional[frozenset] = None
    ...
    def __init__(self) -> None:
        ...
        #: measure() used to rebuild nine sets from class tuples on EVERY
        #: call -- once per function body, per class body and once more per
        #: file for module scope. The tables never change after __init__,
        #: so build the membership sets once here. Same lookups, same
        #: semantics, no per-symbol allocation.
        ...
        self._fs_flags = self.FLAG_NODES
        self._prune_kinds = frozenset(self.FUNC_KINDS) | frozenset(
            self.TYPE_KINDS)
        #: One merged probe for the two per-node table lookups in measure().
        #: A type present in BOTH COUNTERS and FLAG_NODES keeps both effects
        #: -- the pair is applied exactly as the two separate lookups were.
        self._counter_flag: dict[str, tuple[Optional[str], Optional[str]]] = {
            k: (v, None) for k, v in self.COUNTERS.items()}
        for _fk, _fv in self.FLAG_NODES.items():
            _prev = self._counter_flag.get(_fk)
            self._counter_flag[_fk] = ((_prev[0] if _prev else None), _fv)
    ...
    def walk_scope(self, node: Any, rec: FileRec, db: sqlite3.Connection,
        ...
        # Attribute lookups hoisted out of the per-node loop: FUNC_KINDS and
        # TYPE_KINDS are class attributes, and `stack.pop`/`stack.append`
        # were re-resolved once per visited node.
        pop = stack.pop
        push = stack.append
    ...
    def insert_symbol(self, db: sqlite3.Connection, rec: FileRec, name: str,
        ...
        row = [sid]
        row.extend(vals)
        # ~110 columns per symbol. bools are the singletons True/False, so
        # an identity test replaces an isinstance call per column; for every
        # other value the row cell is v itself, exactly as before.
        get = m.get
        for k, dflt in spec:
            v = get(k, dflt)
            row.append(1 if v is True else 0 if v is False else v)
    ...
    def measure(self, body: Any, rec: FileRec,
        ...
        counter_flag = self._counter_flag
        ...
        on_node_types = self.ON_NODE_TYPES
        ...
        while True:
            ...
            # so requiring a named node is free everywhere else. The
            # `is_named` property is read only on the branch arms that use
            # it: the conjunctions commute, so the truth table is unchanged
            # and the common node pays one less C-property call.
            ...
            if (t in nests and not is_elif and node is not body
                    and node.is_named):
                nest_stack.append(depth)
                st.max_nesting = max(st.max_nesting, len(nest_stack))
            if (t in loops or node.id in extra_loops) and node.is_named:
                ...
                st.bump("n_loops")
            elif t in branches and node.is_named:
                st.cyclomatic += 1
            ...
            if on_node_types is None or t in on_node_types:
...
HAZARD_CATEGORIES = (
    ...
    "resource",
    # -- pack 3: framework lifecycle. Each name here becomes an n_<cat>
    # column on symbols, materialized from hazards by the base pass, so no
    # walker change is needed to count these.
    "transaction",  # beginTransaction / DB::transaction / commit / rollBack
    "queue",        # dispatch / dispatchSync / Queue::push / Bus::dispatch
    "write",        # ORM/ query-builder mutations: save/create/update/...
    "massassign",   # fill / forceFill -- the mass-assignment front door
    "config",       # env() -- broken at run time once config:cache runs
)
...
HAZARD_CALLS: dict[str, str] = {
    ...
    # -- pack 3: framework lifecycle --------------------------------------
    # Laravel: a job queued inside a transaction races the commit unless it
    # went through afterCommit (docs "Queues / Dispatch After Commit");
    # beginTransaction/commit/rollBack mark the frontier a dispatch may cross.
    # No pre-existing key is redefined here -- every entry is new.
    ...
    # Event/job dispatch. `->dispatch` also fires events, which inside a
    # transaction have exactly the same ordering hazard for queued listeners.
    ...
    "Bus::dispatch": "queue",
    # Eloquent / query-builder mutations: two or more in one call tree with
    # no transaction site is a torn-write frontier.
    ...
    "truncate": "write",
    # The mass-assignment front door (Laravel docs "Mass Assignment"):
    # fill()/forceFill() and the create-family take an array straight into
    # model state; whether that array is the request is a graph question.
    "fill": "massassign", "forceFill": "massassign",
    # Laravel configuration docs: "you should be sure that you are only
    # calling the env function from within your configuration files" -- after
    # config:cache every other call site reads null.
    "env": "config",
}
...
#: Pack 3: Laravel/Symfony service-locator facades. A scoped call whose
#: class is one of these resolves through the container at run time, so
#: `Log::info(...)` in domain code is a hidden global dependency -- the
#: layering fact neither a static call counter nor the unresolved-call
#: counter can show (facades resolve fine; they are still service location).
FACADE_CLASSES = frozenset((
    "DB", "Log", "Cache", "Config", "Route", "Auth", "Session", "Queue",
    "Storage", "Validator", "Event", "View", "Hash", "Cookie", "Crypt",
    "Lang", "File", "URL", "Redirect", "Response", "Input", "Gate", "Blade",
    "Artisan", "Schema", "Bus", "Redis", "Mail", "Notification", "Date",
))

#: Pack 3: callees whose risk lives in an ARGUMENT (arg count / literalness),
#: so on_call must read the arguments node -- the name alone is not evidence.
#: Mirrors the n_dynamic_open check, which started this shape.
ARG_SHAPE_SINKS = frozenset((
    "in_array", "array_search", "setcookie",
    "preg_match", "preg_replace", "preg_split", "preg_match_all",
    "preg_filter", "preg_quote",
    "file_put_contents", "fopen",
))

#: Pack 3: fetch callees whose URL argument is scanned for a clear-text
#: scheme. Kept separate from ARG_SHAPE_SINKS so the membership test reads
#: honestly: these need no literalness check, only the URL text.
CLETEXT_SINKS = frozenset((
    "file_get_contents", "curl_setopt", "curl_init", "fopen",
))
...
#: Node types each hot whole-tree dispatch actually inspects. Every other
#: node -- names, arguments, punctuation-level leaves, the bulk of a tree --
#: used to walk the entire if/elif chain and fail every comparison. One
#: frozenset probe retires the common case first; the chains below are
#: unchanged, so each handled type takes exactly the same branch as before.
_ON_NODE_TYPES = frozenset((
...
_FILE_EXTRA_TYPES = frozenset((
    "variable_name", "function_call_expression", "member_call_expression",
    "nullsafe_member_call_expression", "scoped_call_expression",
    "object_creation_expression", "dynamic_variable_name",
    "include_expression", "include_once_expression", "require_expression",
    "require_once_expression", "variadic_placeholder",
))

#: exactly the types PhpAnalyzer.on_node branches on; measure() uses it to
#: skip the hook call for every other node.
PHP_ON_NODE_TYPES = _ON_NODE_TYPES
...
class PhpAnalyzer(TreeSitterAnalyzer):
    ...
    ON_NODE_TYPES = PHP_ON_NODE_TYPES
    ...
    COUNTERS = {
        ...
        # exit()/die are statements, not calls, so on_call never sees them;
        # this is the one way the debug-residue family can count them.
        "exit_statement": "n_debug_call",
    ...
    ("n_elif", "INT NOT NULL DEFAULT 0"),
        ...
        # -- pack 3: argument-shape sinks and framework coupling. The five
        # hazard categories above cover name-only detection; these ten need
        # the arguments node, the assignment target, or the call's scope.
        ("n_request_input", "INT NOT NULL DEFAULT 0"),    # ->all/->input/...
        ("n_facade_call", "INT NOT NULL DEFAULT 0"),      # Log::/DB::/Cache::
        ("n_debug_call", "INT NOT NULL DEFAULT 0"),       # dd/dump/ray/exit
        ("n_inarray_loose", "INT NOT NULL DEFAULT 0"),    # in_array <3 args
        ("n_cookie_lean", "INT NOT NULL DEFAULT 0"),      # setcookie <3 args
        ("n_preg_dynamic", "INT NOT NULL DEFAULT 0"),     # preg_* var pattern
        ("n_file_write_dyn", "INT NOT NULL DEFAULT 0"),   # var path + write
        ("n_cleartext_fetch", "INT NOT NULL DEFAULT 0"),  # http:// fetch
        ("n_session_write", "INT NOT NULL DEFAULT 0"),    # $_SESSION[k] = v
        ("n_static_prop_write", "INT NOT NULL DEFAULT 0"),  # Cls::$var = v
    )
...
"""
    ...
    def __init__(self) -> None:
        ...
        #: (fid, node start_byte) -> (class-body counts, modifiers). Both
        #: type hooks needed both values, and each used to recompute them:
        #: `_class_body_counts` re-walks every method, property and trait
        #: use in the class body. `emit_type` calls `type_flags` and then
        #: `type_extra` on the SAME node, so one computation serves both.
        #: Cleared per file (node start bytes are only unique within one
        #: tree). Both dicts are read-only at every use site.
        self._type_body_info: dict[tuple[int, int], tuple[dict, set]] = {}
        self.php_version = ""
    ...
    def function_flags(self, node: Any, rec: FileRec,
        ...
        # visibility_of walks the node's children and decodes text; it was
        # called twice here and once more by the caller for the same answer.
    ...
    def _body_info(self, node: Any, rec: FileRec) -> tuple[dict, set]:
        """(class-body counts, modifiers) for one type node, computed once."""
        key = (rec.fid, node.start_byte)
        info = self._type_body_info.get(key)
        if info is None:
            src = rec.data
            body = node.child_by_field_name(self.BODY_FIELD)
            counts = _class_body_counts(body, src) if body is not None else {}
            info = (counts, _modifiers(node, src))
            self._type_body_info[key] = info
        return info

    def type_flags(self, node: Any, rec: FileRec,
                   scope: Scope) -> dict[str, Any]:
        src = rec.data
        counts, mods = self._body_info(node, rec)
        ns = self._namespace_at(rec.fid, node.start_byte)
        return dict(
            is_public=1,
            is_abstract=int("abstract" in mods),
    ...
    def on_call(self, node: Any, src: bytes, st: BodyStats,
        ...
        if t == "function_call_expression":
            ...
            if nm is not None and nm.type == "name" and scope_n is not None:
                ...
                if scope_n.type in ("name", "qualified_name", "relative_scope"):
                    ...
                    # Only this branch knows the scope text: a static call to
                    # a facade class is service location the graph resolves
                    # fine -- and that is exactly why it hides from the
                    # unresolved-call counters.
                    if cls in FACADE_CLASSES:
                        st.bump("n_facade_call")
                else:
        ...
        if name and loop_depth:
            for needle, col in self.LOOP_CALL_COUNTERS.items():
                if needle in name:
                    st.bump(col)
        # `lstrip`/`lower` used to run two and six times per call node
        # respectively; one of each, reused, is the same string work once.
        nb = name.lstrip("->")
        if nb in ESCAPERS or name in ESCAPERS:
        ...
        if _b == "error_log":
            ...
            st.bump("n_log_call")
        _nl = name.lower()
        if any(k in _nl for k in AUTH_MARKERS):
            # G01: an auth-family call -- the marker that makes the
            # unauthenticated-input surface query exclude this function.
            st.bump("n_auth_call")
        if name in ("->all", "->input", "->only", "->except", "->post",
                    "->json"):
            # Request-object reads -- the Laravel/Symfony half of the input
            # surface, which the $_FILES/$_GET capture cannot see.
            st.bump("n_request_input")
        if _b in ("dd", "dump", "ray"):
            # Debug leftovers; exit/die land here via the exit_statement
            # counter, since they are statements and never reach on_call.
            st.bump("n_debug_call")
        if _b in ARG_SHAPE_SINKS or _b in CLETEXT_SINKS:
            # Sinks whose risk is in the ARGUMENTS. The literal test is
            # NODE-SHAPED, not textual: "'/a/' . $p" starts and ends with a
            # quote, so only a lone `string` node counts as a literal (a
            # double-quoted "$p" is an encapsed_string in this grammar --
            # PHP interpolates it, so it is computed by definition).
            args2 = node.child_by_field_name("arguments")
            akids = [] if args2 is None else [
                c for c in args2.named_children if c.type == "argument"]
            n_args = len(akids)
            first_node = (akids[0].named_children[0]
                          if akids and akids[0].named_children else None)
            is_literal = (first_node is not None
                          and len(akids[0].named_children) == 1
                          and first_node.type == "string")
            first_txt = text_of(first_node, src).strip() if first_node \
                is not None else ""
            if _b in ("in_array", "array_search") and n_args < 3:
                # No third argument = loose search: '0' == 0, 'php' == 0
                # pre-8.0, and a needle of 0 matches every string haystack.
                st.bump("n_inarray_loose")
            elif _b == "setcookie" and n_args < 3:
                # No expiry/path/domain/secure/httponly arguments at all.
                st.bump("n_cookie_lean")
            elif (_b.startswith("preg_") and first_node is not None
                    and not is_literal):
                # A computed pattern is regex-injection surface (CodeQL
                # php/regexp-injection) and a ReDoS carrier when it comes
                # from data.
                st.bump("n_preg_dynamic")
            elif _b == "file_put_contents" and first_node is not None \
                    and not is_literal:
                st.bump("n_file_write_dyn")
            elif _b == "fopen" and first_node is not None and not is_literal:
                mode = ""
                if len(akids) > 1 and akids[1].named_children:
                    mode = text_of(
                        akids[1].named_children[0], src).strip().strip("'\"")
                if mode[:1] in ("w", "a", "c", "x", "+"):
                    st.bump("n_file_write_dyn")
            if args2 is not None and _b in CLETEXT_SINKS \
                    and "http://" in text_of(args2, src):
                # SonarPHP S5332: clear-text protocol in a fetch.
                st.bump("n_cleartext_fetch")
        if loop_depth:
            if _b == "in_array":
        ...
        if t == "binary_expression":
            ...
            if left is not None and left.type == "member_access_expression":
                ...
                if nm is not None and nm.type == "variable_name":
                    ...
                    and left.type == "scoped_property_access_expression":
                # Cls::$name = v / self::$name = v -- mutable static state.
                # Under a long-running worker (Octane, Swoole, queue workers)
                # this is cross-request shared memory.
                st.bump("n_static_prop_write")
            elif left is not None and left.type == "subscript_expression":
                for ch in left.named_children:
                    if ch.type == "variable_name" \
                            and _txt(ch, src) == "$_SESSION":
                        # $_SESSION[k] = v -- session state written inside
                        # the request; combined with an input read it is
                        # session poisoning (OWASP session mgmt cheat sheet).
                        st.bump("n_session_write")
                        break
    ...
    def type_extra(self, node: Any, rec: FileRec, db: sqlite3.Connection,
        ...
        c, mods = self._body_info(node, rec)
        body = node.child_by_field_name(self.BODY_FIELD)
    ...
    def parse_imports(self, root: Any, rec: FileRec, bufs: Buffers) -> None:
        ...
        # Spelled inline rather than `for n in _walk_nodes(root)`: the
        # generator resumed 275k times per build cost a frame switch per
        # node. Same cursor traversal, same order, no generator.
        cursor = root.walk()
        done = False
        while not done:
            n = cursor.node
    ...
    def parse_file_extra(self, root: Any, rec: FileRec,
        ...
        # Spelled inline rather than `for n in _walk_nodes(root)`: the
        # generator was resumed once per node -- 275k frame switches per
        # build. Same cursor traversal, same order, no generator. The two
        # early-out guards below are INVERTED (`in` instead of
        # `not in ... continue`) rather than spelled `continue`, because a
        # loop-level continue here would skip the descent at the bottom of
        # the body and stall the cursor on the same node forever.
        cursor = root.walk()
        done = False
        while not done:
            n = cursor.node
            t = n.type
            if t in _FILE_EXTRA_TYPES:
                if t == "variable_name":
                    var = _txt(n, src)
                    if var in SUPERGLOBALS:
                        key = ""
                        par = n.parent
                        ...
                        bufs.rows("superglobal_reads").append(
                            (self._sid_at(n, rec.fid, funcs), rec.fid, var,
                             key, n.start_point[0] + 1,
...
(
    "env-outside-config",
    "env() called outside config paths, with the runtime callers that break under config:cache",
    ...
    """WITH RECURSIVE up(sym, anc, depth) AS (
        -- depth bound 2: two caller hops is enough to prove 'reached at
        -- run time' without dragging in the whole call tree
        SELECT s.id, s.id, 0 FROM symbols s WHERE s.n_config > 0
        UNION
        SELECT u.sym, e.caller_id, u.depth+1
        FROM up u JOIN edges e ON e.callee_id = u.anc
        WHERE u.depth < 2 AND e.is_self = 0)
    SELECT s.name, s.class_name AS class_, s.n_config AS env_reads,
        MIN(CASE WHEN u.anc <> s.id AND a.is_test = 0 THEN u.depth END)
            AS caller_hops,
        COUNT(DISTINCT CASE WHEN u.anc <> s.id AND a.is_test = 0
                            THEN u.anc END) AS callers_up,
        COALESCE(MAX(CASE WHEN u.anc <> s.id AND a.is_test = 0
                          THEN a.is_entrypoint END), 0)
            AS runtime_entry_reaches,
        s.fan_in, s.is_controller AS controller,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN up u ON u.sym = s.id
    JOIN symbols a ON a.id = u.anc
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE f.is_test = 0 AND f.is_generated = 0
      AND instr('/' || f.path, '/config/') = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id
    ORDER BY runtime_entry_reaches DESC, s.fan_in DESC,
        env_reads DESC LIMIT :lim"""),
(
    "dispatch-in-transaction",
    "Queue/event dispatch reachable from a transaction, with afterCommit as counter-evidence",
    ...
    """WITH RECURSIVE walk(tx, sym, depth) AS (
        -- depth bound 3
        SELECT s.id, s.id, 0 FROM symbols s WHERE s.n_transaction > 0
        UNION
        SELECT w.tx, e.callee_id, w.depth+1
        FROM walk w JOIN edges e ON e.caller_id = w.sym
        WHERE w.depth < 3 AND e.is_self = 0)
    SELECT src.name AS opens_tx, sink.name AS dispatches,
        MIN(w.depth) AS hops, sink.n_queue AS dispatch_calls,
        SUM(h.pattern = 'dispatchAfterCommit') AS after_commit_sites,
        COUNT(DISTINCT h.pattern) AS dispatch_kinds,
        GROUP_CONCAT(DISTINCT h.pattern) AS patterns,
        src.n_write AS writes_in_tx_fn, src.fan_in,
        f.path || ':' || sink.line_start AS at
    FROM (SELECT tx, sym, MIN(depth) AS depth FROM walk
          GROUP BY tx, sym) w
    JOIN symbols sink ON sink.id = w.sym AND sink.n_queue > 0
    JOIN symbols src ON src.id = w.tx
    JOIN hazards h ON h.symbol_id = sink.id AND h.category = 'queue'
    JOIN files f ON f.id = sink.file_id
    LEFT JOIN modules m ON m.id = sink.module_id
    WHERE f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    GROUP BY src.id, sink.id
    ORDER BY after_commit_sites ASC, hops ASC, dispatch_calls DESC LIMIT :lim"""),
(
    "multi-write-no-transaction",
    "Two or more write sites in a call tree, no transaction anywhere in the writer",
    ...
    """WITH writes AS (
        SELECT symbol_id, SUM(n) AS n_writes, COUNT(DISTINCT pattern) AS kinds
        FROM hazards WHERE category = 'write' GROUP BY symbol_id),
    callee_writes AS MATERIALIZED (
        SELECT e.caller_id AS sid, SUM(w.n_writes) AS sub_writes
        FROM edges e JOIN writes w ON w.symbol_id = e.callee_id
        WHERE e.is_self = 0 GROUP BY e.caller_id)
    SELECT s.name, s.class_name AS class_, w.n_writes,
        w.kinds AS write_kinds, COALESCE(cw.sub_writes, 0) AS writes_in_callees,
        s.n_transaction AS tx_here, s.is_controller AS controller,
        s.fan_in, f.path || ':' || s.line_start AS at
    FROM writes w
    JOIN symbols s ON s.id = w.symbol_id
    LEFT JOIN callee_writes cw ON cw.sid = s.id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE w.n_writes + COALESCE(cw.sub_writes, 0) >= 2 AND s.n_transaction = 0
      AND s.kind IN ('function','method')
      AND f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    ORDER BY w.n_writes DESC, writes_in_callees DESC, s.fan_in DESC
    LIMIT :lim"""),
("raw-sql-below-controller", "Controller-reachable code that splices SQL text (interp/concat), beyond superglobal-to-sql",
    ...
    reach(root, sym, depth) AS (
        SELECT root, sym, MIN(depth) FROM walk GROUP BY root, sym)
    SELECT s.name AS builds_sql, entry.name AS reached_from,
        MIN(r.depth) AS hops,
        s.n_sql_interp AS sql_interp, s.n_sql_concat AS sql_concat,
        s.n_sql_prepared AS prepared, s.n_sql_literal AS literal_sql,
        s.fan_in, f.path || ':' || s.line_start AS at
    FROM reach r
    JOIN symbols s ON s.id = r.sym
      AND (s.n_sql_interp > 0 OR s.n_sql_concat > 0)
    JOIN symbols entry ON entry.id = r.root
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE r.depth > 0 AND f.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id, entry.id
    ORDER BY hops ASC, sql_interp DESC, sql_concat DESC LIMIT :lim"""),
(
    "file-sink-frontier",
    "File open/write/delete reachable from input, up to 3 hops (CodeQL path-injection)",
    ...
    """WITH RECURSIVE reach(src, sym, depth) AS (
        ...
        FROM reach r JOIN edges e ON e.caller_id = r.sym
        WHERE r.depth < 3 AND e.is_self = 0)
    SELECT src.name AS reads_input, sink.name AS touches_files,
        MIN(r.depth) AS hops, sink.n_dynamic_open AS dyn_opens,
        sink.n_file_write_dyn AS dyn_writes,
        GROUP_CONCAT(DISTINCT h.pattern) AS delete_or_move,
        src.n_get + src.n_post + src.n_request AS psalm_tainted_reads,
        sink.n_file AS file_calls, sink.fan_in,
        f.path || ':' || sink.line_start AS at
    FROM (SELECT src, sym, MIN(depth) AS depth FROM reach
          GROUP BY src, sym) r
    JOIN symbols sink ON sink.id = r.sym
      AND (sink.n_dynamic_open > 0 OR sink.n_file_write_dyn > 0)
    LEFT JOIN hazards h ON h.symbol_id = sink.id
      AND h.pattern IN ('unlink','rmdir','rename','copy')
    JOIN symbols src ON src.id = r.src
    JOIN files sf ON sf.id = src.file_id
    JOIN files f ON f.id = sink.file_id
    LEFT JOIN modules m ON m.id = sink.module_id
    WHERE f.is_test = 0 AND sf.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY src.id, sink.id
    ORDER BY dyn_writes DESC, hops ASC, dyn_opens DESC LIMIT :lim"""),
(
    "nullable-return-hotspot",
    "Nullable-returning functions by call-site count: every site must handle the null",
    ...
    """SELECT s.name, s.class_name AS class_, s.return_type,
        s.n_nullable_types AS nullable_types, s.n_null_check AS null_guards,
        s.n_callsites AS call_sites, s.fan_in,
        s.is_public AS public_, s.has_strict_types AS strict_file,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.kind IN ('function','method')
      AND s.return_type <> ''
      AND (substr(s.return_type, 1, 1) = '?'
           OR instr(s.return_type, 'null') > 0)
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY call_sites DESC, s.fan_in DESC LIMIT :lim"""),
(
    "mixed-propagation-frontier",
    "Entrypoints reaching functions with no return type or untyped params: where mixed flows",
    ...
    """WITH RECURSIVE walk(root, sym, depth) AS (
        ...
        SELECT s.id, s.id, 0 FROM symbols s
        JOIN files sf ON sf.id = s.file_id
        WHERE s.is_entrypoint = 1 AND sf.is_test = 0
        UNION
        SELECT w.root, e.callee_id, w.depth+1
        FROM walk w JOIN edges e ON e.caller_id = w.sym
        WHERE w.depth < 3 AND e.is_self = 0)
    SELECT s.name AS untyped_fn, entry.name AS entry_,
        MIN(r.depth) AS hops,
        s.n_untyped_params AS untyped_params,
        CASE WHEN s.return_type = '' THEN 1 ELSE 0 END AS no_return_type,
        s.n_params, s.n_callsites AS call_sites, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM (SELECT root, sym, MIN(depth) AS depth FROM walk
          GROUP BY root, sym) r
    JOIN symbols s ON s.id = r.sym
      AND (s.return_type = '' OR s.n_untyped_params > 0)
      AND s.kind IN ('function','method','closure')
    JOIN symbols entry ON entry.id = r.root
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE r.depth > 0 AND f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id, entry.id
    ORDER BY untyped_params DESC, call_sites DESC, hops ASC LIMIT :lim"""),
(
    "in-array-loose-input",
    "in_array/array_search without the strict argument, reachable from input, up to 3 hops",
    ...
    """WITH RECURSIVE reach(src, sym, depth) AS (
        ...
        FROM reach r JOIN edges e ON e.caller_id = r.sym
        WHERE r.depth < 3 AND e.is_self = 0)
    SELECT sink.name AS in_fn, MIN(r.depth) AS hops,
        sink.n_inarray_loose AS loose_searches,
        sink.n_loose_compare AS loose_cmp, sink.n_strict_compare AS strict_cmp,
        sink.n_crypto AS crypto_calls, sink.is_entrypoint,
        src.n_get + src.n_post + src.n_request + src.n_cookie
            AS tainted_reads,
        f.path || ':' || sink.line_start AS at
    FROM (SELECT src, sym, MIN(depth) AS depth FROM reach
          GROUP BY src, sym) r
    JOIN symbols sink ON sink.id = r.sym AND sink.n_inarray_loose > 0
    JOIN symbols src ON src.id = r.src
    JOIN files sf ON sf.id = src.file_id
    JOIN files f ON f.id = sink.file_id
    LEFT JOIN modules m ON m.id = sink.module_id
    WHERE f.is_test = 0 AND sf.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY sink.id
    ORDER BY loose_searches DESC, hops ASC, tainted_reads DESC LIMIT :lim"""),
(
    "regex-injection-surface",
    "preg_* with a computed pattern in functions reachable from input, up to 2 hops",
    ...
    """WITH RECURSIVE reach(src, sym, depth) AS (
        ...
        SELECT r.src, e.callee_id, r.depth+1
        FROM reach r JOIN edges e ON e.caller_id = r.sym
        WHERE r.depth < 2 AND e.is_self = 0)
    SELECT sink.name AS in_fn, MIN(r.depth) AS hops,
        sink.n_preg_dynamic AS computed_patterns,
        sink.n_regex_lit AS literal_regexes,
        sink.n_preg_in_loop AS preg_in_loop,
        src.n_get + src.n_post + src.n_request + src.n_cookie
            AS tainted_reads,
        sink.fan_in, f.path || ':' || sink.line_start AS at
    FROM (SELECT src, sym, MIN(depth) AS depth FROM reach
          GROUP BY src, sym) r
    JOIN symbols sink ON sink.id = r.sym AND sink.n_preg_dynamic > 0
    JOIN symbols src ON src.id = r.src
    JOIN files sf ON sf.id = src.file_id
    JOIN files f ON f.id = sink.file_id
    LEFT JOIN modules m ON m.id = sink.module_id
    WHERE f.is_test = 0 AND sf.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY sink.id
    ORDER BY computed_patterns DESC, hops ASC, preg_in_loop DESC LIMIT :lim"""),
(
    "mass-assignment-surface",
    "fill()/forceFill() fed by request input, with $fillable as counter-evidence (Laravel docs)",
    ...
    """SELECT s.name, s.class_name AS class_,
        s.n_massassign AS massassign_calls,
        s.n_request_input AS request_reads,
        s.n_superglobal_reads AS super_reads,
        CASE WHEN EXISTS (SELECT 1 FROM fields fl
                          WHERE fl.symbol_id = s.parent_id
                            AND fl.name IN ('fillable','guarded'))
             THEN 1 ELSE 0 END AS has_fillable,
        s.is_model AS model, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_massassign > 0
      AND (s.n_request_input > 0 OR s.n_superglobal_reads > 0)
      AND f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    ORDER BY has_fillable ASC, s.n_request_input DESC, s.fan_in DESC
    LIMIT :lim"""),
(
    "debug-reach-production",
    "dd/dump/ray/exit reachable from an entrypoint, up to 3 hops",
    ...
    """WITH RECURSIVE walk(root, sym, depth) AS (
        ...
        FROM walk w JOIN edges e ON e.caller_id = w.sym
        WHERE w.depth < 3 AND e.is_self = 0)
    SELECT s.name AS debug_site, entry.name AS reached_from,
        MIN(w.depth) AS hops,
        s.n_debug_call AS debug_calls, s.n_error_suppress AS suppressed,
        s.n_raw_echo AS raw_echo, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM walk w
    JOIN symbols s ON s.id = w.sym AND s.n_debug_call > 0
    JOIN symbols entry ON entry.id = w.root
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id, entry.id
    ORDER BY hops ASC, debug_calls DESC, raw_echo DESC LIMIT :lim"""),
(
    "session-write-from-input",
    "$_SESSION written in functions that also read request input (OWASP session cheat sheet)",
    ...
    """SELECT s.name, s.n_session_write AS session_writes,
        s.n_superglobal_reads AS super_reads,
        s.n_request_input AS request_reads,
        s.n_session AS session_reads, s.n_session_call AS session_calls,
        s.n_escaped_output AS escapes, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_session_write > 0
      AND (s.n_superglobal_reads > 0 OR s.n_request_input > 0)
      AND f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    ORDER BY session_writes DESC, s.n_superglobal_reads DESC LIMIT :lim"""),
(
    "superglobal-to-header",
    "header()/setcookie reachable from input, up to 3 hops (Psalm TaintedHeader)",
    ...
    """WITH RECURSIVE reach(src, sym, depth) AS (
        ...
        FROM reach r JOIN edges e ON e.caller_id = r.sym
        WHERE r.depth < 3 AND e.is_self = 0)
    SELECT src.name AS reads_input, sink.name AS writes_headers,
        MIN(r.depth) AS hops, sink.n_header_call AS header_calls,
        COUNT(DISTINCT h.pattern) AS header_kinds,
        GROUP_CONCAT(DISTINCT h.pattern) AS patterns,
        src.n_get + src.n_post + src.n_cookie AS psalm_tainted_reads,
        sink.fan_in, f.path || ':' || sink.line_start AS at
    FROM (SELECT src, sym, MIN(depth) AS depth FROM reach
          GROUP BY src, sym) r
    JOIN symbols sink ON sink.id = r.sym
      AND (sink.n_header > 0 OR sink.n_header_call > 0)
    LEFT JOIN hazards h ON h.symbol_id = sink.id AND h.category = 'header'
    JOIN symbols src ON src.id = r.src
    JOIN files sf ON sf.id = src.file_id
    JOIN files f ON f.id = sink.file_id
    LEFT JOIN modules m ON m.id = sink.module_id
    WHERE f.is_test = 0 AND sf.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY src.id, sink.id
    ORDER BY hops ASC, header_calls DESC, psalm_tainted_reads DESC
    LIMIT :lim"""),
(
    "facade-in-model-layer",
    "Facade static calls inside model/entity classes: a service locator in the domain",
    ...
    """SELECT s.name, s.class_name AS class_,
        s.n_facade_call AS facade_calls,
        s.n_static_calls AS static_calls, s.is_model AS model,
        s.n_sql_calls AS sql_calls, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_facade_call > 0 AND s.is_model = 1
      AND s.kind IN ('function','method')
      AND f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    ORDER BY facade_calls DESC, s.fan_in DESC LIMIT :lim"""),
(
    "write-in-loop",
    "DB write calls in looping functions: the bulk-write anti-pattern",
    ...
    """WITH writes AS (
        SELECT symbol_id, SUM(n) AS n_writes
        FROM hazards WHERE category = 'write' GROUP BY symbol_id)
    SELECT s.name, s.class_name AS class_, w.n_writes,
        s.max_loop_depth AS loop_depth, s.call_in_loop,
        s.n_loops, s.n_transaction AS tx_here, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM writes w
    JOIN symbols s ON s.id = w.symbol_id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.max_loop_depth > 0 AND (s.call_in_loop > 0
                                    OR s.query_in_loop > 0)
      AND f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    ORDER BY w.n_writes * s.max_loop_depth DESC, s.fan_in DESC LIMIT :lim"""),
(
    "dynamic-dispatch-frontier",
    "Entrypoints reaching eval / variable-include / variable-new sites, up to 4 hops",
    ...
    SELECT s.name AS dynamic_site, entry.name AS reached_from,
        MIN(w.depth) AS hops,
        s.n_eval AS evals, s.n_dynamic_include AS variable_includes,
        s.n_dynamic_new AS variable_news, s.n_variable_var AS variable_vars,
        s.fan_in, f.path || ':' || s.line_start AS at
    FROM (SELECT root, sym, MIN(depth) AS depth FROM walk
          GROUP BY root, sym) w
    JOIN symbols s ON s.id = w.sym
      AND (s.n_eval > 0 OR s.n_dynamic_include > 0 OR s.n_dynamic_new > 0)
    JOIN symbols entry ON entry.id = w.root
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE w.depth > 0 AND f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id, entry.id
    ORDER BY hops ASC, s.n_eval DESC, variable_includes DESC LIMIT :lim"""),
(
    "trait-method-collision",
    "Same method name provided by two traits used by one class (flat-compose conflict)",
    ...
    """WITH RECURSIVE split(cid, rest, tok) AS (
        SELECT symbol_id, traits || ',', '' FROM classes c
        WHERE c.traits <> ''
        UNION ALL
        SELECT cid,
            CASE WHEN instr(rest, ',') > 0
            ...
            CASE WHEN instr(rest, ',') > 0
                 THEN substr(rest, 1, instr(rest, ',') - 1) ELSE rest END
        FROM split WHERE rest <> ''),
    used(cid, trait_name) AS (
        SELECT DISTINCT cid, tok FROM split WHERE tok <> ''),
    collide(cid, method_name, n_traits) AS (
        SELECT u.cid, tm.name, COUNT(DISTINCT u.trait_name)
        FROM used u
        JOIN traits t ON t.name = u.trait_name
        JOIN symbols tm ON tm.parent_id = t.symbol_id
           AND tm.kind = 'method'
        GROUP BY u.cid, tm.name
        HAVING COUNT(DISTINCT u.trait_name) > 1)
    SELECT c.name AS class_, cl.method_name AS colliding_method,
        cl.n_traits,
        GROUP_CONCAT(DISTINCT u.trait_name) AS traits_,
        f.path || ':' || c.line AS at
    FROM collide cl
    JOIN classes c ON c.symbol_id = cl.cid
    JOIN used u ON u.cid = cl.cid
    JOIN traits t2 ON t2.name = u.trait_name
    JOIN symbols tm2 ON tm2.parent_id = t2.symbol_id
      AND tm2.kind = 'method' AND tm2.name = cl.method_name
    JOIN files f ON f.id = c.file_id
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    GROUP BY cl.cid, cl.method_name
    ORDER BY cl.n_traits DESC, c.name ASC LIMIT :lim"""),
(
    "remote-call-in-loop",
    "curl_exec / file_get_contents / fwrite inside looping functions",
    ...
    """SELECT s.name, s.class_name AS class_, s.io_in_loop AS io_in_loop,
        s.max_loop_depth AS loop_depth, s.n_remote_fetch AS remote_calls,
    ...
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.io_in_loop > 0 AND f.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id
    ORDER BY io_in_loop DESC, distinct_callers DESC LIMIT :lim"""),
(
    "regex-compile-in-loop",
    "preg_* inside loops: per-iteration pattern work, weighted by callers",
    ...
    """SELECT s.name, s.class_name AS class_, s.regex_in_loop AS regex_in_loop,
        s.n_preg_in_loop AS preg_in_loop,
        s.max_loop_depth AS loop_depth,
        COUNT(DISTINCT e.caller_id) AS distinct_callers,
        s.fan_in, f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN edges e ON e.callee_id = s.id AND e.is_self = 0
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE (s.regex_in_loop > 0 OR s.n_preg_in_loop > 0)
      AND f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id
    ORDER BY regex_in_loop DESC, preg_in_loop DESC, distinct_callers DESC
    LIMIT :lim"""),
(
    "cookie-without-flags",
    "setcookie called with fewer than 3 arguments: no expiry, path, domain, secure, httponly",
    ...
    """SELECT s.name, s.n_cookie_lean AS lean_cookie_calls,
        s.n_header_call AS header_calls, s.n_session_call AS session_calls,
        s.n_auth_call AS auth_calls, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_cookie_lean > 0 AND f.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY lean_cookie_calls DESC, s.fan_in DESC LIMIT :lim"""),
(
    "global-state-mutation",
    "Functions reaching for global state, weighted by how many callers they have",
    ...
    "     ones to read first.",
    """SELECT s.name, s.class_name AS class_,
        s.n_globals AS global_statements,
        s.n_superglobal_reads AS super_reads, s.n_assign AS assigns,
        s.fan_in, s.n_callsites AS call_sites,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_globals > 0 AND s.kind IN ('function','method','closure')
      AND f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC, global_statements DESC LIMIT :lim"""),
(
    "reflection-frontier",
    "Reflection sinks reachable from input, up to 3 hops",
    ...
    """WITH RECURSIVE reach(src, sym, depth) AS (
        ...
        FROM reach r JOIN edges e ON e.caller_id = r.sym
        WHERE r.depth < 3 AND e.is_self = 0)
    SELECT sink.name AS reflects, MIN(r.depth) AS hops,
        sink.n_reflect AS reflect_calls,
        src.n_get + src.n_post + src.n_request AS tainted_reads,
        sink.n_dynamic_new AS variable_news,
        sink.fan_in, f.path || ':' || sink.line_start AS at
    FROM (SELECT src, sym, MIN(depth) AS depth FROM reach
          GROUP BY src, sym) r
    JOIN symbols sink ON sink.id = r.sym AND sink.n_reflect > 0
    JOIN symbols src ON src.id = r.src
    JOIN files sf ON sf.id = src.file_id
    JOIN files f ON f.id = sink.file_id
    LEFT JOIN modules m ON m.id = sink.module_id
    WHERE f.is_test = 0 AND sf.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY sink.id
    ORDER BY hops ASC, reflect_calls DESC LIMIT :lim"""),
(
    "callable-frontier",
    "call_user_func / usort-style callable sinks reachable from input, up to 3 hops",
    ...
    """WITH RECURSIVE reach(src, sym, depth) AS (
        ...
        FROM reach r JOIN edges e ON e.caller_id = r.sym
        WHERE r.depth < 3 AND e.is_self = 0)
    SELECT sink.name AS invokes, MIN(r.depth) AS hops,
        COUNT(DISTINCT h.pattern) AS callable_kinds,
        GROUP_CONCAT(DISTINCT h.pattern) AS patterns,
        src.n_get + src.n_post + src.n_request AS tainted_reads,
        sink.n_dynamic_call AS dynamic_calls,
        sink.fan_in, f.path || ':' || sink.line_start AS at
    FROM (SELECT src, sym, MIN(depth) AS depth FROM reach
          GROUP BY src, sym) r
    JOIN symbols sink ON sink.id = r.sym
    JOIN hazards h ON h.symbol_id = sink.id AND h.category = 'callable'
    JOIN symbols src ON src.id = r.src
    JOIN files sf ON sf.id = src.file_id
    JOIN files f ON f.id = sink.file_id
    LEFT JOIN modules m ON m.id = sink.module_id
    WHERE f.is_test = 0 AND sf.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY sink.id
    ORDER BY hops ASC, callable_kinds DESC, tainted_reads DESC LIMIT :lim"""),
(
    "constructor-side-effects",
    "Constructors doing IO/SQL/network, ranked by how many sites construct them",
    ...
    "     negative).",
    """SELECT s.name, s.class_name AS class_, s.n_io AS io_ops,
        s.n_sql_calls AS sql_calls, s.n_net AS net_calls,
        s.n_file AS file_ops, s.n_callsites AS construction_sites,
        s.fan_in, f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.name = '__construct' AND s.kind = 'method'
      AND (s.n_io + s.n_sql_calls + s.n_net + s.n_file) > 0
      AND f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    ORDER BY construction_sites DESC, io_ops DESC, s.fan_in DESC
    LIMIT :lim"""),
(
    "static-state-writes",
    "Cls::$var = v writes to mutable static properties (Octane/worker state leaks)",
    ...
    "     still rank.",
    """SELECT s.name, s.class_name AS class_,
        s.n_static_prop_write AS static_writes,
        s.n_static_calls AS scoped_calls, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_static_prop_write > 0
      AND s.kind IN ('function','method','closure')
      AND f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    ORDER BY static_writes DESC, s.fan_in DESC LIMIT :lim"""),
(
    "superglobal-in-model",
    "Model/entity-layer functions reading superglobals directly: a layering violation with taint",
    ...
    """SELECT s.name, s.class_name AS class_,
        s.n_superglobal_reads AS super_reads,
        s.n_get + s.n_post + s.n_request + s.n_cookie AS psalm_tainted_reads,
        s.n_request_input AS request_reads,
        s.n_sql_calls AS sql_calls, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.is_model = 1 AND s.n_superglobal_reads > 0
      AND s.kind IN ('function','method')
      AND f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    ORDER BY psalm_tainted_reads DESC, super_reads DESC LIMIT :lim"""),
(
    "controller-direct-sql",
    "Controllers issuing SQL themselves instead of going through a model/gateway",
    ...
    """SELECT s.name, s.class_name AS class_,
        s.n_sql_calls AS sql_calls, s.n_sql_interp AS sql_interp,
        s.n_sql_concat AS sql_concat, s.n_sql_prepared AS prepared,
        s.n_superglobal_reads AS super_reads, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.is_controller = 1
      AND (s.n_sql_calls > 0 OR s.n_sql_interp > 0 OR s.n_sql_concat > 0)
      AND s.kind IN ('function','method')
      AND f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    ORDER BY sql_interp DESC, sql_concat DESC, s.fan_in DESC LIMIT :lim"""),
(
    "cleartext-remote-fetch",
    "http:// (clear-text) URLs passed to fetch calls (SonarPHP S5332)",
    ...
    """SELECT s.name, s.n_cleartext_fetch AS cleartext_fetches,
        s.n_remote_fetch AS remote_fetches, s.fan_in,
        s.is_controller AS controller,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_cleartext_fetch > 0 AND f.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY cleartext_fetches DESC, s.fan_in DESC LIMIT :lim"""),
(
    "superglobal-to-mail",
    "mail() reachable from input, up to 2 hops (email header injection)",
    ...
    """WITH RECURSIVE reach(src, sym, depth) AS (
        ...
        FROM reach r JOIN edges e ON e.caller_id = r.sym
        WHERE r.depth < 2 AND e.is_self = 0)
    SELECT src.name AS reads_input, sink.name AS sends_mail,
        MIN(r.depth) AS hops,
        GROUP_CONCAT(DISTINCT h.pattern) AS mail_patterns,
        src.n_get + src.n_post + src.n_request AS tainted_reads,
        sink.fan_in, f.path || ':' || sink.line_start AS at
    FROM (SELECT src, sym, MIN(depth) AS depth FROM reach
          GROUP BY src, sym) r
    JOIN symbols sink ON sink.id = r.sym
    JOIN hazards h ON h.symbol_id = sink.id AND h.pattern = 'mail'
    JOIN symbols src ON src.id = r.src
    JOIN files sf ON sf.id = src.file_id
    JOIN files f ON f.id = sink.file_id
    LEFT JOIN modules m ON m.id = sink.module_id
    WHERE f.is_test = 0 AND sf.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY src.id, sink.id
    ORDER BY hops ASC, tainted_reads DESC LIMIT :lim"""),
]
...
(
    "input-surface-by-module",
    "Where attacker-controlled input enters, by module: reads, Psalm-tainted reads, and the sinks they can reach",
    ...
    """SELECT m.name AS namespace_, COUNT(DISTINCT s.id) AS fns,
        COALESCE(SUM(s.n_superglobal_reads), 0) AS super_reads,
        COALESCE(SUM(s.n_psalm_tainted), 0) AS psalm_tainted,
        COALESCE(SUM(s.n_sql_interp + s.n_sql_concat), 0) AS sql_spliced,
        COALESCE(SUM(s.n_shell), 0) AS shell,
        COALESCE(SUM(s.n_raw_echo), 0) AS raw_echo,
        COALESCE(SUM(CASE WHEN s.n_superglobal_reads > 0 THEN 1 ELSE 0 END),
                 0) AS input_fns,
        CAST(100.0 * SUM(CASE WHEN s.n_superglobal_reads > 0 THEN 1
                              ELSE 0 END)
             / NULLIF(COUNT(s.id), 0) AS INT) AS pct_input_facing
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    JOIN modules m ON m.id = s.module_id
    WHERE s.kind IN ('function','method','closure') AND f.is_test = 0
      AND m.name LIKE :mod
    GROUP BY m.id HAVING fns > 0
    ORDER BY psalm_tainted DESC, sql_spliced DESC, super_reads DESC
    LIMIT :lim"""),
(
    "nullability-pressure",
    "Where null lives: nullable types, null-safe calls and null guards per module",
    ...
    "     invisible.",
    """SELECT m.name AS namespace_, COUNT(DISTINCT s.id) AS fns,
        COALESCE(SUM(s.n_nullable_types), 0) AS nullable_types,
        COALESCE(SUM(s.n_null_safe), 0) AS nullsafe_calls,
        COALESCE(SUM(s.n_null_check), 0) AS null_checks,
        COUNT(DISTINCT CASE WHEN s.n_nullable_types > 0 THEN s.id END)
            AS fns_with_nullable,
        COUNT(DISTINCT CASE WHEN s.return_type LIKE '?%'
                             OR instr(s.return_type, 'null') > 0
                            THEN s.id END) AS fns_returning_nullable,
        CAST(100.0 * COUNT(DISTINCT CASE WHEN s.n_nullable_types > 0
                                         THEN s.id END)
             / NULLIF(COUNT(s.id), 0) AS INT) AS pct_nullable_fns
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    JOIN modules m ON m.id = s.module_id
    WHERE s.kind IN ('function','method') AND f.is_test = 0
      AND m.name LIKE :mod
    GROUP BY m.id HAVING fns > 0
    ORDER BY nullable_types DESC, nullsafe_calls DESC LIMIT :lim"""),
(
    "error-swallow-map",
    "Where failures disappear: broad catches, empty catches and suppression vs throws, per module",
    ...
    """SELECT m.name AS namespace_, COUNT(DISTINCT s.id) AS fns,
        COALESCE(SUM(s.n_throw), 0) AS throws,
        COALESCE(SUM(s.n_try), 0) AS trys,
        COALESCE(SUM(s.n_catch), 0) AS catches,
        COALESCE(SUM(s.n_catch_broad), 0) AS broad_catches,
        COALESCE(SUM(s.n_catch_empty), 0) AS empty_catches,
        COALESCE(SUM(s.n_error_suppress), 0) AS suppressed,
        COALESCE(SUM(s.n_catch_broad + s.n_catch_empty
                     + s.n_error_suppress), 0) AS silencers,
        CAST(100.0 * COALESCE(SUM(s.n_catch_broad + s.n_catch_empty
                                  + s.n_error_suppress), 0)
             / NULLIF(COALESCE(SUM(s.n_throw + s.n_catch
                                   + s.n_error_suppress), 0), 0) AS INT)
            AS pct_swallowed
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    JOIN modules m ON m.id = s.module_id
    WHERE s.kind IN ('function','method','closure') AND f.is_test = 0
      AND m.name LIKE :mod
    GROUP BY m.id HAVING fns > 0
    ORDER BY pct_swallowed DESC, silencers DESC LIMIT :lim"""),
(
    "entrypoint-blast-radius",
    "How much of the tree each entrypoint can reach in 3 hops",
    ...
    """WITH RECURSIVE walk(root, sym, depth) AS (
        -- depth bound 3
        SELECT s.id, s.id, 0 FROM symbols s WHERE s.is_entrypoint = 1
        UNION
        SELECT w.root, e.callee_id, w.depth+1
        FROM walk w JOIN edges e ON e.caller_id = w.sym
        WHERE w.depth < 3 AND e.is_self = 0),
    reach AS (
        SELECT root, COUNT(DISTINCT sym) AS reached, MAX(depth) AS max_depth
        FROM walk GROUP BY root)
    SELECT s.name, m.name AS module_, r.reached, r.max_depth,
        s.fan_out, s.cyclomatic AS cyclo,
        s.n_superglobal_reads AS input_reads, s.n_sql_calls AS sql_calls,
        f.path || ':' || s.line_start AS at
    FROM reach r
    ...
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE f.is_test = 0
    ORDER BY r.reached DESC, s.n_superglobal_reads DESC LIMIT :lim"""),
(
    "facade-density",
    "Facade static calls per module: where the service locator hides in plain sight",
    ...
    """SELECT m.name AS namespace_, COUNT(DISTINCT s.id) AS fns,
        COALESCE(SUM(s.n_facade_call), 0) AS facade_calls,
        COALESCE(SUM(s.n_static_calls), 0) AS static_calls,
        COUNT(DISTINCT CASE WHEN s.n_facade_call > 0 THEN s.id END)
            AS fns_with_facades,
        CAST(100.0 * COALESCE(SUM(s.n_facade_call), 0)
             / NULLIF(COALESCE(SUM(s.n_static_calls), 0), 0) AS INT)
            AS pct_facade
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    JOIN modules m ON m.id = s.module_id
    WHERE s.kind IN ('function','method') AND f.is_test = 0
      AND m.name LIKE :mod
    GROUP BY m.id HAVING fns > 0
    ORDER BY facade_calls DESC, pct_facade DESC LIMIT :lim"""),
(
    "gadget-inventory",
    "Gadget magic methods per namespace: the ammunition a deserialization attack composes with",
    ...
    """SELECT COALESCE(NULLIF(c.namespace, ''), '(global)') AS namespace_,
        COUNT(DISTINCT c.symbol_id) AS classes_with_magic,
        COUNT(DISTINCT mm.symbol_id) AS gadget_methods,
        SUM(c.has_destruct) AS destructs, SUM(c.has_wakeup) AS wakeups,
        SUM(c.has_tostring) AS tostrings,
        COALESCE(SUM(mm.n_hazards), 0) AS hazards_in_gadgets,
        COALESCE(SUM(mm.n_calls), 0) AS calls_in_gadgets
    FROM classes c
    JOIN magic_methods mm ON mm.class_id = c.symbol_id AND mm.is_gadget = 1
    JOIN files f ON f.id = c.file_id
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    GROUP BY c.namespace
    ORDER BY hazards_in_gadgets DESC, gadget_methods DESC LIMIT :lim"""),
(
    "transaction-surface",
    "Mutations, transactions and queue dispatches per module: how much writes without a guard",
    ...
    """SELECT m.name AS namespace_, COUNT(DISTINCT s.id) AS fns,
        COALESCE(SUM(s.n_write), 0) AS write_calls,
        COALESCE(SUM(s.n_transaction), 0) AS tx_calls,
        COALESCE(SUM(s.n_queue), 0) AS queue_dispatches,
        COUNT(DISTINCT CASE WHEN s.n_write > 0 THEN s.id END)
            AS fns_writing,
        COUNT(DISTINCT CASE WHEN s.n_write > 0 AND s.n_transaction > 0
                            THEN s.id END) AS fns_with_tx,
        CAST(100.0 * COUNT(DISTINCT CASE WHEN s.n_write > 0
                                          AND s.n_transaction > 0
                                         THEN s.id END)
             / NULLIF(COUNT(DISTINCT CASE WHEN s.n_write > 0
                                          THEN s.id END), 0) AS INT)
            AS pct_tx_guarded
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    JOIN modules m ON m.id = s.module_id
    WHERE s.kind IN ('function','method') AND f.is_test = 0
      AND m.name LIKE :mod
    GROUP BY m.id
    HAVING write_calls > 0
    ORDER BY write_calls DESC, pct_tx_guarded ASC LIMIT :lim"""),
(
    "test-only-coupling",
    "Production functions whose ONLY callers are test files: the reverse dependency no coverage tool shows",
    ...
    """SELECT s.name, s.kind, s.class_name AS class_,
        COUNT(DISTINCT e.caller_id) AS all_callers,
        COUNT(DISTINCT CASE WHEN cf.is_test = 1 OR ce.is_test = 1
                            THEN e.caller_id END) AS test_callers,
        s.fan_in, s.n_callsites AS sites,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN edges e ON e.callee_id = s.id AND e.is_self = 0
    JOIN symbols ce ON ce.id = e.caller_id
    JOIN files cf ON cf.id = ce.file_id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.is_test = 0 AND f.is_test = 0
      ...
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id
    HAVING all_callers = test_callers
    ORDER BY all_callers DESC LIMIT :lim"""),
(
    "debug-residue",
    "Debug calls, exits and suppression per module: what debugging left behind",
    ...
    """SELECT m.name AS namespace_, COUNT(DISTINCT s.id) AS fns,
        COALESCE(SUM(s.n_debug_call), 0) AS debug_calls,
        COUNT(DISTINCT CASE WHEN s.n_debug_call > 0 THEN s.id END)
            AS fns_with_debug,
        COALESCE(SUM(s.n_error_suppress), 0) AS suppressed,
        COALESCE(SUM(s.n_raw_echo), 0) AS raw_echo,
        COALESCE(SUM(s.n_catch_empty), 0) AS empty_catches
    ...
    WHERE s.kind IN ('function','method','closure') AND f.is_test = 0
      AND m.name LIKE :mod
    GROUP BY m.id HAVING fns > 0
    ORDER BY debug_calls DESC, fns_with_debug DESC, suppressed DESC
    LIMIT :lim""")
]
