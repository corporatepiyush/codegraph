...
def discover(analyzer: Analyzer, root: str, db: sqlite3.Connection,
    ...
    for dirpath, dirnames, filenames in os.walk(root, onerror=_walk_error):
        ...
        for fn in sorted(filenames):
            ...
            # One pass instead of four: blank, comment-only, non-blank and
            # max-line-length were four separate generator sweeps over every
            # line of every file. Same counts: a line is blank or it is not,
            # so non-blank == len(lines) - blank, and max length is taken
            # over ALL lines (a whitespace-only line can be the longest).
            blank = 0
            cmt = 0
            max_line = 0
            for line in lines:
                ...
                if not stripped:
                    blank += 1
                elif line.lstrip()[:3] in ("//", "#", "/*", "*", "*/",
                                           '"""', "'''", "--", ";;", "%"):
                    cmt += 1
            n_sloc = len(lines) - blank
            ...
            file_rows.append(
                (fid, rel, os.path.dirname(rel) or ".", fn, ext, analyzer.LANG,
                 mid, st.st_size, len(lines), n_sloc,
                 blank, cmt, max_line,
...
def scan_markers(rec: FileRec, bufs: Buffers) -> None:
    ...
    for i, line in enumerate(rec.text.splitlines(), 1):
        # The marker only counts when a comment token shares the line, and
        # most lines carry none: the substring scan is near-free, the regex
        # is not, so the cheap test runs first. Same rows either way.
...
class TreeSitterAnalyzer(Analyzer):
    ...
    def __init__(self) -> None:
        ...
        #: Node-type tables are class constants, so their set forms are
        #: computed ONCE instead of ten `set()` builds per `measure` call
        #: (5,107 measure calls on gson -- 51k set constructions per build).
        self._s_loops = frozenset(self.LOOP_NODES)
        self._s_branches = frozenset(self.BRANCH_NODES)
        self._s_nests = frozenset(self.NEST_NODES)
        self._s_calls = frozenset(self.CALL_NODES)
        self._s_operators = frozenset(self.OPERATOR_NODES)
        self._s_strings = frozenset(self.STRING_NODES)
        self._s_numbers = frozenset(self.NUMBER_NODES)
        self._s_comments = frozenset(self.COMMENT_NODES)
        self._s_ifs = frozenset(self.IF_NODES)
        self._s_prune = (frozenset(self.FUNC_KINDS)
                         | frozenset(self.TYPE_KINDS))
    ...
    def emit_module_scope(self, root: Any, rec: FileRec,
        ...
        if not self.MODULE_SCOPE_SYMBOL:
            return
        stats = self.measure(root, rec, prune=self._s_prune)
    ...
    def walk_scope(self, node: Any, rec: FileRec, db: sqlite3.Connection,
        ...
        func_kinds = self.FUNC_KINDS
        type_kinds = self.TYPE_KINDS
        body_field = self.BODY_FIELD
        node_name = self.node_name
        while stack:
            cur, sc = stack.pop()
            kind = func_kinds.get(cur.type)
            if kind:
    ...
    def emit_function(self, node: Any, rec: FileRec, db: sqlite3.Connection,
        ...
        self._track_identifiers = bool(
            self.volatile_fields.get(owner)
            or self.datefmt_fields.get(owner))
        stats = self.measure(body, rec)
        self._track_identifiers = False
    ...
    def emit_type(self, node: Any, rec: FileRec, db: sqlite3.Connection,
        ...
        body = node.child_by_field_name(self.BODY_FIELD) or node
        bstats = self.measure(body, rec, prune=self._s_prune)
    ...
    def measure(self, body: Any, rec: FileRec,
        ...
        src = rec.data
        loops = self._s_loops
        branches = self._s_branches
        nests = self._s_nests
        calls = self._s_calls
        counters = self.COUNTERS
        operators = self._s_operators
        strings = self._s_strings
        numbers = self._s_numbers
        comments = self._s_comments
        ifs = self._s_ifs
        ...
        on_string = self.on_string
        text_of_local = text_of
        # text_of keeps ONE decoded string per ASCII file and slices it for
        # every node; within this synchronous walk nothing can evict that
        # cache entry, so resolve it once and slice directly for the token
        # firehose below. A non-ASCII file yields None and keeps the plain
        # text_of path (byte offsets are not str indices there).
        ent = _TEXT_CACHE.get(id(src))
        if ent is None or ent[0] is not src:
            text_of(body, src)                 # prime, exactly as any call
            ent = _TEXT_CACHE[id(src)]
        ascii_txt = ent[1]
        ...
        while True:
            ...
            if named:
                hit_loop = t in loops
                if not hit_loop and extra_loops:
                    hit_loop = node.id in extra_loops
                if hit_loop:
            ...
            if t in calls:
                on_call(node, src, st, loop_depth, nest)
            elif t in operators:
                st.n_operators += 1
                st.operators.add(t)
            elif t in strings:
                txt = (ascii_txt[node.start_byte:node.end_byte]
                       if ascii_txt is not None
                       else text_of_local(node, src))
                bump("n_string_lit")
                st.operands.add(txt[:40])
                st.n_operands += 1
                on_string(node, txt, src, st, loop_depth)
            elif t in numbers:
                txt = (ascii_txt[node.start_byte:node.end_byte]
                       if ascii_txt is not None
                       else text_of_local(node, src)).strip()
                st.n_operands += 1
                st.operands.add(txt)
                ...
                bump("n_comment_lines",
                     node.end_point[0] - node.start_point[0] + 1)
            elif node.child_count == 0:
                st.n_tokens += 1
                st.n_operands += 1
                st.operands.add(
                    (ascii_txt[node.start_byte:node.end_byte]
                     if ascii_txt is not None
                     else text_of_local(node, src))[:40])

            on_node(node, src, st, loop_depth, nest)
...
#: Proxy-managed annotations (Spring Framework reference, "Declarative
#: Transaction Management" / "Cache Abstraction" / "Async"): in proxy mode
#: (the default) these take effect ONLY for calls that arrive through the
#: proxy. A method invoked from a sibling in the same bean -- self-invocation
#: -- never sees them; the annotation is inert at that call site. The docs
#: say this in as many words for @Transactional and the same machinery
#: carries @Async and @Cacheable.
PROXY_ANNOTATIONS = frozenset("""
Transactional Async Cacheable CacheEvict CachePut Caching Retryable
Validated
""".split())

#: Bean stereotypes that default to SINGLETON scope: one instance serves
#: every request thread. Instance state on these beans is shared state.
SINGLETON_STEREOTYPES = frozenset("""
Component Service Repository Controller RestController ControllerAdvice
Configuration Singleton
""".split())

#: Narrow-scope annotations: the bean is prototype / request / session
#: scoped, so a plain field injection into a singleton captures ONE instance
#: for the whole application lifetime (Spring reference, "Bean scopes":
#: prototype-bean dependency injection into a singleton).
NARROW_SCOPE_ANNOTATIONS = frozenset("""
RequestScope SessionScope RefreshScope ApplicationScope
""".split())

#: Non-thread-safe format/helper types. A field of one of these types is a
#: shared mutable object no matter how it is reached; SimpleDateFormat,
#: NumberFormat and DecimalFormat document themselves as not thread-safe,
#: and Calendar is mutable. (DateTimeFormatter IS immutable -- excluded.)
FORMAT_FIELD_TYPES = ("SimpleDateFormat", "NumberFormat", "DecimalFormat",
                      "Calendar")

#: Thread-safe collection types: exact simple-name membership used to EXCLUDE
#: a field from the shared-bean-mutable-collection query. Everything else
#: with a collection type needs external synchronization.
THREAD_SAFE_COLLECTIONS = frozenset("""
...
#: Method calls that GROW a collection, matched against the receiver being a
#: static collection field. `set` is deliberately absent (Map.put vs
#: positional overwrite is indistinguishable and rarely growth).
STATIC_COLL_ADDS = frozenset("""
add addAll put putAll offer push addFirst addLast insertElementAt
addElement incrementAndGet addAndGet
""".split())

#: Method calls that SHRINK or empty a collection.
STATIC_COLL_REMOVES = frozenset("""
remove removeAll clear removeIf poll pop removeFirst removeLast retainAll
trimToSize removeElement decrementAndGet
""".split())
...
#: Modality word checks, precompiled once. Every one of these used to go
#: through `re.search(str_pattern, ...)` and its internal pattern cache --
#: three lookups per symbol in visibility_of alone, ~15k per build on gson --
#: for words that never change.
_WORD_PUBLIC = re.compile(r'\bpublic\b')
_WORD_PROTECTED = re.compile(r'\bprotected\b')
_WORD_PRIVATE = re.compile(r'\bprivate\b')
_WORD_STATIC = re.compile(r'\bstatic\b')
_WORD_ABSTRACT = re.compile(r'\babstract\b')
_WORD_SYNCHRONIZED = re.compile(r'\bsynchronized\b')
_WORD_NATIVE = re.compile(r'\bnative\b')
_WORD_FINAL = re.compile(r'\bfinal\b')
_WORD_VOLATILE = re.compile(r'\bvolatile\b')
...
#: frozensets for on_call's per-call metric columns. The chain stays an
#: elif chain in the original order (several names appear in more than one
#: set, and first-match-wins is the recorded behaviour); only the container
#: changed -- an 8-to-17 tuple scan per call node became one hash probe.
_BOX_VALUEOF_TYPES = frozenset(
    ("Integer", "Long", "Double", "Float", "Short", "Byte", "Character",
     "Boolean"))
_BOX_UNBOX_BASES = frozenset(
    ("intValue", "longValue", "doubleValue", "floatValue", "booleanValue",
     "shortValue", "byteValue", "charValue"))
_SETACCESSIBLE_BASES = frozenset(("setAccessible", "trySetAccessible",
                                  "privateLookupIn"))
_FFM_ARENA_BASES = frozenset(
    ("ofConfined", "ofShared", "ofAuto", "global", "allocateFrom",
     "allocate", "reinterpret", "ofAddress"))
_FFM_ARENA_RECEIVERS = ("Arena.", "MemorySegment.", "SegmentAllocator.")
_FFM_DOWNCALL_BASES = frozenset(
    ("nativeLinker", "downcallHandle", "upcallStub", "libraryLookup",
     "loaderLookup", "invokeExact"))
_UNSAFE_BASES = frozenset(
    ("getUnsafe", "allocateMemory", "reallocateMemory", "freeMemory",
     "objectFieldOffset", "staticFieldOffset", "arrayBaseOffset",
     "copyMemory", "setMemory", "putOrderedObject", "putOrderedLong",
     "allocateDirect", "fullFence", "acquireFence", "releaseFence",
     "loadLoadFence", "storeStoreFence"))
_ATOMIC_BASES = frozenset(
    ("compareAndSet", "weakCompareAndSet", "getAndSet", "incrementAndGet",
     "decrementAndGet", "getAndIncrement", "getAndDecrement", "getAndAdd",
     "addAndGet", "updateAndGet", "accumulateAndGet", "lazySet",
     "compareAndExchange", "getAcquire", "setRelease", "getPlain",
     "setOpaque"))
_WAIT_BASES = frozenset(
    ("wait", "notify", "notifyAll", "await", "join", "sleep", "onSpinWait",
     "park", "awaitTermination"))
_CLOSE_BASES = frozenset(
    ("close", "closeQuietly", "shutdown", "shutdownNow", "dispose", "free"))
_REGEX_METHOD_BASES = frozenset(("matches", "replaceAll", "replaceFirst",
                                 "split"))
_OPTIONAL_BASES = frozenset(
    ("ofNullable", "orElse", "orElseGet", "orElseThrow", "isPresent",
     "ifPresent", "ifPresentOrElse"))
_QUERY_CALL_BASES = frozenset(
    ("executeQuery", "executeUpdate", "executeBatch", "prepareStatement",
     "prepareCall", "createQuery", "createNativeQuery", "getResultList",
...
class JavaAnalyzer(TreeSitterAnalyzer):
    ...
    LOOP_CALL_COUNTERS = {
        ...
        # String.format / MessageFormat.format inside a loop: a fresh
        # Formatter parse per element. Base-name match, so LocalDate.format
        # in a loop counts too -- also a per-element cost, and the query
        # says so.
    ...
    ("n_elif", "INT NOT NULL DEFAULT 0"),
        ...
        # -- wave-3 pack: cross-method defect families -----------------------
        #: `volatileField++` / `+=` on a volatile field: the compound
        #: operation is NOT atomic even though each read/write is (Error
        #: Prone NonAtomicVolatileUpdate). Needs the field's declared
        #: volatility plus the assignment shape -- a name-graph fact.
        ("n_volatile_compound", "INT NOT NULL DEFAULT 0"),
        #: Uses of a non-thread-safe FORMAT field (SimpleDateFormat,
        #: NumberFormat, DecimalFormat, Calendar) declared on the owner
        #: class. The field is shared by every caller; the format methods
        #: mutate internal state (PMD UnsynchronizedStaticFormatter family).
        ("n_shared_datefmt_use", "INT NOT NULL DEFAULT 0"),
        #: addXxxListener/addXxxHandler call sites: a registration with no
        #: matching removal is a listener pinned for the component's life.
        ("n_listener_add", "INT NOT NULL DEFAULT 0"),
        #: removeXxxListener/removeXxxHandler call sites -- the deregistration
        #: side of the same ledger.
        ("n_listener_remove", "INT NOT NULL DEFAULT 0"),
        #: `new Thread(...)` sites: an unmanaged thread that bypasses any
        #: executor/virtual-thread infrastructure the same file already has.
        ("n_thread_alloc", "INT NOT NULL DEFAULT 0"),
        #: Dynamic expression evaluation: SpEL parseExpression, ScriptEngine
        #: eval, ELProcessor evaluate. The sink half of an injection pair.
        ("n_spel_eval", "INT NOT NULL DEFAULT 0"),
        #: String.format / MessageFormat-style `.format(...)` inside a loop:
        #  a per-element cost ranked by how hot the calling path is.
        ("n_format_in_loop", "INT NOT NULL DEFAULT 0"),
        #: add/put-family calls whose receiver is a STATIC collection field:
        #  the growth side of an unbounded-growth ledger.
        ("n_static_coll_add", "INT NOT NULL DEFAULT 0"),
        #: remove/clear-family calls on a static collection field: the
        #: pruning side. add>remove across a class is a growth leak shape.
        ("n_static_coll_remove", "INT NOT NULL DEFAULT 0"),
    )
...
-- wait/notify/notifyAll call sites with their MONITOR and whether the call
-- lexically sits inside a synchronized region. Cross-method view: one
-- method may wait on a monitor while the synchronized(...) that guards it
-- lives in entirely different methods -- the shape no per-file checker can
-- pair.
CREATE TABLE monitor_ops(
    id INTEGER PRIMARY KEY,
    symbol_id INT NOT NULL REFERENCES symbols(id),
    file_id INT NOT NULL REFERENCES files(id),
...
CREATE INDEX idx_mon_sym ON monitor_ops(symbol_id, op);
CREATE INDEX idx_mon_name ON monitor_ops(monitor, op);
...
"""
    ...
    def __init__(self) -> None:
        ...
        #: type name -> {field names whose declared type is a non-thread-safe
        #: format helper (SimpleDateFormat, NumberFormat, DecimalFormat,
        #: Calendar)}
        self.datefmt_fields: dict[str, set[str]] = {}
        #: type name -> {static mutable collection field names}
        self.statcoll_fields: dict[str, set[str]] = {}
        self.java_release = ""
        #: Identifier `extra_rows` exist only so function_extra can test each
        #: identifier against the OWNER's volatile / datefmt field names. When
        #: the owner declares neither, every one of those rows -- one tuple
        #: per identifier token in the body, plus a decode apiece at replay --
        #: was allocated and then ignored. The flag is set per function-body
        #: measure and reset straight after, so type-body and module-scope
        #: measures never collect them at all.
        self._track_identifiers = False
        #: Per-declaration memos, cleared every file. `function_flags` and
        #: `emit_function`/`function_extra` each need the SAME node's modifier
        #: text, annotation set and signature text; each used to re-derive
        #: them (three byte-slice decodes for the signature alone) two or
        #: three times per symbol. Keyed by tree-sitter node id, which is
        #: unique within one tree -- and one file is one tree.
        self._mods_cache: dict[int, str] = {}
        self._annos_cache: dict[int, set] = {}
    ...
    # -- per-declaration memos ---------------------------------------------
    def _mods_of(self, node: Any, src: bytes) -> str:
        """`_modifiers_text`, memoized per declaration node."""
        mods = self._mods_cache.get(node.id)
        if mods is None:
            mods = self._mods_cache[node.id] = _modifiers_text(node, src)
        return mods

    def _annos_of(self, node: Any, src: bytes) -> set[str]:
        """`_annotation_names`, memoized per declaration node.

        The cached set is shared: every caller only tests membership and
        length, none mutate it.
        """
        annos = self._annos_cache.get(node.id)
        if annos is None:
            annos = self._annos_cache[node.id] = _annotation_names(node, src)
        return annos

    def signature_of(self, node: Any, rec: FileRec) -> str:
        """Memoized: `function_flags` and the symbol row both need the same
        text, which used to cost two byte-slice decodes per function."""
        sig = self._sig_cache.get(node.id)
        if sig is None:
            sig = self._sig_cache[node.id] = \
                super().signature_of(node, rec)
        return sig

    # -- symbol shape ------------------------------------------------------
    def visibility_of(self, node: Any, rec: FileRec) -> str:
        mods = self._mods_of(node, rec.data)
        if _WORD_PUBLIC.search(mods):
            return "public"
        if _WORD_PROTECTED.search(mods):
            return "protected"
        if _WORD_PRIVATE.search(mods):
            return "private"
        return "package"
    ...
    def function_flags(self, node: Any, rec: FileRec,
        ...
        mods = self._mods_of(node, src)
        annos = self._annos_of(node, src)
        ...
        return dict(
            ...
            is_static=1 if _WORD_STATIC.search(mods) else 0,
            is_abstract=1 if _WORD_ABSTRACT.search(mods)
            ...
            n_synchronized_methods=1 if _WORD_SYNCHRONIZED.search(mods) else 0,
            n_native_calls=1 if _WORD_NATIVE.search(mods) else 0,
    ...
    def on_call(self, node: Any, src: bytes, st: BodyStats,
        ...
        if node.type == "object_creation_expression":
            ...
            if tname == "Thread":
                # Wave-3: a raw unmanaged thread. Ranks against the same
                # file's executor/virtual-thread infrastructure in SQL.
                st.bump("n_thread_alloc")
        ...
        if _b.startswith("add") and ("Listener" in _b or "Handler" in _b):
            # wave-3 listener ledger: a registration whose removal lives
            # (or does not live) somewhere else entirely.
            st.bump("n_listener_add")
        elif _b.startswith("remove") and ("Listener" in _b or "Handler" in _b):
            st.bump("n_listener_remove")
        if _b in ("parseExpression", "eval", "evaluate",
                  "evaluateExpression"):
            # Dynamic expression evaluation sink: SpEL parseExpression,
            # ScriptEngine.eval, ELProcessor.evaluate. Name-exact; a domain
            # eval() on some other class counts too and the query says so.
            st.bump("n_spel_eval")
        ...
        # unauthenticated-input surface query exclude this method. Same
        # substring tests as the AUTH_MARKERS any(), without the genexpr
        # and the lowercase copy per call site.
        full_lower = full.lower()
        if ("auth" in full_lower or "security" in full_lower
                or "login" in full_lower or "jwt" in full_lower
                or "token" in full_lower or "isauthenticated" in full_lower
                or "hasrole" in full_lower):
            st.bump("n_auth_call")
        ...
        elif base == "valueOf" and full.split(".")[0] in _BOX_VALUEOF_TYPES:
            st.bump("n_boxing_sites")
            if loop_depth:
                st.bump("n_boxing_in_loop")
        elif base in _BOX_UNBOX_BASES:
            st.bump("n_boxing_sites")
            if loop_depth:
                st.bump("n_boxing_in_loop")
        elif base in _SETACCESSIBLE_BASES:
            st.bump("n_setaccessible")
        elif base in ("load", "loadLibrary") and full.startswith(
                ("System.", "Runtime.")):
            st.bump("n_native_calls")
        elif base in _FFM_ARENA_BASES:
            if full.startswith(_FFM_ARENA_RECEIVERS):
                st.bump("n_ffm_arena")
        elif base in _FFM_DOWNCALL_BASES:
            st.bump("n_ffm_downcall")
        elif base in _UNSAFE_BASES:
            st.bump("n_unsafe_calls")
        elif base in _ATOMIC_BASES:
            st.bump("n_atomic_ops")
        elif base in _WAIT_BASES:
            st.bump("n_wait_calls")
        elif base in _CLOSE_BASES:
        ...
        elif base in _REGEX_METHOD_BASES:
            st.bump("n_regex_compile")
            if loop_depth:
                st.bump("regex_in_loop")
        elif base in _OPTIONAL_BASES or (
                base in ("of", "empty") and full.startswith("Optional.")):
            st.bump("n_optional_ops")
        elif base in _QUERY_CALL_BASES:
    ...
    def on_node(self, node: Any, src: bytes, st: BodyStats,
        ...
        if t == "string_literal" and src[node.start_byte:
            ...
            # FIRST, ahead of the rest of the chain: identifiers are the
            # single most common node type, and every one of them used to
            # fall through ~17 type comparisons to reach its branch at the
            # bottom. The branch types are mutually exclusive, so position
            # in the chain is semantics-free.
            if self._track_identifiers:
            ...
            # x++ / --x: replayed in function_extra, where the owner's
            # volatile-field set is known, to catch a non-atomic compound
            # update of a volatile field (Error Prone NonAtomicVolatileUpdate).
    ...
    def parse_imports(self, root: Any, rec: FileRec, bufs: Buffers) -> None:
        src = rec.data
        # One file is one tree and tree-sitter node ids are unique within a
        # tree, so this is the point where the per-declaration memos reset.
        self._mods_cache.clear()
        self._annos_cache.clear()
        ...
        for m in MODULE_IMPORT_RE.finditer(rec.text):
            ...
            bufs.imports.append(
                (rec.fid, m.group(1)[:300], None, None, "import module",
                 line, 1, 0, 1, 0, 0, 1))
        # Module directives exist ONLY inside a `module_declaration` (the
        # grammar places them nowhere else -- anywhere else they recover as
        # ERROR tokens, which these type tests never match). Walking only
        # those subtrees instead of the whole tree keeps every row identical
        # while skipping a full cursor walk of every ordinary file -- 436k
        # node visits per build on gson, all of them finding nothing.
        for top in root.named_children:
            if top.type != "module_declaration":
                continue
                ...
                if n.type == "requires_module_directive":
                    ...
                    bufs.rows("jpms_directives").append(
                        (rec.fid, "requires",
                         ...
                         n.start_point[0] + 1))
                elif n.type in ("exports_module_directive",
                    ...
                    bufs.rows("jpms_directives").append(
                        (rec.fid, n.type.split("_")[0],
                         ...
                         n.start_point[0] + 1))
                elif n.type in ("uses_module_directive",
    ...
    def type_extra(self, node: Any, rec: FileRec, db: sqlite3.Connection,
        ...
        for fl in body.named_children:
            ...
            ftype = _txt(tn, src).strip() if tn is not None else ""
            is_static = 1 if _WORD_STATIC.search(mods) else 0
            is_final = 1 if _WORD_FINAL.search(mods) else 0
            is_volatile = 1 if _WORD_VOLATILE.search(mods) else 0
            ...
            for d in fl.named_children:
                ...
                if "ThreadLocal" in ftype:
                    tls.add(fname)
                if any(k in ftype for k in FORMAT_FIELD_TYPES):
                    dfs.add(fname)
                if is_static and not is_final and _is_collection(ftype):
                    scl.add(fname)
                bufs.fields.append(
        ...
        if dfs:
            self.datefmt_fields[name] = dfs
        if scl:
            self.statcoll_fields[name] = scl
    ...
    def function_extra(self, node: Any, rec: FileRec, db: sqlite3.Connection,
                ...
                if t.type in ("type_identifier", "scoped_type_identifier",
                    ...
                    bufs.rows("exceptions").append(
                        (sid, fid, "throws", tn[:120],
                         int(tn in BROAD_EXCEPTIONS), 0, 0, 0, 0, 0,
                         c.start_point[0] + 1))
        ...
        volatiles = self.volatile_fields.get(owner) or ()
        statics = self.static_fields.get(owner) or ()
        tlocals = self.threadlocal_fields.get(owner) or ()
        datefmts = self.datefmt_fields.get(owner) or ()
        statcolls = self.statcoll_fields.get(owner) or ()
        # Identifier rows are replayed against the field-name sets as BYTES:
        # a decode("utf-8","replace") of the slice equals the field name iff
        # the raw bytes equal the name's UTF-8 bytes (field names are ASCII
        # identifiers; invalid sequences decode to U+FFFD and never match).
        # One bytes slice per identifier instead of a full decode.
        vol_b = frozenset(v.encode() for v in volatiles)
        df_b = frozenset(d.encode() for d in datefmts)
        n_vol = 0
        n_static_write = 0
        n_tl_ops = 0
        n_tl_remove = 0
            ...
            if t == "synchronized_statement":
                ...
                if recv and recv in tlocals:
                    if mname in ("set", "get", "withInitial"):
                        ...
                        n_tl_remove += 1
                if mname in ("wait", "notify", "notifyAll"):
                    # monitor ledger: WHICH object, and is this call inside
                    # the synchronized region that guards it? The query pairs
                    # the monitor name with lock_ops rows OTHER methods hold.
                    bufs.rows("monitor_ops").append(
                        (sid, fid,
                         (recv.rsplit(".", 1)[-1] if recv else "")[:80],
                         mname, int(_in_synchronized(n, body)),
                         n.start_point[0] + 1))
                base_recv = recv.rsplit(".", 1)[-1] if recv else ""
                if base_recv and base_recv in statcolls:
                    if mname in STATIC_COLL_ADDS:
                        n_coll_add += 1
                    elif mname in STATIC_COLL_REMOVES:
                        n_coll_rem += 1
            elif t == "object_creation_expression":
                ...
                # S2142 / Error Prone InterruptedExceptionSwallowed: the
                # interrupt flag must be re-set (Thread.currentThread()
                # .interrupt()) or the exception rethrown; anything else
                # silently eats the cancellation signal.
                restores = int("interrupt" in ctxt)
                for c in n.named_children:
                ...
                bufs.rows("exceptions").append(
                    (sid, fid, "throw", tn[:120],
                     int(tn in BROAD_EXCEPTIONS), 0, 0, 0, int(depth > 0), 0,
                     n.start_point[0] + 1))
            elif t == "identifier":
                nb = src[n.start_byte:n.end_byte]
                if nb in vol_b:
                    n_vol += 1
                if nb in df_b:
                    n_df_use += 1
                ...
                if left is not None:
                    ...
                    if lt.rsplit(".", 1)[-1] in statics:
                        n_static_write += 1
                    op = _field_child(n, "operator")
                    if op is not None and _txt(op, src) != "=" and \
                            lt.rsplit(".", 1)[-1] in volatiles:
                        # volatileField += x: not atomic even though the
                        # field is volatile (Error Prone
                        # NonAtomicVolatileUpdate).
                        n_volcmp += 1
            elif t == "update_expression":
                kids = [c for c in n.named_children if c.type != "comment"]
                if kids:
                    kt = _txt(kids[0], src).strip()
                    if kt.rsplit(".", 1)[-1] in volatiles:
                        n_volcmp += 1
        ...
        if n_volcmp:
            updates["n_volatile_compound"] = n_volcmp
        if n_df_use:
            updates["n_shared_datefmt_use"] = n_df_use
        if n_coll_add:
            updates["n_static_coll_add"] = n_coll_add
        if n_coll_rem:
            updates["n_static_coll_remove"] = n_coll_rem
        if updates:
        ...
        if _WORD_NATIVE.search(self._mods_of(node, src)):
    ...
    def flush_extra(self, db: sqlite3.Connection, bufs: Buffers) -> None:
        for tbl, sql in (
            ...
            ("exceptions",
             ...
             "is_empty,rethrows,logs,in_loop,restores,line) "
             "VALUES(?,?,?,?,?,?,?,?,?,?,?)"),
            ("monitor_ops",
             "INSERT INTO monitor_ops(symbol_id,file_id,monitor,op,in_sync,"
             "line) VALUES(?,?,?,?,?,?)"),
            ("generics",
...
def _in_synchronized(node: Any, stop: Any) -> int:
    """1 when the wait/notify call site sits inside a synchronized region.

    Used by the monitor ledger (monitor_ops): a wait() OUTSIDE every
    synchronized block of its method is only a bug when the monitor protocol
    lives in other methods -- which is the cross-method pairing the query
    makes.
    """
    cur = node.parent
    while cur is not None and cur.id != stop.id:
...
(
    ...
    "equals() overridden without hashCode() or vice versa (SpotBugs EI/EQ)",
    "ANSWERS which types override equals() but not hashCode() (or vice versa),\n"
    ...
    """WITH eq AS (
        SELECT m.parent_id AS class_id, COUNT(*) AS n
        FROM overrides o JOIN symbols m ON m.id=o.symbol_id
        WHERE o.method_name='equals' GROUP BY m.parent_id),
    hc AS (
        SELECT m.parent_id AS class_id, COUNT(*) AS n
        FROM overrides o JOIN symbols m ON m.id=o.symbol_id
        WHERE o.method_name='hashCode' GROUP BY m.parent_id)
    ...
    JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    LEFT JOIN eq ON eq.class_id=s.id
    LEFT JOIN hc ON hc.class_id=s.id
    WHERE s.kind IN ('class','interface') AND f.is_test=0
      AND COALESCE(m.name,'') LIKE :mod
...
(
    ...
    "ANSWERS where Thread.sleep is called while holding a lock, blocking other\n"
    ...
    """SELECT s.name,
        s.n_synchronized_blocks AS sync_blocks,
        s.n_synchronized_methods AS sync_methods,
        s.n_lock_acquire AS lock_acquires,
        s.n_wait_calls AS wait_calls,
        s.fan_in, s.cyclomatic AS cyclo,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_synchronized_blocks + s.n_synchronized_methods > 0
      AND s.n_wait_calls > 0 AND f.is_test=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_wait_calls DESC, s.fan_in DESC LIMIT :lim"""),
(
    "double-checked-locking",
    "Broken double-checked locking pattern (PMD DC)",
    "ANSWERS where the double-checked locking pattern is detected by the\n"
    ...
    """SELECT s.name, h.pattern, h.n AS sites,
        s.n_synchronized_blocks AS sync_blocks,
        s.n_volatile_access AS volatile_access,
        s.cyclomatic AS cyclo, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN hazards h ON h.symbol_id=s.id AND h.pattern='double-checked-locking'
    JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE f.is_test=0 AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC, h.n DESC LIMIT :lim"""),
(
    ...
    "String concatenation with + inside a loop (PMD AppendCharacterWithChar)",
    "ANSWERS where string concatenation happens inside a loop using the +\n"
    ...
    """SELECT s.name, s.n_string_concat AS concat_sites,
...
(
    "executor-without-shutdown",
    "ExecutorService created but never shut down (SpotBugs/sonar)",
    "ANSWERS where an ExecutorService is created but shutdown() is never called,\n"
    ...
    "     lifecycle ownership.",
    """SELECT s.name, s.n_executor_create AS executors_created,
        s.n_submit_in_loop AS submits_in_loop,
        s.n_future_get AS future_gets,
        s.n_close_calls AS close_calls,
        s.is_pooled_executor_root, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_executor_create > 0 AND s.n_close_calls=0 AND f.is_test=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_executor_create DESC, s.fan_in DESC LIMIT :lim"""),
(
    "submit-in-loop",
    "ExecutorService.submit inside a loop (PMD/sonar)",
    "ANSWERS where tasks are submitted to an ExecutorService inside a loop\n"
    ...
    """SELECT s.name, s.n_submit_in_loop AS submits_in_loop,
        s.n_executor_create AS executors,
        s.n_future_get AS future_gets,
        s.n_loops AS loops, s.cyclomatic AS cyclo, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_submit_in_loop > 0 AND f.is_test=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_submit_in_loop DESC, s.fan_in DESC LIMIT :lim"""),
(
    "null-return-ignore",
    "Function that can return null with high fan_in (SpotBugs NP)",
    "ANSWERS which widely-called functions return null, so every caller must\n"
    ...
    """SELECT s.name, s.n_null_returns AS null_returns,
        s.fan_in, s.n_calls, s.cyclomatic AS cyclo,
        s.n_optional_ops AS optional_ops,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_null_returns > 0 AND s.fan_in > 5 AND f.is_test=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC, s.n_null_returns DESC LIMIT :lim"""),
(
    "static-mutable-state",
    "Static mutable fields written from non-static methods (SpotBugs ST)",
    "ANSWERS where a non-static method writes to a static field, creating shared\n"
    ...
    """SELECT s.name, s.n_static_writes AS static_writes,
        s.is_static, s.n_synchronized_blocks AS sync_blocks,
        s.n_atomic_ops AS atomic_ops, s.n_volatile_access AS volatile_access,
        s.fan_in, s.cyclomatic AS cyclo,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_static_writes > 0 AND s.is_static=0 AND f.is_test=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC, s.n_static_writes DESC LIMIT :lim"""),
(
    "weak-random",
    "java.util.Random for security-sensitive operations (SpotBugs/sonar)",
    "ANSWERS where java.util.Random is used, which is predictable and not\n"
    ...
    """SELECT s.name, s.n_weak_random AS weak_random_calls,
        s.n_load_library AS loadLibrary_calls,
...
(
    ...
    "ANSWERS which methods are recorded in the overrides table without the\n"
    ...
    """SELECT s.name, o.owner_type, o.parent_type,
        o.is_annotated, s.fan_in, s.cyclomatic AS cyclo,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN overrides o ON o.symbol_id=s.id
    JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
...
# ======================================================================
# wave-3 pack: cross-method defect families the first three packs left
# open. Sources: Spring Framework reference (proxy semantics, scopes,
# scheduling, events, cache), SonarSource S2222/S2142/S2095 cross-function
# faces, Error Prone (NonAtomicVolatileUpdate, InterruptedExceptionSwallowed),
# IntelliJ inspections (wait/notify outside synchronized), PMD
# UnsynchronizedStaticFormatter, OWASP injection family, general knowledge.
# ======================================================================
(
    "proxy-bypass-self-invocation",
    "@Transactional/@Async/@Cacheable called ONLY from the same bean: the proxy is bypassed",
    "ANSWERS the Spring defect no linter can see: proxy-managed annotations\n"
    ...
    """WITH annotated AS (
        SELECT DISTINCT a.symbol_id FROM attributes a
        WHERE a.name IN ('Transactional','Async','Cacheable','CacheEvict',
                         'CachePut','Caching','Retryable','Validated')),
    self_only AS (
        SELECT DISTINCT e.callee_id
        FROM edges e
        JOIN symbols caller ON caller.id = e.caller_id
        JOIN symbols callee ON callee.id = e.callee_id
        WHERE callee.owner_type <> ''
          AND caller.owner_type = callee.owner_type
          AND NOT EXISTS (
              SELECT 1 FROM edges e2
              JOIN symbols c2 ON c2.id = e2.caller_id
              JOIN files cf2 ON cf2.id = c2.file_id
              WHERE e2.callee_id = e.callee_id
                AND (c2.owner_type <> callee.owner_type
                     OR c2.owner_type = '')
                AND cf2.is_test = 0))
    SELECT s.name, s.owner_type AS owner,
        GROUP_CONCAT(DISTINCT a.name) AS proxy_annos,
        COUNT(DISTINCT a.name) AS n_annos,
        s.n_calls AS calls_made,
        f.path || ':' || s.line_start AS at
    FROM annotated
    JOIN symbols s ON s.id = annotated.symbol_id
    JOIN attributes a ON a.symbol_id = s.id
        AND a.name IN ('Transactional','Async','Cacheable','CacheEvict',
                       'CachePut','Caching','Retryable','Validated')
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.kind IN ('function','method')
      AND s.id IN (SELECT callee_id FROM self_only)
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id
    ORDER BY n_annos DESC, s.n_calls DESC LIMIT :lim"""),
(
    "transactional-checked-exception-commit",
    "@Transactional without rollbackFor on a method that declares checked exceptions: it COMMITS",
    "ANSWERS the default-rollback trap: Spring rolls @Transactional back on\n"
    ...
    "     reads as absent.",
    """WITH tx AS (
        SELECT symbol_id, GROUP_CONCAT(args) AS args_
        FROM attributes WHERE name='Transactional' GROUP BY symbol_id),
    checked AS (
        SELECT symbol_id, COUNT(DISTINCT type) AS n_checked
        FROM exceptions
        WHERE kind='throws' AND type <> ''
          AND type NOT IN ('RuntimeException','Error','Exception',
                           'Throwable')
        GROUP BY symbol_id)
    SELECT s.name, s.owner_type AS owner,
        tx.args_ AS tx_args,
        c.n_checked AS checked_thrown,
        s.fan_in AS callers,
        f.path || ':' || s.line_start AS at
    FROM tx
    JOIN symbols s ON s.id = tx.symbol_id
    JOIN checked c ON c.symbol_id = s.id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE (tx.args_ IS NULL OR tx.args_ NOT LIKE '%rollbackFor%')
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY c.n_checked DESC, s.fan_in DESC LIMIT :lim"""),
(
    "prototype-bean-into-singleton",
    "Prototype/request-scoped bean field-injected into a singleton: ONE instance for the whole app",
    "ANSWERS the Spring scope-capture defect: a field whose declared type is\n"
    ...
    """WITH proto_types AS (
        ...
        FROM symbols t
        JOIN attributes ta ON ta.symbol_id = t.id
        WHERE t.kind IN ('class','interface','record','enum')
          AND (ta.name IN ('RequestScope','SessionScope','RefreshScope',
                           'ApplicationScope')
               OR (ta.name='Scope' AND ta.args LIKE '%prototype%'))),
    singleton_beans AS (
        SELECT DISTINCT b.id
        FROM symbols b
        JOIN attributes ba ON ba.symbol_id = b.id
        WHERE b.kind = 'class'
          AND ba.name IN ('Component','Service','Repository','Controller',
                          'RestController','ControllerAdvice',
                          'Configuration','Singleton')),
    inj AS (
        SELECT fd.symbol_id AS bean_id, COUNT(*) AS n_proto_fields,
            GROUP_CONCAT(fd.name) AS fields_, GROUP_CONCAT(fd.type) AS types_
        FROM fields fd
        JOIN proto_types pt
          ON pt.name = SUBSTR(fd.type || '<', 1,
                              INSTR(fd.type || '<', '<') - 1)
        GROUP BY fd.symbol_id)
    SELECT s.name AS singleton_bean,
        inj.n_proto_fields, inj.fields_, inj.types_,
        f.path || ':' || s.line_start AS at
    FROM singleton_beans
    JOIN inj ON inj.bean_id = singleton_beans.id
    JOIN symbols s ON s.id = singleton_beans.id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY inj.n_proto_fields DESC LIMIT :lim"""),
(
    "returned-resource-never-closed",
    "Method opens a closeable, returns it, and NO caller ever closes anything",
    "ANSWERS the ownership half of Sonar S2095 that per-method checkers\n"
    ...
    "     can appear.",
    """WITH opens AS (
        SELECT symbol_id, COUNT(*) AS n_opens, SUM(in_try_resources) AS n_twr
        FROM resources
        WHERE in_try_resources=0
        GROUP BY symbol_id),
    callers AS (
        SELECT e.callee_id AS sid,
            COUNT(DISTINCT e.caller_id) AS n_callers,
            COUNT(DISTINCT CASE WHEN c.n_close_calls > 0
                                 OR c.n_try_resources > 0
                                THEN c.id END) AS n_closing
        FROM edges e
        JOIN symbols c ON c.id = e.caller_id
        JOIN files cf ON cf.id = c.file_id
        WHERE cf.is_test=0
        GROUP BY e.callee_id)
    SELECT s.name, s.owner_type AS owner, s.return_type,
        o.n_opens, cl.n_callers, cl.n_closing,
        f.path || ':' || s.line_start AS at
    FROM opens o
    JOIN symbols s ON s.id = o.symbol_id
    JOIN callers cl ON cl.sid = s.id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE cl.n_closing = 0
      AND SUBSTR(s.return_type || '<', 1,
                 INSTR(s.return_type || '<', '<') - 1) IN
          ('InputStream','OutputStream','Reader','Writer','Stream',
           'IntStream','LongStream','DoubleStream','Connection','Channel',
           'Socket','ServerSocket','DatagramSocket','RandomAccessFile',
           'Scanner','ObjectInput','ObjectOutput','Closeable',
           'AutoCloseable',
           'FileInputStream','FileOutputStream','FileReader','FileWriter',
           'BufferedReader','BufferedWriter','BufferedInputStream',
           'BufferedOutputStream','InputStreamReader','OutputStreamWriter',
           'PrintWriter','PrintStream','DataInputStream',
           'DataOutputStream','ObjectInputStream','ObjectOutputStream',
           'ZipFile','ZipInputStream','ZipOutputStream','GZIPInputStream',
           'GZIPOutputStream','JarFile','Statement','PreparedStatement',
           'CallableStatement','ResultSet',
           'JsonReader','JsonWriter')
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY cl.n_callers DESC, o.n_opens DESC LIMIT :lim"""),
(
    "lock-acquire-without-release",
    "Explicit Lock acquired, never released in the method -- every caller inherits the leak",
    "ANSWERS the cross-function face of Sonar S2222: the rule only checks\n"
    ...
    """WITH acq AS (
        SELECT symbol_id, COUNT(*) AS n_acq, MIN(line) AS first_line
        FROM lock_ops WHERE op='acquire' AND kind='lock'
        GROUP BY symbol_id),
    rel AS (
        SELECT symbol_id, COUNT(*) AS n_rel
        FROM lock_ops WHERE op='release'
        GROUP BY symbol_id)
    SELECT s.name, s.owner_type AS owner, a.n_acq,
        s.n_io AS io_in_body, s.n_wait_calls AS waits,
        s.fan_in AS callers,
        f.path || ':' || a.first_line AS at
    FROM acq a
    JOIN symbols s ON s.id = a.symbol_id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.id NOT IN (SELECT symbol_id FROM rel)
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY a.n_acq DESC, s.fan_in DESC LIMIT :lim"""),
(
    "transitive-block-under-lock",
    "Guarded region calls out to a method that blocks -- the blocking lives 1-2 hops away",
    "ANSWERS the transitive version of lock-held-across-io: the critical\n"
    ...
    """WITH RECURSIVE down(sym, depth) AS (
        SELECT l.symbol_id, 0 FROM lock_ops l
        WHERE l.op='acquire' AND l.holds_call > 0
        UNION
        SELECT e.callee_id, d.depth+1 FROM down d
        JOIN edges e ON e.caller_id = d.sym
        WHERE d.depth < 2 AND e.is_self = 0),   -- depth bound: 2 hops
    blockers AS (
        SELECT id, n_wait_calls + n_io + n_query_calls AS n_block
        FROM symbols
        WHERE n_wait_calls > 0 OR n_io > 0 OR n_query_calls > 0)
    SELECT s.name, s.owner_type AS owner,
        MIN(d.depth) AS hops,
        b.n_block AS block_ops,
        s.n_lock_acquire + s.n_synchronized_blocks AS locks_held,
        s.fan_in AS callers,
        f.path || ':' || s.line_start AS at
    FROM down d
    JOIN symbols s ON s.id = d.sym
    JOIN blockers b ON b.id = d.sym
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE d.depth > 0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id
    ORDER BY hops ASC, b.n_block DESC LIMIT :lim"""),
(
    "wait-notify-outside-sync",
    "wait()/notify() outside any synchronized block, on a monitor OTHER methods lock",
    "ANSWERS the cross-method monitor mismatch (IntelliJ 'wait()/notify()\n"
    ...
    """WITH class_monitors AS (
        SELECT DISTINCT l.lock_name
        FROM lock_ops l
        WHERE l.op='acquire' AND l.lock_name <> '' AND l.lock_name <> 'this'),
    sites AS (
        SELECT mo.symbol_id, mo.monitor, mo.op,
            COUNT(*) AS n_sites,
            SUM(mo.in_sync) AS n_in_sync,
            MIN(mo.line) AS first_line
        FROM monitor_ops mo
        GROUP BY mo.symbol_id, mo.monitor, mo.op)
    SELECT s.name, s.owner_type AS owner, si.op, si.monitor,
        si.n_sites - si.n_in_sync AS bare_sites,
        si.n_in_sync,
        s.fan_in AS callers,
        f.path || ':' || si.first_line AS at
    FROM sites si
    JOIN symbols s ON s.id = si.symbol_id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE si.n_in_sync < si.n_sites
      AND si.monitor IN (SELECT lock_name FROM class_monitors)
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY bare_sites DESC, s.fan_in DESC LIMIT :lim"""),
(
    "interrupt-swallowed",
    "InterruptedException caught, never rethrown, interrupt flag never restored (S2142)",
    "ANSWERS where the cancellation signal dies: a catch of\n"
    ...
    """SELECT s.name, s.owner_type AS owner,
        COUNT(DISTINCT x.line) AS swallowed,
        x.type AS caught,
        s.fan_in AS callers,
        f.path || ':' || MIN(x.line) AS at
    FROM exceptions x
    JOIN symbols s ON s.id = x.symbol_id
    JOIN files f ON f.id = x.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE x.kind='catch' AND x.type='InterruptedException'
      AND x.rethrows=0 AND x.restores=0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id
    ORDER BY swallowed DESC, s.fan_in DESC LIMIT :lim"""),
(
    "shared-format-field-use",
    "Methods using a shared SimpleDateFormat/NumberFormat/Calendar field: not thread-safe",
    "ANSWERS shared non-thread-safe formatters (PMD UnsynchronizedStatic-\n"
    ...
    """SELECT s.name, s.owner_type AS owner,
        s.n_shared_datefmt_use AS shared_uses,
        s.fan_in AS callers,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    ...
    WHERE s.kind IN ('function','method','constructor')
      AND s.n_shared_datefmt_use > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_shared_datefmt_use DESC, s.fan_in DESC LIMIT :lim"""),
(
    "string-format-in-loop",
    "String.format-style formatting inside a loop, on the hot paths callers actually hit",
    "ANSWERS per-element formatting cost: .format(...) called inside a loop\n"
    ...
    """SELECT s.name, s.owner_type AS owner,
        s.n_format_in_loop AS fmt_in_loop,
        s.max_loop_depth AS depth,
        s.fan_in AS callers,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.kind IN ('function','method','constructor')
      AND s.n_format_in_loop > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_format_in_loop DESC, s.fan_in DESC LIMIT :lim"""),
(
    "common-pool-from-entry",
    "parallelStream() on the COMMON pool, reachable from a request entry point",
    "ANSWERS the transitive version of parallel-stream-hazard: the\n"
    ...
    """WITH RECURSIVE down(root, sym, depth) AS (
        ...
        WHERE (s.is_handler=1 OR s.is_entrypoint=1
               OR (s.is_public=1 AND s.fan_in=0))
    ...
    SELECT s.name, s.owner_type AS owner,
        MIN(d.depth) AS hops,
        COUNT(DISTINCT d.root) AS n_entry_paths,
        s.n_parallel_streams AS parallel_ops,
        f.path || ':' || s.line_start AS at
    FROM down d
    JOIN symbols s ON s.id = d.sym
    JOIN symbols rt ON rt.id = d.root
    JOIN files ef ON ef.id = rt.file_id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE d.depth > 0 AND s.n_parallel_streams > 0 AND ef.is_test=0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id
    ORDER BY hops ASC, s.n_parallel_streams DESC LIMIT :lim"""),
(
    "static-write-from-entry",
    "Static mutable state written on a path reachable from a request entry point",
    "ANSWERS the reachability half of the shared-statics story that Q8/Q23\n"
    ...
    SELECT s.name, s.owner_type AS owner,
        MIN(d.depth) AS hops,
        COUNT(DISTINCT d.root) AS n_entry_paths,
        s.n_static_writes AS static_writes,
        s.n_volatile_access AS volatile_refs,
        f.path || ':' || s.line_start AS at
    FROM down d
    JOIN symbols s ON s.id = d.sym
    JOIN symbols rt ON rt.id = d.root
    ...
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE d.depth > 0 AND s.n_static_writes > 0 AND ef.is_test=0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id
    ORDER BY s.n_static_writes DESC, hops ASC LIMIT :lim"""),
(
    "listener-add-remove-imbalance",
    "Classes registering listeners/handlers with no removal anywhere in the class",
    "ANSWERS the listener-lifetime ledger: addXxxListener/addXxxHandler\n"
    ...
    """WITH ledger AS (
        SELECT owner_type AS otype,
            SUM(n_listener_add) AS adds,
            SUM(n_listener_remove) AS removes,
            COUNT(DISTINCT id) AS registering_fns
        FROM symbols
        WHERE owner_type <> '' AND kind IN ('function','method')
        GROUP BY owner_type)
    SELECT s.name AS class_, l.adds, l.removes,
        l.adds - l.removes AS net_added,
        l.registering_fns,
        f.path || ':' || s.line_start AS at
    FROM ledger l
    JOIN symbols s ON s.name = l.otype AND s.kind IN ('class','interface')
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE l.adds > l.removes
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY l.adds DESC, s.name LIMIT :lim"""),
(
    "expression-eval-from-input",
    "SpEL/ScriptEngine evaluation reachable from entry points or sharing the method's input reads",
    "ANSWERS the dynamic-evaluation sink of the OWASP injection family:\n"
    ...
    """WITH RECURSIVE down(root, sym, depth) AS (
        ...
        WHERE d.depth < 3 AND e.is_self = 0),  -- depth bound: 3 hops
    reachable AS (
        SELECT s.id, MIN(d.depth) AS hops,
            COUNT(DISTINCT d.root) AS n_paths
        FROM down d
        JOIN symbols s ON s.id = d.sym
        WHERE d.depth > 0 AND s.n_spel_eval > 0
        GROUP BY s.id),
    direct_input AS (
        SELECT DISTINCT uis.symbol_id
        FROM user_input_sites uis),
    evals AS (
        SELECT id FROM symbols WHERE n_spel_eval > 0)
    SELECT s.name, s.owner_type AS owner,
        COALESCE(r.hops, 0) AS hops,
        COALESCE(r.n_paths, 0) AS n_entry_paths,
        CASE WHEN s.id IN (SELECT symbol_id FROM direct_input)
             THEN 1 ELSE 0 END AS reads_input,
        s.n_spel_eval AS eval_calls,
        f.path || ':' || s.line_start AS at
    FROM evals
    JOIN symbols s ON s.id = evals.id
    LEFT JOIN reachable r ON r.id = s.id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE (r.id IS NOT NULL
           OR s.id IN (SELECT symbol_id FROM direct_input))
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_spel_eval DESC, r.hops ASC LIMIT :lim"""),
(
    "volatile-compound-update",
    "volatileField++ / += on a volatile field: the compound update is not atomic (Error Prone)",
    "ANSWERS Error Prone NonAtomicVolatileUpdate: a compound update\n"
    ...
    """SELECT s.name, s.owner_type AS owner,
        s.n_volatile_compound AS bad_updates,
        s.n_volatile_access AS volatile_refs,
        s.fan_in AS callers,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    ...
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.kind IN ('function','method','constructor')
      AND s.n_volatile_compound > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_volatile_compound DESC, s.fan_in DESC LIMIT :lim"""),
(
    "manual-thread-allocation",
    "new Thread(...) beside existing executor/virtual-thread infrastructure",
    "ANSWERS unmanaged threads: `new Thread(...)` sites in files that also\n"
    ...
    """SELECT s.name, s.owner_type AS owner,
        s.n_thread_alloc AS raw_threads,
        CASE WHEN EXISTS (SELECT 1 FROM symbols t
             WHERE t.file_id = s.file_id
             AND (t.is_executor_root = 1 OR t.is_pooled_executor_root = 1
                  OR t.is_virtual_thread_root = 1))
             THEN 1 ELSE 0 END AS has_pool_infra,
        s.fan_in AS callers,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.kind IN ('function','method','constructor')
      AND s.n_thread_alloc > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_thread_alloc DESC, has_pool_infra DESC LIMIT :lim"""),
(
    "scheduled-overlap-risk",
    "@Scheduled tasks whose body does IO/locking: overlap and starvation risk",
    "ANSWERS the scheduling defect family from the Spring reference: a\n"
    ...
    """WITH sched AS (
        SELECT symbol_id, GROUP_CONCAT(args) AS args_, COUNT(*) AS n_annos
        FROM attributes WHERE name='Scheduled' GROUP BY symbol_id)
    SELECT s.name, s.owner_type AS owner,
        sched.args_ AS trigger_args,
        CASE WHEN sched.args_ LIKE '%fixedRate%' THEN 1 ELSE 0 END
            AS fixed_rate,
        s.n_io AS io_ops, s.n_query_calls AS db_calls,
        s.n_lock_acquire + s.n_synchronized_blocks AS locks,
        (s.n_io*2 + s.n_query_calls*2 + s.n_wait_calls*3
         + s.n_lock_acquire + s.n_synchronized_blocks) AS overlap_cost,
        f.path || ':' || s.line_start AS at
    FROM sched
    JOIN symbols s ON s.id = sched.symbol_id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY overlap_cost DESC, s.fan_in DESC LIMIT :lim"""),
(
    "event-listener-sync-burden",
    "@EventListener/@Subscribe handlers doing IO: they run synchronously in the PUBLISHER's thread",
    "ANSWERS the eventing defect from the Spring reference (and Guava's\n"
    ...
    """WITH listeners AS (
        SELECT symbol_id, GROUP_CONCAT(name) AS annos, COUNT(*) AS n
        FROM attributes
        WHERE name IN ('EventListener','TransactionalEventListener',
                       'Subscribe')
        GROUP BY symbol_id)
    SELECT s.name, s.owner_type AS owner,
        l.annos AS listener_annos,
        s.n_io AS io_ops, s.n_query_calls AS db_calls,
        s.n_wait_calls AS waits,
        (s.n_io*3 + s.n_query_calls*2 + s.n_wait_calls*3) AS sync_cost,
        s.fan_in AS publishers_reaching,
        f.path || ':' || s.line_start AS at
    FROM listeners l
    JOIN symbols s ON s.id = l.symbol_id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE (s.n_io > 0 OR s.n_query_calls > 0 OR s.n_wait_calls > 0)
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY sync_cost DESC, s.fan_in DESC LIMIT :lim"""),
(
    "cache-evict-drift",
    "@CacheEvict on cache names that NO @Cacheable/@Caching in the tree populates",
    "ANSWERS cache-name drift: @CacheEvict (or @CachePut) naming a cache\n"
    ...
    """SELECT s.name, s.owner_type AS owner,
        COUNT(DISTINCT a.args) AS unmatched_names,
        GROUP_CONCAT(DISTINCT a.args) AS evict_args,
        s.fan_in AS callers,
    ...
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE a.name = 'CacheEvict'
      AND NOT EXISTS (SELECT 1 FROM attributes c
                      WHERE c.name IN ('Cacheable','Caching')
                        AND c.args = a.args)
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id
    ORDER BY unmatched_names DESC, s.fan_in DESC LIMIT :lim"""),
(
    "private-proxy-annotation",
    "@Transactional/@Async/@Cacheable on a PRIVATE method: the proxy never sees it",
    "ANSWERS the hard boundary of Spring proxy semantics the reference\n"
    ...
    """SELECT s.name, s.owner_type AS owner, s.visibility,
        GROUP_CONCAT(DISTINCT a.name) AS proxy_annos,
        COUNT(DISTINCT a.name) AS n_annos,
        s.fan_in AS callers,
        f.path || ':' || s.line_start AS at
    ...
    WHERE a.name IN ('Transactional','Async','Cacheable','CacheEvict',
      ...
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id
    ORDER BY n_annos DESC, s.fan_in DESC LIMIT :lim"""),
(
    "static-collection-grows-never-pruned",
    "Static collection fields with add/put calls and no clear/remove anywhere in the class",
    "ANSWERS unbounded growth: classes whose static collection fields get\n"
    ...
    """WITH growth AS (
        SELECT owner_type AS otype,
            SUM(n_static_coll_add) AS adds,
            SUM(n_static_coll_remove) AS removes,
            COUNT(DISTINCT id) AS touching_fns
        FROM symbols
        WHERE owner_type <> '' AND kind IN ('function','method')
        GROUP BY owner_type),
    coll_fields AS (
        SELECT fd.symbol_id AS cid, COUNT(*) AS n_fields,
            GROUP_CONCAT(fd.name) AS names
        FROM fields fd
        WHERE fd.is_static=1 AND fd.is_const=0
        GROUP BY fd.symbol_id)
    SELECT s.name AS class_, g.adds, g.removes,
        g.adds - g.removes AS net_added,
        g.touching_fns, cf.n_fields AS static_collections,
        cf.names AS collection_fields,
        f.path || ':' || s.line_start AS at
    FROM growth g
    JOIN symbols s ON s.name = g.otype AND s.kind IN ('class')
    JOIN coll_fields cf ON cf.cid = s.id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE g.adds > g.removes
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY net_added DESC, g.adds DESC LIMIT :lim"""),
(
    "shared-bean-mutable-collection",
    "Mutable (non-concurrent) collection fields inside singleton-stereotyped beans",
    "ANSWERS shared-state-on-a-singleton: instance fields whose type is an\n"
    ...
    """WITH shared_beans AS (
        ...
        WHERE b.kind = 'class'
          AND ba.name IN ('Component','Service','Repository','Controller',
                          ...
                          'Configuration','Singleton')),
    unsafe AS (
        SELECT fd.symbol_id AS cid, COUNT(*) AS n_unsafe,
            GROUP_CONCAT(SUBSTR(fd.type || '<', 1,
                INSTR(fd.type || '<', '<') - 1)) AS types
        FROM fields fd
        WHERE fd.is_collection=1 AND fd.is_static=0
          AND SUBSTR(fd.type || '<', 1,
                     INSTR(fd.type || '<', '<') - 1) NOT IN
              ('ConcurrentHashMap','ConcurrentLinkedQueue',
               'ConcurrentLinkedDeque','ConcurrentSkipListMap',
               'ConcurrentSkipListSet','CopyOnWriteArrayList',
               'CopyOnWriteArraySet','LinkedBlockingQueue',
               'ArrayBlockingQueue','PriorityBlockingQueue',
               'LinkedBlockingDeque','DelayQueue','SynchronousQueue')
        GROUP BY fd.symbol_id)
    SELECT s.name AS bean, u.n_unsafe AS unsafe_collections,
        u.types AS field_types,
        s.fan_in AS callers,
        f.path || ':' || s.line_start AS at
    FROM shared_beans
    JOIN unsafe u ON u.cid = shared_beans.id
    JOIN symbols s ON s.id = shared_beans.id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY u.n_unsafe DESC, s.fan_in DESC LIMIT :lim"""),
]
...
(
    "serializable-no-uid",
    "Serializable class without serialVersionUID (SpotBugs SE)",
    "ANSWERS which Serializable types do not declare serialVersionUID, so any\n"
...
(
    ...
    "ANSWERS where printStackTrace is called, which writes to stderr without\n"
    ...
    """SELECT s.name, s.n_print_stacktrace AS stacktrace_calls,
        s.n_throws_declared AS declared_throws,
        s.n_catch AS catches, s.n_catch_broad AS broad_catches,
        s.fan_in, s.cyclomatic AS cyclo,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_print_stacktrace > 0 AND f.is_test=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_print_stacktrace DESC, s.fan_in DESC LIMIT :lim"""),
(
    "god-class",
    "A class with too many methods and high coupling (PMD GodClass)",
    "ANSWERS which classes have too many methods, too much complexity, and too\n"
    ...
    """SELECT s.name,
        (SELECT COUNT(*) FROM symbols c WHERE c.parent_id=s.id
         AND c.kind IN ('function','method')) AS n_methods,
        (SELECT SUM(c.cyclomatic) FROM symbols c WHERE c.parent_id=s.id
         AND c.kind IN ('function','method')) AS total_cyclo,
        s.fan_in, s.sloc,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
...
(
    "deep-nesting",
    "Functions with excessive nesting depth (PMD/sonar)",
...
(
    "too-many-params",
    "Functions with too many parameters (PMD/sonar)",
    "ANSWERS where a function has more than 5 parameters, making the call site\n"
    ...
    """SELECT s.name, s.n_params, s.n_optional_params,
        s.n_generic_params, s.sloc, s.cyclomatic AS cyclo,
...
(
    "suppressed-warnings",
    "Functions with @SuppressWarnings annotations (sonar)",
    "ANSWERS where @SuppressWarnings is used, which silences compiler warnings.\n"
    ...
    """SELECT s.name, s.n_suppressions AS suppressions,
        s.n_annotations AS annotations,
        s.n_unchecked_casts AS unchecked_casts,
        s.n_raw_types AS raw_types,
        s.fan_in, s.cyclomatic AS cyclo,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
...
# ----------------------------------------------------------------------
(
    "spring-proxy-surface",
    "Per module: proxy-managed annotations and how much of the surface is inert",
    "ANSWERS how much of a module's declared Spring semantics actually can\n"
        ...
        WHERE callee.owner_type <> ''
          ...
          AND NOT EXISTS (
              ...
              WHERE e2.callee_id = e.callee_id
                ...
                AND cf2.is_test = 0))
    SELECT m.name AS module_,
        COUNT(DISTINCT s.id) AS proxy_annotated,
        SUM(CASE WHEN s.visibility='private' THEN 1 ELSE 0 END)
            AS private_never_advised,
        SUM(CASE WHEN s.id IN (SELECT callee_id FROM selfonly)
                 THEN 1 ELSE 0 END) AS self_invoked_only,
        SUM(CASE WHEN s.n_throws_declared > 0 THEN 1 ELSE 0 END)
            AS throws_checked,
        MIN(f.path) || ':' || MIN(s.line_start) AS at
    FROM ann
    JOIN symbols s ON s.id = ann.symbol_id
    JOIN modules m ON m.id = s.module_id
    JOIN files f ON f.id = s.file_id
    WHERE f.is_test=0 AND m.name LIKE :mod
    GROUP BY m.id
    ORDER BY proxy_annotated DESC LIMIT :lim"""),
(
    "monitor-pressure-map",
    "Per module: guarded region sizes, monitor operations and the lock-owning functions",
    "ANSWERS where the serialization pressure lives: total guarded SLOC per\n"
    ...
    """WITH per_fn AS (
        SELECT s.id, s.module_id, s.file_id, s.line_start,
            COUNT(l.rowid) AS regions,
            COALESCE(SUM(l.region_sloc), 0) AS guarded_sloc
        FROM symbols s
        LEFT JOIN lock_ops l ON l.symbol_id = s.id AND l.op='acquire'
        WHERE s.kind IN ('function','method')
        GROUP BY s.id),
    mon AS (
        SELECT symbol_id, COUNT(*) AS ops
        FROM monitor_ops GROUP BY symbol_id)
    SELECT m.name AS module_,
        COUNT(DISTINCT p.id) AS locking_fns,
        SUM(p.regions) AS lock_regions,
        SUM(p.guarded_sloc) AS guarded_sloc,
        COALESCE(SUM(mo.ops), 0) AS wait_notify_ops,
        MIN(f.path) || ':' || MIN(p.line_start) AS at
    FROM per_fn p
    JOIN modules m ON m.id = p.module_id
    JOIN files f ON f.id = p.file_id
    LEFT JOIN mon mo ON mo.symbol_id = p.id
    WHERE (p.regions > 0 OR mo.ops IS NOT NULL)
      AND f.is_test=0 AND m.name LIKE :mod
    GROUP BY m.id
    ORDER BY guarded_sloc DESC LIMIT :lim"""),
(
    "listener-registration-surface",
    "Per module: listener/handler registrations versus deregistrations",
    "ANSWERS the module-level listener ledger: how many addXxxListener/\n"
    ...
    """SELECT m.name AS module_,
        SUM(s.n_listener_add) AS add_sites,
        SUM(s.n_listener_remove) AS remove_sites,
        COUNT(DISTINCT CASE WHEN s.n_listener_add > 0 THEN s.id END)
            AS registering_fns,
        CAST(100.0 * MIN(SUM(s.n_listener_remove), SUM(s.n_listener_add))
             / NULLIF(SUM(s.n_listener_add), 0) AS INT) AS pct_removed,
        MIN(f.path) || ':' || MIN(s.line_start) AS at
    FROM modules m
    JOIN symbols s ON s.module_id = m.id
    JOIN files f ON f.id = s.file_id
    WHERE (s.n_listener_add > 0 OR s.n_listener_remove > 0)
      AND s.kind IN ('function','method')
      AND f.is_test=0 AND m.name LIKE :mod
    GROUP BY m.id
    ORDER BY add_sites DESC LIMIT :lim"""),
(
    "thread-creation-mix",
    "Per module: raw `new Thread` sites versus executor and virtual-thread roots",
    "ANSWERS the module's thread budget shape: how many threads are created\n"
    ...
    """SELECT m.name AS module_,
        SUM(s.n_thread_alloc) AS raw_threads,
        COUNT(DISTINCT CASE WHEN s.is_executor_root = 1 THEN s.id END)
            AS executor_roots,
        COUNT(DISTINCT CASE WHEN s.is_pooled_executor_root = 1 THEN s.id END)
            AS pooled_roots,
        COUNT(DISTINCT CASE WHEN s.is_virtual_thread_root = 1 THEN s.id END)
            AS virtual_roots,
        MIN(f.path) || ':' || MIN(s.line_start) AS at
    FROM modules m
    JOIN symbols s ON s.module_id = m.id
    JOIN files f ON f.id = s.file_id
    WHERE (s.n_thread_alloc > 0 OR s.is_executor_root = 1
           OR s.is_pooled_executor_root = 1 OR s.is_virtual_thread_root = 1)
      AND s.kind IN ('function','method','constructor')
      AND f.is_test=0 AND m.name LIKE :mod
    GROUP BY m.id
    ORDER BY raw_threads DESC, executor_roots DESC LIMIT :lim"""),
(
    "closeable-ownership-surface",
    "Per module: resource opens, try-with-resources share, and unmanaged opens",
    "ANSWERS the module's resource-hygiene posture: how many resources are\n"
    ...
    """WITH res AS (
        SELECT s.module_id AS mid,
            COUNT(*) AS opens,
            SUM(r.in_try_resources) AS twr,
            SUM(r.closed_in_fn) AS closed,
            SUM(CASE WHEN r.in_try_resources=0 AND r.closed_in_fn=0
                     THEN 1 ELSE 0 END) AS unmanaged,
            MIN(f.path) AS sample_path,
            MIN(r.line) AS sample_line
        FROM resources r
        JOIN symbols s ON s.id = r.symbol_id
        JOIN files f ON f.id = r.file_id
        WHERE f.is_test=0
    ...
    SELECT m.name AS module_,
        r.opens, r.twr, r.closed, r.unmanaged,
        CAST(100.0 * r.twr / NULLIF(r.opens, 0) AS INT) AS twr_pct,
        r.sample_path || ':' || r.sample_line AS at
    FROM res r
    JOIN modules m ON m.id = r.mid
    WHERE m.name LIKE :mod
    ORDER BY r.unmanaged DESC, r.opens DESC LIMIT :lim"""),
(
    "exception-path-map",
    "Per module: interrupt handling, broad and empty catches, declared throws",
    "ANSWERS the module's exception-path hygiene in one row: how many\n"
    ...
    """SELECT m.name AS module_,
        COUNT(DISTINCT CASE WHEN x.kind='catch'
                             AND x.type='InterruptedException'
                            THEN x.symbol_id END) AS interrupt_catches,
        SUM(CASE WHEN x.kind='catch' AND x.type='InterruptedException'
                  AND x.rethrows=0 AND x.restores=0
                 THEN 1 ELSE 0 END) AS swallowed_interrupts,
        SUM(CASE WHEN x.kind='catch' AND x.is_broad=1
                 THEN 1 ELSE 0 END) AS broad_catches,
        SUM(CASE WHEN x.kind='catch' AND x.is_empty=1
                 THEN 1 ELSE 0 END) AS empty_catches,
        SUM(CASE WHEN x.kind='throws' THEN 1 ELSE 0 END) AS declared_throws,
        MIN(f.path) || ':' || MIN(x.line) AS at
    FROM exceptions x
    JOIN symbols s ON s.id = x.symbol_id
    JOIN modules m ON m.id = s.module_id
    JOIN files f ON f.id = x.file_id
    WHERE f.is_test=0 AND m.name LIKE :mod
    GROUP BY m.id
    ORDER BY swallowed_interrupts DESC, broad_catches DESC LIMIT :lim"""),
(
    "scheduler-surface",
    "Per module: @Scheduled tasks, fixedRate vs fixedDelay split, IO-heavy bodies",
    "ANSWERS the module's background-task surface: how many @Scheduled\n"
    ...
    """SELECT m.name AS module_,
        COUNT(DISTINCT s.id) AS scheduled_methods,
        SUM(CASE WHEN a.args LIKE '%fixedRate%' THEN 1 ELSE 0 END)
            AS fixed_rate,
        SUM(CASE WHEN a.args LIKE '%fixedDelay%' THEN 1 ELSE 0 END)
            AS fixed_delay,
        SUM(CASE WHEN s.n_io > 0 OR s.n_query_calls > 0
                 THEN 1 ELSE 0 END) AS io_heavy,
        MIN(f.path) || ':' || MIN(a.line) AS at
    FROM attributes a
    JOIN symbols s ON s.id = a.symbol_id
    JOIN modules m ON m.id = s.module_id
    JOIN files f ON f.id = s.file_id
    WHERE a.name = 'Scheduled' AND f.is_test=0 AND m.name LIKE :mod
    GROUP BY m.id
    ORDER BY scheduled_methods DESC, io_heavy DESC LIMIT :lim"""),
(
    "expression-eval-surface",
    "Per module: SpEL/ScriptEngine evaluation sites and how many also read request input",
    "ANSWERS the module's dynamic-code-execution surface: parseExpression /\n"
    ...
    """WITH uis AS (
        SELECT DISTINCT symbol_id FROM user_input_sites)
    SELECT m.name AS module_,
        SUM(s.n_spel_eval) AS eval_calls,
        COUNT(DISTINCT s.id) AS eval_fns,
        COUNT(DISTINCT CASE WHEN u.symbol_id IS NOT NULL THEN s.id END)
            AS with_input_sites,
        MIN(f.path) || ':' || MIN(s.line_start) AS at
    FROM modules m
    JOIN symbols s ON s.module_id = m.id
    LEFT JOIN uis u ON u.symbol_id = s.id
    JOIN files f ON f.id = s.file_id
    WHERE s.n_spel_eval > 0 AND s.kind IN ('function','method')
      AND f.is_test=0 AND m.name LIKE :mod
    GROUP BY m.id
    ORDER BY eval_calls DESC, with_input_sites DESC LIMIT :lim"""),
]
