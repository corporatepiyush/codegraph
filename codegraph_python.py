...
#: The comment-prefix set `discover` tests each line's `lstrip()[:3]` against.
#: Hoisted to a constant so the per-line loop loads one name instead of
...
class FileRec:
    ...
    `sloc_lines` is the non-blank line count discover already computed for
    the `files` row; `<module>` symbols reuse it instead of re-stripping
    every line of the file a second time.
    """
    fid: int
    mid: int
    rel: str
...
def discover(analyzer: Analyzer, root: str, db: sqlite3.Connection,
    ...
    for dirpath, dirnames, filenames in os.walk(root, onerror=_walk_error):
        ...
        for fn in sorted(filenames):
            ...
            # what is read, not what parsing it costs. Measured with find()
            # rather than max(len(l) for l in data.split(b"\n")): the split
            # materialised every line of the file as a fresh bytes object
            ...
            # One pass instead of four: blank/sloc/comment/max-len each used
            # to re-scan `lines` (and strip() every line twice more).
            lines = text.splitlines()
            blank = cmt = sloc = max_len = 0
            for l in lines:
                n = len(l)
                if n > max_len:
                    max_len = n
                if not l or l.isspace():
                    blank += 1
                elif l.lstrip()[:3] in _COMMENT_PREFIXES:
                    cmt += 1
                    sloc += 1
                else:
                    sloc += 1
            ...
            if parse:
                out.append(FileRec(fid, mid, rel, full, text, data,
                                   analyzer.LANG, test, gen, vend, sloc))
...
def _has_surrogates(text: str) -> bool:
    # isascii() is one C call and covers virtually every path; the
    # per-character scan only runs for non-ASCII names.
    return not text.isascii() and any(
        "\ud800" <= ch <= "\udfff" for ch in text)
...
def scan_markers(rec: FileRec, bufs: Buffers) -> None:
    ...
    for i, line in enumerate(rec.text.splitlines(), 1):
        # MARKER_RE only produces a row when the line also carries a comment
        # character, and NOQA_RE's pattern starts with '#'. Both searches are
        # skipped outright when the cheaper `in` check cannot pass.
        hash_hit = "#" in line
        if hash_hit or "//" in line or "*" in line or "--" in line:
            ...
            if hash_hit:
                ...
                if sm:
                    bufs.markers.append(
                        (rec.fid, None, sm.group(1).upper(), i,
                         line.strip()[:200]))
...
#: Queryset READ methods called per loop iteration -- the Django/SQLAlchemy
#: N+1 shape (`for pk in ids: User.objects.get(pk)`). `get` requires the
#: manager in the dotted chain (dict.get would otherwise flood the counter);
#: the unambiguous verbs (filter/count/exists/...) count on any receiver.
QUERYSET_READS = frozenset((
    "get", "filter", "exclude", "all", "first", "last", "count",
    "exists", "iterator", "one", "scalar"))

#: ORM write-family methods (Django transactions doc / SQLAlchemy session
#: docs). update/add/set are gated: dict.update and set.add spell the same
#: method name and would flood the counter without a receiver hint.
ORM_WRITE_METHODS = frozenset((
    "save", "create", "delete", "bulk_create", "bulk_update",
    "update_or_create", "get_or_create", "incr", "decr"))
ORM_WRITE_GATED = frozenset(("update", "add", "set"))

#: Resource constructors whose RESULT escaping a function via `return`
#: hands every caller a handle it must close.
RESOURCE_RETURNS = frozenset((
    "open", "urlopen", "connect", "NamedTemporaryFile", "TemporaryFile",
    "Session", "cursor", "mkstemp"))
...
def dotted(node: ast.AST) -> str:
    ...
    if type(node) is ast.Name:
        # the overwhelmingly common shape, hit once per Call node
        return node.id
    parts: list[str] = []
    cur: Any = node
    while True:
...
def _unparse(node: ast.AST) -> str:
    """ast.unparse that never raises -- '' where the tree resists."""
    try:
...
#: frozenset mirrors of the tuples above, for `type(node) in SET` tests in the
#: per-node hot path. ast.parse always produces exact node classes, so a set
#: membership on `type(node)` is equivalent to isinstance over the tuple and
#: one hash lookup instead of a C-level tuple scan. The tuples stay for any
#: external reader that imports them.
_LOOP_NODE_SET = frozenset(LOOP_NODES)
_NEST_NODE_SET = frozenset(NEST_NODES)
_DEF_NODE_SET = frozenset((ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef))
_OPERAND_NODE_SET = frozenset((ast.Name, ast.Constant))
_OPERATOR_NODE_SET = frozenset(OPERATOR_NODES)

#: Every node type `_measure` can affect: the if/elif chain plus the
#: operator/operand tail. A node whose type is outside this set provably
#: matches no branch, so the whole chain walk is skipped for the ~30% of
#: nodes that are structural (Load/Store contexts, List, Expr, arg, alias...)
#: -- they used to traverse every test just to fall through.
_MEASURED_TYPES = frozenset((
    ast.If, ast.IfExp, ast.Match, ast.BoolOp, ast.Compare, ast.Return,
    ast.Raise, ast.Try, ast.ExceptHandler, ast.With, ast.AsyncWith,
    ast.Assert, ast.Global, ast.Nonlocal, ast.Await, ast.Yield,
    ast.YieldFrom, ast.Lambda, ast.NamedExpr, ast.Delete,
    ast.ListComp, ast.SetComp, ast.DictComp, ast.GeneratorExp,
    ast.Assign, ast.AugAssign, ast.AnnAssign, ast.Subscript,
    ast.Attribute, ast.BinOp, ast.JoinedStr, ast.Constant, ast.Call,
    ast.ClassDef, ast.FunctionDef, ast.AsyncFunctionDef, ast.Name,
    ast.UnaryOp, ast.Starred, ast.Slice,
)) | _OPERATOR_NODE_SET | _OPERAND_NODE_SET
if getattr(ast, "TryStar", None) is not None:
    _MEASURED_TYPES |= {ast.TryStar}
...
class FunctionMetrics(ast.NodeVisitor):
    ...
    def __init__(self, fn: ast.AST, src_lines: list[str],
        ...
        #: Names bound with Store context, collected during the same walk that
        #: measures everything else. `n_symbols.n_locals` is the size of this
        #: set -- gathering it here removes one full `ast.walk` per function.
    ...
    def generic_visit(self, node: ast.AST) -> None:
        ...
        if (self.prune_defs and node is not self._root
                and t in _DEF_NODE_SET):
            return
        nested = (t in _NEST_NODE_SET and node is not self._root
                  and id(node) not in self.elifs)
        loop = t in _LOOP_NODE_SET
        ...
        # A SYNCHRONOUS with-block does not release across a suspension.
        # Await measured while this counter is up is the shape of a lock
        # (or file) held across a yield point inside a coroutine.
        sync_with = t is ast.With
        if sync_with:
            self.sync_with_depth += 1

        if t is ast.If and len(node.orelse) == 1 \
        ...
        # Direct recursion instead of NodeVisitor.generic_visit -> visit ->
        # getattr("visit_<name>") -> generic_visit: no visit_ methods exist on
        # this class, so the stdlib dispatch was three calls per node to get
        # here. Same children in the same order, one frame per depth instead
        # of two.
    ...
    def _measure(self, node: ast.AST) -> None:
        t = type(node)
        # Local bindings: this runs once per AST node, and self.X attribute
        # loads are a measurable share of the whole build at that volume.
        # depth/loop_depth are read-only in this method.
        bump = self.bump
        depth = self.depth
        loop_depth = self.loop_depth

        # Structural nodes (contexts, List/Expr/arg/alias, ...) match no
        # branch below; skip the whole chain for them.
        if t not in _MEASURED_TYPES:
            return

        # Frequency-ordered dispatch: Constant is ~56% and Name ~9% of nodes
        # on a typical tree, so they are tested first (with their operand
        # bookkeeping folded in); everything else that provably matches no
        # branch returned at the guard above.
        if t is ast.Constant:
            self._constant(node)
            operands = self.operands
            key = repr(node.value)[:40]
            operands[key] = operands.get(key, 0) + 1
            return
        if t is ast.Name:
            operands = self.operands
            nid = node.id
            operands[nid] = operands.get(nid, 0) + 1
            if isinstance(node.ctx, ast.Store):
                self.store_names.add(nid)
            return

        if t in (ast.If, ast.IfExp):
            self.cyclomatic += 1
            # an `elif` arm costs one point flat; only real nesting compounds
            self.cognitive += 1 if id(node) in self.elifs else max(1, depth)
            bump("n_branches")
            ...
            bump("n_cmp", len(node.ops))
            # `x in some_container` inside a loop is an O(n) scan per
            # iteration unless the container is a set/dict -- which a static
            # reader usually cannot prove, so the count is a fact, not a bug.
            if loop_depth \
                    and any(isinstance(op, (ast.In, ast.NotIn))
                            for op in node.ops) \
                    and any(isinstance(c, (ast.Name, ast.Attribute,
                                           ast.Subscript))
            ...
            bump("n_returns")
            if depth > 0:
                bump("n_early_returns")
            if node.value is not None and any(
                    isinstance(n2, ast.Call)
                    and dotted(n2.func).split(".")[-1] in RESOURCE_RETURNS
                    for n2 in ast.walk(node.value)):
                # the handle escapes: every caller now owns closing it
                bump("n_resource_return")
        elif t is ast.Raise:
            bump("n_throw")
            if node.exc is None:
                bump("n_reraise")
            ...
            if not any(isinstance(n2, (ast.Raise, ast.Return))
                ...
                bump("n_catch_swallow")
        elif t in (ast.With, ast.AsyncWith):
            bump("n_with")
            bump("n_ctx_managers", len(node.items))
            ...
            bump("n_global_stmt", len(node.names))
        elif t is ast.Nonlocal:
            bump("n_nonlocal", len(node.names))
        elif t is ast.Await:
            bump("n_await")
            if loop_depth:
                bump("await_in_loop")
            if self.sync_with_depth:
                # clippy await_holding_lock analogue: the sync with-block is
                # still open across this suspension point
                bump("n_await_in_sync_with")
            ...
            bump("n_del", len(node.targets))
        elif t in COMP_NODES:
            bump("n_comprehension")
            bump("n_comp_generators", len(node.generators))
            if len(node.generators) > 1:
                bump("n_nested_comprehension")
            for g in node.generators:
                bump("n_comp_ifs", len(g.ifs))
                if g.is_async:
                    bump("n_async_comprehension")
            ...
            bump("n_assign", len(node.targets))
            self._string_build(node)
        elif t is ast.AugAssign:
            bump("n_compound_assign")
            if isinstance(node.op, ast.Add) and loop_depth:
                bump("concat_in_loop")
        elif t is ast.AnnAssign:
            bump("n_assign")
            bump("n_annotated_assign")
        elif t is ast.Subscript:
            bump("n_subscript")
        elif t is ast.Attribute:
            bump("n_member_access")
            ...
            bump("n_inner_function")
            if loop_depth:
                bump("n_loop_closure")

        if t in _OPERATOR_NODE_SET:
            key = t.__name__
    ...
    def _constant(self, node: ast.Constant) -> None:
        ...
        if isinstance(v, str):
            bump("n_string_lit")
            # SQL_TEXT_RE's shortest possible match is "SELECT": anything
            # shorter cannot match, so skip the search outright.
            if len(v) >= 6 and SQL_TEXT_RE.search(v):
                bump("n_sql_literal")
                if self.loop_depth:
                    bump("query_in_loop")
    ...
    def _call(self, node: ast.Call) -> None:
        ...
        if name:
            ...
            # -- pack 3: ORM / framework shapes for the cross-function pack --
            if self.loop_depth and "." in name and base in QUERYSET_READS \
                    and (base != "get" or "objects" in name):
                self.bump("n_orm_query_in_loop")    # Django/SQLAlchemy N+1
            if self.loop_depth and base == "commit":
                self.bump("n_commit_in_loop")       # SQLAlchemy unit-of-work
            if "." in name and (base in ORM_WRITE_METHODS or (
                    base in ORM_WRITE_GATED and (
                        "objects" in name or "instance" in name
                        or name.startswith("self.")))):
                self.bump("n_orm_write")            # multi-write/atomic pair
            if base == "atomic" or name.endswith(".atomic"):
                self.bump("n_atomic")
            if name == "mark_safe" or name.endswith(".mark_safe"):
                self.bump("n_mark_safe")            # Django docs: XSS sink
            for kw in node.keywords:
                # OWASP mass assignment: **request.form / **request.get_json()
                # splatted straight into a model constructor or .update()
                if kw.arg is None and "request" in (
                        dotted(kw.value) or _unparse(kw.value)).lower():
                    self.bump("n_mass_assign")
                    break
            if name.startswith(("requests.", "httpx.")) and any(
                    kw.arg == "verify" and isinstance(kw.value, ast.Constant)
                    and kw.value.value is False for kw in node.keywords):
                self.bump("n_verify_false")         # bandit/ruff S501
        else:
...
class PythonAnalyzer(Analyzer):
    ...
    EXTRA_SYMBOL_COLS = (
        ...
        # -- pack 3: cross-function ORM/concurrency/import-time facts -------
        # Each is a count of SITES, never a verdict: whether an ORM read per
        # loop iteration is an N+1 depends on the receiver's type and trip
        # count, which no static reader knows. The queries rank; the reader
        # decides.
        ("n_orm_query_in_loop", "INT NOT NULL DEFAULT 0"),  # Django/SQLAlchemy N+1
        ("n_commit_in_loop", "INT NOT NULL DEFAULT 0"),     # SQLAlchemy unit-of-work
        ("n_orm_write", "INT NOT NULL DEFAULT 0"),          # save/create/update family
        ("n_atomic", "INT NOT NULL DEFAULT 0"),             # transaction.atomic() seen
        ("n_await_in_sync_with", "INT NOT NULL DEFAULT 0"), # await under sync with
        ("n_in_scan_loop", "INT NOT NULL DEFAULT 0"),       # `x in var` per iteration
        ("n_resource_return", "INT NOT NULL DEFAULT 0"),    # open()/connect() returned
        ("n_mark_safe", "INT NOT NULL DEFAULT 0"),          # Django mark_safe XSS sink
        ("n_mass_assign", "INT NOT NULL DEFAULT 0"),        # **request.* splat
        ("n_verify_false", "INT NOT NULL DEFAULT 0"),       # bandit S501
    )
...
CREATE TABLE module_vars(
 ...
 -- Cross-function framework shapes, one row per site: Thread(target=...),
 -- Session()/sessionmaker() construction, render_template_string() calls,
 -- and module-level monkeypatching / settings writes. `kind` picks the
 -- family and `expr` carries the target text so SQL can resolve a target
 -- name against symbols without re-reading source.
...
CREATE INDEX idx_api_kind ON api_sites(kind, symbol_id);
CREATE INDEX idx_api_sym ON api_sites(symbol_id);
CREATE INDEX idx_api_expr ON api_sites(expr);
"""
...
"""
    ...
    def _imports(self, tree: ast.Module, rec: FileRec, bufs: Buffers) -> None:
        ...
        # One walk for both the `if TYPE_CHECKING` probe and the import
        # statements: the probe used to be a separate full `ast.walk` per
        # file, doubling this function's traversal cost. `has_if` is only
        # read AFTER the walk in the original, so the import rows are
        # staged and the `is_type_only` column is finalized here once the
        # walk has seen everything -- same rows, same order, one traversal.
        pending: list[list] = []
        has_if = False
            ...
            if isinstance(node, ast.If):
                ...
                for a in node.names:
                    ...
                    pending.append(
                        [rec.fid, a.name, None, a.asname, "import",
                         node.lineno, int(self._external(a.name)),
                         0, 0, 0, 0, 1])
                ...
                pending.append(
                    [rec.fid, ("." * rel) + mod, None,
                     ...
                     int(wildcard),
                     # bool placeholder; finalized after the walk below
                     mod == "typing",
                     0, len(node.names)])
                root = mod.split(".")[0]
                if root in HAZARD_IMPORTS:
        ...
        if pending:
            for r in pending:
                flag = r[9]
                if flag is True:
                    r[9] = 1 if has_if else 0
                elif flag is False:
                    r[9] = 0
            bufs.imports.extend(map(tuple, pending))
    ...
    def _function(self, node: ast.AST, rec: FileRec, db: sqlite3.Connection,
        ...
        m.update(
            ...
            # store_names was filled by the same walk that measured the body;
            # this used to be a separate full ast.walk per function.
            n_locals=len(fm.store_names),
            ...
            is_test=1 if name.startswith("test_")
                        or any("pytest" in d for d in decs) else 0,
    ...
    def _module_scope(self, tree: ast.Module, rec: FileRec,
        ...
        m.update(
            ...
            n_tokens=sum(fm.operators.values()) + sum(fm.operands.values()),
            n_lines=end,
            # discover already counted non-blank lines for the files row
            sloc=rec.sloc_lines,
            is_generated=int(rec.is_generated),
    ...
    def _dynamic_sites(self, node: ast.AST, sid: int, rec: FileRec,
        ...
        much of its own answer to trust. Framework shapes the pack needs for
        cross-function queries (Thread targets, Session builds, SSTI sinks,
        module-level patching) land in `api_sites` with a kind per family;
        module scope is the only place attribute targets are recorded, since
        import-time patching is the defect and per-statement `self.x = ...`
        rows everywhere else would be noise.
        """
        module_level = isinstance(node, ast.Module)
        if module_level:
            # Module-scope patching only: DIRECT statements of the file. The
            # full walk below re-visits every function body, and recording
            # `self.x = ...` / `g.user = ...` from there would flood this
            # table with ordinary per-function assignment.
            for stmt in node.body:
                ...
                for tgt in stmt.targets:
                    if not isinstance(tgt, ast.Attribute):
                        continue
                    patch_base = dotted(tgt.value).split(".")[-1]
                    kind = ("settings-write" if patch_base == "settings"
                            else "monkeypatch")
                    bufs.rows("api_sites").append(
                        (sid, rec.fid, kind, _unparse(stmt)[:200],
                         stmt.lineno, 0))
        for c in ast.walk(node):
            if isinstance(c, ast.Call):
                ...
                if base in ("eval", "exec", "compile"):
                    ...
                    kind = "computed"
                if kind:
                    literal = bool(c.args) and isinstance(c.args[-1],
                                                          ast.Constant)
                    try:
                ...
                # -- api_sites families. Recorded on the per-function pass
                # only: the module-scope walk re-visits every function body
                # (dynamic_sites already double-counts that way) and the
                # caller symbol is the useful owner of the row.
                if module_level:
                    continue
                if base == "Thread" or name.endswith(".Thread"):
                    for kw in c.keywords:
                        if kw.arg == "target" and isinstance(kw.value,
                                                             ast.Name):
                            bufs.rows("api_sites").append(
                                (sid, rec.fid, "thread-target",
                                 kw.value.id, c.lineno, 0))
                if base in ("Session", "sessionmaker", "scoped_session"):
                    bufs.rows("api_sites").append(
                        (sid, rec.fid, "session", name[:200], c.lineno, 0))
                if base == "render_template_string":
                    lit = bool(c.args) and isinstance(c.args[0], ast.Constant)
                    bufs.rows("api_sites").append(
                        (sid, rec.fid, "ssti", name[:200], c.lineno,
                         int(lit)))
            ...
            if isinstance(n, ast.AnnAssign) and isinstance(n.target, ast.Name):
                ...
                for t in n.targets:
                    if isinstance(t, ast.Name) and t.id != "__slots__":
                        # is_mutable used to be hardcoded 0 with the real
                        # signal parked in is_collection; a class attribute
                        # initialized from a literal container IS the
                        # shared-mutable-state shape (RUF012), so record it
                        # under both names.
                        mutable = int(isinstance(n.value,
                                                 (ast.List, ast.Dict,
                                                  ast.Set)))
                        bufs.fields.append(
                            (sid, ordinal, t.id, "", "", n.lineno, 1, 0,
                             mutable, 0, mutable, 1, 1, 0))
                        ordinal += 1
    ...
    def resolve_calls(self, db: sqlite3.Connection, bufs: Buffers) -> None:
        ...
        for tbl, sql in (
            ...
            ("secret_candidates",
             ...
             "VALUES(?,?,?,?)"),
            ("api_sites",
             "INSERT INTO api_sites(symbol_id,file_id,kind,expr,line,"
             "is_literal_arg) VALUES(?,?,?,?,?,?)"),
        ):
...
# ---- pack 3: cross-function ORM / framework / import-time pack -------------
(
    "orm-query-in-loop",
    "Queryset reads inside a loop: the ORM N+1 (Django/SQLAlchemy perf docs)",
    "ANSWERS where an ORM query fires per loop iteration -- .get/.filter/\n"
    ...
    """SELECT s.name, s.n_orm_query_in_loop AS orm_queries_in_loop,
        s.max_loop_depth AS depth, s.n_orm_write AS orm_writes,
        s.n_sql AS raw_sql, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_orm_query_in_loop > 0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_orm_query_in_loop DESC, s.fan_in DESC LIMIT :lim"""),
(
    "commit-in-loop",
    ".commit() inside a loop: one transaction per item (SQLAlchemy session docs)",
    "ANSWERS where the unit of work is a single row: a commit per iteration\n"
    ...
    "     a commit verb.",
    """SELECT s.name, s.n_commit_in_loop AS commits_in_loop,
        s.max_loop_depth AS depth, s.n_sql AS sql_calls, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_commit_in_loop > 0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_commit_in_loop DESC, s.fan_in DESC LIMIT :lim"""),
(
    "multi-write-no-atomic",
    "Multiple ORM writes with no transaction.atomic in sight (Django transactions doc)",
    "ANSWERS functions performing two or more ORM writes with no atomic block\n"
    ...
    """SELECT s.name, s.n_orm_write AS orm_writes, s.n_sql AS raw_sql,
        s.fan_in, f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_orm_write >= 2 AND s.n_atomic = 0
      AND NOT EXISTS (SELECT 1 FROM attributes a WHERE a.symbol_id=s.id
                      AND a.name LIKE '%atomic%')
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_orm_write DESC, s.fan_in DESC LIMIT :lim"""),
(
    "celery-task-sync-call",
    "Celery task called as a plain function -- the queue is bypassed (Celery calling-tasks doc)",
    "ANSWERS task-decorated functions that other in-tree code calls directly.\n"
    ...
    """SELECT s.name,
        (SELECT MIN(a.name) FROM attributes a WHERE a.symbol_id=s.id
         AND a.name LIKE '%task%') AS task_decorator,
        s.fan_in, s.n_params, f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.fan_in > 0 AND EXISTS (
            SELECT 1 FROM attributes a2 WHERE a2.symbol_id=s.id
              AND a2.name LIKE '%task%')
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC LIMIT :lim"""),
(
    "celery-task-reliability",
    "Task with no acks_late and no time limits (Celery task docs)",
    "ANSWERS tasks whose decorator sets neither acks_late nor a time limit:\n"
    ...
    WHERE EXISTS (
            SELECT 1 FROM attributes a2 WHERE a2.symbol_id=s.id
              AND a2.name LIKE '%task%'
              AND COALESCE(a2.args,'') NOT LIKE '%acks_late%'
              AND COALESCE(a2.args,'') NOT LIKE '%time_limit%')
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC LIMIT :lim"""),
(
    "lock-across-await",
    "Await while a sync with-block is open -- the lock is held across the suspension (clippy await_holding_lock analogue)",
    "ANSWERS async functions that await inside a SYNCHRONOUS with-block: the\n"
    ...
    """SELECT s.name, s.n_await_in_sync_with AS awaits_in_sync_with,
        s.n_concurrency AS lock_sites, s.n_await AS awaits, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.is_async=1 AND s.n_await_in_sync_with > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_await_in_sync_with DESC, s.n_concurrency DESC LIMIT :lim"""),
(
    "thread-target-shared-state",
    "threading.Thread(target=...) aimed at code that writes shared state",
    "ANSWERS spawn sites whose target function uses global/nonlocal writes:\n"
    ...
    "     not modelled at all.",
    """WITH spawns AS (
        SELECT d.symbol_id AS sid, d.expr AS target_fn, d.line, d.file_id
        FROM api_sites d WHERE d.kind='thread-target')
    SELECT s.name AS spawner, sp.target_fn,
        COUNT(DISTINCT tgt.id) AS defs,
        MAX(tgt.n_global_stmt) AS target_global_writes,
        MAX(tgt.n_concurrency) AS target_concurrency,
        df.path || ':' || sp.line AS at
    FROM spawns sp
    JOIN symbols s ON s.id=sp.sid
    JOIN files df ON df.id=sp.file_id
    LEFT JOIN symbols tgt ON tgt.name=sp.target_fn
        AND tgt.kind IN ('function','method')
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE df.is_test=0 AND COALESCE(m.name,'') LIKE :mod
    GROUP BY sp.sid, sp.target_fn, sp.line
    ORDER BY target_global_writes DESC, target_concurrency DESC LIMIT :lim"""),
(
    "import-monkeypatch",
    "Module-level attribute assignment: monkey-patching at import time",
    "ANSWERS module-scope statements that assign into a dotted target (os.cwd\n"
    ...
    """SELECT f.path, d.expr, d.line,
        f.path || ':' || d.line AS at
    FROM api_sites d
    JOIN files f ON f.id=d.file_id
    LEFT JOIN modules m ON m.id=f.module_id
    WHERE d.kind='monkeypatch' AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY f.path, d.line LIMIT :lim"""),
(
    "settings-mutation-import",
    "settings.* written at import time (Django settings docs)",
    "ANSWERS module-scope writes like settings.DEBUG = True: Django reads\n"
    ...
    JOIN files f ON f.id=d.file_id
    LEFT JOIN modules m ON m.id=f.module_id
    WHERE d.kind='settings-write' AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY f.path, d.line LIMIT :lim"""),
(
    "import-time-side-effects",
    "Work done at import time: I/O, network, subprocess or open() at module scope",
    "ANSWERS modules that read files, touch the network, shell out or open a\n"
    ...
    """SELECT f.path,
        s.n_io AS io, s.n_net AS net, s.n_open AS opens, s.n_exec AS execs,
        s.n_shell AS shell, s.n_subprocess AS subprocesses,
        s.n_io + s.n_net + s.n_open + s.n_exec + s.n_shell
            + s.n_subprocess AS import_time_work,
        f.path || ':1' AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.kind='module'
      AND (s.n_io + s.n_net + s.n_open + s.n_exec + s.n_shell
           + s.n_subprocess) > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY import_time_work DESC, s.n_net DESC LIMIT :lim"""),
(
    "membership-scan-in-loop",
    "`x in container` inside a loop: O(n) scan per iteration (flake8-perf family)",
    "ANSWERS membership tests against a variable container inside a loop\n"
    ...
    """SELECT s.name, s.n_in_scan_loop AS membership_scans,
        s.max_loop_depth AS depth, s.n_loops AS loops, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_in_scan_loop > 0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_in_scan_loop DESC, s.fan_in DESC LIMIT :lim"""),
(
    "resource-return-escape",
    "open()/connect() result returned to the caller -- the handle escapes",
    "ANSWERS functions that hand a freshly opened file, socket, session or\n"
    ...
    """SELECT s.name, s.n_resource_return AS escaped_handles,
        s.n_ctx_managers AS ctx_managers, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_resource_return > 0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC, s.n_resource_return DESC LIMIT :lim"""),
(
    "taint-frontier-input",
    "Dangerous sinks reachable from request-input readers, with hop distance",
    "ANSWERS which subprocess/shell/eval/pickle/mark_safe sinks sit within 4\n"
    ...
    """WITH RECURSIVE down(root, sym, depth) AS (
        SELECT DISTINCT u.symbol_id, u.symbol_id, 0 FROM user_input_sites u
        WHERE u.symbol_id IS NOT NULL
        UNION
    ...
    SELECT s.name AS sink_fn, MIN(d.depth) AS hops,
        COUNT(DISTINCT d.root) AS input_readers,
        s.n_subprocess AS subprocesses, s.n_shell_true AS shell_true,
        s.n_eval_exec AS evals, s.n_pickle_load AS pickles,
        s.n_mark_safe AS mark_safe,
        f.path || ':' || s.line_start AS at
    FROM down d
    ...
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.id=d.sym
      AND (s.n_subprocess + s.n_shell_true + s.n_eval_exec
           + s.n_pickle_load + s.n_mark_safe) > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id
    ORDER BY input_readers DESC, hops ASC LIMIT :lim"""),
(
    "mark-safe-surface",
    "mark_safe() in functions that read request input (Django docs / Semgrep)",
    "ANSWERS same-function co-occurrence of mark_safe (or a .mark_safe\n"
    ...
    """SELECT s.name, s.n_mark_safe AS mark_safe_calls,
        COUNT(DISTINCT u.id) AS input_sites,
        GROUP_CONCAT(DISTINCT u.kind) AS kinds,
        s.fan_in, f.path || ':' || s.line_start AS at
    FROM symbols s
    ...
    LEFT JOIN modules m ON m.id = s.module_id
    LEFT JOIN user_input_sites u ON u.symbol_id = s.id
    WHERE s.n_mark_safe > 0 AND u.kind IS NOT NULL
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id
    ORDER BY s.fan_in DESC, s.n_mark_safe DESC LIMIT :lim"""),
(
    "mass-assignment-surface",
    "**request.<form/json/args> splatted into a call (OWASP mass assignment)",
    "ANSWERS calls that unpack a request-derived mapping into keyword\n"
    ...
    """SELECT s.name, s.n_mass_assign AS splat_sites,
        COUNT(DISTINCT u.id) AS input_sites,
        GROUP_CONCAT(DISTINCT u.kind) AS kinds,
        s.fan_in, f.path || ':' || s.line_start AS at
    FROM symbols s
...
(
    "ssti-surface",
    "render_template_string() calls: server-side template injection surface (CWE-1336)",
    "ANSWERS templates constructed from runtime strings: a template argument\n"
    ...
    "     captured.",
    """SELECT s.name, d.expr, d.line, d.is_literal_arg,
        COUNT(DISTINCT u.id) AS input_sites,
        f.path || ':' || d.line AS at
    FROM api_sites d
    ...
    WHERE d.kind = 'ssti'
      ...
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY d.id
    ORDER BY d.is_literal_arg ASC, input_sites DESC LIMIT :lim"""),
(
    "session-created-per-call",
    "Session()/sessionmaker() built inside functions (SQLAlchemy session-per-request)",
    "ANSWERS functions that construct a Session themselves instead of\n"
    ...
    """SELECT s.name, COUNT(DISTINCT ap.id) AS session_builds,
        s.fan_in, f.path || ':' || MIN(ap.line) AS at
    FROM api_sites ap
    JOIN symbols s ON s.id = ap.symbol_id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE ap.kind = 'session' AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id
    ORDER BY s.fan_in DESC, session_builds DESC LIMIT :lim"""),
(
    "tls-verify-disabled",
    "verify=False on an HTTP client call (bandit S501 / ruff S501)",
    "ANSWERS requests/httpx calls with TLS certificate verification\n"
    ...
    """SELECT s.name, s.n_verify_false AS verify_disabled, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_verify_false > 0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_verify_false DESC, s.fan_in DESC LIMIT :lim"""),
(
    "test-only-callers",
    "Production functions whose only callers are test files",
    "ANSWERS functions every resolved caller of which lives in a test tree:\n"
    ...
    """SELECT s.name, s.fan_in, s.cyclomatic AS cyclo, s.sloc,
        s.is_public AS public, f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.kind IN ('function','method') AND f.is_test=0 AND s.is_test=0
      AND f.is_generated=0 AND s.fan_in > 0
      AND COALESCE(m.name,'') LIKE :mod
      AND s.id IN (SELECT e.callee_id FROM edges e
                   JOIN symbols cs ON cs.id=e.caller_id
                   JOIN files cf ON cf.id=cs.file_id
                   WHERE cf.is_test=1 OR cs.is_test=1)
      AND s.id NOT IN (SELECT e2.callee_id FROM edges e2
                       JOIN symbols cs2 ON cs2.id=e2.caller_id
                       JOIN files cf2 ON cf2.id=cs2.file_id
                       WHERE cf2.is_test=0 AND cs2.is_test=0)
    ORDER BY s.fan_in DESC, s.cyclomatic DESC LIMIT :lim"""),
(
    "mutable-class-attribute",
    "Mutable class attribute without ClassVar -- shared by every instance (bugbear/RUF012)",
    "ANSWERS class-level list/dict/set initializers that every instance\n"
    ...
    """SELECT s.name AS class_, fl.name AS attribute, fl.type,
        (SELECT COUNT(*) FROM callsites cs WHERE cs.callee_id=s.id)
            AS instantiations,
        f.path || ':' || fl.line AS at
    FROM fields fl
    JOIN symbols s ON s.id=fl.symbol_id AND s.kind='class'
    JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE fl.is_static=1 AND fl.is_mutable=1
      AND fl.type NOT LIKE '%ClassVar%'
      AND (SELECT COUNT(*) FROM callsites cs2 WHERE cs2.callee_id=s.id) > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY instantiations DESC LIMIT :lim"""),
(
    "global-write-reachable",
    "Global-variable writers reachable from public entry points",
    "ANSWERS functions that execute global writes and how far they sit from a\n"
    ...
    """WITH RECURSIVE up(root, sym, depth) AS (
        SELECT s.id, s.id, 0 FROM symbols s
        WHERE s.n_global_stmt > 0 AND s.kind IN ('function','method')
        UNION
        SELECT u.root, e.caller_id, u.depth+1 FROM up u
        JOIN edges e ON e.callee_id=u.sym
        WHERE u.depth < 4 AND e.is_self=0)          -- depth bound: 4 hops
    SELECT w.name AS writer, w.n_global_stmt AS global_writes,
        MIN(u.depth) AS hops_from_public,
        COUNT(DISTINCT CASE WHEN pub.is_public=1 THEN u.sym END)
            AS public_reach,
        f.path || ':' || w.line_start AS at
    FROM up u
    CROSS JOIN symbols w
    LEFT JOIN symbols pub ON pub.id=u.sym
    JOIN files f ON f.id=w.file_id
    LEFT JOIN modules m ON m.id=w.module_id
    WHERE w.id=u.root AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY w.id
    HAVING public_reach > 0
    ORDER BY public_reach DESC, global_writes DESC LIMIT :lim"""),
(
    "prod-imports-test",
    "Production code importing from the test tree",
    "ANSWERS non-test files whose imports resolve into a test file: helpers\n"
    ...
    """SELECT f.path AS importer, tf.path AS test_module, i.line,
        tf.n_symbols AS test_symbols,
        f.path || ':' || i.line AS at
    FROM imports i
    JOIN files f ON f.id=i.file_id
    JOIN files tf ON tf.id=i.target_id AND tf.is_test=1
    LEFT JOIN modules m ON m.id=f.module_id
    WHERE f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY tf.n_symbols DESC, f.path LIMIT :lim"""),
(
    "dict-get-in-loop",
    ".get() inside a loop body (ruff PERF family)",
    "ANSWERS functions calling .get() inside loop bodies: when the same key\n"
    ...
    "     type is not known.",
    """SELECT s.name, s.n_dict_get_in_loop AS gets_in_loop,
        s.max_loop_depth AS depth, s.n_loops AS loops, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_dict_get_in_loop > 0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_dict_get_in_loop DESC, s.fan_in DESC LIMIT :lim"""),
(
    "format-in-loop",
    ".format() inside a loop body (ruff PERF family)",
    "ANSWERS str.format calls inside loop bodies: the template is re-parsed\n"
    ...
    """SELECT s.name, s.n_format_in_loop AS formats_in_loop,
        s.max_loop_depth AS depth, s.concat_in_loop AS concats, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_format_in_loop > 0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_format_in_loop DESC, s.fan_in DESC LIMIT :lim"""),
(
    "loop-else",
    "for/while with an else clause (pylint W0120 shape)",
    "ANSWERS loop-else constructs: the else runs when the loop ends without\n"
    ...
    """SELECT s.name, s.n_loop_else AS loop_elses,
        s.n_loops AS loops, s.cyclomatic AS cyclo, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_loop_else > 0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_loop_else DESC, s.fan_in DESC LIMIT :lim"""),
(
    "print-statement-shipping",
    "print() in public non-test code",
    "ANSWERS public functions that write to stdout outside tests: print\n"
    ...
    """SELECT s.name, s.n_print AS prints, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_print > 0 AND s.is_public=1
      AND s.kind IN ('function','method')
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC, s.n_print DESC LIMIT :lim"""),
(
    "insecure-tempfile",
    "tempfile.mktemp -- predictable temp name (bandit S306)",
    "ANSWERS insecure temp-FILE NAME generation: mktemp returns a path\n"
    ...
    "     race) is invisible.",
    """SELECT s.name, s.n_insecure_temp AS insecure_temp_sites, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_insecure_temp > 0 AND f.is_test=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC, s.n_insecure_temp DESC LIMIT :lim"""),
(
    "unused-public-api",
    "Public functions nothing in the tree calls -- exported API nobody uses",
    "ANSWERS public functions with zero resolved in-tree callers and no\n"
    ...
    """SELECT s.name, s.kind, s.sloc, s.cyclomatic AS cyclo,
        s.n_doc_lines, f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.fan_in=0 AND s.is_public=1
      AND s.kind IN ('function','method')
      AND s.is_dunder=0 AND s.is_entrypoint=0 AND s.is_override=0
      AND s.is_abstract=0 AND s.is_overload=0 AND s.is_test=0
      AND f.is_test=0 AND f.is_generated=0
      AND NOT EXISTS (SELECT 1 FROM attributes a WHERE a.symbol_id=s.id)
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.sloc DESC LIMIT :lim""")
...
# ---- pack 3: module-posture metrics ----------------------------------------
(
    "import-surface",
    "Which modules do real work at import time, and what kind",
    "ANSWERS the import-time bill per module: I/O, network, subprocess, exec\n"
    ...
    """SELECT m.name AS module,
        COUNT(DISTINCT s.id) AS module_symbols,
        SUM(s.n_io) AS io, SUM(s.n_net) AS net, SUM(s.n_open) AS opens,
        SUM(s.n_exec) AS execs, SUM(s.n_shell) AS shell,
        SUM(s.n_subprocess) AS subprocesses,
        SUM(s.n_io + s.n_net + s.n_open + s.n_exec + s.n_shell
            + s.n_subprocess) AS import_time_work,
        MIN(f.path) || ':1' AS at
    FROM symbols s
    JOIN files f ON f.id=s.file_id
    JOIN modules m ON m.id=s.module_id
    WHERE s.kind='module' AND f.is_test=0 AND f.is_generated=0
      AND m.name LIKE :mod
    GROUP BY m.id
    HAVING import_time_work > 0
    ORDER BY import_time_work DESC, net DESC LIMIT :lim"""),
(
    "exception-posture",
    "How each module handles failure: handler volume and how broad it is",
    "ANSWERS the shape of error handling per module -- how many except\n"
    ...
    """SELECT m.name AS module,
        COUNT(DISTINCT h.id) AS handler_rows,
        SUM(h.is_broad) AS broad, SUM(h.is_bare) AS bare,
        SUM(h.is_empty) AS empty, SUM(h.in_loop) AS in_loop,
        SUM(h.n_body_lines) AS body_lines,
        CAST(100.0 * SUM(h.is_broad) / NULLIF(COUNT(*), 0) AS INT)
            AS pct_broad,
        MIN(f.path || ':' || h.line) AS at
    FROM handlers h
    JOIN symbols s ON s.id=h.symbol_id
    JOIN files f ON f.id=s.file_id
    ...
    GROUP BY m.id
    HAVING handler_rows > 0
    ORDER BY handler_rows DESC, broad DESC LIMIT :lim"""),
(
    "resource-posture",
    "Files and handles per module: how many opens vs how many context managers",
    "ANSWERS the resource discipline per module -- total open()/resource\n"
    ...
    """SELECT m.name AS module,
        COUNT(DISTINCT s.id) AS functions,
        SUM(s.n_open) AS opens, SUM(s.n_resource) AS resources,
        SUM(s.n_ctx_managers) AS ctx_managers, SUM(s.n_with) AS with_blocks,
        CAST(100.0 * SUM(s.n_open)
             / NULLIF(SUM(s.n_open) + SUM(s.n_ctx_managers), 0) AS INT)
            AS pct_open_unmanaged,
        MIN(f.path || ':' || s.line_start) AS at
    FROM symbols s
    JOIN files f ON f.id=s.file_id
    JOIN modules m ON m.id=f.module_id
    WHERE s.kind IN ('function','method') AND f.is_test=0
      AND f.is_generated=0 AND m.name LIKE :mod
    GROUP BY m.id
    HAVING SUM(s.n_open) > 0
    ORDER BY SUM(s.n_open) DESC, pct_open_unmanaged DESC LIMIT :lim"""),
(
    "concurrency-posture",
    "Shared-state writes per module: spawns, locks, globals and mutable module vars",
    "ANSWERS where the free-threaded build has to be correct, aggregated per\n"
    ...
    """WITH mv AS (
        SELECT v.module_id AS mid, COUNT(DISTINCT v.rowid) AS n
        FROM module_vars v JOIN files f ON f.id=v.file_id
        WHERE v.is_mutable_container=1 AND v.is_constant=0 AND f.is_test=0
        GROUP BY v.module_id)
    SELECT m.name AS module,
        COUNT(DISTINCT s.id) AS functions,
        SUM(s.n_concurrency) AS spawn_lock_sites,
        SUM(s.n_global_stmt) AS global_writes,
        SUM(s.is_async) AS async_fns,
        SUM(s.lock_in_loop) AS locks_in_loop,
        COALESCE((SELECT mv.n FROM mv WHERE mv.mid=m.id), 0)
            AS mutable_module_vars,
        SUM(s.n_concurrency + s.n_global_stmt)
            + COALESCE((SELECT mv.n FROM mv WHERE mv.mid=m.id), 0)
            AS shared_state_sites,
        MIN(f.path || ':' || s.line_start) AS at
    FROM symbols s
    JOIN files f ON f.id=s.file_id
    JOIN modules m ON m.id=f.module_id
    WHERE s.kind IN ('function','method') AND f.is_test=0
      AND f.is_generated=0 AND m.name LIKE :mod
    GROUP BY m.id
    HAVING shared_state_sites > 0
    ORDER BY shared_state_sites DESC LIMIT :lim"""),
(
    "sql-construction",
    "SQL usage per module: how much is built by string surgery",
    "ANSWERS where raw SQL lives per module and what share of it is\n"
    ...
    """SELECT m.name AS module,
        COUNT(DISTINCT s.id) AS functions,
        SUM(s.n_sql) AS sql_calls,
        SUM(s.n_sql_fstring) AS fstring, SUM(s.n_sql_concat) AS concat,
        SUM(s.n_sql_format) AS fmt,
        SUM(s.n_sql_fstring + s.n_sql_concat + s.n_sql_format) AS hand_built,
        SUM(s.query_in_loop) AS queries_in_loop,
        CAST(COALESCE(MIN(100, 100.0
             * SUM(s.n_sql_fstring + s.n_sql_concat + s.n_sql_format)
             / NULLIF(SUM(s.n_sql), 0)), 100) AS INT) AS pct_hand_built,
        MIN(f.path || ':' || s.line_start) AS at
    FROM symbols s
    JOIN files f ON f.id=s.file_id
    JOIN modules m ON m.id=f.module_id
    ...
    GROUP BY m.id
    HAVING hand_built > 0
    ORDER BY hand_built DESC, queries_in_loop DESC LIMIT :lim"""),
(
    "third-party-coupling",
    "External dependency pull per module: distinct roots, wildcards, relative reach",
    "ANSWERS how much of each module's import block is third-party, and how\n"
    ...
    """SELECT m.name AS module,
        ...
        SUM(i.is_external) AS external_rows,
        COUNT(DISTINCT CASE WHEN i.is_external=1 THEN
            lower(substr(i.target, 1, instr(i.target || '.', '.') - 1)) END)
            AS external_roots,
        SUM(i.is_relative) AS relative_rows,
        SUM(i.is_wildcard) AS wildcard_rows,
        MIN(f.path || ':' || i.line) AS at
    FROM imports i
    JOIN files f ON f.id=i.file_id
    JOIN modules m ON m.id=f.module_id
    WHERE f.is_test=0 AND f.is_generated=0 AND m.name LIKE :mod
    GROUP BY m.id
    HAVING external_roots > 0
    ORDER BY external_roots DESC, external_rows DESC LIMIT :lim"""),
(
    "recursive-hotspots",
    "Functions that call themselves, by complexity and fan-in",
    "ANSWERS where recursion lives and which recursive functions the most\n"
    ...
    """SELECT s.name, s.cyclomatic AS cyclo, s.fan_in, s.sloc,
        s.max_loop_depth, s.n_params,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.is_recursive=1 AND s.kind IN ('function','method')
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.cyclomatic DESC, s.fan_in DESC LIMIT :lim"""),
(
    "registration-surface",
    "Framework-registered entry points per module: code invoked without a caller",
    "ANSWERS where implicit invocation concentrates -- functions registered by\n"
    ...
    """SELECT m.name AS module,
        COUNT(DISTINCT a.id) AS registered_entry_points,
        COUNT(DISTINCT s.id) AS functions,
    ...
    JOIN modules m ON m.id=f.module_id
    WHERE f.is_test=0 AND f.is_generated=0 AND m.name LIKE :mod
      AND (a.name LIKE '%route%' OR a.name LIKE '%task%'
           OR a.name LIKE '%receiver%' OR a.name LIKE '%signal%'
           OR a.name LIKE '%command%' OR a.name LIKE '%listener%'
           OR a.name LIKE '%subscribe%' OR a.name LIKE '%event%'
           OR a.name LIKE '%handler%' OR a.name LIKE '%fixture%'
           OR a.name LIKE '%connect%' OR a.name LIKE '%hook%'
           OR a.name LIKE '%register%')
    GROUP BY m.id
    ORDER BY registered_entry_points DESC LIMIT :lim"""),
(
    "input-surface",
    "Request-input reads per module, and how much of the module checks auth",
    "ANSWERS the attack surface per module -- how many request-input sites it\n"
    ...
    """SELECT m.name AS module,
        COUNT(DISTINCT u.id) AS input_reads,
        COUNT(DISTINCT s.id) AS functions,
        COUNT(DISTINCT CASE WHEN s.n_auth_call > 0 THEN s.id END)
            AS fns_with_auth_checks,
        CAST(100.0 * COUNT(DISTINCT CASE WHEN s.n_auth_call > 0 THEN s.id END)
             / NULLIF(COUNT(DISTINCT s.id), 0) AS INT) AS pct_auth,
        MIN(m.name || ':1') AS at
    FROM user_input_sites u
    CROSS JOIN files f
    JOIN symbols s ON s.module_id = f.module_id
    JOIN modules m ON m.id = f.module_id
    WHERE s.kind IN ('function','method') AND f.is_test = 0
      AND f.is_generated = 0 AND m.name LIKE :mod
      AND f.id = u.file_id
    GROUP BY f.module_id
    HAVING input_reads > 0
    ORDER BY input_reads DESC, pct_auth ASC LIMIT :lim"""),
(
    "network-hygiene",
    "Outbound calls per module: how many carry no timeout",
    "ANSWERS the per-module split of outbound network calls: fetch sites,\n"
    ...
    """SELECT m.name AS module,
        COUNT(DISTINCT s.id) AS functions,
        SUM(s.n_net) AS net_calls,
        SUM(s.n_request_no_timeout) AS no_timeout_sites,
        SUM(s.n_fetch) AS fetch_sites,
        CAST(COALESCE(MIN(100, 100.0 * SUM(s.n_request_no_timeout)
             / NULLIF(SUM(s.n_net), 0)), 0) AS INT) AS pct_no_timeout,
        MIN(f.path || ':' || s.line_start) AS at
    FROM symbols s
    JOIN files f ON f.id=s.file_id
    JOIN modules m ON m.id=f.module_id
    WHERE s.kind IN ('function','method') AND f.is_test=0
      AND f.is_generated=0 AND m.name LIKE :mod
    GROUP BY m.id
    HAVING no_timeout_sites > 0
    ORDER BY no_timeout_sites DESC, net_calls DESC LIMIT :lim""")
]
