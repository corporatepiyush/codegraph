...
**Event loops (epoll / io_uring / kqueue).** C has no portable poller, so each
of the three APIs is captured by name — call sites, argument text, and every
flag the contract turns on — and the documented failure modes are queries
rather than prose. These cover `epoll(7)`'s stated pitfalls, `io_uring(7)`'s
error-channel and lifetime rules, and `kqueue(2)`'s `EV_EOF`/`EV_ERROR`/
`EV_RECEIPT` semantics:

- **epoll** — `et-without-drain`, `et-without-nonblocking`,
  `event-wait-no-timeout`, `event-wait-eintr-blind`,
  `write-ready-never-cleared`, `event-error-flag-unhandled`,
  `oneshot-never-rearmed`
- **io_uring** — `uring-sqe-null`, `uring-res-unchecked`,
  `uring-errno-confusion`, `uring-cqe-never-seen`,
  `uring-user-data-unset`, `uring-link-ordering`,
  `uring-sqpoll-free-race`, `uring-ring-without-barrier`,
  `uring-teardown-incomplete`
- **kqueue** — `kevent-timer-zero`, `kevent-batch-errors-ignored`
- **Loop cost** — `event-fd-leak`, `event-loop-alloc-per-event`,
  `blocking-call-in-event-loop`, `event-api-inventory` (M),
  `event-loop-hotspots` (M), `event-api-coverage` (M)

**Toolchain API (libc / libm / libpthread / builtins).** Thirty namespaces,
ranked by measured call sites across real corpora rather than guessed, held in
one `apiuse` table rather than thirty columns. Every "immature use" rule is an
API call plus a *missing guard*, and the guards are a shared vocabulary of
twelve — that sharing is why the pack costs 12 columns instead of 30:

- **stdio & formatting** — `snprintf-truncation-ignored`, `stdio-result-unchecked`,
  `scanf-unchecked`
- **Strings & conversion** — `string-conversion-unchecked`,
  `ctype-signed-char-ub`
- **Math** — `math-domain-error`
- **Memory & mapping** — `mman-result-unchecked`, `mman-never-unmapped`,
  `calloc-transposed-args`
- **Sockets & processes** — `socket-partial-write`, `socket-not-nonblocking`,
  `wait-status-unchecked`
- **Time & signals** — `time-not-threadsafe`, `sleep-eintr-unhandled`,
  `signal-deprecated-api`, `signal-stack-size-constant`,
  `time-elapsed-not-monotonic`
- **Variadic & pointers** — `varargs-lifetime`, `varargs-array-type`,
  `pointer-overflow-check-broken`
- **Event batching** — `event-wait-batch-one`
- **Inventory** — `api-namespace-inventory` (M), `api-discipline-score` (M)

These deliberately overlap with what GCC 14/15 and Clang 20 detect — but every
one of those checks sits behind an opt-in flag (`-fanalyzer`,
`-Wcalloc-transposed-args`, `-Wvarargs`, `_FORTIFY_SOURCE`). A source-level
analyzer that needs no build flags covers the builds that never opted in.

**Ownership, resource and API discipline** (CERT rules the layout queries could
not express): `realloc-idiom-risk`, `alloc-size-off-by-one`,
`resource-leak-surface`, `unchecked-alloc-result`, `free-then-deref`,
`vla-from-input`, `weak-randomness`, `shift-count-risk`, `errno-dependence`,
`env-trust-boundary`, `assert-with-side-effect`, `unterminated-strncpy`,
`struct-padding-leak`, `false-sharing-risk`, `lock-order-inversion`,
`io-in-loop`, `lock-convoy`, `branch-per-iteration`, `dead-static`,
`missing-prototype`, `header-hygiene`, `module-cycles`, `naming-coherence`,
`untested-surface`, with `marker-debt` (M), `clone-candidates` (M),
`aliasing-blocked` (M), `file-hotspots` (M), `api-surface` (M),
`macro-hygiene` (M), `init-discipline` (M), `lock-scope` (M),
`typedef-pointer-opacity` (M), `static-inline-in-header` (M),
`error-code-convention` (M), `extern-in-source` (M)
