...
(
    ...
    """SELECT g.name, g.type, g.ptr_depth AS ptr, g.is_array AS arr,
        g.is_static AS stat, g.has_init AS init, m.name AS module,
        f.path || ':' || g.line AS at,
        (SELECT COUNT(*) FROM symbols s2
         WHERE s2.module_id=g.module_id AND s2.n_concurrency>0) AS mod_conc_fns
    ...
    ORDER BY mod_conc_fns DESC, g.ptr_depth DESC, g.name LIMIT :lim"""),
(
    "module-coupling",
...
CAnalyzer.QUERIES = CAnalyzer.QUERIES + [
(
    "dead-code",
