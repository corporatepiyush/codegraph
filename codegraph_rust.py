...
def walk_nodes(node: Any) -> Iterator[Any]:
    """Every node in the subtree, parents before children, cursor-driven.

    Same visit order as `walk()` -- preorder, left to right, unnamed nodes
    included -- but never materialises a children list per node, so a body
    of N nodes costs N cursor steps instead of N list allocations.
    """
    cursor = node.walk()
    while True:
        yield cursor.node
    ...
    for dirpath, dirnames, filenames in os.walk(root, onerror=_walk_error):
        ...
        for fn in sorted(filenames):
            ...
            try:
                ...
                # gets pasted into reports. (One realpath walk per file, not
                # two -- the old condition called os.path.realpath twice.)
                rp = os.path.realpath(full)
                if rp != full and not rp.startswith(real_root + os.sep):
                    n_skipped_escape += 1
                    continue
            ...
            # One pass over the lines instead of four: blank/sloc/comment/
            # max-len each used to walk the list separately, stripping every
            # line two or three times. Same counters, same file row.
            lines = text.splitlines()
            blank = sloc_n = cmt = maxlen = 0
            for l in lines:
                ll = len(l)
                if ll > maxlen:
                    maxlen = ll
                ...
                if not t:
                    ...
                    sloc_n += 1
                    # leading chars are untouched by rstrip, so t[:3] is the
                    # same prefix the old lstrip()[:3] test read
                    if t[:3] in ("//", "#", "/*", "*", "*/", '"""',
                                 "'''", "--", ";;", "%"):
                        cmt += 1
            ...
            file_rows.append(
                (fid, rel, os.path.dirname(rel) or ".", fn, ext, analyzer.LANG,
                 mid, st.st_size, len(lines), sloc_n, blank, cmt, maxlen,
...
def scan_markers(rec: FileRec, bufs: Buffers) -> None:
    ...
    for i, line in enumerate(rec.text.splitlines(), 1):
        # A marker row requires one of these comment introducers IN the line
        # as well as a keyword match, so lines with none of the four can skip
        # the regex entirely -- a C-speed substring scan each.
...
#: Bit flags for `measure()`'s per-type dispatch map. One dict lookup on the
#: node type replaces the ten set-membership tests the loop used to run; the
#: flags are tested in the exact order the original if/elif chain applied
#: them, so the side effects fire in the same order.
BIT_ELIF = 1        # candidate for the elif flattening (only when want_elif)
BIT_NEST = 2        # raises nesting depth
BIT_LOOP = 4        # a loop
BIT_BRANCH = 8      # a branch (elif of the loop test, as before)
BIT_CALL = 16       # on_call hook
BIT_OP = 32         # operator node (Halstead proxy)
BIT_STR = 64        # string literal
BIT_NUM = 128       # numeric literal
BIT_CMT = 256       # comment
BIT_COUNTER = 512   # in COUNTERS -> counts a metric column
BIT_FLAG = 1024     # in FLAG_NODES -> sets a boolean column
BIT_ONNODE = 2048   # subclass on_node inspects this type
...
class TreeSitterAnalyzer(Analyzer):
    ...
    def __init__(self) -> None:
        ...
        # Dispatch tables derived once from the class-level node-type tables.
        # `measure()` used to rebuild nine Python sets from them on EVERY body
        # -- one set() per table per function, several thousand per file --
        # and then test a node's type against up to ten of them. A single
        # dict lookup now answers all of those membership questions at once.
        self._fs_ifs = frozenset(self.IF_NODES)
        self._fs_loops = frozenset(self.LOOP_NODES)
        self._prune_kinds = (frozenset(self.FUNC_KINDS)
                             | frozenset(self.TYPE_KINDS))
        self._loop_call_items = tuple(self.LOOP_CALL_COUNTERS.items())
        self._hazard_memo: dict[str, Optional[tuple[str, str]]] = {}
        # type -> bit mask of every table the measure loop cares about
        bits: dict[str, int] = {}

        def _mark(types: Any, b: int) -> None:
            for x in types:
                bits[x] = bits.get(x, 0) | b

        if self.IF_NODES and self.ELSE_FIELD:      # want_elif, folded in
            _mark(self.IF_NODES, BIT_ELIF)
        _mark(self.NEST_NODES, BIT_NEST)
        _mark(self.LOOP_NODES, BIT_LOOP)
        _mark(self.BRANCH_NODES, BIT_BRANCH)
        _mark(self.CALL_NODES, BIT_CALL)
        _mark(self.OPERATOR_NODES, BIT_OP)
        _mark(self.STRING_NODES, BIT_STR)
        _mark(self.NUMBER_NODES, BIT_NUM)
        _mark(self.COMMENT_NODES, BIT_CMT)
        _mark(self.COUNTERS, BIT_COUNTER)
        _mark(self.FLAG_NODES, BIT_FLAG)
        _mark(self.ON_NODE_TYPES, BIT_ONNODE)
        self._kind_bits = bits
    ...
    def emit_module_scope(self, root: Any, rec: FileRec,
        ...
        if not self.MODULE_SCOPE_SYMBOL:
            return
        stats = self.measure(root, rec, prune=self._prune_kinds)
    ...
    def walk_scope(self, node: Any, rec: FileRec, db: sqlite3.Connection,
        ...
        # Bound once per file walk: attribute chases inside the loop were
        # per-node dict lookups on self for tables that never change.
    ...
    def emit_type(self, node: Any, rec: FileRec, db: sqlite3.Connection,
        ...
        body = node.child_by_field_name(self.BODY_FIELD) or node
        bstats = self.measure(body, rec, prune=self._prune_kinds)
    ...
    def insert_symbol(self, db: sqlite3.Connection, rec: FileRec, name: str,
        ...
        if spec is None:
            ...
            for _cid, cname, ctype, _nn, dflt, _pk in db.execute(
                ...
                spec.append((cname, d))
            self._sym_first_spec = first = 1 + len(cols)
            self._row_tmpl = [0] * (first + len(spec))
            pos: dict[str, int] = {}
            for i, (cname, dflt) in enumerate(spec):
                self._row_tmpl[first + i] = dflt
                pos[cname] = first + i
            self._col_pos = pos
            self._sym_sql = (
        ...
        self._n_sym += 1
        sid = self._n_sym
        # Template fill: start from the DDL defaults and overwrite only what
        # this symbol actually produced. Same cells as the old per-column
        # `m.get(key, default)` sweep -- m carries ~20 produced keys against
        # ~170 columns -- but len(m) dict operations instead of len(spec).
        row = self._row_tmpl.copy()
        row[0] = sid
        row[1:self._sym_first_spec] = vals
    ...
    def measure(self, body: Any, rec: FileRec,
        ...
        while True:
            ...
            named = node.is_named
            bits = kind_bits.get(t)
            if bits is None:
                # No table names this type: every membership test in the old
                # chain missed it and only the bare-token branch could fire.
                if node.child_count == 0:
                    st.n_tokens += 1
                    st.n_operands += 1
                    operands_add(text_of(node, src)[:40])
            else:
                ...
                # (BIT_ELIF is only set when IF_NODES and ELSE_FIELD are, so
                # this test folds the old `want_elif` gate.)
                is_elif = False
                if bits & BIT_ELIF:
                    par = node.parent
                    if par is not None:
                ...
                if named:
                    # tree-sitter-ruby gives its anonymous keyword tokens the
                    # same `type` strings as the named nodes (`if`, `while`,
                    # `when`, `rescue`); requiring a named node is what keeps
                    # that grammar from double-counting, and it is free
                    # everywhere else.
                    if (bits & BIT_NEST) and not is_elif and node is not body:
                        nest_stack.append(depth)
                        if st.max_nesting < len(nest_stack):
                            st.max_nesting = len(nest_stack)
                    if (bits & BIT_LOOP) or (extra_loops
                                             and node.id in extra_loops):
                        loop_stack.append(depth)
                        loop_depth += 1
                        if st.max_loop_depth < loop_depth:
                            st.max_loop_depth = loop_depth
                        st.cyclomatic += 1
                        nest = len(nest_stack)
                        st.cognitive += nest if nest > 1 else 1
                        counts["n_loops"] = cget("n_loops", 0) + 1
                    elif bits & BIT_BRANCH:
                        st.cyclomatic += 1
                        nest = len(nest_stack)
                        st.cognitive += 1 if is_elif else (nest
                                                           if nest > 1 else 1)
                ...
                if bits & BIT_CALL:
                    ...
                    counts["n_comment_lines"] = (
                        cget("n_comment_lines", 0)
    ...
    #: Node types a subclass's `on_node` actually inspects. `measure()` calls
    #: the hook only for these; every other node skipped a call that would
    #: have walked the whole dispatch chain and done nothing. A subclass that
    #: widens `on_node` must widen this set with it.
    ON_NODE_TYPES: frozenset = frozenset()
...
SQL_LIT_RE = re.compile(
    r'\b(SELECT\s|INSERT\s+INTO|UPDATE\s+\w|DELETE\s+FROM)\b', re.I)
...
# --- per-callee dispatch tables for RustAnalyzer.on_call -------------------
# The old code tested tuple literals membership-wise on every call site;
# frozensets and one dict answer the same questions with one hash each.
_LOCK_IN_LOOP_BASES = frozenset(("lock", "read", "write"))
_TO_OWNED_LOOP_BASES = frozenset(("to_string", "to_owned", "to_vec"))
_SAFE_FALLBACK_BASES = frozenset(("unwrap_or_else", "unwrap_or_default",
                                  "ok_or_else"))
_ITER_LOOP_BASES = frozenset(("iter", "into_iter", "chars", "bytes"))
_PUSH_LOOP_BASES = frozenset(("push", "insert", "extend"))
_IO_LOOP_BASES = frozenset(("read_to_string", "read_to_end", "write_all"))
_BORROW_MUT_BASES = frozenset(("get_mut", "borrow_mut"))
_UNWRAP_ERR_BASES = frozenset(("expect_err", "unwrap_err"))
_UNCHECKED_BASES = frozenset(("from_utf8_unchecked", "get_unchecked",
                              "get_unchecked_mut"))
_JOIN_BASES = frozenset(("join", "join_all", "try_join", "join_next",
                         "try_join_all"))
#: base name -> metric columns on_call bumps for it, in the order the old
#: elif chain bumped them (`borrow`/`borrow_mut` fed two columns). One base
#: never matched more than one branch of that chain, so a single lookup is
#: the same verdict.
_CALL_BASE_COLS: dict[str, tuple[str, ...]] = {
    "unwrap": ("n_unwrap",),
    "expect": ("n_expect",),
    "clone": ("n_clone",), "cloned": ("n_clone",),
    "to_owned": ("n_to_owned",), "to_vec": ("n_to_owned",),
    "to_string": ("n_to_owned",),
    "collect": ("n_collect",),
    "with_capacity": ("n_with_capacity",),
    "borrow": ("n_borrow_calls", "n_lock_acquire"),
    "borrow_mut": ("n_borrow_calls", "n_lock_acquire"),
    "lock": ("n_lock_acquire",), "try_lock": ("n_lock_acquire",),
    "read": ("n_lock_acquire",), "write": ("n_lock_acquire",),
    "lock_owned": ("n_lock_acquire",),
    "spawn": ("n_spawn",),
    "spawn_blocking": ("n_spawn_blocking",),
    "block_in_place": ("n_spawn_blocking",),
    "send": ("n_channel_ops",), "recv": ("n_channel_ops",),
    "try_send": ("n_channel_ops",), "try_recv": ("n_channel_ops",),
    "blocking_send": ("n_channel_ops",), "blocking_recv": ("n_channel_ops",),
    "load": ("n_atomic_ops",), "store": ("n_atomic_ops",),
    "fetch_add": ("n_atomic_ops",), "fetch_sub": ("n_atomic_ops",),
    "fetch_update": ("n_atomic_ops",), "fetch_or": ("n_atomic_ops",),
    "compare_exchange": ("n_atomic_ops",),
    "compare_exchange_weak": ("n_atomic_ops",),
    "fence": ("n_atomic_ops",),
    "transmute": ("n_transmute",), "transmute_copy": ("n_transmute",),
    "from_raw": ("n_from_raw",), "from_raw_parts": ("n_from_raw",),
    "from_raw_parts_mut": ("n_from_raw",), "from_raw_fd": ("n_from_raw",),
    "into_raw": ("n_into_raw",), "into_raw_fd": ("n_into_raw",),
}
...
class RustAnalyzer(TreeSitterAnalyzer):
    ...
    #: exactly the types RustAnalyzer.on_node dispatches on
    ON_NODE_TYPES = frozenset((
        "let_declaration", "binary_expression", "compound_assignment_expr",
        "unary_expression", "index_expression", "scoped_identifier",
        "await_expression", "generic_type", "return_expression",
        "match_arm",
    ))
    ...
    ("n_elif", "INT NOT NULL DEFAULT 0"),
        ...
        # -- task/thread bookkeeping (tokio anti-patterns, wave-3 pack) ------
        # `thread::spawn` specifically: an OS thread, not a green task. The
        # plain n_spawn counts both and cannot separate a runtime task from a
        # thread the runtime will never see.
        ("n_thread_spawn", "INT NOT NULL DEFAULT 0"),
        # A spawn whose call site sits inside a loop body: one task per item
        # is the unbounded-concurrency shape tokio's docs warn about.
        ("n_spawn_in_loop", "INT NOT NULL DEFAULT 0"),
        # join / join_all / join_next: handle reclamation, for the
        # spawn-vs-join balance that says whether detached tasks are intended.
        ("n_join_calls", "INT NOT NULL DEFAULT 0"),
    )
...
CREATE TABLE macros(
    ...
    n_unsafe INT NOT NULL DEFAULT 0,        -- `unsafe` tokens the expansion hides
    line INT NOT NULL DEFAULT 0
) STRICT;
...
"""
    ...
    def function_flags(self, node: Any, rec: FileRec,
        ...
        # Six counting passes over the same four small subtrees (one per
        # metric, each materialising `.children` lists on the way) used to
        # sit here; two passes counting all six counters collect exactly the
        # same totals -- counting is order-free and no type belongs to two
        # of these counters.
        n_generic = 0
        n_bounds = 0
        n_life = 0
        ...
        for sub in (tp, where):
            if sub is None:
                continue
            for n in walk_nodes(sub):
                nt = n.type
                if nt in ("type_parameter", "const_parameter"):
                    n_generic += 1
                elif nt == "trait_bounds":
                    n_bounds += 1
                elif nt in ("lifetime", "lifetime_parameter"):
                    n_life += 1
                elif nt in ("higher_ranked_trait_bound", "for_lifetimes"):
                    n_hrtb += 1
                elif nt == "where_predicate":
                    n_where += 1
        ...
        for sub in (params, ret):
            ...
            for n in walk_nodes(sub):
                ...
                if nt == "dynamic_type":
                    n_dyn += 1
                elif nt == "abstract_type":
                    n_impl_trait += 1
                elif nt == "pointer_type":
                    n_raw += 1
                elif nt == "lifetime":
                    n_life += 1
    ...
    def type_flags(self, node: Any, rec: FileRec,
        ...
        head = txt.split("{", 1)[0]
        attrs = _attr_names(node, src)
        # One decode-and-slice of the sibling attribute run, shared by the
        # three readers below instead of walking it twice more.
        arg_list = _attr_args(node, src)
        tp = node.child_by_field_name("type_parameters")
        n_generic = sum(1 for n in walk_nodes(tp)
        ...
        return dict(
            ...
            n_expect_attrs=sum(1 for a in attrs if a == "expect"),
            n_cfg_blocks=sum(1 for a in attrs if a.startswith("cfg")),
            n_cfg_features=len(CFG_FEATURE_RE.findall(" ".join(arg_list))),
            is_test=int(node.type == "mod_item"
                        and any("cfg(test)" in a for a in arg_list)),
        )
    ...
    def on_call(self, node: Any, src: bytes, st: BodyStats,
        ...
        if node.type == "macro_invocation":
            ...
            if loop_depth:
                for needle, col in self._loop_call_items:
                    if needle == name:
                        st.bump(col)
            return
        ...
        if base in BLOCKING_IO or name in BLOCKING_IO or \
                "::".join(name.replace(".", "::").split("::")[-2:]) \
                in BLOCKING_IO:
    ...
    def hazard_of(self, callee: str) -> Optional[tuple[str, str]]:
        # One memo for the whole run: this is called once per recorded call
        # site and is a pure function of the callee text, while real crates
        # call `.clone()`/`.lock()` thousands of times under a few hundred
        # distinct spellings.
                ...
                bufs.rows("macros").append(
                    (sid, rec.fid, text_of(m, src)[:120], "invocation", 0,
                     ...
                     int(bool(re.search(r'\b(fn|struct|impl|enum|trait)\s',
                                        inner))),
                     len(re.findall(r'\bunsafe\b', inner)),
                     n.start_point[0] + 1))
        ...
        if t == "trait_item":
            ...
            bufs.rows("macros").append(
                (sid, rec.fid, self.node_name(node, rec)[:120], "definition",
                 ...
                 len(re.findall(r'\bunsafe\b', txt)),
                 node.start_point[0] + 1))
            ...
            if cur.type == "block":
                for c in cur.named_children:
                    ...
                    span = src[c.end_byte:n.start_byte].decode(
                        "utf-8", "replace")
                    if _drop_re(gname).search(span):
                        dropped = 1
                        continue
                    guards.append(gname[:40])
            cur = cur.parent
        depth = _loop_depth(n, body, loop_types)
    ...
    def flush_extra(self, db: sqlite3.Connection, bufs: Buffers) -> None:
        for tbl, sql in (
            ...
            ("macros",
             ...
             "body_bytes,defines_items,n_unsafe,line) VALUES(?,?,?,?,?,?,?,?,?)"),
            ("cfg_blocks",
...
_DROP_SEARCH_CACHE: dict[str, "re.Pattern[str]"] = {}

def _drop_re(gname: str) -> "re.Pattern[str]":
    """Compiled `drop(<name>)` finder, cached per guard name.

    The pattern used to be recompiled through the re module's cache on every
    guard of every await; guard names repeat heavily, so one compile per
    distinct name is the same matching for less work.
    """
    pat = _DROP_SEARCH_CACHE.get(gname)
    if pat is None:
        pat = re.compile(r'\bdrop\s*\(\s*%s\s*\)' % re.escape(gname))
        _DROP_SEARCH_CACHE[gname] = pat
    return pat
...
(
    ...
    LIMIT :lim"""),
# ===================== wave-3 pack: graph-context defects =====================
("transitive-panic-surface",
    "unwrap, expect, panic! and indexing reachable THROUGH the call graph from a pub or main entry",
    "ANSWERS the panic surface no per-function lint can see: clippy::unwrap_used\n"
    ...
    """WITH RECURSIVE walk(root, sym, depth) AS (
        SELECT s.id, s.id, 0 FROM symbols s
        WHERE (s.is_public=1 OR s.is_entrypoint=1)
          AND s.kind IN ('function','method') AND s.is_test=0
        UNION
        SELECT w.root, e.callee_id, w.depth+1
        FROM walk w JOIN edges e ON e.caller_id=w.sym
    ...
    reach(root, sym, depth) AS (
        SELECT root, sym, MIN(depth) FROM walk GROUP BY root, sym)
    SELECT s.name, entry.name AS reached_from, r.depth AS hops,
        s.n_unwrap AS unwraps, s.n_expect AS expects,
        s.n_panic_macro AS panic_macros, s.n_index_expr AS index_ops,
        s.n_slice_range AS slices, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM reach r
    JOIN symbols s ON s.id=r.sym
    JOIN symbols entry ON entry.id=r.root
    ...
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE r.depth > 0
      AND (s.n_unwrap + s.n_expect + s.n_panic_macro
           + s.n_index_expr + s.n_slice_range) > 0
      AND s.is_test=0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY (s.n_unwrap + s.n_expect + s.n_panic_macro + s.n_index_expr) DESC,
        r.depth ASC LIMIT :lim"""),
("result-unwrapped-by-caller",
    "callee returns Result/Option, caller .unwrap()s it -- the seam where error handling is abandoned",
    "ANSWERS the pairing the type system flags but no per-function tool can\n"
    ...
    """SELECT c.name AS unwrapping_caller, k.name AS returns_result,
        k.return_type, c.n_unwrap AS caller_unwraps, c.n_expect AS caller_expects,
        k.n_question_mark AS callee_question_marks, k.fan_in AS callee_fan_in,
        c.cyclomatic AS cyclo,
        f.path || ':' || c.line_start AS at
    FROM edges e
    JOIN symbols c ON c.id=e.caller_id
    JOIN symbols k ON k.id=e.callee_id
    JOIN files f ON f.id=c.file_id
    LEFT JOIN modules m ON m.id=c.module_id
    WHERE e.is_self=0
      AND (k.return_type LIKE '%Result<%' OR k.return_type LIKE '%Option<%')
      AND (c.n_unwrap + c.n_expect) > 0
      AND c.is_test=0 AND k.is_test=0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY (c.n_unwrap + c.n_expect) DESC, k.fan_in DESC LIMIT :lim"""),
("mutex-poison-cascade",
    "lock().unwrap() sites -- the second dominoes of a poisoned mutex, ranked by how many callers they take down",
    "ANSWERS the cascade the std docs describe but nothing measures: one panic\n"
    ...
    """SELECT s.name, s.impl_type AS on_type,
        s.n_lock_acquire AS lock_acquires, s.n_unwrap AS unwraps,
        s.n_expect AS expects, s.n_await AS awaits,
        (s.n_unwrap + s.n_expect) * MAX(s.fan_in, 1) AS cascade_blast,
        s.fan_in, f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_lock_acquire > 0 AND (s.n_unwrap + s.n_expect) > 0
      AND s.is_test=0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY cascade_blast DESC, s.n_lock_acquire DESC LIMIT :lim"""),
("panic-in-drop-impl",
    "Drop impl methods that can panic -- a panic while unwinding aborts the process",
    "ANSWERS Drop impls whose methods contain panic sites. A panic raised while\n"
    ...
    """SELECT d.type_name AS type_, s.name AS drop_method,
        s.n_unwrap AS unwraps, s.n_expect AS expects,
        s.n_panic_macro AS panic_macros, s.n_index_expr AS index_ops,
        s.n_borrow_calls AS refcell_borrows, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM impls d
    JOIN symbols s ON s.parent_id=d.symbol_id
        AND s.kind IN ('function','method')
    JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE d.trait_name = 'Drop'
      AND (s.n_unwrap + s.n_expect + s.n_panic_macro
           + s.n_index_expr + s.n_borrow_calls) > 0
      AND s.is_test=0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY (s.n_unwrap + s.n_expect + s.n_panic_macro) DESC,
        s.fan_in DESC LIMIT :lim"""),
("panic-in-extern-fn",
    "extern \"C\" fns containing panic sites -- since Rust 1.81 the process aborts at the boundary",
    "ANSWERS body-ful `extern \"C\" fn` items that can panic. Before 1.81 that\n"
    ...
    """SELECT s.name, s.n_unwrap AS unwraps, s.n_expect AS expects,
        s.n_panic_macro AS panic_macros, s.n_index_expr AS index_ops,
        s.n_extern_calls AS ffi_calls_out, s.n_unsafe_blocks AS unsafe_blocks,
        s.fan_in, f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.is_extern_fn = 1
      AND (s.n_unwrap + s.n_expect + s.n_panic_macro + s.n_index_expr) > 0
      AND s.is_test=0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY (s.n_unwrap + s.n_expect + s.n_panic_macro) DESC,
        s.fan_in DESC LIMIT :lim"""),
("await-in-loop",
    ".await inside a loop body -- N items take N sequential round-trips (tokio tutorial anti-pattern)",
    "ANSWERS sequential awaiting: each iteration parks this task until the\n"
    ...
    """SELECT s.name, s.impl_type AS on_type,
        s.await_in_loop AS awaits_in_loop, s.n_await AS awaits_total,
        s.n_channel_ops AS channel_ops, s.n_lock_acquire AS locks,
        s.is_async_fn AS async_fn, s.is_public AS pub_, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.await_in_loop > 0
      AND s.is_test=0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.await_in_loop DESC, s.fan_in DESC LIMIT :lim"""),
("blocking-in-critical-section",
    "a lock acquired, then a blocking call reached while it is held -- here or up to 3 frames down",
    "ANSWERS the sync cousin of await_holding_lock: a guard still alive while\n"
    ...
    """WITH RECURSIVE walk(root, sym, depth) AS (
        ...
        SELECT w.root, e.callee_id, w.depth+1
        FROM walk w JOIN edges e ON e.caller_id=w.sym
        WHERE w.depth < 3 AND e.is_self=0),
    reach(root, sym, depth) AS (
        SELECT root, sym, MIN(depth) FROM walk GROUP BY root, sym)
    SELECT h.name AS takes_lock, s.name AS blocking_callee, r.depth AS hops,
        s.n_blocking_io AS blocking_calls, s.n_thread_sleep AS sleeps,
        s.n_block_on AS block_ons, s.io_in_loop AS blocking_in_loop,
        f.path || ':' || s.line_start AS at
    FROM reach r
    JOIN symbols s ON s.id=r.sym
    JOIN symbols h ON h.id=r.root
    JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE (s.n_blocking_io > 0 OR s.n_thread_sleep > 0 OR s.n_block_on > 0)
      AND f.is_test=0 AND COALESCE(m.name,'') LIKE :mod
    ORDER BY (s.n_blocking_io + s.n_thread_sleep + s.n_block_on) DESC,
        r.depth ASC LIMIT :lim"""),
("sync-lock-in-async-fn",
    "async fns acquiring std-sync locks -- prefer tokio::sync or a critical section too short to care",
    "ANSWERS async functions that call lock/read/write/borrow at all. A std\n"
    ...
    """SELECT s.name, s.impl_type AS on_type,
        s.n_lock_acquire AS lock_acquires, s.n_await AS awaits,
        s.n_lock_across_await AS guards_across_await,
        s.n_arc_mutex AS arc_mutex_fields, s.n_rc_refcell AS rc_refcell_fields,
        s.is_public AS pub_, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.is_async_fn = 1 AND s.n_lock_acquire > 0
      AND s.is_test=0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_lock_acquire DESC, s.n_lock_across_await DESC LIMIT :lim"""),
("block-on-in-async-context",
    ".block_on() reached from an async fn -- blocking the thread that should be polling",
    "ANSWERS block_on called from an async fn directly (hops=0) or through up\n"
    ...
    """WITH RECURSIVE walk(root, sym, depth) AS (
        SELECT s.id, s.id, 0 FROM symbols s WHERE s.is_async_fn = 1
        UNION
        SELECT w.root, e.callee_id, w.depth+1
        FROM walk w JOIN edges e ON e.caller_id=w.sym
        WHERE w.depth < 3 AND e.is_self=0),
    reach(root, sym, depth) AS (
        SELECT root, sym, MIN(depth) FROM walk GROUP BY root, sym)
    SELECT a.name AS async_fn, s.name AS blocks_in, r.depth AS hops,
        s.n_block_on AS block_ons, s.n_blocking_io AS other_blocking,
        a.fan_in AS async_fan_in,
        f.path || ':' || s.line_start AS at
    FROM reach r
    JOIN symbols s ON s.id=r.sym
    ...
    WHERE s.n_block_on > 0 AND f.is_test=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_block_on DESC, r.depth ASC LIMIT :lim"""),
("os-thread-in-async",
    "thread::spawn reached from an async fn -- OS threads the runtime cannot see, count, or cancel",
    "ANSWERS raw OS threads created under an async fn, directly or up to 3\n"
    ...
    reach(root, sym, depth) AS (
        SELECT root, sym, MIN(depth) FROM walk GROUP BY root, sym)
    SELECT a.name AS async_fn, s.name AS spawns_thread, r.depth AS hops,
        s.n_thread_spawn AS thread_spawns, s.n_spawn AS total_spawns,
        s.n_spawn_blocking AS spawn_blockings, a.fan_in AS async_fan_in,
        f.path || ':' || s.line_start AS at
    FROM reach r
    JOIN symbols s ON s.id=r.sym
    JOIN symbols a ON a.id=r.root
    JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_thread_spawn > 0 AND f.is_test=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_thread_spawn DESC, r.depth ASC LIMIT :lim"""),
("spawned-task-panic",
    "closures handed to spawn that contain panic sites -- the task dies silently when the handle is dropped",
    "ANSWERS task bodies with unwraps/panics inside. A panicked task stores the\n"
    ...
    "     invisible here.",
    """SELECT p.name AS spawns_tasks, c.name AS task_body,
        c.n_unwrap AS unwraps, c.n_expect AS expects,
        c.n_panic_macro AS panic_macros, c.n_index_expr AS index_ops,
        p.n_spawn AS spawns_here, p.is_async_fn AS spawner_is_async,
        f.path || ':' || c.line_start AS at
    FROM symbols p
    JOIN symbols c ON c.parent_id=p.id AND c.kind='closure'
    JOIN files f ON f.id=c.file_id
    LEFT JOIN modules m ON m.id=p.module_id
    WHERE p.n_spawn > 0
      AND (c.n_unwrap + c.n_expect + c.n_panic_macro + c.n_index_expr) > 0
      AND p.is_test=0 AND c.is_test=0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY (c.n_unwrap + c.n_expect + c.n_panic_macro) DESC,
        p.n_spawn DESC LIMIT :lim"""),
("spawn-in-loop",
    "spawn calls inside loop bodies -- one task per item is unbounded concurrency",
    "ANSWERS spawns (tasks or threads) whose call site sits in a loop: input\n"
    ...
    """SELECT s.name, s.impl_type AS on_type,
        s.n_spawn_in_loop AS spawns_in_loop, s.n_spawn AS spawns_total,
        s.n_spawn_blocking AS spawn_blockings, s.n_join_calls AS joins,
        s.max_loop_depth AS loop_nesting, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_spawn_in_loop > 0
      AND s.is_test=0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_spawn_in_loop DESC, s.fan_in DESC LIMIT :lim"""),
("spawn-join-balance",
    "module-level task accounting: spawns minus handle joins -- the fire-and-forget ledger",
    "ANSWERS whether a module's concurrency is reclaimed. A large positive\n"
    ...
    """SELECT m.name AS module_,
        SUM(s.n_spawn) AS spawns, SUM(s.n_spawn_blocking) AS spawn_blocking,
        SUM(s.n_thread_spawn) AS thread_spawns, SUM(s.n_join_calls) AS joins,
        SUM(s.n_spawn) - SUM(s.n_join_calls) AS detached,
        COUNT(DISTINCT CASE WHEN s.n_spawn > 0 THEN s.id END) AS spawning_fns
    FROM symbols s
    JOIN files f ON f.id=s.file_id
    JOIN modules m ON m.id=s.module_id
    ...
    GROUP BY m.id
    HAVING spawns > joins
    ORDER BY detached DESC, spawns DESC LIMIT :lim"""),
("channel-op-in-loop",
    "send/recv inside loops -- unbounded queue growth or a self-deadlock when the buffer fills",
    "ANSWERS channel traffic inside loop bodies. send-per-iteration into an\n"
    ...
    """SELECT s.name, s.impl_type AS on_type,
        s.n_channel_ops AS channel_ops, s.n_await AS awaits,
        s.await_in_loop AS awaits_in_loop, s.max_loop_depth AS loop_nesting,
        s.fan_in, f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.n_channel_ops > 0 AND s.max_loop_depth > 0
      AND s.is_test=0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_channel_ops DESC, s.fan_in DESC LIMIT :lim"""),
("deep-async-call-chain",
    "async call chains 4+ hops deep -- every frame's state is embedded in the root's future",
    "ANSWERS the giant-future problem: an async fn's future embeds the state\n"
    ...
    """WITH RECURSIVE down(root, sym, depth) AS (
        SELECT s.id, s.id, 0 FROM symbols s
        WHERE s.is_async_fn = 1 AND s.is_test=0
        UNION
        SELECT d.root, e.callee_id, d.depth+1
        FROM down d JOIN edges e ON e.caller_id=d.sym
        WHERE d.depth < 8 AND e.is_self=0)
    SELECT a.name AS async_entry, MAX(d.depth) AS chain_depth,
        COUNT(DISTINCT d.sym) AS callees_downstream,
        a.fan_in, a.sloc,
        f.path || ':' || a.line_start AS at
    FROM down d
    JOIN symbols a ON a.id=d.root
    JOIN files f ON f.id=a.file_id
    LEFT JOIN modules m ON m.id=a.module_id
    GROUP BY a.id
    HAVING chain_depth >= 4
    ORDER BY chain_depth DESC, callees_downstream DESC LIMIT :lim"""),
("swallowed-error-below-pub-api",
    "let _ = fallible() / .map_err(|_| ...) reachable from a pub API -- the caller gets Ok while it failed",
    "ANSWERS the cross-frame half of clippy::let_underscore_must_use: the\n"
    ...
    reach(root, sym, depth) AS (
        SELECT root, sym, MIN(depth) FROM walk GROUP BY root, sym)
    SELECT s.name, entry.name AS reached_from, r.depth AS hops,
        s.n_error_swallow AS swallows, s.n_question_mark AS propagates,
        s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM reach r
    JOIN symbols s ON s.id=r.sym
    ...
    JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE r.depth > 0 AND s.n_error_swallow > 0
      AND s.is_test=0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_error_swallow DESC, r.depth ASC LIMIT :lim"""),
("exit-skipping-drop",
    "process::exit / abort reachable from a pub API -- destructors do not run on the way out",
    "ANSWERS where the process dies without unwinding. std::process::exit runs\n"
    ...
    reach(root, sym, depth) AS (
        SELECT root, sym, MIN(depth) FROM walk GROUP BY root, sym)
    SELECT s.name AS calls_exit, h.pattern AS sink, h.n AS exit_sites,
        entry.name AS reached_from, r.depth AS hops, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM reach r
    JOIN symbols s ON s.id=r.sym
    JOIN hazards h ON h.symbol_id=s.id
        AND h.pattern IN ('exit','process::exit','*::exit','abort')
    JOIN symbols entry ON entry.id=r.root
    JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE f.is_test=0 AND COALESCE(m.name,'') LIKE :mod
    ORDER BY r.depth ASC, h.n DESC, s.fan_in DESC LIMIT :lim"""),
("explicit-leak-surface",
    "mem::forget, Box::leak and .leak() call sites -- memory intentionally never freed",
    "ANSWERS the leak sites: forget destroys the value without running Drop,\n"
    ...
    """SELECT s.name, h.pattern AS leak_site, h.n AS leak_calls,
        h.category, s.is_public AS pub_, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM hazards h
    JOIN symbols s ON s.id=h.symbol_id
    JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE h.pattern IN ('mem::forget','Box::leak','leak')
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY h.n DESC, s.fan_in DESC LIMIT :lim"""),
("needless-unsafe-fn",
    "unsafe fn with no unsafe op in its body -- callers write unsafe {} for nothing",
    "ANSWERS unsafe fn whose body contains no unsafe block, transmute or raw\n"
    ...
    """SELECT s.name, s.impl_type AS on_type,
        s.is_unsafe_fn AS unsafe_fn, s.n_unsafe_blocks AS unsafe_blocks,
        s.n_unsafe_ops AS unsafe_ops, s.n_transmute AS transmutes,
        s.n_raw_ptr AS raw_ptrs, s.is_public AS pub_, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.is_unsafe_fn = 1
      AND s.n_unsafe_blocks = 0 AND s.n_unsafe_ops = 0
      AND s.n_transmute = 0 AND s.n_from_raw = 0
      AND s.is_test=0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC, s.sloc DESC LIMIT :lim"""),
("gratuitous-unsafe-impl",
    "unsafe impl Send/Sync/... on a type whose impl block shows no unsafe op at all",
    "ANSWERS the manual promises with nothing visible behind them. clippy's\n"
    ...
    "    more often.",
    """WITH impl_body AS (
        SELECT i.id AS impl_id, i.symbol_id, i.type_name, i.trait_name,
            i.line, i.file_id,
            COALESCE(SUM(m.n_unsafe_blocks),0) AS unsafe_blocks,
            COALESCE(SUM(m.n_raw_ptr),0) AS raw_ptrs,
            COALESCE(SUM(m.n_transmute),0) AS transmutes
        FROM impls i
        LEFT JOIN symbols m ON m.parent_id=i.symbol_id
            AND m.kind IN ('function','method')
        WHERE i.is_unsafe = 1
        GROUP BY i.id)
    SELECT b.type_name AS type_, b.trait_name AS trait_,
        b.unsafe_blocks, b.raw_ptrs, b.transmutes,
        ts.fan_in AS impl_fan_in,
        (SELECT COUNT(*) FROM impls i2
         WHERE i2.trait_name=b.trait_name) AS impls_of_trait,
        f.path || ':' || b.line AS at
    FROM impl_body b
    JOIN symbols ts ON ts.id=b.symbol_id
    JOIN files f ON f.id=b.file_id
    LEFT JOIN modules m2 ON m2.id=f.module_id
    WHERE b.unsafe_blocks = 0 AND b.raw_ptrs = 0 AND b.transmutes = 0
      AND f.is_test=0 AND COALESCE(m2.name,'') LIKE :mod
    ORDER BY ts.fan_in DESC, b.trait_name, b.type_name LIMIT :lim"""),
("macro-defined-unsafe",
    "unsafe inside macro_rules! bodies -- re-expanded (and re-unreviewed) at every call site",
    "ANSWERS unsafe code the unsafe_blocks table never sees: definitions whose\n"
    ...
    """SELECT md.name AS macro_, md.n_unsafe AS unsafe_tokens,
        md.body_bytes, md.defines_items,
        (SELECT COUNT(*) FROM macros mi WHERE mi.kind='invocation'
           AND mi.name=md.name) AS call_sites,
        f.path || ':' || md.line AS at
    FROM macros md
    JOIN files f ON f.id=md.file_id
    LEFT JOIN modules m ON m.id=f.module_id
    WHERE md.kind='definition' AND md.n_unsafe > 0
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY call_sites DESC, md.n_unsafe DESC LIMIT :lim"""),
("mutual-recursion",
    "indirect recursion cycles in the call graph (a->b->a) -- the stack-overflow paths is_recursive cannot see",
    "ANSWERS call-graph cycles of length 2..6 through the RESOLVED edge table.\n"
    ...
    """WITH RECURSIVE walk(start, cur, depth) AS (
        ...
        UNION
        SELECT w.start, e.callee_id, w.depth+1
        FROM walk w JOIN edges e ON e.caller_id=w.cur
        WHERE w.depth < 6 AND e.is_self=0)
    SELECT s.name, s.impl_type AS on_type, MIN(w.depth) AS cycle_len,
        s.fan_in, s.is_public AS pub_,
        f.path || ':' || s.line_start AS at
    FROM walk w
    JOIN symbols s ON s.id=w.start
    JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE w.start = w.cur AND w.depth > 0
      AND f.is_test=0 AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id
    ORDER BY cycle_len ASC, s.fan_in DESC LIMIT :lim"""),
("uncalled-pub-api",
    "pub functions nothing in this tree calls -- what pub hides from rustc's dead-code lint",
    "ANSWERS the over-exposed surface: pub fns with fan_in=0. rustc's\n"
    ...
    """SELECT s.name, s.impl_type AS on_type,
        s.is_trait_method AS trait_disp,
        s.sloc, s.cyclomatic AS cyclo, s.has_doc,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.is_public = 1 AND s.fan_in = 0
      AND s.kind IN ('function','method')
      AND s.is_entrypoint = 0 AND s.is_abstract = 0
      AND s.is_test=0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.sloc DESC, s.cyclomatic DESC LIMIT :lim"""),
("global-mutable-state",
    "statics holding shared mutability -- static mut, Mutex/RwLock, atomics: one lock (or race) per process",
    "ANSWERS file-level `static` items that carry mutable shared state. Every\n"
    ...
    "    not modelled.",
    """SELECT s.name AS global_, s.kind,
        s.n_static_mut AS static_mut_, s.n_arc_mutex AS nested_locks,
        s.signature,
        (SELECT COUNT(*) FROM symbols acc
         WHERE acc.module_id = s.module_id AND acc.is_test = 0
           AND acc.kind IN ('function','method')
           AND acc.n_lock_acquire > 0) AS locking_fns_in_module,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.kind = 'static'
      AND (s.n_static_mut > 0 OR s.n_arc_mutex > 0
           OR s.signature LIKE '%Mutex<%' OR s.signature LIKE '%RwLock<%'
           OR s.signature LIKE '%Atomic%')
      AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_static_mut DESC, locking_fns_in_module DESC LIMIT :lim"""),
("duplicate-dependency-versions",
    "same crate declared at two different versions -- cargo-deny's duplicates check over the manifests",
    "ANSWERS crates the workspace pulls at more than one version: two\n"
    ...
    """SELECT d.name, COUNT(DISTINCT d.version) AS versions,
        GROUP_CONCAT(DISTINCT d.version) AS version_list,
        COUNT(*) AS declarations,
        SUM(d.is_dev) AS dev_declarations
    FROM deps d
    GROUP BY d.name
    HAVING versions > 1
    ORDER BY versions DESC, declarations DESC LIMIT :lim"""),
("needlessly-async-fn",
    "async fn with no await and no async block -- clippy::unused_async, ranked by caller cost",
    "ANSWERS signatures that promise suspension the body never does. Every\n"
    ...
    "    they are.",
    """SELECT s.name, s.impl_type AS on_type,
        s.n_await AS awaits, s.n_async_blocks AS async_blocks,
        s.is_trait_method AS trait_method, s.is_public AS pub_, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.is_async_fn = 1 AND s.n_await = 0 AND s.n_async_blocks = 0
      AND s.is_test=0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC, s.sloc DESC LIMIT :lim"""),
("allocation-loop-under-pub-api",
    "clone/collect/format inside loops, reachable from a pub fn -- churn every caller pays",
    "ANSWERS the allocation-churn signal promoted to a caller-facing property:\n"
    ...
    SELECT s.name, entry.name AS reached_from, r.depth AS hops,
        ...
        s.n_to_owned_in_loop AS to_owned_in_loop,
        s.n_with_capacity AS with_capacity, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM reach r
    JOIN symbols s ON s.id=r.sym
    JOIN symbols entry ON entry.id=r.root
    ...
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE r.depth > 0
      AND (s.n_clone_in_loop > 0 OR s.alloc_in_loop > 0
           OR s.n_to_owned_in_loop > 0)
      AND s.is_test=0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY (s.n_clone_in_loop + s.alloc_in_loop) DESC,
        r.depth ASC LIMIT :lim"""),
("rc-in-public-signature",
    "pub APIs taking or returning Rc/RefCell -- the signature infects every caller with single-threadedness",
    "ANSWERS public functions whose parameter types carry Rc< or RefCell<.\n"
    ...
    """SELECT s.name, p.name AS param, p.type AS param_type,
        s.is_public AS pub_, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM params p
    ...
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE s.is_public = 1
      AND (p.type LIKE '%Rc<%' OR p.type LIKE '%Rc <%'
           OR p.type LIKE '%RefCell<%' OR p.type LIKE '%RefCell <%'
           OR p.type LIKE '%rc::Rc<%' OR p.type LIKE '%cell::RefCell<%')
      AND s.is_test=0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC LIMIT :lim"""),
("ptr-arg-surface",
    "&mut Vec<T> / &mut String / &mut HashMap parameters (clippy::ptr_arg), ranked by callers to migrate",
    "ANSWERS mutable references to whole collections. clippy::ptr_arg says it\n"
    ...
    JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE (p.type LIKE '%&mut Vec<%' OR p.type LIKE '%&mut Vec <%'
           OR p.type LIKE '%&mut String%' OR p.type LIKE '%&mut PathBuf%'
           OR p.type LIKE '%&mut HashMap<%' OR p.type LIKE '%&mut HashSet<%')
      AND s.is_test=0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC, s.sloc DESC LIMIT :lim"""),
("test-only-callers",
    "production functions whose entire caller set is test code -- dead in prod, kept alive by their tests",
    "ANSWERS the graph fact no per-file tool can state: fan_in > 0, and every\n"
    ...
    """WITH test_in AS (
        SELECT e.callee_id AS id, COUNT(*) AS n
        FROM edges e
        JOIN symbols c ON c.id=e.caller_id
    ...
    SELECT s.name, s.impl_type AS on_type,
        s.fan_in AS callers_total, COALESCE(ti.n, 0) AS from_tests,
        s.is_public AS pub_,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id=s.file_id
    LEFT JOIN modules m ON m.id=s.module_id
    LEFT JOIN test_in ti ON ti.id=s.id
    WHERE s.kind IN ('function','method')
      AND s.fan_in > 0 AND COALESCE(ti.n, 0) = s.fan_in
      AND s.is_test=0 AND f.is_test=0 AND f.is_generated=0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.fan_in DESC LIMIT :lim""")
]
...
# ===================== wave-3 pack: design/triage metrics =====================
("unsafe-density",
    "unsafe surface per module: blocks, ops, undocumented share, normalized per ksloc",
    "ANSWERS how much of each module the compiler cannot check: unsafe blocks\n"
    ...
    "    the density.",
    """SELECT m.name AS module_,
        COUNT(DISTINCT s.id) AS fns_total,
        COUNT(DISTINCT CASE WHEN s.n_unsafe_blocks > 0 THEN s.id END)
            AS fns_unsafe,
        SUM(s.n_unsafe_blocks) AS unsafe_blocks,
        SUM(s.n_unsafe_ops) AS unsafe_ops,
        SUM(s.n_unsafe_blocks - s.n_safety_comments) AS undocumented_blocks,
        SUM(s.n_transmute) AS transmutes, SUM(s.n_raw_ptr) AS raw_ptrs,
        SUM(s.sloc) AS sloc_,
        CAST(1000.0 * COALESCE(SUM(s.n_unsafe_ops),0)
             / NULLIF(SUM(s.sloc),0) AS INT) AS ops_per_ksloc
    FROM symbols s
    JOIN files f ON f.id=s.file_id
    JOIN modules m ON m.id=s.module_id
    WHERE s.kind IN ('function','method')
      ...
      AND m.name LIKE :mod
    GROUP BY m.id
    HAVING unsafe_blocks > 0
    ORDER BY unsafe_ops DESC, undocumented_blocks DESC LIMIT :lim"""),
("async-blocking-ratio",
    "async/blocking texture per module: what fraction of blocking I/O sits inside async fns",
    "ANSWERS where latency incidents will come from: async fns, awaits,\n"
    ...
    """SELECT m.name AS module_,
        COUNT(DISTINCT CASE WHEN s.is_async_fn = 1 THEN s.id END) AS async_fns,
        SUM(s.n_await) AS awaits, SUM(s.n_spawn) AS spawns,
        SUM(s.n_spawn_blocking) AS spawn_blocking,
        SUM(s.n_blocking_io) AS blocking_io,
        SUM(s.n_blocking_in_async) AS blocking_in_async,
        SUM(s.n_lock_across_await) AS guards_across_await,
        SUM(s.n_thread_sleep) AS sleeps, SUM(s.n_block_on) AS block_ons,
        CAST(100.0 * COALESCE(SUM(s.n_blocking_in_async),0)
             / NULLIF(SUM(s.n_blocking_io),0) AS INT)
            AS pct_blocking_in_async
    FROM symbols s
    JOIN files f ON f.id=s.file_id
    JOIN modules m ON m.id=s.module_id
    WHERE s.kind IN ('function','method')
      ...
      AND m.name LIKE :mod
    GROUP BY m.id
    HAVING async_fns > 0 OR blocking_io > 0
    ORDER BY blocking_in_async DESC, blocking_io DESC LIMIT :lim"""),
("panic-density",
    "panic surface per module: unwraps, expects, panic macros, indexing -- total and per ksloc",
    "ANSWERS which modules fail loudly and by which mechanism: the\n"
    ...
    """SELECT m.name AS module_,
        COUNT(DISTINCT CASE WHEN s.n_unwrap + s.n_expect
             + s.n_panic_macro > 0 THEN s.id END) AS fns_panicky,
        SUM(s.n_unwrap) AS unwraps, SUM(s.n_expect) AS expects,
        SUM(s.n_panic_macro) AS panic_macros,
        SUM(s.n_index_expr) AS index_ops,
        SUM(s.n_slice_range) AS slices,
        SUM(s.n_borrow_calls) AS refcell_borrows,
        SUM(s.n_safe_fallback) AS safe_fallbacks,
        SUM(s.sloc) AS sloc_,
        CAST(1000.0 * COALESCE(SUM(s.n_unwrap + s.n_expect
             + s.n_panic_macro + s.n_index_expr),0)
             / NULLIF(SUM(s.sloc),0) AS INT) AS panics_per_ksloc
    FROM symbols s
    JOIN files f ON f.id=s.file_id
    JOIN modules m ON m.id=s.module_id
    WHERE s.kind IN ('function','method')
      ...
      AND m.name LIKE :mod
    GROUP BY m.id
    HAVING (unwraps + expects + panic_macros + index_ops) > 0
    ORDER BY panics_per_ksloc DESC, unwraps DESC LIMIT :lim"""),
("trait-coupling",
    "how much code is welded to each trait: implementors plus generic bounds, the change-blanket rustdoc cannot sum",
    "ANSWERS the blast radius of changing a trait: COUNT of impl blocks plus\n"
    ...
    """SELECT t.name AS trait_, t.n_required AS required_methods,
        t.is_public AS pub_,
        COUNT(DISTINCT i.id) AS impls,
        COUNT(DISTINCT gb.symbol_id) AS generic_users,
        SUM(CASE WHEN gb.in_where = 1 THEN 1 ELSE 0 END) AS where_clauses,
        GROUP_CONCAT(DISTINCT i.type_name) AS implementors,
        f.path || ':' || s.line_start AS at
    FROM traits t
    JOIN symbols s ON s.id=t.symbol_id
    JOIN files f ON f.id=t.file_id
    LEFT JOIN impls i ON i.trait_name = t.name
    LEFT JOIN generic_bounds gb ON gb.bound = t.name
    LEFT JOIN modules m ON m.id=s.module_id
    WHERE f.is_test=0 AND COALESCE(m.name,'') LIKE :mod
    GROUP BY t.symbol_id
    HAVING (impls + generic_users) > 0
    ORDER BY impls DESC, generic_users DESC LIMIT :lim"""),
("await-risk-density",
    "yield-point exposure per module: awaits, awaits in loops, guard-live sites, RefCell guards",
    "ANSWERS the module-level exposure map for the await_holding family:\n"
    ...
    """SELECT m.name AS module_,
        COUNT(DISTINCT ap.symbol_id) AS fns_awaiting,
        COUNT(ap.id) AS await_points,
        SUM(ap.in_loop) AS in_loop_points,
        SUM(ap.n_guards_live) AS guard_live_sites,
        SUM(ap.guard_dropped) AS explicit_drops,
        SUM(ap.has_refcell_guard) AS refcell_guard_sites,
        CAST(100.0 * COALESCE(SUM(ap.n_guards_live),0)
             / NULLIF(COUNT(ap.id),0) AS INT) AS pct_guarded
    FROM async_points ap
    JOIN files f ON f.id=ap.file_id
    JOIN modules m ON m.id=f.module_id
    WHERE f.is_test=0 AND m.name LIKE :mod
    GROUP BY m.id
    HAVING await_points > 0
    ORDER BY guard_live_sites DESC, in_loop_points DESC LIMIT :lim"""),
("error-propagation-style",
    "each module's error dialect: ? vs unwrap vs discard -- pct of fallible sites that propagate",
    "ANSWERS the error-handling style a module actually speaks, as a leading\n"
    ...
    """SELECT m.name AS module_,
        COUNT(DISTINCT CASE WHEN s.n_question_mark > 0 THEN s.id END)
            AS fns_propagating,
        SUM(s.n_question_mark) AS question_marks,
        SUM(s.n_unwrap + s.n_expect) AS unwraps_expects,
        SUM(s.n_error_swallow) AS swallows,
        SUM(s.n_safe_fallback) AS safe_fallbacks,
        CAST(100.0 * COALESCE(SUM(s.n_question_mark),0)
             / NULLIF(SUM(s.n_question_mark) + SUM(s.n_unwrap)
                      + SUM(s.n_expect) + SUM(s.n_error_swallow), 0) AS INT)
            AS pct_propagated
    FROM symbols s
    JOIN files f ON f.id=s.file_id
    JOIN modules m ON m.id=s.module_id
    WHERE s.kind IN ('function','method')
      ...
      AND m.name LIKE :mod
    GROUP BY m.id
    HAVING (question_marks + unwraps_expects + swallows) > 0
    ORDER BY pct_propagated ASC, unwraps_expects DESC LIMIT :lim"""),
("task-structure",
    "concurrency build-out per module: task spawns vs spawn_blocking vs raw threads vs joins vs channels",
    "ANSWERS which modules create concurrent work and which reclaim it: the\n"
    ...
    """SELECT m.name AS module_,
        SUM(s.n_spawn) AS spawns, SUM(s.n_spawn_blocking) AS spawn_blocking,
        SUM(s.n_thread_spawn) AS thread_spawns,
        SUM(s.n_join_calls) AS joins,
        SUM(s.n_channel_ops) AS channel_ops,
        COUNT(DISTINCT CASE WHEN s.n_spawn > 0 THEN s.id END)
            AS spawning_fns
    FROM symbols s
    JOIN files f ON f.id=s.file_id
    JOIN modules m ON m.id=s.module_id
    WHERE s.kind IN ('function','method')
      ...
      AND m.name LIKE :mod
    GROUP BY m.id
    HAVING (spawns + thread_spawns) > 0
    ORDER BY spawns DESC, thread_spawns DESC LIMIT :lim"""),
("macro-opaque-bytes",
    "how much of each module the parser counted but could not read: macro bytes vs module SLOC",
    "ANSWERS the honesty metric for every other number in this catalogue:\n"
    ...
    """SELECT m.name AS module_, m.sloc AS module_sloc,
        COUNT(DISTINCT mc.file_id) AS files_with_macros,
        COUNT(mc.id) FILTER (WHERE mc.kind='invocation') AS invocations,
        COUNT(mc.id) FILTER (WHERE mc.kind='definition') AS definitions,
        COALESCE(SUM(mc.body_bytes),0) AS hidden_bytes,
        CAST(1000.0 * COALESCE(SUM(mc.body_bytes),0)
             / NULLIF(m.sloc,0) AS INT) AS hidden_per_ksloc
    FROM modules m
    LEFT JOIN files f ON f.module_id=m.id
    LEFT JOIN macros mc ON mc.file_id=f.id
    WHERE m.name LIKE :mod
    GROUP BY m.id
    HAVING hidden_bytes > 0
    ORDER BY hidden_bytes DESC, invocations DESC LIMIT :lim"""),
("api-surface-bloat",
    "exposure per module: pub symbol share and pub functions nothing internal calls",
    "ANSWERS the surface area an attacker reads, a semver policy protects,\n"
    ...
    """SELECT m.name AS module_,
        COUNT(DISTINCT s.id) AS symbols_,
        SUM(CASE WHEN s.is_public=1 THEN 1 ELSE 0 END) AS pub_symbols,
        CAST(100.0 * SUM(CASE WHEN s.is_public=1 THEN 1 ELSE 0 END)
             / NULLIF(COUNT(DISTINCT s.id),0) AS INT) AS pct_public,
        COUNT(DISTINCT CASE WHEN s.is_public=1 AND s.fan_in=0
             AND s.kind IN ('function','method') THEN s.id END)
            AS pub_no_callers
    FROM symbols s
    JOIN files f ON f.id=s.file_id
    JOIN modules m ON m.id=s.module_id
    WHERE s.kind IN ('function','method','struct','enum','trait','impl')
      AND f.is_test=0 AND f.is_generated=0
      AND m.name LIKE :mod
    GROUP BY m.id
    HAVING pub_symbols > 0
    ORDER BY pub_no_callers DESC, pct_public DESC LIMIT :lim"""),
("external-coupling",
    "which external crates each module actually reaches -- the audit and upgrade surface",
    "ANSWERS dependency risk by module: distinct external crate heads among\n"
    ...
    "    types flow.",
    """SELECT m.name AS module_,
        COUNT(DISTINCT CASE WHEN i.is_external=1
            THEN substr(i.target, 1, instr(i.target || '::', '::') - 1) END)
            AS external_crates,
        SUM(CASE WHEN i.is_external=1 THEN 1 ELSE 0 END) AS external_uses,
        COUNT(DISTINCT i.target) AS distinct_paths,
        SUM(i.is_wildcard) AS wildcard_imports
    FROM imports i
    JOIN files f ON f.id=i.file_id
    JOIN modules m ON m.id=f.module_id
    WHERE m.name LIKE :mod
    GROUP BY m.id
    HAVING external_uses > 0
    ORDER BY external_crates DESC, external_uses DESC LIMIT :lim""")
]
