...
def discover(analyzer: Analyzer, root: str, db: sqlite3.Connection,
    ...
    # "some line exceeds the per-line cap" in one C-level search: a line of
    # MAX_LINE_BYTES+1 non-newline bytes exists iff the longest line is over
    # the cap (bytes.split(b"\n") lines, so \n is the only break).
    longline_re = re.compile(rb"[^\n]{%d,}" % (analyzer.MAX_LINE_BYTES + 1))
    mod_id: dict[str, int] = {}
    out: list[FileRec] = []
    ...
    for dirpath, dirnames, filenames in os.walk(root, onerror=_walk_error):
        ...
        for fn in sorted(filenames):
            ...
            # what is read, not what parsing it costs. One regex search
            # answers "is any line over the cap" without splitting the file
            # into a list of every line's bytes.
            if not too_big and data:
                if longline_re.search(data):
                    n_skipped_big += 1
                    too_big = True

            lines = text.splitlines()
            sloc = sum(map(bool, map(str.strip, lines)))
            blank = len(lines) - sloc
            cmt = sum(map(_is_comment_line, lines))
            ...
            file_rows.append(
                (fid, rel, os.path.dirname(rel) or ".", fn, ext, analyzer.LANG,
                 mid, st.st_size, len(lines), sloc,
                 blank, cmt, max(map(len, lines), default=0),
                 hashlib.sha1(data).hexdigest() if data else "",
                 int(parse), int(test), int(gen), int(vend)))
            analyzer.file_id[rel] = fid
            if parse:
                # text/data are NOT kept here. The parse loop re-reads each
                # file when it is that file's turn: discover used to hold
                # every file's bytes AND decoded text for the whole walk,
                # which set peak RSS at "entire source tree, twice" before
                # the graph had stored a single row. `abspath` is on the rec.
                out.append(FileRec(fid, mid, rel, full, "", b"",
...
#: discover's comment-line test, hoisted so the per-file loop can run it
#: through map() at C dispatch speed (a genexp re-evaluates the slice and
#: the membership tuple per line in bytecode).
_CMT_PREFIXES = ("//", "#", "/*", "*", "*/", '"""', "'''", "--", ";;", "%")

def _is_comment_line(line: str) -> bool:
    return line.lstrip()[:3] in _CMT_PREFIXES and bool(line.strip())

def scan_markers(rec: FileRec, bufs: Buffers) -> None:
    """TODO/FIXME/HACK and friends, with their line and text.

    (A whole-text finditer with a splitlines-compatible line index was tried
    and REVERTED: marker matches are rare -- 361 in the whole profiling
    corpus -- so the per-line `search` the original does scans the same bytes
    without paying to build a per-file break index first.)
    """
...
def build(analyzer: Analyzer, root: str, db: sqlite3.Connection, *,
    ...
    for i, rec in enumerate(recs):
        try:
            # Load this file's bytes now (see the note in discover): the
            # decode is the same `utf-8/replace` the walk used, so the text
            # the parser sees is byte-for-byte what it always saw.
            with open(rec.abspath, "rb") as fh:
                rec.data = fh.read()
            rec.text = rec.data.decode("utf-8", "replace")
...
#: Precompiled for the hot per-function/per-field paths below: every
#: re.search(pattern_string, ...) call pays the re-cache dict lookup, and
#: these run once per function or per struct field.
_WS_RUN = re.compile(r'\s+')
_HAS_STATIC = re.compile(r'\bstatic\b')
_HAS_INLINE = re.compile(r'\binline\b')
_FNPTR_DECL = re.compile(r'\(\s*\*\s*(\w+)\s*\)')
_TAG_CLOSE_NAME = re.compile(r'}\s*(\w+)\s*;')
_SIZEOF_ARG = re.compile(r'sizeof\s*\(\s*(.*?)\s*\)')
_TYPED_SIZEOF = re.compile(
    r'(?:struct|union|enum)\s+[A-Za-z_]\w*'
    r'|u?int(?:8|16|32|64)_t|char|int|long|short|float'
    r'|double|unsigned\s+\w+|size_t')
_TRAILING_IDENT = re.compile(r'([A-Za-z_]\w*)\s*$')
_LEADING_DIGIT = re.compile(r'^\d')
_PATH_TAIL_SPLIT = re.compile(r'[.>\[\]]')
_FOR_INT_DECL = re.compile(
    r'\b((?:long\s+|short\s+)*int)\s+([A-Za-z_]\w*)\s*=')
_ARR_STRIP = re.compile(r'[\[\]]')
_UNION_HEAD = re.compile(r'\bunion\b[^;{}]*$')
...
#: The "absent metric" default for _insert_symbol's C-level column fill.
_REPEAT_ZERO = _repeat(0)
...
_DEREF_AFTER_CACHE: dict[str, "re.Pattern[str]"] = {}

def _deref_after(freed: str) -> "re.Pattern[str]":
    rx = _DEREF_AFTER_CACHE.get(freed)
    if rx is None:
        rx = _DEREF_AFTER_CACHE[freed] = re.compile(
...
#: matches its quote char); the literal body is consumed by a second regex
#: with the exact escape rules the original per-character loop used, so edge
#: cases keep their meaning: a newline inside an unterminated string ends it
#: WITHOUT being blanked (it stays a real newline), an escaped quote never
#: closes, and `\<newline>` continues the literal across the line (the
#: newline is blanked, exactly as `j += 2` did).
_BLANK_TOKEN = re.compile(r'/\*|//|"|\'')

#: A CLOSED literal: the closing quote is required, so a match failure means
#: unterminated, and the body regex below finds where it stops. DOTALL makes
#: `\\.` span an escaped newline, matching the old `j += 2`.
_BLANK_DQ = re.compile(r'"(?:[^"\\\n]|\\.)*"', re.DOTALL)
_BLANK_SQ = re.compile(r"'(?:[^'\\\n]|\\.)*'", re.DOTALL)
_BLANK_DQ_BODY = re.compile(r'"(?:[^"\\\n]|\\.)*', re.DOTALL)
_BLANK_SQ_BODY = re.compile(r"'(?:[^'\\\n]|\\.)*", re.DOTALL)
...
def blank_c(src: str) -> str:
    ...
    for m in _BLANK_TOKEN.finditer(src):
        ...
        if tok == "/*":
            ...
            closed, body = ((_BLANK_DQ, _BLANK_DQ_BODY) if tok == '"'
                            else (_BLANK_SQ, _BLANK_SQ_BODY))
            cm = closed.match(src, start)
            if cm is not None:
                end = cm.end()
            else:
                # Unterminated: stop at the first unescaped newline (which
                # survives as a real newline) or at end of input. A literal
                # ending in a bare backslash at its last byte consumed one
                # past the end in the old loop (`j += 2` past the final
                # index), which blanked everything to end-of-input either way.
                end = body.match(src, start).end()
                if end < n and src[end] == '\\':
                    end = n
        out.append(src[prev:start])
        # Blank the span: every non-newline becomes a space. Splitting on
        # newlines and joining space-runs keeps the work at C level (the
        # first version ran a Python generator per CHARACTER; a re.sub was
        # measured slower still on the short spans real code produces).
        out.append("\n".join(" " * len(p)
                             for p in src[start:end].split("\n")))
        prev = end
        pos = end
    out.append(src[prev:])
    return ''.join(out)

#: Every byte except `{` and `}` -- the deletion table that reduces a whole
#: blanked file to its braces in one C pass (bytes.translate).
_NON_BRACE_BYTES = bytes(
    b for b in range(256) if b != 0x7B and b != 0x7D)
...
def scan_defects(raw: str, blank: str) -> tuple[int, int]:
    ...
    The brace walk used to be a Python loop over every character, clamping the
    depth at 0 and recording whether it ever dipped negative. That walk is
    reproducible without per-character Python: strip everything but the braces
    at C speed, then remove `{}` pairs until fixed point. The remainder is
    `}`a`{`b where a is exactly the walk's deepest overshoot and b its final
    clamped depth -- so "depth != 0 or ever negative" is precisely "the
    remainder is non-empty" (differential-tested against the loop, including
    on random brace soup).
    """
    errors = 0
    braces = blank.encode("utf-8", "replace").translate(
        None, _NON_BRACE_BYTES)
    while True:
        reduced = braces.replace(b"{}", b"")
        if len(reduced) == len(braces):
            break
        braces = reduced
    if braces:
        errors += 1
            ...
            if name not in KEYWORDS:
                ...
                if k < n and blank[k] == "{":
                    ...
                    while e < n:
                        ...
                        e += 1
                    ss = raw.rfind('\n', 0, i) + 1
                    sig = _WS_RUN.sub(' ', raw[ss:k].strip())[:400]
                    res.append((name, sig, line_of(blank, i), line_of(blank, e),
                                1 if _HAS_STATIC.search(sig) else 0,
                                1 if _HAS_INLINE.search(sig) else 0,
                                blank[k:e + 1], k))
                    i = e + 1
                    continue
        i = mm.end()
...
def expr_counts(body: str) -> dict[str, int]:
    ...
    (A fused single-alternation pass over the eight context classes was
    tried and REVERTED: it measured 1.63s -> 2.57s on the profiling corpus,
    because these patterns match so often that a Python tally per match
    costs more than the C scans it replaces.)
    """
    arrow = body.count("->")
    out = dict(
        ...
        n_addrof=len(_EC_ADDROF.findall(body)) if "&" in body else 0,
...
#: Division, as loop_analysis counts it: a `/` with a following character
#: that is neither `/` nor `=` (comments are already blanked; the trailing
#: byte of the body has no `body[i+1]` and never counted).
_DIV_IN_LOOP = re.compile(r'/(?![/=]).', re.DOTALL)

#: Braces only, for loop_analysis's depth walk.
_LOOP_BRACE = re.compile(r'[{}]')
...
def loop_analysis(body: str) -> dict[str, int]:
    ...
    The first version walked every character of the body in Python. Braces
    come from one finditer (C speed between hits); the content of each
    OUTERMOST loop body is exactly the region where loop depth > 0, so the
    per-character in-loop test collapses into per-REGION scans of that
    content: one IDENT pass per region for the token counts, one regex for
    the divisions.
    """
    max_loop_depth = switch_in_loop = alloc_in_loop = call_in_loop = 0
    libm_in_loop = div_in_loop = strlen_in_loop = branch_in_loop = 0
    io_in_loop = lock_in_loop = 0
    stack: list[bool] = []
    depth = 0
    seg_start = -1                    # open outermost loop region, if any
    segs: list[tuple[int, int]] = []
    for m in _LOOP_BRACE.finditer(body):
        i = m.start()
        if body[i] == "{":
            is_loop = opener_kind(body, i) in ("for", "while", "do")
            ...
            if stack.pop():
                if depth == 1 and seg_start >= 0:
                    segs.append((seg_start, i))
                    seg_start = -1
                if depth:
                    depth -= 1
    if seg_start >= 0:                # unclosed loop at EOF: still in-loop
        segs.append((seg_start, len(body)))
    n = len(body)
    for a, b in segs:
        div_in_loop += len(_DIV_IN_LOOP.findall(
            body[a:b + 1] if b < n else body[a:b]))
        for im in IDENT.finditer(body, a, b):
            tok = im.group(0)
            if tok == "switch":
                switch_in_loop += 1
            elif tok == "if" or tok == "case":
                branch_in_loop += 1
            e = im.end()
            while e < n and body[e] in " \t\r\n":
                e += 1
            if e < n and body[e] == "(" and tok not in KEYWORDS:
                call_in_loop += 1
                ...
                if cat == "io":
                    ...
                    lock_in_loop += 1
    return dict(max_loop_depth=max_loop_depth, switch_in_loop=switch_in_loop,
                alloc_in_loop=alloc_in_loop, call_in_loop=call_in_loop,
                libm_in_loop=libm_in_loop, div_in_loop=div_in_loop,
                strlen_in_loop=strlen_in_loop, branch_in_loop=branch_in_loop,
                io_in_loop=io_in_loop, lock_in_loop=lock_in_loop)

#: One pass feeds both max_nesting and cognitive in metrics().
_METRICS_BRACE = re.compile(r'[{}]|\b(?:if|for|while|switch|catch)\b')

#: METRIC_KW and _METRICS_BRACE fused: one finditer computes cyclomatic's
#: keyword tallies AND the brace depth AND cognitive (which needs the depth
#: at each decision keyword) in a single left-to-right scan. Identical
#: numbers: cognitive reads the same depth evolution at the same token
#: positions, just without a second pass over every body byte.
_METRICS_ALL = re.compile(
    r'[{}]|\b(?:for|while|do|if|switch|case|goto|return|catch)\b|&&|\|\||\?')
...
def metrics(body: str) -> dict[str, int]:
    ...
    combined finditer pass produces both numbers. The keyword tallies were
    a third pass; they are folded into the same scan.
    """
    nl = nb = ns = ng = nr = nc = nlog = 0
    mx = cog = 0
    d = 0
    for m in _METRICS_ALL.finditer(body):
        t = m.group(0)
        if t == "{":
            d += 1
            if d > mx:
            ...
            if t == "if" or t == "for" or t == "while" or t == "switch" \
                    or t == "catch":
                cog += d if d > 1 else 1
            if t == "if" or t == "for" or t == "while" or t == "case" \
                    or t == "catch":
                nc += 1
            if t == "for" or t == "while" or t == "do":
                nl += 1
    ...
    return dict(
        cyclomatic=1 + nc + nlog, cognitive=cog, max_nesting=mx,
        sloc=sum(map(bool, map(str.strip, body.splitlines()))),
        n_loops=nl,
        n_branches=nb,
        n_switch=ns,
        n_returns=nr,
...
def struct_fields(blank: str, raw: str, open_brace: int) -> tuple[list[tuple[Any, ...]], int]:
    ...
    def emit(stmt: str, off: int) -> None:
        ...
        if "(" in stmt:                       # function-pointer member
            m = _FNPTR_DECL.search(stmt)
            if m:
                out.append((ordinal, _WS_RUN.sub(' ', stmt.strip(" ;\n"))[:160],
        ...
        for decl in stmt.rstrip(";").split(","):
            ...
            if arr:
                a = _ARR_STRIP.sub('', arr).strip()
                alen = int(a) if a.isdigit() else -1
            out.append((ordinal, _WS_RUN.sub(' ', ftype)[:160], fname[:80],
    ...
    while i < len(body):
        ...
        if ch == "{":
            ...
            stack.append("union" if _UNION_HEAD.search(back) else "struct")
            start = i + 1
        elif ch == "}":
            if stack:
                stack.pop()
...
"""
    ...
    def parse_file(self, rec: FileRec, db: sqlite3.Connection,
        ...
        # One splitlines per FILE: _tags used to re-split the whole text for
        # every aggregate body just to read the comment block above it -- a
        # struct-dense header paid a full split per tag.
        raw_lines = raw.splitlines()
        self._tags(rec, raw, blank, db, bufs, tag_spans, raw_lines)
    ...
    def _tags(self, rec: FileRec, raw: str, blank: str,
        ...
        for m in TAG_BODY.finditer(blank):
            ...
            if not name:
                close = blank.find("}", ob)
                mm = _TAG_CLOSE_NAME.search(raw[close:close + 120])
                name = mm.group(1) if mm else "(anon@%d)" % line_of(raw, m.start())
            ln = line_of(raw, m.start())
            tag_spans.append((ln, endline))
            doc, ndoc = has_leading_comment(raw_lines, ln)
            sid = self._new_symbol(
                db,
                ["file_id", "module_id", "name", "qual_name", "kind",
    ...
    def _prototypes(self, rec: FileRec, blank: str, in_function,
        ...
        for m in PROTO_RE.finditer(blank):
            ...
            if nm not in KEYWORDS:
                ...
                bufs.rows("declarations").append((rec.fid, nm, ln))
        ...
        for am in FILE_SCOPE_REF_RE.finditer(blank):
            ln = line_of(blank, am.start())
            if in_function(ln) or ln in directive_lines:
                continue
            nm = am.group(2)
            if nm in KEYWORDS or nm in TYPE_WORDS:
                continue
            kind = "addr" if am.group(1) else "ref"
            bufs.rows("addr_taken").append((None, rec.fid, nm, ln, kind))
    ...
    def _globals(self, rec: FileRec, blank: str, db: sqlite3.Connection,
        ...
        for m in GLOBAL_RE.finditer(blank):
            ...
            rows.append((
                rec.fid, rec.mid, gname, _WS_RUN.sub(' ', ctype).strip()[:120],
                ln,
    ...
    def _functions(self, rec: FileRec, raw: str, blank: str,
        ...
        if raw_lines is None:
            raw_lines = raw.splitlines()
        # The file's file-scope objects are fixed for the whole file; the
        # per-function global read/write pass tests against this one set.
        gnames = {gn for gn, _ in globals_} if globals_ else frozenset()
        for name, sig, ls, le, st, inl, body, boff in funcs:
            ...
            mags = [m for m in NUM_RE.finditer(body)
                    if not _is_ok_magic_cached(m.group(1))]
            ...
            # (A body without the callee names cannot match either regex; the
            # substring tests are the same literals the patterns spell.)
            chk_vars = {m.group(1) for m in TOCTOU_CHECK_RE.finditer(body)} \
                if ("stat" in body or "access" in body) else set()
            opn_vars = {m.group(1) for m in TOCTOU_OPEN_RE.finditer(body)} \
                if ("open" in body or "creat" in body) else set()
            ...
            # Both regexes spell their casings literally ([Ll]ock...), so a
            # body without "lock", "Lock" or "sem_wait" cannot match either.
            if "lock" in body or "Lock" in body or "sem_wait" in body:
                ...
                for lm in LOCK_ARG_RE.finditer(body):
                    nm = lm.group(1)
                    if LOCK_NAME_RE.search(nm) and \
                            nm.lower() not in LOCK_GENERIC:
                        lock_rows.append((nm, blines.of(lm.start())))
            else:
                n_lock_acquire = n_lock_release = 0
                lock_rows = []
            # Narrow casts: (uint32_t)x and friends -- silent truncation.
            n_narrow_cast = len(NARROW_CAST_RE.findall(body)) \
                if ("8_t" in body or "16_t" in body or "32_t" in body) else 0
            ...
            for_decl = set()
            for m4 in _FOR_INT_DECL.finditer(body):
            ...
            n_variadic_fmt = (
                ...
                if any(p[6] for p in ps)
                and ("printf" in body or "syslog" in body) else 0)
            ...
            memop_rows = []
            if ("mem" in body or "str" in body or "printf" in body):
                for m3 in MEMOP_ARG_RE.finditer(body):
                    dst = m3.group(2).strip()
                    sarg = (m3.group(4) or "").strip()
                    buf = ""
                    sm = _SIZEOF_ARG.search(sarg)
                    if sm:
                        operand = sm.group(1)
                        tm = _TYPED_SIZEOF.match(operand)
                        if not tm:
                            nm = _TRAILING_IDENT.search(operand)
                            if nm:
                                buf = nm.group(1)
                    else:
                        idm = IDENT.search(sarg)
                        if idm and not _LEADING_DIGIT.match(sarg):
                            buf = idm.group(0)
                    dtail = _PATH_TAIL_SPLIT.split(dst)[-1].strip() or dst
                    memop_rows.append(
                        (m3.group(1), dst[:120],
                         m3.group(3).strip()[:120], sarg[:120], buf,
                         dtail[:80], blines.of(m3.start())))
            ...
            if ("epoll" in body or "uring" in body or "kqueue" in body
                    or "kevent" in body or "EV_SET" in body):
                for em in EVENTOP_RE.finditer(body):
                    ...
                    event_rows.append(
                        (em.group(1),
                         _WS_RUN.sub(' ', body[em.end():j - 1]).strip()[:180],
                         blines.of(em.start())))
    ...
    def _scan_calls(self, body: str,
        ...
        (A `ident[ \\t\\r\\n]*\\(`-shaped regex that visits only called
        identifiers was tried and REVERTED: its optional-ws tail makes the
        engine retry and backtrack at every position inside every word, and
        that measured slower than this plain IDENT walk despite visiting far
        fewer tokens.)
        """
    ...
    def _hazards(self, sid: int, body: str,
        ...
        # Literal-spelled patterns skip bodies that cannot match: the `in`
        # test is a C-level scan an order of magnitude cheaper than the
        # regex, and most bodies lack `<<`/`>>`/`sizeof` outright.
        for pat, cat, rx in HAZARD_RE:
            if pat == "shift":
                if "<<" not in body and ">>" not in body:
                    continue
            elif pat == "mul_sizeof":
                if "sizeof" not in body:
                    continue
    ...
    def _insert_symbol(self, db: sqlite3.Connection, rec: FileRec, name: str,
        ...
        # map(m.get, cols, repeat(0)) == [int(g(c, 0)) for c in cols]: every
        # value stored in `m` is already an int, and absent metrics take the
        # 0 default -- but the loop runs at C level (~150 columns/function).
        vals += map(g, metric_cols, _REPEAT_ZERO)
...
def _is_ok_magic(tok: str) -> bool:
    ...
    try:
        return int(tok.rstrip("uUlL"), 0) in MAGIC_OK
    except ValueError:
        return False

#: Same integers, memoized: `0`, `1`, `8`, `0x0` and friends repeat across
#: hundreds of thousands of literal sites, and `int(x, 0)` per site is the
#: only reason this function showed in the profile at all.
_MAGIC_OK_CACHE: dict[str, bool] = {}

def _is_ok_magic_cached(tok: str) -> bool:
    r = _MAGIC_OK_CACHE.get(tok)
    if r is None:
        r = _is_ok_magic(tok)
        _MAGIC_OK_CACHE[tok] = r
    return r
...
CAnalyzer.QUERIES = QUERIES
CAnalyzer.METRICS = METRICS
