...
#: The same word list as MARKER_RE without the `[:\-(]` tail: one search over
#: the whole FILE decides whether any line can match, so a file with no
#: marker word anywhere skips the per-line scan. Conservative by
#: construction -- it can only let files through, never hold a marker back.
MARKER_ANY_RE = re.compile(
    r'\b(TODO|FIXME|XXX|HACK|BUG|NOTE|WARNING|OPTIMIZE|REVIEW|DEPRECATED|'
    r'SAFETY|PANIC|UNSAFE)\b', re.I)
    ...
    for dirpath, dirnames, filenames in os.walk(root, onerror=_walk_error):
        ...
        for fn in sorted(filenames):
            ...
            try:
                ...
                # gets pasted into reports. One realpath per file, not two.
            ...
            # One pass instead of four: blank/sloc/comment/max-length were
            # four separate generator sweeps over every line of every file.
            blank = sloc = cmt = 0
            maxlen = 0
            for l in lines:
                n = len(l)
...
def scan_markers(rec: FileRec, bufs: Buffers) -> None:
    ...
    text = rec.text
    if MARKER_ANY_RE.search(text) is None:
        return
...
class TreeSitterAnalyzer(Analyzer):
    ...
    IF_NODES: tuple[str, ...] = ()
    #: node types that may count as loops even though they are not loop
    #: keywords; `is_extra_loop` decides per node during the measure walk.
    EXTRA_LOOP_NODES: tuple[str, ...] = ()
    ...
    def __init__(self) -> None:
        ...
        # Membership sets for the measure() walk are per-class constants.
        # Building ten sets per measured body put ~13k set constructions on
        # the sinatra profile for content that never changes; frozensets are
        # built once here and shared by every walk.
        self._fs_loops = frozenset(self.LOOP_NODES)
        self._fs_branches = frozenset(self.BRANCH_NODES)
        self._fs_nests = frozenset(self.NEST_NODES)
        self._fs_calls = frozenset(self.CALL_NODES)
        self._fs_operators = frozenset(self.OPERATOR_NODES)
        self._fs_strings = frozenset(self.STRING_NODES)
        self._fs_numbers = frozenset(self.NUMBER_NODES)
        self._fs_comments = frozenset(self.COMMENT_NODES)
        self._fs_ifs = frozenset(self.IF_NODES)
        self._fs_xloops = frozenset(self.EXTRA_LOOP_NODES)
        self._prune_types = (frozenset(self.FUNC_KINDS)
    ...
    def walk_scope(self, node: Any, rec: FileRec, db: sqlite3.Connection,
        ...
        # Hot-loop bindings: attribute lookups hoisted out of the walk.
        func_get = self.FUNC_KINDS.get
        type_get = self.TYPE_KINDS.get
        body_field = self.BODY_FIELD
        node_name = self.node_name
        emit_function = self.emit_function
        emit_type = self.emit_type
        while stack:
            cur, sc = stack.pop()
            kind = func_get(cur.type)
            if kind:
                sid = emit_function(cur, rec, db, bufs, sc, kind)
            ...
            if kind:
                sid = emit_type(cur, rec, db, bufs, sc, kind)
    ...
    def insert_symbol(self, db: sqlite3.Connection, rec: FileRec, name: str,
        ...
        if spec is None:
            ...
            # Indices into the FULL row: [sid] + the 14 fixed vals sit ahead
            # of the spec columns.
            self._sym_idx = {n: i + 1 + len(cols)
        ...
        self._n_sym += 1
        sid = self._n_sym
        # Start from the default row (one list copy) and apply only the keys
        # the caller actually produced -- O(len(m)), not O(width of the
        # table). Same values in the same columns: an absent key keeps the
        # DDL default exactly as m.get(k, dflt) did, and keys with no column
    ...
    def measure(self, body: Any, rec: FileRec,
        ...
        st = BodyStats()
        src = rec.data
        # Per-class constant membership sets, built once in __init__.
        loops = self._fs_loops
        branches = self._fs_branches
        nests = self._fs_nests
        calls = self._fs_calls
        counters = self.COUNTERS
        operators = self._fs_operators
        strings = self._fs_strings
        numbers = self._fs_numbers
        comments = self._fs_comments
        ifs = self._fs_ifs
        flags = self.FLAG_NODES

        else_field = self.ELSE_FIELD
        want_elif = bool(ifs) and bool(else_field)
        # Iterators that hide behind non-loop syntax (Ruby's `each {}`) are
        # decided AT the node: the predicate looks only at the node's parent,
        # so it needs no up-front traversal of the whole body to collect ids.
        # That second full walk per measured body is gone.
        xloop_types = self._fs_xloops
        is_xloop = self.is_extra_loop
        cursor = body.walk()
        depth = 0
        loop_depth = 0
        ...
        nest_stack: list[int] = []
        loop_stack: list[int] = []
        # Hot-loop bindings: one attribute lookup per walk instead of one per
        # node visited.
        ...
        while True:
            ...
            # so requiring a named node is free everywhere else. `is_named`
            # is read only when the type is one that needs it -- most nodes
            # are none of loop/branch/nest.
            ...
            if t in nests or t in loops or t in branches or t in xloop_types:
                named = node.is_named
            else:
                named = False
            ...
            if named and (t in loops or (t in xloop_types
                                         and is_xloop(node, src))):
    ...
    def is_extra_loop(self, node: Any, src: bytes) -> bool:
        """True if `node` (whose type is in EXTRA_LOOP_NODES) iterates.

        Called from `measure` at the node itself, so a language decides from
        the node's own shape -- usually the call it is attached to -- with no
        second traversal of the body. Default: there are no extra loops.
        """
        return False
...
#: Job enqueue family (Sidekiq, ActiveJob, ActionMailer, Resque). One bump per
#: call site; the `_in_loop` twin is bumped only inside an iteration, which is
#: the bulk-enqueue anti-pattern.
ENQUEUE_APIS = frozenset("""
perform_async perform_later perform_in perform_at perform_all_at
enqueue enqueue_job deliver_later deliver_later!
""".split())

#: Mutating calls whose receiver is a CONSTANT: `CONST << x`. The constant is
#: process-global shared mutable state whether or not it was frozen. Only
#: unambiguous container mutators -- `update`/`delete`/`store` are class
#: methods (`User.update(id, ...)`) far more often than Hash mutation.
CONST_MUTATE_APIS = frozenset("""
<< push append concat unshift shift pop delete_if clear
merge! replace insert
""".split())
...
class RubyAnalyzer(TreeSitterAnalyzer):
    ...
    # Ruby's real loops are blocks: `xs.each do ... end`. The type gate for
    # the per-node `is_extra_loop` predicate below.
    EXTRA_LOOP_NODES = ("do_block", "block")
    ...
    COUNTERS = {
        ...
        # a namespaced constant reference (Sinatra::Base) -- cross-namespace
        # coupling; bare `Array` is a plain constant and is NOT counted here.
        "scope_resolution": "n_const_ref",
    }
    LOOP_CALL_COUNTERS = {
    ...
    ("n_elif", "INT NOT NULL DEFAULT 0"),
        ...
        # -- B pack: coupling + sink surfaces the linters can only guess at --
        ("n_const_ref", "INT NOT NULL DEFAULT 0"),     # Foo::Bar references
        ("n_to_sym", "INT NOT NULL DEFAULT 0"),        # Symbol DoS sink
        ("n_regex_dyn", "INT NOT NULL DEFAULT 0"),     # Regexp.new / interp re
        ("n_enqueue", "INT NOT NULL DEFAULT 0"),       # job enqueue family
        ("n_enqueue_in_loop", "INT NOT NULL DEFAULT 0"),
        ("n_zoneless_time", "INT NOT NULL DEFAULT 0"), # Time.now / DateTime.now
        ("n_const_mutate", "INT NOT NULL DEFAULT 0"),  # CONST << x etc.
        ("n_thread_join", "INT NOT NULL DEFAULT 0"),   # argless join/value: Thread#join
        # -- file/class role -------------------------------------------------
...
CREATE TABLE ruby_modules(
    ...
    n_delegates INT NOT NULL DEFAULT 0,
...
CREATE TABLE blocks(
    ...
    line INT NOT NULL,
    -- return / break / throw statements inside the block body, attributed to
    -- EVERY enclosing block: a `return` inside `xs.each` inside `transaction`
    -- exits the whole method, so the transaction row must see it.
    n_exits INT NOT NULL DEFAULT 0
) STRICT;
...
-- B pack: block-level sinks (transaction exits, per-iteration locks) and
-- bulk enqueues. Partial on the exact predicates the new queries probe so
-- Rails-scale block tables do not get scanned per query.
CREATE INDEX idx_blk_exit ON blocks(method, symbol_id, line) WHERE n_exits > 0;
CREATE INDEX idx_blk_lock ON blocks(method, symbol_id, line) WHERE depth > 0;
CREATE INDEX idx_fn_enqueue ON symbols(name, file_id) WHERE n_enqueue_in_loop > 0;
"""
...
"""
    ...
    def parse_file(self, rec: FileRec, db: sqlite3.Connection,
        ...
        # The role regexes scan the whole file; each has a literal that MUST
        # be present for the regex to match, so a substring precheck keeps
        # the regex off files that cannot match. ("Record" covers
        # ApplicationRecord/ActiveRecord::Base; ActiveModel::Base is checked
        # by its own literal, and likewise InheritedResources::Base.)
        text = rec.text
        ctrl = ("app/controllers/" in rel
                or "Controller" in text
                or "InheritedResources::Base" in text) \
            and bool(CONTROLLER_RE.search(text))
        model = ("app/models/" in rel
                 or "Record" in text or "ActiveModel::Base" in text) \
            and bool(MODEL_RE.search(text))
        job = ("app/jobs/" in rel or "app/workers/" in rel
               or "Job" in text or "Sidekiq" in text or "Resque" in text) \
            and bool(JOB_RE.search(text))
        self._role = (int(ctrl), int(model), int(job))
        self._vis = []
    ...
    def type_flags(self, node: Any, rec: FileRec,
        ...
        # One decode for both class-text probes: text_of over a whole class
        # span is not free, and it was paid twice per class for the same
        # 2,000-character prefix.
        head2000 = text_of(node, rec.data)[:2000]
        out = {
            "is_public": 1,
            ...
            "is_concern": int("ActiveSupport::Concern" in head2000),
            "is_abstract": int("abstract_class" in head2000),
        }
        out.update(meta)
        return out
    ...
    def on_call(self, node: Any, src: bytes, st: BodyStats,
        ...
        # unauthenticated-input surface query exclude this method. An explicit
        # loop over the markers, not any(): no generator frame per call site.
        low = (full + " " + meth).lower()
        for _k in AUTH_MARKERS:
            if _k in low:
                st.bump("n_auth_call")
                break
        else:
        ...
        # -- B pack: sink/surface counters. Independent families, so plain
        # `if`s -- none of these interact with the elif chain above.
        if meth in ("to_sym", "to_proc"):
            # brakeman SymbolDoS: user input interned as a Symbol lives in
            # the process symbol table forever.
            st.bump("n_to_sym")
        if full in ("Regexp.new", "Regexp.compile"):
            # brakeman RegexDoS / CodeQL "Regular expression injection":
            # a regex built from a variable at run time.
            st.bump("n_regex_dyn")
        if meth in ENQUEUE_APIS:
            st.bump("n_enqueue")
        if full in ("Time.now", "DateTime.now"):
            # Rails/TimeZone surface: wall-clock time where zone matters.
            st.bump("n_zoneless_time")
        if recv is not None \
                and recv.type in ("constant", "scope_resolution") \
                and meth in CONST_MUTATE_APIS:
            # `CONST << x`: mutating a shared constant. The constant's OWN
            # freeze call would be in its defining file, not here.
            st.bump("n_const_mutate")
        if meth in ("join", "value", "value!"):
            _args = node.child_by_field_name("arguments")
            if _args is None or not _args.named_children:
                # Argumentless join/value: Thread#join, not Array#join
                # (which almost always passes a separator). Heuristic --
                # the thread-new-without-join query states it.
                st.bump("n_thread_join")
    ...
    def on_node(self, node: Any, src: bytes, st: BodyStats,
                loop_depth: int, nest: int) -> None:
        t = node.type
        # Dispatch ordered by observed frequency: identifiers dominate Ruby
        # bodies, and every branch below is an exclusive equality test, so
        # order cannot change what is counted.
        if t == "identifier":
            ...
            # `CONST << x` is a BINARY node here, not a call -- the most
            # common constant-mutation spelling would be invisible from the
            # call path alone.
            _left = node.child_by_field_name("left")
            _op = node.child_by_field_name("operator")
            if _left is not None and _op is not None \
                    and _left.type in ("constant", "scope_resolution") \
                    and text_of(_op, src).strip() == "<<":
                st.bump("n_const_mutate")
        elif t == "regex":
            # An interpolated regex literal (/a#{x}c/) is rebuilt from parts
            # at run time -- the literal-literal shape of RegexDoS.
    ...
    def is_extra_loop(self, node: Any, src: bytes) -> bool:
        ...
        Decided at the block node during the single measure walk: the only
        fact that matters is the parent call's method name.
        """
        call = node.parent
        ...
        if meth is None:
            return False
        return text_of(meth, src).strip() in ITERATOR_METHODS
    ...
    def parse_imports(self, root: Any, rec: FileRec, bufs: Buffers) -> None:
        src = rec.data
        # A full-tree cursor walk per file is only worth it if an import can
        # exist. Every import call's method text is one of REQUIRE_KINDS,
        # spelled verbatim in the source, and three byte scans cover the set:
        # require_relative/require_dependency contain "require",
        # autoload/autoload_at contain "load". A file with none of the three
        # cannot yield an import row.
        if b"require" not in src and b"load" not in src and b"gem" not in src:
            return
    ...
    def _scan_body(self, node: Any, rec: FileRec,
        ...
        # Every block (iteration or not) gets a row in _pend_blocks; this
        # stack tracks which rows are still open, so a return/break/throw
        # inside the body can be attributed to each enclosing block. A
        # `return` inside `xs.each` inside a `transaction` exits the METHOD,
        # so the transaction row must see it even though it is not the
        # innermost block. One counter bump per exit site, no second walk.
        block_rows: list[tuple[int, int]] = []
        block_exits: dict[int, int] = {}
        max_block = 0
        # Hot-loop bindings and call-first dispatch: calls dominate Ruby
        # bodies, and every branch below is an exclusive test on t.
        tof = text_of
        pend_append = self._pend_blocks.append
        call_detail = self._call_detail
        while True:
            n = cursor.node
            t = n.type
            while iter_stack and iter_stack[-1] >= depth:
                iter_stack.pop()
            while block_rows and block_rows[-1][0] >= depth:
                block_rows.pop()
            ...
            if t == "call":
                ...
                if mn is not None:
                    ...
                    if mn.end_byte - mn.start_byte == 12:
                        self._pend_has_yield = True
                    if meth == "throw":
                        for _d, bi in block_rows:
                            block_exits[bi] = block_exits.get(bi, 0) + 1
                    call_detail(n, meth, src, rec, idepth, singleton,
                                bump)
            elif t == "do_block" or t == "block":
                parent = n.parent
                meth = ""
                recv = ""
                if parent is not None and parent.type == "call":
                    mn = parent.child_by_field_name("method")
                    meth = tof(mn, src) if mn is not None else ""
                ...
                block_rows.append((depth, len(self._pend_blocks) - 1))
            elif t == "return" or t == "break":
                # `throw` is a CALL in this grammar and is counted in the
                # call branch above; `next` is a legal loop exit, not counted.
                for _d, bi in block_rows:
                    block_exits[bi] = block_exits.get(bi, 0) + 1
            elif t == "yield":
                self._pend_has_yield = True
            ...
            while not cursor.goto_next_sibling():
                if not cursor.goto_parent():
                    # Drain the exit counters into the pending block rows
                    # (n_exits rides at the end of each row; absent means 0)
                    # before handing control back -- function_extra flushes
                    # these rows.
                    self._pend_blocks = [
                        row + (block_exits.get(i, 0),)
                        for i, row in enumerate(self._pend_blocks)]
                ...
                (rec.fid, model[:80], meth[:40], kind,
                 ...
                 int(args is not None and bool(args.named_children)
                     and args.named_children[0].type in ("string",
                                                         "heredoc_beginning")),
                 idepth, chain, line))
        # Bulk-enqueue shape: an enqueue call nested inside iteration blocks.
        # Rides on the SECOND walk's block-depth (idepth), which counts the
        # find_each/in_batches iterations that measure()'s loop_depth misses
        # -- those two iterator names are absent from ITERATOR_METHODS.
        if idepth and meth in ENQUEUE_APIS:
            bump("n_enqueue_in_loop")
    ...
    def type_extra(self, node: Any, rec: FileRec, db: sqlite3.Connection,
        ...
        bufs.rows("ruby_modules").append(
            (sid, rec.fid, name[:200], int(node.type == "module"),
             ...
             st["ivars"], st["cvars"], st["globals"], st["delegates"],
             int(is_core), node.start_point[0] + 1))
    ...
    def _class_body(self, owner: Any, body: Any, src: bytes, rec: FileRec,
        ...
        for n in body.named_children:
            ...
            elif meth in ("delegate", "delegate_missing_to"):
                # ActiveSupport delegation: `delegate :name, to: :profile`.
                # One count per call -- the to:/prefix options are not worth
                # parsing here; the count is the Law-of-Demeter surface.
                st["delegates"] += 1
    ...
    def flush_extra(self, db: sqlite3.Connection, bufs: Buffers) -> None:
        for tbl, sql in (
            ("ruby_modules",
             ...
             "n_class_vars,n_globals,n_delegates,reopens_core,line) "
             "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"),
            ("mixins",
            ...
            ("blocks",
             ...
             "captures_outer,line,n_exits) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)"),
            ("ar_queries",
...
(
    ...
    LIMIT :lim"""),
(
    "transaction-exit-statement",
    "return/break/throw inside a transaction or with_lock block (Rails/TransactionExitStatement)",
    "ANSWERS where a transaction ends by falling off the method instead of by\n"
    "     committing or raising. On Rails 7.1 a `return` inside the block can\n"
    "     leave the transaction OPEN (active_record."
    ...
    "     built-in names transaction and with_lock are matched.",
    """SELECT s.name AS in_method, COALESCE(m.name,'') AS module_,
        b.method AS block_api, b.n_exits AS exits, b.depth AS block_depth,
        b.n_queries AS queries_in_block, b.is_iteration AS in_iterator,
        b.body_sloc, s.cyclomatic AS cyclo, s.n_ar_write AS ar_writes,
        s.fan_in,
        f.path || ':' || b.line AS at
    FROM blocks b
    JOIN symbols s ON s.id = b.symbol_id
    JOIN files f ON f.id = b.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE b.method IN ('transaction', 'with_lock')
      AND b.n_exits > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY b.n_exits DESC, s.fan_in DESC, b.n_queries DESC
    LIMIT :lim"""),
(
    "validation-skip-reachable",
    "update_attribute / increment! / toggle! reachable from a params reader (Rails/SkipsModelValidations)",
    "ANSWERS writes that bypass validations AND callbacks, reached through the\n"
    ...
    reach(root, sym, depth) AS (
        SELECT root, sym, MIN(depth) FROM walk GROUP BY root, sym),
    skips AS MATERIALIZED (
        SELECT u.caller_id AS sym_id, u.n,
            CASE WHEN INSTR(u.name, '.') > 0
                 THEN SUBSTR(u.name, INSTR(u.name, '.') + 1)
                 ELSE u.name END AS base
        FROM unresolved_calls u)
    SELECT src.name AS reads_params, s.name AS skips_validations,
        GROUP_CONCAT(DISTINCT sk.base) AS skipped_apis,
        SUM(sk.n) AS sites, MIN(reach.depth) AS hops,
        s.is_model AS in_model, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM reach
    JOIN symbols src ON src.id = reach.root
    JOIN symbols s ON s.id = reach.sym
    JOIN skips sk ON sk.sym_id = s.id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE sk.base IN ('update_attribute', 'update_column', 'update_columns',
                      'increment!', 'decrement!', 'toggle!', 'update_counters',
                      'increment_counter', 'decrement_counter')
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY src.id, s.id
    ORDER BY sites DESC, hops ASC, s.fan_in DESC
    LIMIT :lim"""),
(
    "action-filter-unresolved",
    "before_action etc. naming a method this tree cannot find (Rails/LexicallyScopedActionFilter)",
    "ANSWERS action filters whose target method has NO definition in the same\n"
    ...
    "     block filters are excluded.",
    """SELECT c.host, c.hook, c.method AS filter_target,
        c.is_conditional AS conditional,
        (SELECT COUNT(*) FROM ar_callbacks c2
         WHERE c2.host = c.host AND c2.target_id IS NOT NULL)
            AS resolved_siblings,
        s.name AS declared_in, s.is_controller AS ctrl,
        f.path || ':' || c.line AS at
    FROM ar_callbacks c
    JOIN symbols s ON s.id = c.symbol_id
    JOIN files f ON f.id = c.file_id
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE c.method <> '' AND c.target_id IS NULL AND c.is_block = 0
      AND c.hook IN ('before_action', 'after_action', 'around_action',
                     'prepend_before_action', 'append_before_action',
                     'prepend_around_action', 'append_around_action',
                     'before_filter', 'after_filter', 'around_filter')
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY ctrl DESC, c.host, c.line
    LIMIT :lim"""),
(
    "abstract-method-unimplemented",
    "A module's abstract (NotImplementedError) method missing from an including host",
    "ANSWERS interface drift: a module declares a method abstract -- body is a\n"
    ...
    "    actually include.",
    """WITH abs_mods AS MATERIALIZED (
        SELECT ms.id AS mod_id, ms.name AS mod_name, a.name AS meth,
            a.sloc AS stub_sloc
        FROM symbols ms JOIN symbols a ON a.parent_id = ms.id
        WHERE ms.kind IN ('module', 'class') AND a.kind = 'method'
          AND a.is_abstract = 1)
    SELECT h.name AS host, am.mod_name AS mixed_in, am.meth AS unimplemented,
        x.kind AS mixin_kind, am.stub_sloc,
        (SELECT COUNT(*) FROM symbols own
         WHERE own.parent_id = h.id AND own.kind = 'method') AS own_methods,
        f.path || ':' || x.line AS at
    FROM mixins x
    JOIN symbols h ON h.id = x.host_id
    JOIN abs_mods am ON am.mod_name = x.mixin_short
    JOIN files f ON f.id = h.file_id
    LEFT JOIN modules m ON m.id = h.module_id
    WHERE f.is_test = 0 AND f.is_generated = 0
      ...
      AND NOT EXISTS (
          ...
          WHERE own.parent_id = h.id AND own.name = am.meth)
    ORDER BY am.stub_sloc DESC, x.line
    LIMIT :lim"""),
(
    "method-missing-protocol",
    "method_missing without super, and what does not implement respond_to_missing?",
    "ANSWERS the two halves of the method_missing contract, ranked together.\n"
    ...
    "    def only, so one inherited from an ancestor reads as missing.",
    """SELECT s.name AS missing_def, COALESCE(m.name,'') AS module_,
        s.n_super AS super_calls,
        (SELECT COUNT(*) FROM symbols sib
         WHERE sib.parent_id = s.parent_id AND sib.id <> s.id
           AND sib.name IN ('respond_to_missing?', 'respond_to?'))
            AS respond_to_defined,
        s.n_send AS send_calls, s.cyclomatic AS cyclo, s.sloc, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.name = 'method_missing' AND s.kind = 'method'
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY super_calls ASC, respond_to_defined ASC, s.fan_in DESC
    LIMIT :lim"""),
(
    "duplicate-method-in-class",
    "The same method defined twice in one class -- the second silently wins (Lint/DuplicateMethods)",
    "ANSWERS redefinitions that overwrite without warning: a class reopened\n"
    ...
    "    they produce no symbol.",
    """WITH dupes AS MATERIALIZED (
        SELECT d.parent_id, d.name, COUNT(*) AS n_defs,
            COUNT(DISTINCT d.file_id) AS n_files,
            MIN(d.line_start) AS first_line, MAX(d.line_start) AS last_line
        FROM symbols d
        WHERE d.kind = 'method' AND d.parent_id IS NOT NULL
          AND d.name NOT IN ('(anonymous)')
        GROUP BY d.parent_id, d.name
        HAVING COUNT(*) > 1)
    SELECT h.name AS owner, dupes.name AS method_, dupes.n_defs,
        dupes.n_files, dupes.first_line, dupes.last_line,
        o.sloc, o.fan_in,
        f.path || ':' || dupes.first_line AS at
    FROM dupes
    JOIN symbols h ON h.id = dupes.parent_id
    JOIN symbols o ON o.parent_id = dupes.parent_id
                  AND o.name = dupes.name
                  AND o.line_start = dupes.first_line
    JOIN files f ON f.id = h.file_id
    LEFT JOIN modules m ON m.id = h.module_id
    WHERE f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY dupes.n_defs DESC, o.fan_in DESC
    LIMIT :lim"""),
(
    "class-reopened-across-files",
    "One class declared in several files: the split nobody reviews",
    "ANSWERS classes and modules whose declarations are spread over more than\n"
    ...
    "    decorators do this deliberately and legitimately.",
    """WITH spread AS (
        SELECT r.name, COUNT(*) AS n_decls,
            COUNT(DISTINCT r.file_id) AS n_files,
            SUM(r.n_defs) AS total_defs, SUM(r.n_mixins) AS total_mixins,
            SUM(r.n_class_vars + r.n_class_ivars) AS class_state,
            SUM(r.n_delegates) AS total_delegates,
            MAX(r.reopens_core) AS touches_core,
            SUM(r.is_module) AS module_decls, MIN(r.line) AS line
        FROM ruby_modules r
        JOIN files rf ON rf.id = r.file_id
        WHERE rf.is_test = 0 AND rf.is_generated = 0
        GROUP BY r.name
        HAVING COUNT(DISTINCT r.file_id) > 1)
    SELECT sp.name, sp.n_decls, sp.n_files, sp.total_defs,
        sp.total_mixins, sp.class_state, sp.total_delegates,
        sp.touches_core, sp.module_decls,
        f.path || ':' || sp.line AS at
    FROM spread sp
    JOIN files f ON f.id = (
        SELECT r2.file_id FROM ruby_modules r2
        WHERE r2.name = sp.name ORDER BY r2.line LIMIT 1)
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE COALESCE(m.name,'') LIKE :mod
    ORDER BY sp.n_files DESC, sp.total_defs DESC
    LIMIT :lim"""),
(
    "inherit-exception-base",
    "Exception subclass inheriting Exception/Object -- invisible to bare rescue (Lint/InheritException)",
    "ANSWERS custom error classes whose parentage puts them OUTSIDE\n"
    ...
    "    plain classes that are never raised at all.",
    """SELECT r.name AS exception_class, r.superclass AS inherits_from,
        r.n_defs AS own_methods,
        (SELECT COUNT(*) FROM symbols m2
         WHERE m2.file_id = r.file_id AND m2.kind = 'method'
           AND m2.n_rescue_exception > 0) AS file_rescues_exception,
        (SELECT COUNT(*) FROM symbols m3
         WHERE m3.file_id = r.file_id AND m3.kind = 'method'
           AND m3.n_raise > 0) AS file_raises,
        f.path || ':' || r.line AS at
    FROM ruby_modules r
    JOIN files f ON f.id = r.file_id
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE r.is_module = 0
      AND r.superclass IN ('Exception', 'Object', 'BasicObject')
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY file_rescues_exception DESC, r.n_defs DESC
    LIMIT :lim"""),
(
    "n-plus-one-reachable",
    "A query-in-a-loop method reachable from a controller action (CodeQL: Database query in a loop)",
    "ANSWERS the cross-file N+1: the loop-and-query lives in a service or\n"
    ...
    "    counted from whichever action reaches it, not from its text.",
    """WITH RECURSIVE walk(root, sym, depth) AS (
        SELECT s.id, s.id, 0 FROM symbols s
        JOIN files f ON f.id = s.file_id
        WHERE s.is_controller = 1 AND s.is_public = 1
          AND s.kind = 'method' AND f.is_test = 0
        UNION
        SELECT w.root, e.callee_id, w.depth + 1
        FROM walk w JOIN edges e ON e.caller_id = w.sym
        WHERE w.depth < 4 AND e.is_self = 0),
    reach(root, sym, depth) AS (
        SELECT root, sym, MIN(depth) FROM walk GROUP BY root, sym)
    SELECT entry.name AS controller_action, s.name AS queries_in_loop,
        MIN(reach.depth) AS hops,
        MAX(s.n_ar_query_in_block) AS ar_in_loops,
        MAX(s.query_in_loop) AS raw_in_loops,
        MAX(s.n_ar_write_in_loop) AS writes_in_loops,
        MAX(s.max_block_depth) AS block_depth, MAX(s.fan_in) AS fan_in,
        f.path || ':' || MIN(s.line_start) AS at
    FROM reach
    JOIN symbols entry ON entry.id = reach.root
    JOIN symbols s ON s.id = reach.sym
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE reach.depth > 0
      AND (s.n_ar_query_in_block > 0 OR s.query_in_loop > 0)
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY entry.id, s.id
    ORDER BY ar_in_loops DESC, hops ASC, writes_in_loops DESC
    LIMIT :lim"""),
(
    "enqueue-in-loop",
    "Job enqueues inside an iteration -- the bulk-enqueue stampede (Sidekiq anti-pattern)",
    "ANSWERS loops that enqueue one background job per element: ten thousand\n"
    ...
    "    the receiver, so a local object's own `enqueue` method counts.",
    """SELECT s.name AS in_method, COALESCE(m.name,'') AS module_,
        s.n_enqueue_in_loop AS enqueues_in_loop, s.n_enqueue AS enqueues,
        s.n_iter_blocks AS iter_blocks, s.max_block_depth AS block_depth,
        s.is_job AS in_job, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_enqueue_in_loop > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_enqueue_in_loop DESC, s.n_enqueue DESC, s.fan_in DESC
    LIMIT :lim"""),
(
    "mutex-held-across-io",
    "A Mutex taken in a method that also does network/disk/DB work",
    "ANSWERS critical sections that span I/O: every other thread that wants\n"
    ...
    "    from Concurrent::Semaphore or the GVL are invisible.",
    """SELECT s.name, COALESCE(m.name,'') AS module_,
        s.n_mutex AS mutexes, s.n_io AS io_, s.n_net AS net_,
        s.n_sql + s.n_rails_query AS db_, s.n_ar_query AS ar_calls,
        s.n_sleep_call AS sleeps, s.n_timeout AS timeouts,
        s.lock_in_loop AS lock_in_loop_, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_mutex > 0
      AND (s.n_io + s.n_net + s.n_sql + s.n_rails_query + s.n_ar_query
           + s.n_sleep_call) > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY (s.n_io + s.n_net + s.n_sql + s.n_ar_query) DESC,
        s.fan_in DESC
    LIMIT :lim"""),
(
    "thread-new-without-join",
    "Thread.new with no join/value in the spawning method -- fire-and-forget failure modes",
    "ANSWERS where threads are spawned and never waited on IN THE SAME METHOD.\n"
    ...
    "    construction.",
    """SELECT s.name, COALESCE(m.name,'') AS module_,
        s.n_thread_new AS threads_spawned, s.n_ractor AS ractors,
        s.n_thread_join AS joins_seen,
        s.n_mutex AS mutexes, s.n_class_level_write AS class_writes,
        s.fan_out, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE (s.n_thread_new + s.n_ractor) > 0 AND s.n_thread_join = 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_thread_new DESC, s.fan_in DESC
    LIMIT :lim"""),
(
    "thread-current-reachable",
    "Thread.current usage reachable from something that runs on a request/worker thread",
    "ANSWERS where request-scoped state rides on Thread.current. Under Puma\n"
    ...
    "    not appear. Fibres change the scoping rules and are not modeled.",
    """WITH RECURSIVE down(root, sym, depth) AS (
        SELECT s.id, s.id, 0 FROM symbols s WHERE s.is_threaded_entry = 1
        UNION
        SELECT d.root, e.callee_id, d.depth + 1
        FROM down d JOIN edges e ON e.caller_id = d.sym
        WHERE d.depth < 4 AND e.is_self = 0)
    SELECT entry.name AS threaded_entry, s.name AS uses_thread_current,
        MIN(down.depth) AS hops,
        MAX(s.n_thread_local) AS thread_local_sites,
        MAX(entry.n_thread_new) AS spawns,
        MAX(entry.is_controller) AS ctrl, MAX(entry.is_job) AS job,
        f.path || ':' || MIN(s.line_start) AS at
    FROM down
    JOIN symbols entry ON entry.id = down.root
    JOIN symbols s ON s.id = down.sym
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_thread_local > 0 AND f.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY entry.id, s.id
    ORDER BY thread_local_sites DESC, hops ASC
    LIMIT :lim"""),
(
    "constant-mutation",
    "Methods that mutate a constant receiver (CONST << x) -- shared mutable state",
    "ANSWERS writes into process-global constants: `CACHE << item`,\n"
    ...
    "    freeze state are not checked -- this is the write side only.",
    """SELECT s.name, COALESCE(m.name,'') AS module_,
        s.n_const_mutate AS mutations, s.n_freeze AS freezes,
        s.n_const_get AS dynamic_gets, s.n_thread_new AS spawns,
        s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_const_mutate > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_const_mutate DESC, s.fan_in DESC
    LIMIT :lim"""),
(
    "symbol-dos-surface",
    "to_sym / to_proc in a method that reads request input (brakeman SymbolDoS)",
    "ANSWERS where request input can be interned: `params[:x].to_sym` puts an\n"
    ...
    "    gem or via `String#capitalize!`-style C paths is invisible.",
    """SELECT s.name, COALESCE(m.name,'') AS module_,
        s.n_to_sym AS to_sym, s.n_params_read AS params_reads,
        COUNT(DISTINCT u.id) AS input_sites,
        GROUP_CONCAT(DISTINCT u.kind) AS kinds,
        s.is_controller AS ctrl, s.fan_in,
        f.path || ':' || s.line_start AS at
    ...
    LEFT JOIN modules m ON m.id = s.module_id
    LEFT JOIN user_input_sites u ON u.symbol_id = s.id
    WHERE s.n_to_sym > 0 AND s.n_params_read > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id
    ORDER BY s.n_to_sym DESC, input_sites DESC
    LIMIT :lim"""),
(
    "regex-dos-surface",
    "A regex built at run time (Regexp.new / interpolated literal) next to request input (brakeman RegexDoS)",
    "ANSWERS the two ReDoS entry shapes: a regex COMPILED from a variable\n"
    ...
    "    harmless `#{col}` interpolation ranks like `(a+)+$`.",
    """SELECT s.name, COALESCE(m.name,'') AS module_,
        s.n_regex_dyn AS dynamic_regex, s.n_params_read AS params_reads,
        s.regex_in_loop AS regex_in_loop_, s.n_regex_lit AS regex_lits,
        COUNT(DISTINCT u.id) AS input_sites,
        s.is_controller AS ctrl, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    ...
    LEFT JOIN modules m ON m.id = s.module_id
    LEFT JOIN user_input_sites u ON u.symbol_id = s.id
    WHERE s.n_regex_dyn > 0 AND s.n_params_read > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY s.id
    ORDER BY s.n_regex_dyn DESC, input_sites DESC
    LIMIT :lim"""),
(
    "html-safe-frontier",
    "html_safe / raw reachable, through the call graph, from a method that reads request input",
    "ANSWERS the cross-function XSS frontier. html-safe-xss catches the mark\n"
    ...
    "    method_missing) produces no edge, so the frontier is a floor.",
    """WITH RECURSIVE walk(root, sym, depth) AS (
        SELECT s.id, s.id, 0 FROM symbols s
        WHERE s.n_params_read > 0 AND s.kind = 'method'
        UNION
    ...
    reach(root, sym, depth) AS (
        SELECT root, sym, MIN(depth) FROM walk GROUP BY root, sym)
    SELECT src.name AS reads_params, s.name AS marks_html_safe,
        MIN(reach.depth) AS hops, SUM(s.n_html_safe) AS html_safe_calls,
        s.is_controller AS ctrl, s.fan_in,
        f.path || ':' || MIN(s.line_start) AS at
    FROM reach
    JOIN symbols src ON src.id = reach.root
    ...
    WHERE s.n_html_safe > 0 AND f.is_test = 0
      ...
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY src.id, s.id
    ORDER BY hops ASC, html_safe_calls DESC, s.fan_in DESC
    LIMIT :lim"""),
(
    "file-open-frontier",
    "File.open/read with a variable path, reachable from a params reader (brakeman FileAccess)",
    "ANSWERS the path-traversal frontier across functions: same-function\n"
    ...
    "    the File. capture, so those sinks are missed entirely.",
    """WITH RECURSIVE walk(root, sym, depth) AS (
        SELECT s.id, s.id, 0 FROM symbols s
        WHERE s.n_params_read > 0 AND s.kind = 'method'
        UNION
    ...
    reach(root, sym, depth) AS (
        SELECT root, sym, MIN(depth) FROM walk GROUP BY root, sym)
    SELECT src.name AS reads_params, s.name AS opens_path,
        MIN(reach.depth) AS hops, SUM(s.n_dynamic_open) AS open_sites,
    ...
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_dynamic_open > 0 AND f.is_test = 0
      AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY src.id, s.id
    ORDER BY hops ASC, open_sites DESC
    LIMIT :lim"""),
(
    "sql-interp-reachable",
    "Interpolated-SQL query code reachable from a params reader (brakeman SQLInjection, graph frontier)",
    "ANSWERS the injection frontier: the string-interpolated where/heredoc\n"
    ...
    "    floor, not a proof of safety elsewhere.",
    """WITH RECURSIVE walk(root, sym, depth) AS (
        SELECT s.id, s.id, 0 FROM symbols s
        WHERE s.n_params_read > 0 AND s.kind = 'method'
        UNION
    ...
    reach(root, sym, depth) AS (
        SELECT root, sym, MIN(depth) FROM walk GROUP BY root, sym)
    SELECT src.name AS reads_params, s.name AS builds_sql,
        MIN(reach.depth) AS hops,
        MAX(s.n_sql_interp) AS interpolated_sql,
        MAX(s.n_sql_literal) AS sql_literals,
        MAX(s.is_controller) AS sink_is_ctrl, MAX(s.fan_in) AS fan_in,
        f.path || ':' || MIN(s.line_start) AS at
    FROM reach
    JOIN symbols src ON src.id = reach.root
    JOIN symbols s ON s.id = reach.sym
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE (s.n_sql_interp > 0
             OR EXISTS (SELECT 1 FROM ar_queries aq
                        WHERE aq.symbol_id = s.id AND aq.has_interpolation = 1))
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY src.id, s.id
    ORDER BY hops ASC, interpolated_sql DESC
    LIMIT :lim"""),
(
    "zoneless-time-surface",
    "Time.now / DateTime.now where a zoned time was meant (Rails/TimeZone)",
    "ANSWERS wall-clock reads -- Time.now, DateTime.now -- which ignore\n"
    ...
    "    only the literal Time./DateTime. receiver shapes match.",
    """SELECT s.name, COALESCE(m.name,'') AS module_,
        s.n_zoneless_time AS now_calls, s.is_controller AS ctrl,
        s.is_job AS in_job, s.is_model AS in_model, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.n_zoneless_time > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_zoneless_time DESC, s.fan_in DESC
    LIMIT :lim"""),
(
    "lock-in-loop",
    "transaction / with_lock / synchronize taken inside an iteration",
    "ANSWERS per-iteration locks and transactions: one BEGIN/COMMIT per row\n"
    ...
    "    excluded), not the loop trip count.",
    """SELECT s.name AS in_method, COALESCE(m.name,'') AS module_,
        b.method AS lock_api, b.depth AS block_depth,
        b.n_queries AS queries_in_block, b.n_exits AS exits,
        b.body_sloc, s.fan_in,
        f.path || ':' || b.line AS at
    FROM blocks b
    JOIN symbols s ON s.id = b.symbol_id
    JOIN files f ON f.id = b.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE b.depth > 0
      AND b.method IN ('transaction', 'with_lock', 'synchronize', 'lock')
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY b.depth DESC, b.n_queries DESC, s.fan_in DESC
    LIMIT :lim"""),
(
    "include-shadows-own-method",
    "An included module defines a method the host also defines -- the module silently wins",
    "ANSWERS MRO accidents: `include` puts the module ABOVE the class in the\n"
    ...
    "    directions.",
    """WITH mod_methods AS MATERIALIZED (
        SELECT ms.id AS mod_id, ms.name AS mod_name, md.name AS meth,
            md.sloc AS sloc_in_mixin
        FROM symbols ms JOIN symbols md ON md.parent_id = ms.id
        WHERE ms.kind = 'module' AND md.kind = 'method'),
    host_methods AS MATERIALIZED (
        SELECT hd.parent_id AS host_id, hd.name AS meth,
            hd.sloc AS sloc_in_host, hd.line_start
        FROM symbols hd
        WHERE hd.kind = 'method' AND hd.parent_id IS NOT NULL)
    SELECT h.name AS host_class, x.mixin_short AS mixin,
        mm.meth AS shadowed_method, mm.sloc_in_mixin, hm.sloc_in_host,
        x.kind AS mixin_kind, x.line AS include_line,
        f.path || ':' || x.line AS at
    FROM mixins x
    JOIN symbols h ON h.id = x.host_id
    JOIN mod_methods mm ON mm.mod_name = x.mixin_short
    JOIN host_methods hm ON hm.host_id = x.host_id AND hm.meth = mm.meth
    JOIN files f ON f.id = h.file_id
    LEFT JOIN modules m ON m.id = h.module_id
    WHERE f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY mm.sloc_in_mixin DESC, x.line
    LIMIT :lim"""),
(
    "nested-method-def",
    "A def inside a def (Lint/NestedMethodDefinition)",
    "ANSWERS methods defined inside methods: the inner def is CREATED at each\n"
    ...
    "    metaprogramming shape and appear here too.",
    """SELECT s.name AS inner_def, p.name AS outer_method,
        COALESCE(m.name,'') AS module_,
        s.sloc, s.cyclomatic AS cyclo, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN symbols p ON p.id = s.parent_id AND p.kind = 'method'
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.kind = 'method'
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.sloc DESC, s.fan_in DESC
    LIMIT :lim"""),
(
    "only-called-from-tests",
    "Production methods whose only callers are test files",
    "ANSWERS the code the test suite keeps alive: every caller is in a test\n"
    ...
    "    have no edges at all and never appear in either direction.",
    """WITH prod_calls AS MATERIALIZED (
        SELECT e.callee_id, COUNT(*) AS n
        FROM edges e
        JOIN symbols c ON c.id = e.caller_id
        JOIN files cf ON cf.id = c.file_id
        WHERE cf.is_test = 0 AND c.is_test = 0
        GROUP BY e.callee_id),
    test_calls AS MATERIALIZED (
        SELECT e.callee_id, COUNT(*) AS n
        FROM edges e
        JOIN symbols c ON c.id = e.caller_id
        JOIN files cf ON cf.id = c.file_id
        WHERE cf.is_test = 1 OR c.is_test = 1
        GROUP BY e.callee_id)
    SELECT s.name, COALESCE(m.name,'') AS module_,
        t.n AS test_callers, s.sloc, s.cyclomatic AS cyclo,
        s.fan_in AS static_fan_in,
        f.path || ':' || s.line_start AS at
    FROM test_calls t
    JOIN symbols s ON s.id = t.callee_id
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE NOT EXISTS (
              SELECT 1 FROM prod_calls p WHERE p.callee_id = s.id)
      AND s.kind IN ('method', 'function')
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY t.n DESC, s.sloc DESC
    LIMIT :lim"""),
(
    "eql-without-hash",
    "A class defines == but no hash/eql? -- silently broken as a Hash key",
    "ANSWERS the value-object contract gap: `==` defined, `hash` not. The\n"
    ...
    "    matching uses it, and a class that is never hashed still appears.",
    """WITH eq_defs AS (
        SELECT d.parent_id, COUNT(*) AS n_eq_defs, MIN(d.line_start) AS first_line
        FROM symbols d
        WHERE d.kind = 'method' AND d.name = '=='
          AND d.parent_id IS NOT NULL
        GROUP BY d.parent_id)
    SELECT h.name AS owner, COALESCE(m.name,'') AS module_,
        eq_defs.n_eq_defs, eq_defs.first_line,
        (SELECT COUNT(*) FROM symbols sib
         WHERE sib.parent_id = h.id
           AND sib.name IN ('hash', 'eql?', 'deconstruct_keys')) AS keyed_siblings,
        (SELECT COUNT(*) FROM symbols meths
         WHERE meths.parent_id = h.id AND meths.kind = 'method') AS own_methods,
        f.path || ':' || eq_defs.first_line AS at
    FROM eq_defs
    JOIN symbols h ON h.id = eq_defs.parent_id
    JOIN files f ON f.id = h.file_id
    LEFT JOIN modules m ON m.id = h.module_id
    WHERE (SELECT COUNT(*) FROM symbols sib
           WHERE sib.parent_id = h.id
             AND sib.name IN ('hash', 'eql?', 'deconstruct_keys')) = 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY eq_defs.n_eq_defs DESC, own_methods DESC
    LIMIT :lim"""),
(
    "unguarded-recursion",
    "A recursive method with no conditional in its body",
    "ANSWERS recursion with no visible base case: is_recursive comes from the\n"
    ...
    """SELECT s.name, COALESCE(m.name,'') AS module_,
        s.cyclomatic AS cyclo, s.n_branches AS branches,
        s.n_cases AS cases, s.n_calls AS calls_, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.is_recursive = 1 AND s.n_branches = 0 AND s.n_cases = 0
      AND s.kind IN ('method', 'function')
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.cyclomatic DESC, s.fan_in DESC
    LIMIT :lim"""),
(
    "regex-in-loop",
    "gsub / match / Regexp.new inside an iteration (regex recompilation and repeated backtracking)",
    "ANSWERS per-iteration regex work: gsub and match re-scan on every trip,\n"
    ...
    "    and a loop whose collection is empty at run time pays nothing.",
    """SELECT s.name, COALESCE(m.name,'') AS module_,
        s.regex_in_loop AS re_in_loop, s.n_regex_dyn AS dynamic_regex,
        s.n_regex_lit AS regex_lits, s.n_iter_blocks AS iter_blocks,
        s.max_block_depth AS block_depth,
        (s.regex_in_loop * (1 + s.max_block_depth)
         + s.n_regex_dyn * 2) AS cost,
        s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.regex_in_loop > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY cost DESC, s.fan_in DESC
    LIMIT :lim"""),
]
...
(
    "callback-load",
    "Lifecycle callbacks declared per class, and how many of them query",
    "ANSWERS the ActiveRecord callback load a model carries: every\n"
    ...
    "    macros are excluded (association-fanout ranks those).",
    """SELECT c.host, COUNT(*) AS callbacks,
        SUM(c.issues_query) AS query_callbacks,
        SUM(c.is_conditional) AS conditional_hooks,
        SUM(c.is_block) AS block_hooks,
        SUM(CASE WHEN c.target_id IS NULL AND c.method <> '' AND c.is_block = 0
                 THEN 1 ELSE 0 END) AS unresolved_targets,
        COUNT(DISTINCT c.hook) AS distinct_hooks,
        f.path || ':' || MIN(c.line) AS at
    FROM ar_callbacks c
    JOIN files f ON f.id = c.file_id
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    GROUP BY c.host
    ORDER BY callbacks DESC, query_callbacks DESC
    LIMIT :lim"""),
(
    "association-fanout",
    "Associations declared per model -- the join surface every query can touch",
    "ANSWERS how many belongs_to/has_many/has_and_belongs_to_many edges each\n"
    ...
    "    parsed, so a well-audited hub and a neglected one rank the same.",
    """SELECT c.host, COUNT(*) AS associations,
        SUM(c.is_conditional) AS conditional,
        (SELECT COUNT(*) FROM mixins x WHERE x.host_id = c.symbol_id)
            AS mixins_included,
        f.path || ':' || MIN(c.line) AS at
    FROM ar_callbacks c
    JOIN files f ON f.id = c.file_id
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE c.is_association = 1 AND f.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY c.host, c.symbol_id
    ORDER BY associations DESC, mixins_included DESC
    LIMIT :lim"""),
(
    "mixin-fanout",
    "Hosts ranked by how many modules they mix in -- inclusion coupling per class",
    "ANSWERS the receive side of Ruby's composition: how many modules each\n"
    ...
    "    extend self-style internal use counts like a real dependency.",
    """SELECT rm.name AS host, rm.n_mixins AS mixins_,
        COUNT(DISTINCT x.mixin_short) AS distinct_mixins,
        SUM(x.in_singleton) AS singleton_mixins,
        rm.n_defs AS own_defs, rm.superclass,
        f.path || ':' || rm.line AS at
    FROM ruby_modules rm
    LEFT JOIN mixins x ON x.host_id = rm.symbol_id
    JOIN files f ON f.id = rm.file_id
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE f.is_test = 0 AND rm.n_mixins > 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY rm.symbol_id
    ORDER BY mixins_ DESC, distinct_mixins DESC
    LIMIT :lim"""),
(
    "metaprogram-density",
    "Classes ranked by define_method/method_missing/eval sites per explicit def",
    "ANSWERS how much of a class's real API is generated rather than\n"
    ...
    "    100 by construction.",
    """SELECT rm.name AS owner, COALESCE(m.name,'') AS module_,
        rm.n_defs AS explicit_defs,
        COALESCE(SUM(s.n_define_method),0) AS define_method,
        COALESCE(SUM(s.n_method_missing),0) AS method_missing,
        COALESCE(SUM(s.n_class_eval + s.n_instance_eval + s.n_eval),0)
            AS eval_family,
        COALESCE(SUM(s.n_metaprogram_total),0) AS meta_total,
        CAST(100.0 * COALESCE(SUM(s.n_metaprogram_total),0)
             / NULLIF(rm.n_defs + COALESCE(SUM(s.n_metaprogram_total),0), 0)
             AS INT) AS pct_meta,
        f.path || ':' || rm.line AS at
    FROM ruby_modules rm
    LEFT JOIN symbols s ON s.parent_id = rm.symbol_id AND s.kind = 'method'
    JOIN files f ON f.id = rm.file_id
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    GROUP BY rm.symbol_id
    HAVING meta_total > 0
    ORDER BY pct_meta DESC, meta_total DESC
    LIMIT :lim"""),
(
    "delegate-density",
    "Classes ranked by ActiveSupport delegate statements vs their own defs",
    "ANSWERS how much of a class's interface is forwarded elsewhere:\n"
    ...
    "    invisible; only class-body statements are captured.",
    """SELECT rm.name AS host, COALESCE(m.name,'') AS module_,
        rm.n_delegates AS delegate_statements, rm.n_defs AS own_defs,
        CAST(100.0 * rm.n_delegates
             / NULLIF(rm.n_delegates + rm.n_defs, 0) AS INT)
            AS pct_delegated,
        f.path || ':' || rm.line AS at
    FROM ruby_modules rm
    JOIN files f ON f.id = rm.file_id
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE rm.n_delegates > 0 AND f.is_test = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY rm.n_delegates DESC, pct_delegated DESC
    LIMIT :lim"""),
(
    "attr-exposure",
    "Generated attribute accessors as a share of each class's public surface",
    "ANSWERS the ratio lens on accessor exposure: attr-coupling ranks by raw\n"
    ...
    "    so the denominator is the written surface only.",
    """SELECT s.name AS owner, COALESCE(m.name,'') AS module_,
        s.n_attr_accessor + s.n_attr_reader + s.n_attr_writer AS exposed_attrs,
        (SELECT COUNT(*) FROM symbols m2 WHERE m2.parent_id = s.id
           AND m2.kind = 'method') AS explicit_methods,
        CAST(100.0 * (s.n_attr_accessor + s.n_attr_reader + s.n_attr_writer)
             / NULLIF((s.n_attr_accessor + s.n_attr_reader + s.n_attr_writer)
               + (SELECT COUNT(*) FROM symbols m2 WHERE m2.parent_id = s.id
                  AND m2.kind = 'method'), 0) AS INT) AS pct_generated_api,
        s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE s.kind IN ('class', 'module')
      AND (s.n_attr_accessor + s.n_attr_reader + s.n_attr_writer) > 0
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY pct_generated_api DESC, exposed_attrs DESC
    LIMIT :lim"""),
(
    "class-variable-surface",
    "Class-level state per class: @@vars, class ivars and where they are written",
    "ANSWERS the static surface of shared mutable state: class variables,\n"
    ...
    "    separates them.",
    """SELECT rm.name AS owner, COALESCE(m.name,'') AS module_,
        rm.n_class_vars AS class_vars, rm.n_class_ivars AS class_ivars,
        COALESCE(SUM(s.n_class_level_write),0) AS write_sites,
        COALESCE(SUM(s.n_class_var),0) AS class_var_uses,
        (rm.n_class_vars + rm.n_class_ivars
         + COALESCE(SUM(s.n_class_level_write),0)) AS surface,
        f.path || ':' || rm.line AS at
    FROM ruby_modules rm
    LEFT JOIN symbols s ON s.parent_id = rm.symbol_id AND s.kind = 'method'
    JOIN files f ON f.id = rm.file_id
    ...
    GROUP BY rm.symbol_id
    HAVING surface > 0
    ORDER BY surface DESC, write_sites DESC
    LIMIT :lim"""),
(
    "class-weight",
    "Classes and modules ranked by total method load and lines (Metrics/ClassLength family)",
    "ANSWERS the big classes: method count, class-method count and total\n"
    ...
    "    defs) are excluded by the HAVING-style WHERE.",
    """SELECT rm.name AS owner, COALESCE(m.name,'') AS module_,
        rm.is_module AS is_module_, rm.n_defs AS methods_,
        rm.n_class_defs AS class_methods_, rm.n_mixins AS mixins_,
        cls.sloc, cls.cyclomatic AS cyclo,
        CAST(cls.sloc / MAX(rm.n_defs, 1) AS INT) AS sloc_per_method,
        f.path || ':' || rm.line AS at
    FROM ruby_modules rm
    JOIN symbols cls ON cls.id = rm.symbol_id
    JOIN files f ON f.id = rm.file_id
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE rm.n_defs > 0 AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY cls.sloc DESC, rm.n_defs DESC
    LIMIT :lim"""),
(
    "constant-coupling",
    "Methods reaching across namespaces the most (Foo::Bar references + const_get)",
    "ANSWERS namespace coupling at the call level: methods that reference\n"
    ...
    "    your file reopens or shadows.",
    """SELECT s.name AS in_method, COALESCE(m.name,'') AS module_,
        s.n_const_ref AS const_refs, s.n_const_get AS dynamic_gets,
        s.n_constantize AS constantizes, s.fan_out, s.fan_in,
        f.path || ':' || s.line_start AS at
    FROM symbols s
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE (s.n_const_ref + s.n_const_get) > 3 AND s.kind = 'method'
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    ORDER BY s.n_const_ref DESC, s.n_const_get DESC
    LIMIT :lim"""),
(
    "yield-spread",
    "Modules that hand control to caller blocks the most (block-yield fan-out)",
    "ANSWERS which classes and modules are block-driven: how many of their\n"
    ...
    "    filtered, but the owners' yield counts are not).",
    """SELECT mo.name AS owner, COALESCE(m.name,'') AS module_,
        mo.kind AS owner_kind,
        COUNT(DISTINCT y.id) AS yielding_methods,
        COALESCE(SUM(y.n_yield),0) AS yields,
        COALESCE(SUM(y.n_block_given),0) AS block_given_checks,
        COALESCE(SUM(y.fan_in),0) AS callers,
        f.path || ':' || MIN(y.line_start) AS at
    FROM symbols mo
    JOIN symbols y ON y.parent_id = mo.id
      AND y.n_yield > 0 AND y.kind = 'method'
    JOIN files f ON f.id = mo.file_id
    LEFT JOIN modules m ON m.id = mo.module_id
    WHERE mo.kind IN ('module', 'class')
      AND f.is_test = 0 AND f.is_generated = 0
      AND COALESCE(m.name,'') LIKE :mod
    GROUP BY mo.id
    ORDER BY yields DESC, callers DESC
    LIMIT :lim"""),
(
    "enqueue-spread",
    "Job-enqueue calls per owning class -- who launches background work",
    "ANSWERS which classes reach into background processing: perform_async,\n"
    ...
    "    knowledge of which worker class it targeted.",
    """SELECT rm.name AS owner, COALESCE(m.name,'') AS module_,
        COALESCE(SUM(s.n_enqueue),0) AS enqueues,
        COUNT(DISTINCT s.id) AS enqueue_methods,
        COALESCE(SUM(s.n_enqueue_in_loop),0) AS enqueues_in_loops,
        cls.is_job AS owner_is_job,
        f.path || ':' || rm.line AS at
    FROM ruby_modules rm
    JOIN symbols cls ON cls.id = rm.symbol_id
    LEFT JOIN symbols s ON s.parent_id = rm.symbol_id AND s.n_enqueue > 0
    JOIN files f ON f.id = rm.file_id
    LEFT JOIN modules m ON m.id = f.module_id
    WHERE f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    GROUP BY rm.symbol_id
    HAVING enqueues > 0
    ORDER BY enqueues DESC, enqueues_in_loops DESC
    LIMIT :lim"""),
(
    "dynamic-fanin",
    "Methods invoked through literal send(:name) sites -- the API the call graph cannot see",
    "ANSWERS the invisible fan-in: methods whose name appears as a LITERAL\n"
    ...
    "    name have no symbol and cannot appear as targets at all.",
    """WITH targets AS MATERIALIZED (
        SELECT SUBSTR(ms.arg, 2) AS meth, ms.symbol_id AS caller_id
        FROM metaprogram_sites ms
        WHERE ms.api IN ('send', 'public_send', '__send__')
          AND ms.is_literal = 1
          AND SUBSTR(ms.arg, 1, 1) = ':' AND length(ms.arg) > 1),
    fanin AS (
        SELECT t.meth, COUNT(DISTINCT t.caller_id) AS send_callers
        FROM targets t GROUP BY t.meth)
    SELECT s.name AS invoked_dynamically, COALESCE(m.name,'') AS module_,
        fanin.send_callers, s.is_public AS public_,
        s.fan_in AS static_fan_in,
        f.path || ':' || s.line_start AS at
    FROM fanin
    JOIN symbols s ON s.name = fanin.meth AND s.kind = 'method'
    JOIN files f ON f.id = s.file_id
    LEFT JOIN modules m ON m.id = s.module_id
    WHERE f.is_test = 0 AND COALESCE(m.name,'') LIKE :mod
    ORDER BY fanin.send_callers DESC, s.fan_in DESC
    LIMIT :lim"""),
]
